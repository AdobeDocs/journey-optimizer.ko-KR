...
**사용자 인터페이스의**&#x200B;데이터 세트[!DNL Adobe Journey Optimizer] 작업 영역에서 데이터를 탐색하고 데이터 세트를 만들 수 있습니다. 데이터 세트 대시보드를 열려면 왼쪽 탐색에서 **데이터 세트**&#x200B;를 선택하십시오.

![](assets/datasets-home.png)

**찾아보기** 탭을 선택하여 조직에서 사용할 수 있는 데이터 세트 전체의 목록을 표시합니다. 이름, 데이터 세트가 준수하는 스키마, 가장 최근 수집 실행 상태 등 나열된 각 데이터 세트에 대한 세부 사항이 표시됩니다. 기본적으로는 수집한 데이터 세트만 표시됩니다. 시스템에서 생성한 데이터 세트를 보려면 필터에서 **시스템 데이터 세트 표시** 토글을 켭니다.
...
데이터 집합을 미리 보려면 화면 오른쪽 상단 모서리에서 **데이터 집합 미리 보기**&#x200B;를 선택하여 이 데이터 집합에서 가장 최근에 성공한 일괄 처리를 미리 봅니다. 데이터 세트가 비어 있으면 미리 보기 링크가 비활성화됩니다.

![](assets/dataset-preview.png)

## [!DNL Journey Optimizer]개의 시스템 데이터 세트 {#system-datasets}

이 섹션에는 [!DNL Journey Optimizer]에서 사용하는 시스템 데이터 세트가 나열됩니다. 각 스키마의 전체 필드와 속성 목록을 보려면 [Journey Optimizer 스키마 사전](https://experienceleague.adobe.com/tools/ajo-schemas/schema-dictionary.html?lang=ko){target="_blank"}을 찾아봅니다.
...
* [엔드 투 엔드 사용 사례](../audience/creating-test-profiles.md) - 스키마, 데이터 세트를 만들고 데이터를 수집하여 [!DNL Adobe Journey Optimizer]에 테스트 프로필을 추가합니다.
* [쿼리 예제](../data/datasets-query-examples.md) - [!DNL Adobe Journey Optimizer]개 데이터 세트 및 관련 사용 사례.
