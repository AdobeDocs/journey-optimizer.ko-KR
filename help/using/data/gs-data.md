...
>[!TIP]
>Journey Optimizer에서 데이터 관리를 처음 사용하십니까? 스키마, 데이터 세트 및 소스에 대한 실용적이고 초보자 친화적인 설명을 보려면 [데이터 설정 개요 튜토리얼](https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-management/set-up-data-overview){target="_blank"}을 시청하십시오.
...
>[!NOTE]
>[Adobe Experience Platform 용어집](https://experienceleague.adobe.com/ko/docs/experience-platform/landing/glossary){target="_blank"}은(는) &quot;데이터 소스&quot;를 일반적으로 데이터 원본(CRM, 모바일 앱 등)으로 정의합니다. Journey Optimizer에서 **데이터 원본**&#x200B;은(는) 여정 및 메시지 내에 노출되는 필드를 제어하는 UI 구성이라는 특정한 의미를 갖습니다.
...
### &#x200B;2. 프로필 및 이벤트 데이터에 대한 스키마 디자인 {#design-schemas}
...
### &#x200B;3. 프로필이 활성화된 데이터 세트 만들기 {#create-datasets}
...
➡️ [자습서: 데이터 집합 만들기 및 데이터 수집](https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-management/create-datasets-and-ingest-data){target="_blank"}

### &#x200B;5. Journey Optimizer에서 데이터 소스 구성 {#configure-data-sources}
...
>[!NOTE]
>기본 제공 Adobe Experience Platform 데이터 소스를 통해 경험 이벤트 데이터에 직접 액세스하는 방법은 더 이상 사용되지 않으며 점진적으로 비활성화됩니다. [자세히 알아보기](https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/orchestrate-journeys/journey-use-cases/exp-event-lookup){target="_blank"}.
...
### &#x200B;6. 추적, 피드백 및 여정 데이터 세트 확인 {#verify-datasets}
...
* **[스키마 시작하기](get-started-schemas.md)** — Adobe Experience Platform에서 XDM 스키마를 만들고 올바른 클래스와 필드 그룹을 선택하고 프로필 특성과 동작 이벤트를 모델링하는 방법에 대해 알아봅니다.

* **[데이터 세트 작업](get-started-datasets.md)** - 프로필이 활성화된 데이터 세트와 이벤트 데이터 세트를 만들고, 데이터 수집을 모니터링하고, Journey Optimizer이 추적, 피드백 및 여정 단계 이벤트를 위해 자동으로 만드는 시스템 생성 데이터 세트를 탐색하는 방법을 이해합니다.

* **[데이터 소스 구성](../datasource/about-data-sources.md)** - 여정 내에 프로필 필드와 외부 API 응답을 노출하기 위한 기본 제공 Adobe Experience Platform 데이터 소스 및 선택적 외부 데이터 소스 설정에 대한 단계별 지침입니다.

* **[Adobe Experience Platform 데이터 사용(조회)](lookup-aep-data.md)** - 실시간 고객 프로필에 해당 데이터를 저장하지 않고 AEP 데이터 세트의 참조 또는 트랜잭션 데이터를 사용하여 런타임에 메시지를 보강하는 방법을 알아봅니다.

* **[쿼리 시작](get-started-queries.md)** — 쿼리 서비스를 사용하여 Journey Optimizer 데이터 세트를 분석하고, 이벤트가 올바르게 흐르는지 확인하고, 데이터를 보내고, 열고, 클릭하고, 바운스할 때 보고 쿼리를 만듭니다.

* **[프로필 시작](../audience/get-started-profiles.md)** — Journey Optimizer에서 실시간 고객 프로필이 작동하는 방식과 Platform UI에서 개별 고객 프로필을 탐색, 검사 및 확인하는 방법을 살펴봅니다.

* **[데이터 개요 설정 튜토리얼](https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-management/set-up-data-overview){target="_blank"}** - Journey Optimizer의 데이터 설정에 대한 초보자 친화적인 비디오 연습으로, 스키마, 데이터 세트 및 소스의 전체 내용을 다룹니다.

* **[데이터 세트 만들기 및 데이터 수집 튜토리얼](https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-management/create-datasets-and-ingest-data){target="_blank"}** - Adobe Experience Platform에서 데이터 세트를 만들고 소스 커넥터를 사용하여 데이터를 수집하는 방법을 보여 주는 실습형 튜토리얼로서, 고유한 샌드박스에서 따를 수 있는 단계별 지침을 제공합니다.
