...
* 인바운드 채널이 있는 익명 프로필 타기팅에 대한 보호 기능이 [보호 기능 및 제한 사항](../start/guardrails.md#profile-management-inbound) 페이지에 추가되었습니다. 이 가드레일은 인증되지 않았거나 아직 알려지지 않은 프로필을 타겟팅하면 총 참여 가능한 프로필 수가 증가하고 청구에 영향을 줄 수 있음을 지정합니다. 따라서 Adobe에서는 프로필 수 및 관련 비용을 관리하기 위해 자동 프로필 삭제에 TTL(Time-To-Live)을 설정할 것을 권장합니다.
