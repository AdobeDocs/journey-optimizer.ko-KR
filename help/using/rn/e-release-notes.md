...
* **동적 오퍼 특성** - 이제 Decisioning의 오퍼 특성을 프로필, 컨텍스트 및 대상 데이터를 사용하여 배달 시간에 개인화할 수 있습니다. 이를 통해 사소한 콘텐츠 변형에 대해 중복 오퍼를 유지할 필요가 없으므로 마케터는 보다 적고 유연한 결정 항목을 관리할 수 있습니다.

* **의사 결정의 배치 수준 빈도 제한** - 이제 의사 결정의 빈도 제한 규칙을 개별 배치로 지정할 수 있으므로 오퍼가 지정된 표면에 표시되는 빈도를 보다 세밀하게 제어할 수 있습니다. 두 가지 모드를 사용할 수 있습니다.

   * 배치별 한도: 선택한 배치에 오퍼가 표시되는 경우에만 적용되는 한도를 정의합니다.
   * 배치당 한도 설정: 오퍼가 나타나는 모든 배치에서 캡을 독립적으로 적용하여 각 배치가 자체 한도 설정 카운터를 유지하도록 합니다.
