...
**이번 달 CX Enterprise Coworker의 새로운 기능**
...
<thead>
<tr>
<th><strong>동료 레일에서 여정 생성</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>이제 Coworker 오른쪽 레일에서 AI를 사용하여 <strong>여정을 직접 만들 수 있으므로 이전의 AI Assistant 경험을 여정 생성을 위한 브랜드 변경 및 통합 진입점으로 바꿀 수 있습니다.</strong></p>
</td>
</tr>
</tbody>
</table>
...
<thead>
<tr>
<th><strong>여정 경고에 대한 AI 추천 카드</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>이제 Journey Optimizer 홈 페이지에는 여정 경고가 실행될 때 <strong>사용자 지정 작업 실패</strong> 및 <strong>예외 항목 탐지</strong>에 대한 <strong>AI 권장 사항 여정</strong>이 표시됩니다. 카드를 선택하면 이미 수행된 분석으로 미리 채워진 오른쪽 레일이 있는 여정이 열립니다.</p>
<!-- GIF placeholder: to be added -->
<!-- Documentation link: TBD -->
</td>
</tr>
...
* **여정 시뮬레이션에서 보조 ID 지원** - **보조 ID**&#x200B;이(가) 이제 여정 시뮬레이션에서 지원되므로 읽기 대상 및 이벤트가 트리거된 여정 모두에 대해 복잡한 사용자 시나리오를 테스트할 수 있습니다.

* **대상 자격 여정에 대한 이동 지원** - **대상 자격**(으)로 시작하는 여정은 이제 **이동** 활동을 사용하여 이벤트 기반 시작 여정을 시작할 수 있습니다. 대상 자격 기반 여정으로 이동하는 것은 지원되지 않습니다.

* **Coworker와 여정 버전 비교** - 오늘 한 여정의 두 버전 간에 변경된 사항을 검토하려면 각 노드별로 Journey Optimizer 노드 내에서 수동으로 비교해야 합니다. 특히 여정이 더 복잡해짐에 따라 변경 검토, 감사 및 게시 전 확인 작업이 느려지고 오류가 발생하기 쉬운 구조적 차이는 없습니다. 이 기능을 사용하면 고객 또는 AI 에이전트가 Coworker Chat을 통해 두 버전의 여정을 비교하고 Journey Optimizer을 열지 않고도 필드 수준 세부 사항, 변경된 연결, 여정 수준 속성 변경 및 롤업 카운트와 함께 완전무결성의 **구조화된 차이** 노드를 다시 가져올 수 있습니다.
...
* **위생 분석 동료 기술** - 동료의 새로운 위생 분석 기술은 활성 및 초안 여정에서 손상된 구성, 자동 오류, 오래된 초안 여정, 고립된 데이터 소스, 지속적인 사용자 지정 작업 오류와 같은 손상되거나 사용되지 않는 에셋에 대한 검사와 채팅에서 바로 권장 수정 사항을 표시합니다. <!-- Documentation link: TBD -->

* **비즈니스 성과 분석 동료의 스킬** - 동료의 새로운 **비즈니스 성과 분석** 스킬은 여정의 성과를 분석하고, 성과가 낮은 영역을 설명하고, 재참여 대기, 채널 에스컬레이션 및 전송 시간 최적화와 같은 구체적인 최적화를 권장합니다.  <!-- Documentation link: TBD -->

* **여정 속성의 자동 이벤트 복구 시간 초과** - 이제 여정 속성에 **이벤트 복구 시간 초과 설정** 설정이 포함됩니다. 기본적으로 영향을 받은 여정 이벤트는 서비스 중단 후 최대 72시간 동안 별도의 조치 없이 자동으로 재생됩니다. 이 설정을 켜면 시간에 민감한 여정의 재생 창(0-72시간)을 제어할 수 있습니다. 두 설정 간의 혼동을 방지하기 위해 기존 **시간 초과 또는 오류** 필드의 이름이 **사용자 지정 작업/IDS 작업 시간 초과**(으)로 변경되었습니다.
...
<thead>
<tr>
<th><strong>Android 푸시 알림 템플릿 개선</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>이전에 단일 고정 레이아웃으로 렌더링된 Android 푸시 알림: 이미지는 항상 가운데로 잘리고 긴 본문 텍스트는 잘렸습니다. 이 릴리스에서는 작성 시 템플릿 선택기를 도입하여 마케터가 Android 푸시 알림의 레이아웃을 제어할 수 있도록 했습니다.</p>
<p>다음과 같은 개선 사항을 사용할 수 있습니다.</p>
<ul>
<li><b>레이아웃 선택</b>: Android 푸시를 작성할 때 새 푸시 알림 레이아웃 선택기(표준/확장).</li>
<li><b>전체 이미지 표시"가 있는 표준 레이아웃</b>: 자른 이미지와 크기 조정된 이미지를 선택하십시오.</li>
<li><b>확장된 레이아웃</b>: 잘리지 않은 여러 줄 본문 텍스트와 선택적인 큰 아이콘 축소판 그림입니다.</li>
<li><b>축소된 본문(확장된 레이아웃)</b>: 축소된 상태에 대해 별도의 짧은 본문 텍스트를 설정합니다.</li>
</ul>
</td>
</tr>
</tbody>
...
* **사용자 지정 SMS BYOP 인증 유연성** - 이제 SMS 공급자의 OAuth 설정에 연결할 때 보내는 메시지에 토큰을 배치하는 위치와 토큰 요청 자체의 형식을 포함하여 **사용자 지정 인증 헤더**&#x200B;를 구성할 수 있습니다.

* **DM - 대용량 파일을 자동으로 분할** - 이제 약 20GB를 초과하는 경우 DM 파일을 여러 부분으로 자동으로 분할하거나 파일 라우팅 구성에서 대상 파일 크기를 선택하여 수동으로 분할할 수 있습니다.

* **DM - 대상 제한 늘림** - DM 채널 대상 제한이 3백만 프로필에서 1억 프로필로 늘어 파일 생성 오류가 발생하지 않고 훨씬 더 많은 대상을 타깃팅할 수 있습니다.

### 이메일 채널 {#sep-26-email-channel}
...
* **이메일 작업 수준에서 제외 목록 재정의** - 이제 Journey Optimizer을 사용하여 여정 및 캠페인의 이메일 작업 수준에서 직접 제외 목록 동작을 재정의할 수 있습니다. 이를 통해 팀은 전용 전송 구성이 필요한 운영 또는 규정 준수에 중요한 커뮤니케이션을 보다 유연하게 수행할 수 있을 뿐만 아니라, 다른 모든 전송에 대한 기존 전역 제외 목록 제어를 유지할 수 있습니다. 이러한 향상된 기능은 조직이 광범위한 제외 거버넌스 모델을 변경하지 않고 정확하게 예외 시나리오를 처리할 수 있도록 지원합니다.

* **전자 메일 작성의 URL 구문 유효성 검사** - 이제 Journey Optimizer은 전자 메일 작성 흐름에서 이전 버전의 URL의 유효성을 검사하며 잘못된 구문이 검색되면 더 명확한 지침을 표시합니다. 이를 통해 작성자는 완료 전에 문제를 포착하고 게시 오류를 줄이며 게재 신뢰도를 향상시킬 수 있습니다.
...
<thead>
<tr>
<th><strong>이메일 테마 변형에 대한 다크 모드 지원</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>이제 이메일 테마가 어두운 모드를 지원하므로 각 색상 변형은 어두운 모드가 활성화된 클라이언트에서 이메일을 보는 수신자에게 적합한 모양으로 렌더링될 수 있습니다.</p>
<p>활성화되면 모든 변형에 대해 기본 어두운 팔레트가 자동으로 생성되며, 라이트 모드 디자인과는 별도로 다른 팔레트 또는 사용자 정의 색상으로 기본 어두운 팔레트를 추가로 사용자 정의할 수 있습니다. 따라서 한 모드에서 변경 사항이 다른 모드에 영향을 주지 않습니다.</p>
</td>
</tr>
</tbody>
</table>
...
* **전자 메일 테마의 사용자 지정 글꼴에 대한 대체 글꼴** - 이제 전자 메일 테마를 통해 적용된 모든 사용자 지정(웹) 글꼴에 대한 대체 글꼴을 정의할 수 있습니다. 가입자의 이메일 클라이언트가 사용자 정의 글꼴을 지원하지 않는 경우 Adobe Journey Optimizer은 이메일 클라이언트의 기본값으로 선택을 유지하는 대신 지정된 대체 글꼴을 자동으로 표시합니다. 이렇게 하면 이메일 타이포그래피를 브랜드 지침에 더 가깝게 유지할 수 있으며 이메일 클라이언트 간의 글꼴 렌더링 불일치를 줄일 수 있습니다.
...
* **오케스트레이션된 캠페인을 위한 LINE 채널** - 이제 LINE을 이메일, SMS 및 푸시와 함께 오케스트레이션된 캠페인에서 기본 아웃바운드 채널로 사용할 수 있습니다. 텍스트, 스티커, 이미지, 비디오, 위치 데이터 및 Flex 메시지를 포함한 LINE 메시지를 캠페인 캔버스에서 직접 작성하고 전달할 수 있으며, 이는 일본 및 APAC와 같은 LINE 주력 시장에서 프로모션, 트랜잭션 및 지속적인 참여 사용 사례를 지원합니다. 이전에 제한된 가용성으로 릴리스된 이 기능은 이제 일반적으로 사용할 수 있습니다.

* **오케스트레이션된 새 캠페인 모니터링 API** - 이제 오케스트레이션된 캠페인에 새 **API 사양**&#x200B;을 사용할 수 있습니다. 이를 통해 오케스트레이션된 캠페인을 프로그래밍 방식으로 만들고, 관리하고, 트리거할 수 있으므로 외부 시스템 및 자동화 파이프라인과의 긴밀한 통합을 지원합니다.
...
* **Campaign 오케스트레이션 모니터링** — 이제 오케스트레이션된 캠페인 세분화에 사용되는 관계형 저장소 데이터의 수집 상태 및 최신 상태를 추적하는 새 사용자 인터페이스를 사용할 수 있습니다. 배치 대상자에게 데이터를 제공하는 상태의 상태를 직접 볼 수 있습니다. Adobe Experience Platform의 모니터링 대시보드에 있는 새 Campaign Orchestration 탭에는 드릴다운 그래프 및 계보를 포함한 데이터흐름/데이터 세트 분류와 함께 관계형 저장소 데이터 흐름(레코드 수집/업데이트/삭제/실패/건너뜀)의 상태가 표시됩니다.
