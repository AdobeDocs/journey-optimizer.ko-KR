...
>title="새로운 인라인 작성 기능에 대해 자세히 알아보십시오"
>abstract="2022년 7월 25일부터 메시지는 여정에서 직접 작성됩니다. 기존 메시지는 자동으로 새 모델로 마이그레이션됩니다. 현재 여정 내에서 메시지를 사용하는 경우 마이그레이션 후 추가 작업이 필요합니다."
...
* **제품 변경 사항**: ga(7월 25일)에 각 여정 내에서 채널 컨텐츠가 생성 및 관리됩니다. 다음 **메시지** 메뉴 아래의 왼쪽 탐색 메뉴에서 를 더 이상 사용할 수 없습니다.[자세히 알아보기](../rn/inline-messages.md#change)). 기존 여정에 대한 마이그레이션을 진행합니다.

* **타임라인**: 마이그레이션은 야간에 각 지역에 대해 몇 가지를 통해 발생합니다 [반복](../rn/inline-messages.md#iterations).
...
Adobe의 목표는 제품을 지속적으로 단순화하여 효율적이고 최적화된 사용자 흐름을 제공하는 것입니다. 이러한 새로운 메시지 작성 방법을 사용하면 보다 능률적인 사용자 프로세스가 제공됩니다.

Adobe는 이 새로운 워크플로우를 설계하여 한 위치에서 바로 콘텐츠를 중앙 집중화했습니다.
...
라이브 또는 닫히지 않은 여정에 대해 마이그레이션은 완전히 자동으로 수행됩니다. 프로덕션 영향을 방지하기 위해 라이브 또는 닫힌 여정에 영향을 주지 않으려고 합니다. 새로 만든 버전을 게시해 주십시오.

고객 조직의 모든 샌드박스는 동시에 처리됩니다. 변경 배포 중에 다음 작업이 수행됩니다.
...
메시지 초안 버전은 마이그레이션 중에 수정됩니다. 더 이상 메시지를 참조하지 않습니다. 다음 **메시지** 활동은 적절한 채널 작업 활동으로 대체됩니다. 각 페이지에는 채널 매개 변수와 콘텐츠가 포함되어 있습니다.
...
마이그레이션 중에 이 여정의 새 초안 버전이 만들어집니다. 이 새 초안 버전은 라이브 버전의 사본이지만 메시지가 인라인 작성된 채널 작업으로 대체됩니다. 각 채널 작업 활동에는 채널 매개 변수와 컨텐츠가 포함됩니다. 콘텐츠가 유실되지는 않습니다. 보고는 손실되지 않습니다.

이 초안 버전을 검토하고 테스트한 후 게시하여 라이브 버전이 되기를 바랍니다.

**하나 이상의 메시지를 사용하여 여정 완료 또는 중지됨**

이러한 여정도 마이그레이션됩니다.

여정 보고서를 볼 때 이제 보고서가 더 풍부해지고 메시지 보고서에서 이전에 사용할 수 있었던 정보 수준을 포함합니다.
...
메시지를 작성하는 방식에 변화가 있을 것입니다. 기능은 손실되지 않습니다. 7월 25일(인라인 작성 기능 릴리스) 이후에는 을 사용하여 여정에서 수행할 수 있는 작업에 제한 사항이 적용됩니다 **메시지** 활동:

* 복사/붙여넣기를 수행할 수 없습니다 **메시지** 활동. 하지만 새 채널 작업 활동을 복사/붙여넣을 수 있습니다.
* 이러한 여정의 경우 다음 작업을 수행할 수 없습니다. 다른 샌드박스에 테스트, 게시, 복제, 새 버전 만들기, 닫기 또는 복사 등을 수행할 수 있습니다. 이러한 제한 사항은 **메시지** 활동이 해당 여정에 있습니다. 마이그레이션 프로세스가 실행되고 업데이트될 때까지 대기할 수 있습니다 **메시지** 활동을 채널 작업 활동에 추가하거나 제거할 수 있습니다 **메시지** 수동으로 설정합니다.

### 마이그레이션 프로세스 중에 환경에 액세스할 수 있습니까?

마이그레이션은 야간에 발생합니다. 제품을 사용할 수 있습니다. 하지만 여정을 만들거나, 편집하거나, 삭제하지 마십시오.
