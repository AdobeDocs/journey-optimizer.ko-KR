...
* **시드 목록** - 이제 를 사용할 때 변형이 지원됩니다. **시드 목록**. 시드 주소는 동일한 메시지의 모든 변형(예: 콘텐츠 실험의 다양한 처리)의 사본을 받습니다. [자세히 보기](../configuration/seed-lists.md)
...
* **여정 필터링** - 이제 다음을 사용할 수 있습니다 **여정을 필터링할 사용자 정의 날짜** 기존 사전 정의된 날짜 필터 외에 인벤토리 이렇게 하면 특정 날짜, 특정 달 내, 전체 연도 또는 지정된 시간 범위 내에서 만들거나 게시한 여정을 표시하여 목록을 개선할 수 있습니다. [자세히 보기](../building-journeys/journey-gs.md#filter)
...
* **옵트인/옵트아웃 키워드** - 이제 SMS 채널을 구성할 때 **옵트인 및 옵트아웃 키워드** 기본 설정에 따라. Journey Optimizer은 지정된 이러한 키워드를 기반으로 응답을 트리거합니다. [자세히 알아보기](../sms/sms-configuration.md#create-api)

**캠페인**

* **API 트리거된 캠페인** - API 트리거 캠페인을 활성화한 후 생성된 cURL 코드가 향상되었습니다. 이제 메시지에 사용된 모든 개인화(프로필 및 컨텍스트) 변수가 포함됩니다. [자세히 보기](../campaigns/api-triggered-campaigns.md#execute)
