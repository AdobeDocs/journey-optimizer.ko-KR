...
* 새로운 **조절 API** 에서는 외부 시스템 또는 API에서 급격한 트래픽 스파이크를 방지하기 위해 초당 전송된 이벤트 수에 제한을 설정할 수 있습니다. 설정 제한에 도달하면 모든 후속 API 호출이 수신되는 순서로 가능한 한 빨리 큐에 올라가 처리됩니다. 이 기능은 모든 샌드박스에서 하나의 조절 구성만 지원합니다.
...
* 이제 일별, 주별 또는 월별 기준으로 오퍼 한도 카운터를 재설정할 수 있습니다. <!--[Learn more](../offers/offer-library/add-constraints.md#capping)-->
