...
<tr>
<td>
<p>새 API를 사용하면 작업 캠페인을 검색하고 주요 특성별로 필터링하여 자동화 및 보고 워크플로우를 지원할 수 있습니다.</p>
...
<thead>
<tr>
<th><strong>여정 경고</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>새로운 여정 경고는 주요 여정 상태 신호를 모니터링하는 데 도움이 됩니다. 이 릴리스에는 구성 가능한 임계값 및 여정 인벤토리의 여정 수준 경고 구독과 함께 프로필 폐기 비율, 사용자 지정 작업 오류 비율 및 프로필 오류 비율에 대한 경고 유형이 도입됩니다.</p>
<p>임계값은 모든 여정에서 전역입니다.</p>
...
<tr>
<td>
<p>이메일 콘텐츠를 만들 때 재사용 가능한 테마로 이메일 Designer의 일관된 스타일을 적용합니다.</p>
...
* **의사 결정 항목에 조각 첨부** - 이제 Journey Optimizer에서 의사 결정 정책을 통해 코드 기반 경험 캠페인에서 활용할 수 있는 의사 결정 항목에 <strong>조각</strong>을(를) 첨부할 수 있는 기능을 제공합니다. [자세히 보기](../experience-decisioning/items.md)

  **참고**: 이전에 제한된 가용성으로 릴리스된 이후에는 모든 환경에서 이 개선 사항을 사용할 수 있습니다(일반 가용성).

#### 여정

* **여정 사용자 지정 작업에서 오류 응답 페이로드를 활용합니다** - 이제 사용자 지정 작업에 대해 선택적 <strong>오류 응답 페이로드</strong>를 정의할 수 있습니다. 호출이 실패하면 오류 페이로드가 여정 컨텍스트에 노출되며 `jo_status_code`과(와) 함께 시간 초과/오류 분기에서 사용할 수 있으므로 더 풍부한 대체 논리 및 디버깅을 지원할 수 있습니다. [자세히 보기](../action/action-response.md)

* **기본 및 Adobe Campaign 메시지 동작 결합** - 이제 Journey Optimizer을 사용하여 Adobe Campaign v7/v8 메시지 동작을 동일한 여정의 기본 채널 동작과 결합할 수 있습니다. [자세히 보기](../building-journeys/using-adobe-campaign-v7-v8.md)

* **여정의 여정 페이로드 크기 확인** - Journey Optimizer이 이제 여정 페이로드 크기를 확인하여 최적의 성능과 시스템 안정성을 보장합니다. 여정을 작성하거나 게시할 때 페이로드 크기가 권장 한도에 근접하거나 초과하는 경우 명확한 경고 및 오류가 표시되고, 여정 구성을 최적화하는 실행 가능한 지침이 제공됩니다. 이 사전 유효성 검사는 잠재적 문제를 조기에 식별하고 여정 성능을 유지하는 데 도움이 됩니다. [자세히 보기](../start/guardrails.md#message-content-size)
...
* **사전 정의된 리타겟팅 필터** - 오케스트레이션된 캠페인 사용 사례에 대해 더 쉬운 리타겟팅을 지원하기 위해 이 릴리스에서는 새로운 <strong>캠페인 피드백 필터</strong>를 도입했습니다. 이러한 필터를 사용하면 보냄, 열기만, 열림 또는 클릭됨, 열림 및 클릭됨 등의 메시지 참여를 기반으로 대상을 직접 타기팅하고 다시 타기팅할 특정 캠페인 또는 전환 중 캠페인을 선택할 수 있습니다. [자세히 보기](../orchestrated/retarget.md)

* **매개 변수가 있는 사전 정의된 필터** - 이제 재사용 가능한 편집 가능한 규칙에 대해 오케스트레이션된 캠페인에서 <strong>매개 변수</strong>을(를) 사용하여 사전 정의된 필터를 만들 수 있습니다. [자세히 보기](../orchestrated/predefined-filters.md)
...
* **사용자 생성 메타데이터 지원** - 이제 오케스트레이션된 캠페인에 대한 <strong>executionMetadata 도우미 함수</strong>를 개인화 편집기에서 사용할 수 있으므로 컨텍스트 정보를 모든 기본 작업에 첨부하고 외부 시스템으로 내보내기 위해 데이터 집합에 저장할 수 있습니다. [자세히 보기](../personalization/functions/helpers.md#execution-metadata)

* **다시 시작 단추** - 오케스트레이션된 캠페인에는 이제 <strong>다시 시작 단추</strong>가 포함되어 있으므로 필요할 때 실행을 빠르게 다시 시작한 후 캠페인을 게시할 수 있습니다. [자세히 보기](../orchestrated/start-monitor-campaigns.md)

* **속도 제어 지원** - 이제 오케스트레이션된 캠페인이 <strong>속도 제어</strong>를 지원하므로 게재 속도를 높이고 볼륨 제한에 맞출 수 있습니다. [자세히 보기](../orchestrated/activities/channels.md#rate-control)
...
<p>이전에 제한된 가용성으로 릴리스된 이 기능은 모든 환경에서 사용할 수 있습니다(일반 가용성). 이 일반 가용성 릴리스에는 방해 금지 모드가 완료될 때까지 캠페인 작업을 큐에 넣는 기능과 활성화된 방해 금지 모드 규칙을 미리 보는 기능도 포함됩니다.</p>
<p>가용성 일자: 2026년 1월 28일 목요일</p>
</td>
</tr>
</tbody>
...
<tr>
<td>
<p>이전에 캠페인으로 제한되었던 <strong>DM 채널</strong>은 여정 캔버스에서 사용할 수 있으므로 DM을 여정에 통합할 수 있습니다. DM은 파일 추출 구성과 시간 기반 빈도 설정을 통해 일괄 처리 및 1:1 여정 시나리오 모두에서 지원됩니다.</p>
...
<tr>
<td>
<p>DM 채널은 오케스트레이션된 캠페인에서 사용할 수 있습니다. <strong>DM 활동</strong>은(는) 일회성 메시지와 반복 메시지 모두에 대해 오케스트레이션된 캠페인 내에서 DM을 쉽게 보낼 수 있습니다. DM 공급자에 필요한 <strong>추출 파일</strong> 생성을 자동화합니다. 채널 활동을 오케스트레이션된 캠페인 캔버스에 결합하여 고객 행동 및 데이터를 기반으로 작업을 트리거하는 크로스 채널 캠페인을 만들 수 있습니다.</p>
<p>가용성 일자: 2026년 1월 28일 목요일</p>
</td>
</tr>
</tbody>
...
<tr>
<td>
<p>새로운 <strong>모니터링 대시보드</strong> 및 보강된 여정 단계 이벤트 데이터를 통해 사용자 지정 작업 끝점의 상태 및 성능에 대한 자세한 insight을 얻을 수 있습니다. 성공한 호출, 오류, 처리량, 응답 시간 및 대기열 대기 시간을 추적하여 예외 항목이 발생하는 시기, 장소 및 이유를 신속하게 파악합니다.</p>
<p>이전에 제한된 가용성으로 릴리스된 이 기능은 모든 환경에서 사용할 수 있습니다(일반 가용성).</p>
<p>가용성 일자: 2026년 1월 28일 목요일</p>
</td>
</tr>
...
<tr>
<td>
<p><strong>Decisioning</strong>을(를) 통해 푸시 및 SMS 메시지의 콘텐츠를 개인화하고 최적화할 수 있습니다. 의사 결정 정책, <strong>우선 순위 점수</strong>, 수식 또는 AI 모델을 사용하여 고객에게 최상의 콘텐츠를 표시합니다.</p>
...
* **프로필 시간대를 사용하여 캠페인 예약** - Campaign 예약은 각 프로필의 <strong>시간대</strong>를 사용하여 원하는 현지 시간에 메시지를 배달할 수 있습니다. **참고**: 이 개선 사항은 조직 집합(제한된 가용성)에서만 사용할 수 있습니다. 사용 가능한 날짜: 2026년 1월 28일
