...
1. 양식 디자이너가 열립니다. [구성 요소](../email/content-components.md#add-content-components)를 추가하여 양식 콘텐츠를 빌드합니다. [텍스트](../email/content-components.md#text) 구성 요소와 **[!UICONTROL 필드]** 구성 요소를 사용할 수 있습니다.

1. **[!UICONTROL Field]** 구성 요소를 사용하여 선택한 데이터 세트 스키마를 기반으로 특성을 선택할 수 있습니다.

   >[!NOTE]
   >
   >수집된 데이터를 프로필에 매핑하려면 프로필 ID 필드를 선택합니다. 특성 목록에서 ID 필드를 식별하려면 **[!UICONTROL 필수]**.<!--Explain-->(으)로 표시된 필드를 찾으십시오

   예를 들어 이메일 및 개인 ID를 설정할 수 있습니다. 사용자가 이러한 필드를 채우면 입력한 정보가 선택한 데이터 세트에 저장됩니다.

   ![](assets/lp_create-form-fields.png)

1. 지침, 기본값, 유효성 검사 메시지, 최대 길이 등과 같은 각 **[!UICONTROL 필드 세부 정보]**&#x200B;를 지정할 수 있습니다.
