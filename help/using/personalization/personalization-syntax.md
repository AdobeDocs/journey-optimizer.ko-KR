...
의 개인화 [!DNL Journey Optimizer] 는 Handlebars라는 템플릿 구문을 기반으로 합니다.
...
Handlebars에서 반환된 값은 {{expression}} 은(는) **HTML 이스케이프 처리**. 표현식에 다음이 포함된 경우 `&`그러면 반환된 HTML 이스케이프 출력이 다음과 같이 생성됩니다. `&amp;`. Handlebars가 값을 이스케이프 처리하지 않게 하려면 &quot;triple-stash&quot;를 사용합니다.

리터럴 함수 인수와 관련하여 템플릿 언어 파서는 이스케이프 처리되지 않은 단일 백슬래시(`\`) 기호를 사용할 수 없습니다. 이 문자는 추가 백슬래시(`\`) 기호를 사용할 수 없습니다. 예 :

`{%= regexGroup("abc@xyz.com","@(\\w+)", 1)%}`

## 프로필

이 네임스페이스는에 설명된 프로필 스키마에 정의된 모든 속성을 참조할 수 있습니다. [Adobe Experience Platform 데이터 모델(XDM) 설명서](https://experienceleague.adobe.com/docs/experience-platform/xdm/home.html?lang=ko-KR){target="_blank"}.

스키마에서 참조하기 전에 스키마에서 속성을 정의해야 합니다. [!DNL Journey Optimizer] 개인화 블록.
...
* `Attributes` 은 오퍼 유형에 따라 오퍼별 속성입니다. 예: `deliveryUrl` 이미지용

Decisions API 및 오퍼 표시에 대한 자세한 내용은 다음을 참조하십시오. [이 페이지](../offers/api-reference/offer-delivery-api/decisioning-api.md)

모든 참조는에 설명된 유효성 검사 메커니즘을 사용하여 오퍼 스키마에 대해 유효성이 검사됩니다. [이 페이지](personalization-validation.md)
...
[!DNL Adobe Journey Optimizer] 는 다음 리터럴 유형을 지원합니다.
...
>[!CAUTION]
>
>사용 **x이벤트** 변수는 개인화 표현식에서 사용할 수 없습니다. xEvent에 대한 모든 참조는 유효성 검사 실패를 초래합니다.
