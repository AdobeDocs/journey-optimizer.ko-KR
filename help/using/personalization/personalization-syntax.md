...
이 페이지에서는 Journey Optimizer의 Handlebars 및 PQL 개인화 구문(일반 규칙, 예약된 키워드, 네임스페이스 구조, 유형 시스템 및 일반적인 런타임 오류를 방지하기 위한 모범 사례)에 대해 설명합니다.

**의도**

* Handlebars(`{{...}}`)와 PQL(`{%= ... %}`) 구문을 사용하는 시점을 이해합니다.
* 일반 구문 규칙 적용: 예약된 문자, 대/소문자 구분, HTML 이스케이프, 백슬래시 처리
* 예약된 키워드 및 특수 속성 키(하이픈이 있는 이름, 숫자 이벤트 ID)를 올바르게 이스케이프 처리합니다.
* 일치하지 않는 유형의 값을 비교하거나 전달할 때 유형 강제 변환 적용
* 사용 가능한 네임스페이스의 개인화 참조: 프로필, 대상자, 오퍼
* 가장 일반적인 런타임 및 유효성 검사 오류를 방지하려면 모범 사례를 따르십시오

>[!TAB 용어집]

* **Handlebars**: 특성을 렌더링하고 배열을 반복하며 블록 도우미를 호출하는 데 사용되는 `{{...}}` 템플릿 구문입니다. 기본적으로 HTML은 출력을 이스케이프 처리합니다. *(제품별)*
* **Profile Query Language(PQL)**: 기본 제공 함수(예: `upperCase()`, `formatDate()`)를 호출하고 조건부 식을 평가하는 데 사용되는 `{%= ... %}` 식 구문입니다. *(제품별)*
* **Triple-stash(`{{{ }}}`)**: HTML 이스케이프 처리 없이 값을 출력하는 Handlebars 구문 변형입니다. 값 자체에 인코딩해서는 안 되는 HTML 문자가 포함되어 있을 때 유용합니다.
* **예약된 키워드**: 필드 또는 변수 이름으로 직접 사용할 수 없는 PQL 식별자(`next`, `last`, `this`)는 스키마 필드에서 이러한 이름 중 하나를 사용할 경우 백틱으로 래핑해야 합니다.
* **형식 강제 변환**: PQL에서 비교 또는 산술 전에 필요한 `stringToNumber()` 또는 `toBool()`과(와) 같은 함수를 사용하여 한 데이터 형식에서 다른 데이터 형식(예: 문자열 → 숫자)으로 값을 명시적으로 변환합니다.
* **네임스페이스**: 프로필, 대상, 오퍼 등 각 개인화 데이터의 최상위 그룹이며 고유한 경로 구조와 액세스 규칙을 가지고 있습니다.
* **블록 도우미**: 도우미 이름 앞에 `#`(으)로 식별되는 Handlebars 도우미와 일치하는 닫는 `/`이(가) `{{#each}}`과(와) 같은 블록 구문에 사용됩니다.

>[!TAB 용어]

* **정식 이름:** Handlebars — `{{...}}` 구문, PQL — `{%= ... %}` 구문
* **혼동하지 않음:** `{{...}}`(Handlebars — 변수 및 도우미 렌더링, HTML 이스케이프) ≠ `{%= ... %}`(PQL — 함수 및 표현식 평가) ≠ `{%#if%}` / `{%/if%}`(조건부 블록 구문, % 중괄호)
* **혼동하지 않음:** `{{profile.person.name}}`(단일 스택 — HTML 이스케이프 출력) ≠ `{{{profile.person.name}}}`(트리플 스택 — 이스케이프 처리되지 않은 출력)
* **혼동하지 마십시오.** 예약된 키워드 백틱 이스케이프(`{{...}}` 및 `{%= ... %}`에 모두 적용)≠ 하이픈 키 백틱 이스케이프(`{{...}}`이(가) 아닌 `{%= ... %}` PQL 표현식 내에서만 지원됨)
* **혼동하지 않음:** `=`(PQL 같음 연산자 — 수정) ≠ `==`(유효한 PQL이 아님 — 구문 오류 발생)

>[!TAB 보호 기능 및 제한 사항]

* `xEvent` 변수는 개인화 식에서 사용할 수 없습니다. `xEvent`을(를) 참조하면 유효성 검사가 실패합니다.
* `{{...}}`개의 Handlebars 블록 내의 PQL 함수 호출이 실패합니다. 대신 `{%= ... %}`을(를) 사용하십시오.
* `{% if %}` / `{% elseif %}` / `{% endif %}` 조건부 구문은 지원되지 않습니다. `{%#if%}` / `{%else if%}` / `{%/if%}`을(를) 사용하십시오.
* 하이픈이 있는 필드 이름에 대한 백틱 이스케이프는 PQL 식(`{%= ... %}`) 내에서만 지원됩니다. `{{...}}` Handlebars 보간에서 백틱 구문이 실패하지만 하이픈이 있는 필드 이름은 여전히 직접 참조할 수 있습니다(예: `{{profile.my-custom-field}}`).
* 예약된 키워드(`next`, `last`, `this`)는 스키마 필드 이름으로 사용될 때 백틱으로 래핑해야 합니다. `{{...}}` 및 `{%= ... %}`에 모두 적용됩니다.
* 단일 백슬래시 `\`은(는) 리터럴 함수 인수로 지원되지 않습니다. 이중 백슬래시 `\\`을(를) 사용하십시오.
* PQL은 강력한 형식이므로 비교 또는 산술에서 일치하지 않는 형식에는 `stringToNumber()`, `toBool()` 또는 유사한 강제 변환 함수를 사용하는 명시적 변환이 필요합니다.

>[!TAB FAQ]

**Q: 언제 `{{...}}`과(와) `{%= ... %}`을(를) 사용해야 합니까?**

`{{...}}`(Handlebars)을(를) 사용하여 특성 값을 렌더링하고, 배열을 반복하고, 블록 도우미를 호출합니다. `{%= ... %}`(PQL)을(를) 사용하여 `upperCase()` 및 `formatDate()`과(와) 같은 기본 제공 함수를 호출하고 조건부 표현식을 평가합니다.

**Q: HTML 인코딩을 사용하지 않고 값을 출력하려면 어떻게 합니까?**

`{{...}}` 대신 트리플 스태시 `{{{ }}}`을(를) 사용합니다. 단일 중괄호 Handlebars HTML 이스케이프 출력(예: `&`이(가) `&amp;`이(가) 됨), 트리플 스태시 우회 이스케이프.

**Q: PQL에서 올바른 같음 연산자는 무엇입니까?**

PQL의 같음 비교를 위해 단일 `=`을(를) 사용합니다. `==`을(를) 사용하는 것은 구문 오류입니다.
...
아니요. `{{...}}`은(는) Handlebars 변수 및 도우미만 확인합니다. `{{...}}` 내의 PQL 함수로 인해 &quot;도우미를 찾을 수 없습니다&quot; 오류가 발생합니다. 대신 `{%= functionName(...) %}`을(를) 사용합니다.
