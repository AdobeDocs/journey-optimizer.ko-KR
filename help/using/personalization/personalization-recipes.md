...
필요한 경우 중간 단계로 PQL 표현식 `{%= profile.events.\`order-total\` > 100 %}`. Backticks are not supported in plain Handlebars interpolation — use a `{% let %}` 변수의 백틱에서 하이픈이 연결된 키를 줄바꿈합니다.
