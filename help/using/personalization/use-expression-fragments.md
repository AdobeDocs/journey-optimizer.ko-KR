...
이 페이지에서는 암시적 변수, 루프 내 조각 사용, 편집 가능한 필드, 동적 해결 방법 및 상속 해제 등 개인화 편집기에서 표현식 조각을 삽입, 사용자 지정 및 관리하는 방법에 대해 설명합니다.

**의도**

* 조각 메뉴에서 표현식 조각을 삽입하고 자동 변경 전달 이해
* 암시적 변수 사용: 입력 변수(조각 외부로 선언됨, 내부에 사용됨) 및 출력 변수(조각 내부로 선언됨, 주변 메시지 콘텐츠에 사용됨)
* 루프 내의 표현식 조각 사용 — 조각 액세스에 전역 변수를 활용합니다. 루프 범위 변수를 매개 변수로 전달하는 제한 사항을 이해합니다.
* `<fieldId>="<value>"` 구문을 사용하여 사용자 지정 가능한 조각의 편집 가능한 필드를 재정의합니다.
* 프로필 속성, 데이터 세트 조회 또는 컨텍스트 데이터를 기반으로 런타임 시 조각 ID를 동적으로 해결
* 조각 콘텐츠를 편집기에 바로 붙여넣어 상속 중단

>[!TAB 용어집]

* **표현식 조각**: 캠페인 및 여정에서 ID가 참조하는 재사용 가능한 개인화 표현식 구성 요소입니다. 조각의 변경 사항은 이를 참조하는 모든 콘텐츠에 자동으로 전파됩니다. *(제품별)*
* **암시적 변수**: 조각 기능을 확장하는 변수 — 입력 변수(캠페인/여정 콘텐츠에서 선언됨, 조각 내에서 사용됨) 및 출력 변수(조각 내에서 선언됨, 주변 메시지 콘텐츠에서 사용 가능). *(제품별)*
* **입력 변수**: 조각이 내부적으로 참조하고 사용할 수 있는 조각 외부(캠페인 또는 여정 컨텐츠에서)에 선언된 변수입니다.
* **출력 변수**: 조각 호출 후 주변 메시지 콘텐츠에서 사용할 수 있게 되는 조각 내에서 선언되거나 계산되는 변수입니다.
* **편집 가능한 필드**: 조각 소스를 편집하지 않고 삽입 사용자가 `<fieldId>="<value>"` 구문을 사용하여 기본값을 재정의할 수 있도록 하는 조각 변수가 노출됩니다. *(제품별)*
* **동적 조각 확인**: 디자인 타임에 정적 조각 ID를 포함하는 대신 런타임 시 조각 ID를 확인하는 기능(프로필 특성, 데이터 세트 조회 또는 컨텍스트 데이터 기반)입니다. *(제품별)*
* **상속 중단**: 상황별 메뉴에서 &quot;조각 붙여넣기&quot;를 사용하면 조각의 콘텐츠가 원본 조각과 더 이상 동기화되지 않는 독립 실행형 요소로 편집기에 복사됩니다. *(제품별)*

>[!TAB 용어]

* **정식 이름:** 식 조각 — 변형: 조각, 식 조각
* **동의어:** &quot;조각 ID&quot; = 식에서 조각을 참조하는 데 사용되는 식별자입니다.
* **ID별로 조각을 삽입하면(참조됨, 변경 내용이 모든 콘텐츠에 자동으로 전파됨) 상속/붙여넣기 조각(편집기로 복사된 콘텐츠, 독립 실행형 요소, 더 이상 원본에 연결되지 않음≠이 중단됩니다.**
* **혼동하지 마십시오.** 입력 변수(조각 외부로 선언됨, 내부에서 사용됨)≠ 출력 변수(조각 내부로 선언됨, 주변 메시지 콘텐츠에서 외부에서 사용됨)입니다.
* **혼동하지 마십시오.** 초안 조각(콘텐츠에 추가할 수 있지만 승인될 때까지 여정/캠페인 게시를 차단함) ≠ 라이브 조각(완전히 게시됨, 활성 여정 및 캠페인에 안전함)

>[!TAB 보호 기능 및 제한 사항]

* 주어진 게재에서 최대 30개의 조각을 추가할 수 있습니다.
* 조각은 최대 1개 수준까지만 중첩할 수 있습니다.
* 초안 상태의 조각이 포함된 여정 또는 캠페인은 활성화하거나 게시할 수 없습니다. 게시하기 전에 초안 조각을 승인해야 합니다.
* 표현식 조각은 루프 범위 변수(현재 `{{#each}}` 반복 항목)를 매개 변수로 수신할 수 없습니다. 이는 알려진 제한 사항입니다. 해결 방법으로 전역 변수 또는 인라인 논리를 사용하십시오.
* 여러 줄 바꿈이 포함된 조각을 SMS나 푸시 콘텐츠에서 사용하는 경우 줄 바꿈이 유지됩니다. 전송하기 전에 콘텐츠를 테스트합니다.

>[!TAB FAQ]

**Q: 한 번에 몇 개의 조각을 추가할 수 있습니까?**

최대 30개 조각.

**Q: 조각이 다른 조각 내에 중첩될 수 있습니까?**

네, 하지만 최대 1단계까지만 중첩됩니다.

**Q: 여정 또는 캠페인에서 초안 조각을 사용하면 어떻게 됩니까?**

초안 조각을 콘텐츠에 추가할 수 있지만, 조각이 승인되고 상태가 라이브로 변경될 때까지 여정 또는 캠페인을 활성화하거나 게시할 수 없습니다.

**Q: 식 조각이 현재 루프 항목(예: `{{#each}}`의 `product`)을 매개 변수로 받을 수 있습니까?**

아니요. 표현식 조각은 루프 범위 변수를 매개 변수로 수신할 수 없습니다. 루프 외부에 선언된 전역 변수(조각이 액세스할 수 있음)를 사용하거나 조각을 사용하는 대신 루프 내에 직접 개인화 논리를 포함합니다.

**Q: 상속을 중단하는 이유는 무엇이며 언제 사용해야 합니까?**

상속을 중단한다는 것은 상황별 메뉴에서 &quot;조각 붙여넣기&quot;를 사용하여 조각의 콘텐츠를 편집기에 직접 복사하는 것을 의미합니다. 붙여넣은 콘텐츠는 원래 조각과 더 이상 동기화되지 않는 독립형 요소가 됩니다. 편집 가능한 필드에서 허용하는 범위를 넘어 콘텐츠를 사용자 정의해야 할 때 이를 사용합니다. 원래 조각에 대한 향후 변경 사항은 이 사본에 전파되지 않습니다.
