...
Journey Optimizer에서는 [메시지 개인화](personalize.md) 동안 여러 소스의 컨텍스트 데이터에 액세스할 수 있습니다. 기본 채널([email](../email/get-started-email-design.md), [push](../push/create-push.md), [SMS](../mobile/create-mobile-message.md))의 Handlebars 구문을 사용하여 이러한 원본에서 배열을 반복하여 제품 목록, 권장 사항 또는 기타 반복 요소와 같은 동적 콘텐츠를 표시할 수 있습니다.
...
* **오케스트레이션된 캠페인 데이터 보강 컬렉션**(오케스트레이션된 캠페인만 해당): [오케스트레이션된 캠페인에 개인화 추가](../orchestrated/add-personalization.md#enrichment-collections)를 참조하십시오.
...
>[!NOTE]
>
>**숫자 이벤트 ID에는 백틱이 필요합니다.** 이벤트 ID가 숫자인 경우(예: `1697323153`) 식 경로에서 백틱(`` ` ``)으로 래핑합니다. 백틱이 없으면 PQL 파서에서 구문 오류가 발생합니다.
>
>```handlebars
>context.journey.events.`1697323153`.fieldName
>```
>
>이벤트 타임스탬프의 날짜 형식을 포함한 자세한 내용과 전체 예제는 [컨텍스트 이벤트에서 타임스탬프 형식 지정](../personalization/functions/dates.md#format-date)을 참조하십시오.
...
반복 중 데이터 집합 조회 결과를 필터링하고 특정 기준과 일치하는 항목(예: 특정 범주의 제품)만 표시하려면 `{{#each}}` 루프 내에서 조건부 `{{#if}}` 문을 사용하십시오. 아래 예를 참조하십시오.
...
* **[Target 특성](../orchestrated/add-personalization.md#attributes)**(오케스트레이션된 캠페인만 해당): 데이터 보강 컬렉션 배열을 포함하여 캠페인 캔버스에서 계산된 특성
...
>[!IMPORTANT]
>
>여정 표현식은 Handlebars 개인화와 다른 구문을 사용합니다. 여정 구성(사용자 지정 작업 매개 변수 또는 조건 등)에서는 `first`, `all`, `serializeList` 등의 함수와 함께 [여정 표현식 편집기](../building-journeys/expression/expressionadvanced.md)를 사용합니다. 메시지 콘텐츠에서 `{{#each}}` 루프가 있는 Handlebars 구문을 사용합니다.
...
**사용 사례**: GET 요청에서 쿼리 매개 변수로 전달할 이벤트 배열에서 특정 필드를 가져옵니다.
...
1. 사용자 지정 작업에서 `string` 형식의 쿼리 매개 변수(예: `sku`)를 구성합니다.
...
[컬렉션 관리 함수](../building-journeys/expression/collection-management-functions.md)에서 `first` 함수에 대해 자세히 알아보세요.
...
1. 형식이 `string`인 쿼리 매개 변수(예: `ids`) 구성
2. `Variable`(으)로 표시
...
* [`all` 함수](../building-journeys/expression/collection-management-functions.md)
* [`serializeList` 함수](../building-journeys/functions/list-functions.md#serializeList)
...
**사용 사례**: 요청 본문에 전체 개체 배열을 보냅니다(본문이 있는 POST 또는 GET용).
...
`{{#each}}` 루프 내에서 [표현식 조각](use-expression-fragments.md)을(를) 사용하는 경우 루프 범위 변수를 조각 매개 변수로 전달할 수 없습니다. 하지만 조각은 조각 외부의 메시지 콘텐츠에 정의된 전역 변수에 액세스할 수 있습니다.
...
조각은 메시지에 전역적으로 정의되어 있으므로 `globalDiscount`을(를) 참조할 수 있습니다.
...
자세한 예제와 추가 해결 방법을 포함하여 [루프 내에서 식 조각 사용](use-expression-fragments.md#fragments-in-loops)에 대해 자세히 알아보세요.
...
1. **루프 변수를 매개 변수로 전달하려고 합니다**: 식 조각은 루프 범위 변수(현재 반복 항목 등)를 매개 변수로 받을 수 없습니다. 이는 알려진 제한 사항입니다.
...
2. **조각에 사용할 수 없는 매개 변수가 필요합니다**: 조각이 특정 입력 매개 변수를 받도록 디자인된 경우 해당 매개 변수를 루프 내에서 전달할 수 없으면 올바르게 작동하지 않습니다.

   **솔루션**: 조각이 액세스할 수 있는 전역 변수를 사용하도록 접근 방식을 재구성하십시오. 예제는 [모범 사례 - 루프의 식 조각](#best-practices)을 참조하십시오.

3. **잘못된 변수 범위**: 단편에서 루프 범위 내에만 있는 변수를 참조하려고 할 수 있습니다.

   **솔루션**: 메시지 수준(루프 외부)에서 조각에 필요한 변수를 정의하여 전체적으로 액세스할 수 있도록 합니다.

자세한 설명, 예제 및 권장 패턴을 포함하여 [루프 내에서 식 조각을 사용](use-expression-fragments.md#fragments-in-loops)하는 방법에 대해 자세히 알아보세요.
...
**메시지 디자인:** [이메일 디자인 시작](../email/get-started-email-design.md) | [푸시 알림 만들기](../push/create-push.md) | [SMS 메시지 만들기](../mobile/create-mobile-message.md) | [콘텐츠 미리 보기 및 테스트](../content-management/preview-test.md)
...
이 페이지에서는 메시지 개인화를 통해 Handlebars `{{#each}}` 구문을 사용하여 컨텍스트 소스(이벤트, 사용자 지정 작업 응답, 데이터 세트 조회 및 기술 속성)에서 배열을 반복하는 방법과 여정 활동을 구성할 때 여정 표현식 구문에서 배열을 사용하는 방법에 대해 설명합니다.

**의도**

* `{{#each}}`을(를) 사용하여 메시지 개인화에서 이벤트 배열 데이터(예: 장바구니 항목, 주문 항목)를 반복합니다
* 메시지의 사용자 지정 작업 응답 배열(예: 제품 권장 사항)을 반복합니다
* 메시지의 데이터 세트 조회 결과 배열 반복
* 여러 컨텍스트 소스의 데이터를 하나의 개인화된 메시지로 결합
* 여정 표현식 구문을 사용하여 배열 값을 사용자 지정 작업 매개 변수에 전달
* 데이터 세트 조회 활동에서 조회 키로 배열 사용
* 빈 배열 폴백, 변수 이름 지정, 성능 및 표현식 조각 범위에 대한 우수 사례 적용

>[!TAB 용어집]

* **Handlebars**: 반복(`{{#each}}`) 및 조건부 렌더링(`{{#if}}`)을 위한 Journey Optimizer 메시지 개인화에 사용되는 템플릿 언어입니다. *(제품별)*
* **`{{#each}}`helper**: 배열을 반복하기 위한 Handlebars 구문입니다. 각 반복은 명명된 변수(예: `|product|`)를 통해 현재 항목을 노출합니다. *(제품별)*
* **컨텍스트 데이터**: 정적 프로필 특성과 대조적으로 여정 소스(이벤트, 사용자 지정 작업 응답, 데이터 세트 조회 및 여정 기술 속성)에서 메시지를 보낼 때 사용할 수 있는 데이터입니다. *(제품별)*
* **`currentEventField`**: 필터링 또는 매핑 작업 중에 이벤트 배열의 각 항목을 참조하기 위해 여정 표현식(Handlebars 아님)에 사용되는 참조입니다.
* **`currentActionField`**: 사용자 지정 작업 응답 컬렉션의 각 항목을 참조하는 여정 식에 사용됩니다.
* **`currentDataPackField`**: 데이터 원본 컬렉션의 각 항목을 참조하는 여정 식에 사용됩니다.
* **`serializeList`**: 값 목록을 쿼리 매개 변수로 사용하기에 적합한 구분된 문자열(예: 쉼표로 구분)로 변환하는 여정 표현식 함수입니다.
* **보조 식별자**: 같은 프로필에서 트리거된 동시 여정 인스턴스를 구별하는 여정 수준 식별자로, 현재 인스턴스와 관련된 항목으로 배열을 필터링하는 데 사용됩니다.

>[!TAB 용어]

* **정식 이름:** Handlebars 반복 — 변형: `{{#each}}` 루프, 각 루프, 배열 반복
* **혼동하지 마십시오.** Handlebars `{{#each}}` 구문(반복 및 표시에 대한 메시지 콘텐츠에 사용됨) ≠ 여정 표현식 구문(여정 활동 구성에 사용됨 - `first`, `all`, `serializeList` 등의 함수 사용)
* **혼동하지 마십시오.** `currentEventField`(이벤트 배열에 대한 여정 식) ≠ `currentActionField`(사용자 지정 작업 응답 컬렉션) ≠ `currentDataPackField`(데이터 원본 컬렉션)
* **혼동하지 않음:** `@index` / `@first` / `@last`(Handlebars 특수 변수, 메시지 콘텐츠에서 `{{#each}}` 루프 내에서만 사용 가능) ≠ `first` / `head` 함수(단일 항목을 추출하기 위한 여정 식 함수, 여정 활동 구성에 사용)

>[!TAB 보호 기능 및 제한 사항]

* 여정은 하나의 작업 노드가 배열 항목당 여러 번 실행되는 동적 루프를 만들 수 없습니다. 이는 성능 문제를 방지하기 위한 것입니다. 대신 전체 배열 또는 직렬화된 목록을 단일 사용자 지정 작업에 전달합니다.
* 총 50KB 미만으로 이벤트 페이로드를 유지합니다.
* 사용자 지정 작업 응답 페이로드는 100KB 미만이어야 합니다.
* 성능을 위해 데이터 세트 조회 키 및 반환된 엔티티의 수를 제한하십시오.
* 표현식 조각은 루프 범위 변수(예: 현재 `{{#each}}` 반복 항목)를 매개 변수로 수신할 수 없습니다. 이는 알려진 제한 사항입니다. 대신 전역 변수 또는 인라인 논리를 사용하십시오.
* 숫자 이벤트 ID는 식 경로(예: `` context.journey.events.`1697323153`.fieldName ``)에서 백틱으로 래핑해야 합니다. 백틱이 없으면 PQL 파서에서 구문 오류가 발생합니다.

>[!TAB FAQ]

**Q: 배열을 사용하여 작업할 때 Handlebars 구문과 여정 식 구문의 차이점은 무엇입니까?**

Handlebars `{{#each}}`은(는) 반복 및 표시를 위한 메시지 콘텐츠에 사용됩니다. `first`, `all` 및 `serializeList`과(와) 같은 함수를 사용하는 여정 표현식 구문은 여정 활동 구성(예: 사용자 지정 작업 매개 변수, 조건)에 사용됩니다. 서로 다른 컨텍스트에서 사용되는 고유한 구문입니다.

**Q: 배열 항목당 한 번씩 실행되도록 여정 작업 노드를 반복할 수 있습니까?**

아니요. 여정은 항목당 작업 노드를 여러 번 실행하는 동적 루프를 만들 수 없습니다. 대신 전체 배열 또는 직렬화된 목록을 모든 항목을 처리하는 단일 사용자 지정 작업에 전달하거나 외부 합계를 사용합니다.

**Q: 현재 루프 항목을 `{{#each}}` 루프 내의 식 조각에 전달할 수 있습니까?**

아니요. 표현식 조각은 루프 범위 변수를 매개 변수로 수신할 수 없습니다. 루프 외부에 정의된 전역 변수를 사용하거나 조각을 사용하는 대신 루프 내에 직접 개인화 논리를 포함하십시오.

**Q: 배열이 비어 있을 때 대체 콘텐츠를 어떻게 표시합니까?**

`{{#each}}` 블록 내에서 `{{else}}` 절을 사용합니다. 배열에 항목이 없으면 `{{else}}` 내의 콘텐츠가 렌더링됩니다.

**Q: `{{#each}}` 루프 내에서 `@index`, `@first` 및 `@last`은(는) 무엇을 의미합니까?**

다음은 메시지 콘텐츠의 `{{#each}}` 루프 내에서만 사용할 수 있는 특수 Handlebars 변수입니다. `@index`은(는) 0 기반 현재 반복 인덱스이고, `@first`은(는) 첫 번째 반복에 대해 true이고, `@last`은(는) 마지막 반복에 대해 true입니다.
