...
이 페이지에서는 Journey Optimizer 개인화 편집기를 사용하여 프로필 속성, 대상, 오퍼 의사 결정 및 컨텍스트 속성을 포함한 소스에서 개인화 표현식을 선택, 빌드, 사용자 정의 및 확인하는 방법을 설명합니다.

**의도**

* Journey Optimizer에서 개인화를 추가할 수 있는 위치 이해(메시지, 이메일 Designer, URL, 이메일 구성, 오퍼)
* 표현식에 적합한 개인화 소스 선택
* 편집기 작업 영역에서 속성 추가 및 표현식 작성
* 편집기 도구 사용: 찾기/바꾸기, 자동 완성, 유효성 검사, 알약, 조각으로 저장
* 탐색 창 기능 사용: 도우미 기능, 즐겨찾기, 조건, 조각
* 표현식의 유효성 검사 및 일반적인 오류 해결

>[!TAB 용어집]

* **Personalization 편집기**: 개인화 표현식을 작성, 사용자 지정 및 확인하기 위한 Journey Optimizer의 중앙 UI 도구입니다. 개인화를 정의할 수 있는 모든 위치에서 사용할 수 있습니다. *(제품별)*
* **Personalization 소스**: 식을 만드는 데 사용할 수 있는 데이터 범주(프로필 특성, 타겟 특성, 대상, 오퍼 결정 및 컨텍스트 특성)입니다.
* **컨텍스트 특성**: 여정 또는 캠페인별 데이터(이벤트, 속성, 사용자 지정 작업 응답)는 여정 또는 캠페인에서 채널 작업을 사용하는 경우에만 개인화에 사용할 수 있습니다. *(제품별)*
* **알약**: 가독성을 개선하기 위해 긴 특성 경로를 압축되고 클릭 가능한 토큰으로 렌더링하는 개인화 편집기 표시 모드입니다. 프로필 속성, 컨텍스트 속성 및 다이내믹 미디어에만 사용할 수 있습니다. *(제품별)*
* **자동 완성**: 입력할 때 코드를 자동으로 제안하고 완료하는 편집기 기능입니다. HTML 및 텍스트 형식에만 사용할 수 있으며 프로필 및 컨텍스트 특성만 지원합니다. *(제품별)*
* **표현식 조각**: 캠페인 및 여정 전반에서 참조할 수 있는 재사용 가능한 개인화 표현식 구성 요소입니다. *(제품별)*
* **대체 텍스트**: 지정된 프로필에 대해 문자열 유형 프로필 특성이 비어 있을 때 표시되는 기본 문자열입니다. &quot;대체 텍스트로 삽입&quot;을 통해 특성별로 구성됩니다.

>[!TAB 용어]

* **표준 이름:** 개인화 편집기
* **혼동하지 마십시오.** Personalization 편집기(메시지, 이메일, 푸시 알림 및 오퍼에서 콘텐츠 식을 만드는 데 사용됨 - Handlebars 및 PQL 구문 모두 지원)≠ 고급 표현식 편집기(데이터 소스 및 이벤트 정보에 대한 조건, 사용자 지정 대기 활동 및 작업 매개 변수 매핑에 대한 여정에서 사용됨 - 개인화 편집기와 다른 기본 제공 함수 및 연산자를 제공합니다.)
* **프로필 특성(XDM 스키마 기반, 모든 컨텍스트에서 사용 가능) ≠ 컨텍스트 특성(여정/캠페인별, 해당 컨텍스트에서만 사용 가능) ≠ Target 특성(오케스트레이션된 캠페인만 해당)**
* **혼동하지 마십시오:** HTML/Text에 대한 자동 완성(개인화 특성 완성 제안) ≠ 기본 HTML 코드 자동 완성(토글이 비활성화될 때 편집기 기본값)

>[!TAB 보호 기능 및 제한 사항]

* 자동 완성은 HTML 및 텍스트 형식에만 사용할 수 있으며 프로필 및 컨텍스트 속성만 지원합니다.
* 환약 표시 모드는 프로필 속성, 컨텍스트 속성 및 다이내믹 미디어에만 사용할 수 있습니다.
* URL 개인화는 외부 링크, 구독 취소 링크 및 옵트아웃 링크 유형에만 사용할 수 있습니다.
* 기본적으로 속성 창에는 채워진 속성만 표시됩니다. 모든 스키마 속성을 표시하려면 &quot;채워진 속성만 표시&quot;를 끄십시오.
* 오퍼 모델 사용에는 프로필 속성만 포함되어야 합니다. 의사 결정에 프로필이 아닌 속성이 있으면 유효성 검사 오류가 발생합니다.

>[!TAB FAQ]

**Q: Journey Optimizer에서 개인화를 추가할 수 있는 위치는 어디입니까?**

이메일 제목 줄, 푸시 알림 필드(제목, 본문, 사용자 정의 사운드, 배지, 사용자 정의 데이터), 이메일 Designer 텍스트 요소, URL(외부 링크, 구독 취소 링크, 옵트아웃), 이메일 구성 하위 도메인/헤더/URL 추적 매개 변수 및 오퍼 텍스트 유형 표시를 포함하여 개인화 추가 아이콘이 있는 모든 필드.

**Q: 사용 가능한 개인화 원본은 무엇입니까?**

프로필 속성, 타겟 속성(오케스트레이션된 캠페인만 해당), 대상, 오퍼 의사 결정 및 컨텍스트 속성(여정/캠페인 이벤트 및 사용자 지정 작업 응답)입니다.

**Q: 식을 확인하는 방법은 무엇입니까?**

추가 를 클릭하여 편집기를 닫으면 유효성 검사가 자동으로 실행됩니다. 유효성 검사 버튼을 사용하여 수동으로 트리거할 수도 있습니다. 일반적인 오류: 경로를 찾을 수 없음(스키마에 필드가 없음), 유형 불일치(문자열을 배열로 반복), 잘못된 Handlebars 구문 및 잘못된 세그먼트 정의.

**Q: 알약 옵션은 어떤 기능을 합니까?**

편집기에서 가독성을 높이기 위해 긴 속성 경로를 압축되고 클릭 가능한 토큰으로 렌더링합니다. 프로필 속성, 컨텍스트 속성 및 다이내믹 미디어에만 사용할 수 있습니다.

**Q: 특성 창에 일부 특성만 표시되는 이유는 무엇입니까?**

기본적으로 창에는 채워진 속성만 표시됩니다. 검색 필드 위에 있는 설정 아이콘을 선택하고 &quot;채워진 속성만 표시&quot;를 끄면 모든 스키마 속성이 표시됩니다.
