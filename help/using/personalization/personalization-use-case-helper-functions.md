...
* 다음 `upperCase` 문자열 함수입니다. [추가 정보](functions/string.md#upper).
* 다음 `each` helper, 장바구니에 있는 항목을 나열합니다. [추가 정보](functions/helpers.md#each).
* 다음 `if` helper를 사용하여 관련 제품이 장바구니에 있는 경우 제품별 메모를 삽입합니다. [추가 정보](functions/helpers.md#if-function).
...
* 단일 이벤트. [추가 정보](../event/about-events.md).
* 이벤트로 시작하는 여정. [추가 정보](../building-journeys/using-the-journey-designer.md).
* 여정의 이메일 메시지. [추가 정보](../email/create-email.md)
* 이메일 본문. [추가 정보](../email/content-from-scratch.md).
...
1. 에서 **이메일** 활동, 클릭 **[!UICONTROL Edit content]**&#x200B;를 클릭한 다음 **[!UICONTROL Email Designer]**.
...
1. 클릭 **[!UICONTROL Validate]**&#x200B;를 클릭한 다음 **[!UICONTROL Save]**.
...
1. 추가 `productListItems` 배열에 있는 값:
   ...
   1. 왼쪽 메뉴에서 **[!UICONTROL Contextual attributes]**.
      ...
      **[!UICONTROL Contextual attributes]** 여정 컨텍스트가 메시지에 전달된 후에만 사용할 수 있습니다.
