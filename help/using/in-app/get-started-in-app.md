...
인앱 메시지는 사용자가 이미 앱에 참여하는 동안 사용자를 안내하거나 영향을 미치기를 원할 때 가장 잘 작동하며 해당 순간을 적극 활용합니다.

| 이점 | 이유 | 예시 사용 사례 |
| --- | --- | --- |
| 높은 사용자 참여 | 사용자가 앱 세션에서 활동 중일 때 도달합니다. | 기능 공지, 온보딩 팁 |
| 상황별 관련 트리거 | 인앱 동작 또는 위치를 기반으로 트리거할 수 있습니다. | 사용자가 관련 화면을 방문한 직후 기능 강조 표시 |
| 실시간 게재 | 푸시 토큰 또는 외부 게재 서비스에 대한 종속성 없음 | 현재 세션 중에 표시되는 시간 구분 프롬프트 |
| 외부 채널에 대한 종속성 없음 | 다른 채널의 옵트인 상태에 관계없이 앱 내에서 완전히 작동합니다 | 푸시 알림을 옵트아웃한 사용자에게 연결 |
| 전환 가능성 향상 | 능동적인 관심을 갖는 순간 전달되므로 응답률이 높아짐 | 업셀 또는 크로스셀 오퍼, 설문 조사 프롬프트 |
| 사용자 지정 및 세그멘테이션 | 레이아웃, 텍스트 및 버튼은 특정 대상에 맞게 조정할 수 있습니다 | 다양한 사용자 세그먼트에 대한 개인화된 온보딩 흐름 |
| 업무에 영향을 미치지 않는 설계 | 사용자 경험을 중단하지 않고 보완하도록 설계 가능 | 앱의 UI에 맞는 배너 또는 양식 |

## 사용하지 않을 때

인앱 메시지는 활성 세션에 따라 다르므로 모든 시나리오에 적합하지 않습니다. 다음 상황에서 다른 채널을 고려하십시오.

* 메시지가 표시되지 않으므로 사용자가 앱을 적극적으로 사용하지 않습니다.
* 메시지는 앱 외부의 사용자에게 도달해야 하는 중대하거나 시간에 민감한 문제입니다(예: 중단 또는 보안 경고)
* 통신은 규제 또는 합법적이며 인앱 메시지가 제공할 수 없는 읽기 확인이 필요합니다
* 목표는 앱을 열 가능성이 낮은 비활성 사용자를 위한 계정 재활성화 또는 윈-백 캠페인입니다
* 이 메시지는 주문 확인과 같은 대량 트랜잭션 업데이트로 이메일 또는 SMS에 더 적합합니다
* 과다 사용은 사용자가 너무 자주 나타나는 메시지를 무시하기 시작하는 배너 맹인으로 이어질 수 있습니다
* 메시지가 전달되어야 하는 경우 사용자가 오프라인 상태이거나 앱 연결이 없을 수 있습니다
