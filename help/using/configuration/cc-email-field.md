...
* **컨텍스트 특성** - 이 값은 **API 트리거 캠페인에서만 사용할 수 있음**. 문자열로 전달된 CC 주소 값이 있는 컨텍스트 변수 `context.channel.email.ccvalues`을(를) 포함해야 하는 API 페이로드에서 검색됩니다.
...
>[!CAUTION]
>
>기존 전자 메일 채널 구성을 편집할 때 **[!UICONTROL CC 전자 메일]** 필드에 새 [프로필 특성](../personalization/personalization-build-expressions.md#sources)을 추가할 수 없습니다. [새 채널 구성을 만들어야](channel-surfaces.md#create-channel-surface).
...
* **동의 및 제외:** CC 전자 메일 주소가 동의 또는 제외에 대해 확인되지 않습니다.

* **보고:** 열기, 클릭 수 및 CC 수신자의 기타 참여가 전자 메일 보고 지표에 포함됩니다. CC 주소로 전송된 전자 메일을 열거나 클릭하지 마십시오. [보고서](../reports/report-gs-cja.md)에서 계산 착오를 일으킬 수 있는 전송 분석의 총 열림 및 클릭 수에 고려됩니다.
