...
>additional-url="https://experienceleague.adobe.com/docs/journey-optimizer/using/configuration/delegate-subdomains/about-subdomain-delegation.html?lang=ko-KR#subdomain-delegation-methods" text="하위 도메인 구성 메서드"
...
도메인 이름 위임은 도메인 이름(기술적으로 DNS 영역)의 소유자가 그 하위 분할(기술적으로 하위 영역이라고 할 수 있는 하위 DNS 영역)을 다른 엔티티에 위임할 수 있는 방법입니다. 기본적으로 고객은 &quot;example.com&quot; 영역을 처리하는 경우 하위 영역 &quot;marketing.example.com&quot;을 Adobe에 위임할 수 있습니다. 자세히 알아보기 [하위 도메인 위임](about-subdomain-delegation.md)
...
>[!CAUTION]
>
>전체 하위 도메인 위임은 권장되는 방법입니다. 둘 사이의 차이점에 대해 자세히 알아보기 [하위 도메인 구성 메서드](about-subdomain-delegation.md#subdomain-delegation-methods).
>
...
>additional-url="https://experienceleague.adobe.com/docs/journey-optimizer/using/configuration/delegate-subdomains/delegate-subdomain.html?lang=ko-KR#cname-subdomain-delegation" text="CNAME 하위 도메인 위임"
...
1. 선택 **[!UICONTROL 완전히 위임됨]** 다음에서 **[!UICONTROL 메서드 설정]** 섹션.
...
1. 확인이 성공하면 하위 도메인이 **[!UICONTROL 성공]** 상태. 메시지를 전달하는 데 사용할 준비가 되었습니다.
   ...
   >[!NOTE]
   >
   >하위 도메인이 다음으로 표시됨: **[!UICONTROL 실패]** 호스팅 솔루션에서 유효성 검사 레코드를 만들지 못한 경우.

하위 도메인이 의 Adobe에게 위임되면 [!DNL Journey Optimizer], PTR 기록이 자동으로 생성되고 이 하위 도메인과 연결됩니다. [자세히 알아보기](ptr-records.md)

>[!CAUTION]
>
>하위 도메인의 병렬 실행은 현재 다음에서 지원되지 않습니다. [!DNL Journey Optimizer]. 위임할 하위 도메인을 다른 사용자에게 제출하려고 할 때 **[!UICONTROL 처리 중]** 상태, 오류 메시지가 표시됩니다.
...
1. 다음 항목 선택 **[!UICONTROL CNAME 설정]** 메서드를 사용합니다.
...
1. 검사가 성공하면<!--i.e Adobe validates the record you created and installs it-->, 하위 도메인은 **[!UICONTROL 성공]** 상태. 메시지를 전달하는 데 사용할 준비가 되었습니다.
...
1. **도메인에 대한 DNS 구성**:
   ...
   * **MX 레코드**: 메일 Exchange 레코드 - 하위 도메인으로 전송된 인바운드 이메일을 처리하는 메일 서버 레코드입니다.
   * **SPF 레코드**: 발신자 정책 프레임워크 레코드 - 하위 도메인에서 이메일을 보낼 수 있는 메일 서버의 IP를 나열합니다.
   * **DKIM 레코드**: DomainKeys Identified Mail 표준 레코드 - 공개-개인 키 암호화를 사용하여 메시지를 인증하여 스푸핑을 방지합니다.
   * **A**: 기본 IP 매핑.
   * **CNAME**: 정식 이름 또는 CNAME 레코드는 별칭 이름을 실제 또는 정식 도메인 이름에 매핑하는 DNS 레코드 유형입니다.

1. **추적 및 미러 URL 생성**: 도메인이 email.example.com이면 추적/미러 도메인은 data.email.example.com입니다. SSL 인증서를 설치하여 보안을 유지합니다.

1. **CDN CloudFront 프로비저닝**: CDN이 아직 설정되지 않은 경우 Adobe은 조직의 ID에 대해 CDN을 프로비저닝합니다.

1. **CDN 도메인 만들기**: 도메인이 email.example.com이면 CDN 도메인은 cdn.email.example.com입니다.

1. **CDN SSL 인증서 생성 및 첨부**: Adobe은 CDN 도메인에 대한 CDN 인증서를 만들고 인증서를 CDN 도메인에 연결합니다.

1. **순방향 DNS 만들기**: 위임하는 첫 번째 하위 도메인인 경우 Adobe은 PTR 레코드를 만드는 데 필요한 순방향 DNS를 각 IP에 대해 하나씩 만듭니다.

1. **PTR 레코드 만들기**: 이메일을 스팸으로 표시하지 않도록 ISP에 역방향 DNS 레코드라고도 하는 PTR 기록이 필요합니다. 또한 Gmail에서는 각 IP에 대해 PTR 레코드를 사용할 것을 권장합니다. Adobe은 각 IP에 대해 하나씩, 하위 도메인을 가리키는 모든 IP에 대해 처음으로 하위 도메인을 위임할 때만 PTR 레코드를 생성합니다. 예를 들어 IP가 *192.1.2.1* 하위 도메인은 입니다. *email.example.com*, PTR 기록: *192.1.2.1 PTR r1.email.example.com*. 이후 PTR 레코드를 업데이트하여 새 위임된 도메인을 가리킬 수 있습니다. [PTR 레코드에 대한 자세한 정보](ptr-records.md)
