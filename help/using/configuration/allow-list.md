...
이 허용 목록을 사용하면 특정 샌드박스에서 보내는 이메일을 수신하도록 허가된 유일한 수신자 또는 도메인인 개별 이메일 주소 또는 도메인을 지정할 수 있습니다.

>[!NOTE]
>
>이 기능은 프로덕션 및 비프로덕션 샌드박스에서 사용할 수 있습니다.

예를 들어, 오류가 발생할 수 있는 비프로덕션 인스턴스에서 허용 목록에 따라 실제 고객 주소로 원치 않는 메시지를 보낼 위험이 없으므로 테스트 목적으로 안전한 환경을 제공합니다.
...
## 허용 목록에 액세스 {#access-allowed-list}
...
>[!CAUTION]
>
>허용 목록을 보고, 내보내고, 관리할 수 있는 권한은 다음으로 제한됩니다 [여정 관리자](../administration/ootb-product-profiles.md#journey-administrator). 관리에 대해 자세히 알아보기 [!DNL Journey Optimizer] 사용자 액세스 권한 [이 섹션](../administration/permissions-overview.md).
...
## 허용된 목록 활성화 {#enable-allow-list}

허용된 목록을 활성화하려면 아래 단계를 수행하십시오.

1. 액세스 권한  **[!UICONTROL Channels]** > **[!UICONTROL Email configuration]** > **[!UICONTROL Allow list]** 메뉴 아래의 제품에서 사용할 수 있습니다.

1. 전환 단추를 선택합니다.

   ![](assets/allow-list-edit.png)

1. 선택 **[!UICONTROL Activate allowed list]**. 이제 허용된 목록이 활성 상태입니다.

   ![](assets/allow-list-enable.png)

   >[!NOTE]
   >
   >허용 목록을 활성화한 후 여정 및 캠페인에서 이 목록을 적용할 때까지 5분 지연이 있습니다.

기능이 활성 상태일 때 허용된 목록 논리가 적용됩니다. 추가 정보 [이 섹션](#logic).

>[!NOTE]
>
>활성화된 경우 여정 실행 시 허용되는 목록 기능이 허용되지만 [증명](../email/preview.md#send-proofs) 를 사용하여 여정 테스트 [테스트 모드](../building-journeys/testing-the-journey.md).

## 허용된 목록을 비활성화합니다 {#deactivate-allow-list}

허용된 목록을 비활성화하려면 아래 단계를 따르십시오.
...
1. 선택 **[!UICONTROL Deactivate allowed list]**. 허용된 목록이 더 이상 활성화되어 있지 않습니다.

   ![](assets/allow-list-deactivate.png)

   >[!NOTE]
   >
   >허용된 목록을 비활성화하면 여정 및 캠페인에서 이 목록을 적용할 수 있는 5분 지연이 있습니다.

기능을 비활성화하면 허용된 목록 논리가 적용되지 않습니다. 추가 정보 [이 섹션](#logic).
...
>[!NOTE]
>
>허용된 목록에는 최대 1,000개의 항목이 포함될 수 있습니다.

### 허용 목록을 수동으로 채웁니다 {#manually-populate-list}
...
수동으로 [!DNL Journey Optimizer] 사용자 인터페이스를 통해 이메일 주소 또는 도메인을 추가하여 허용 목록입니다.
...
1. 을(를) 선택합니다 **[!UICONTROL Add email or domain]** 버튼을 클릭합니다.
...
허용된 목록을 채우기 위해 `ALLOWED` 값 `listType` 속성을 사용합니다. 예:
...
>title="허용된 목록 관리"
>abstract="허용된 목록이 활성화되면 허용 목록에 포함된 수신자만 이 샌드박스에서 이메일 메시지를 수신하게 됩니다. 비활성화되면 모든 수신자는 이메일을 수신하게 됩니다."

허용 목록이 [활성](#enable-allow-list), 다음 논리가 적용됩니다.

* 허용 목록이 **비어 있음**: 이메일을 전송하지 않습니다.

* 엔티티가 **허용 목록에 추가**&#x200B;제외 목록에 없으면 이메일이 해당 수신자에게 전송됩니다. 그러나 엔티티가 [제외 목록](../reports/suppression-list.md)로 설정되면 해당 수신자가 이메일을 받지 못합니다. 이유 **[!UICONTROL Suppressed]**.
...
허용 목록이 활성화되면 허용 목록에 없으므로 전송에서 제외된 이메일 주소 또는 도메인을 검색할 수 있습니다. 이렇게 하려면 [Adobe Experience Platform 쿼리 서비스](https://experienceleague.adobe.com/docs/experience-platform/query/api/getting-started.html){target=&quot;_blank&quot;} 를 사용하여 아래 API를 호출할 수 있습니다.

를 가져오려면 **이메일 수** 수신자가 허용 목록에 없으므로 전송되지 않은 경우 다음 쿼리를 사용하십시오.
...
를 가져오려면 **이메일 주소 목록** 수신자가 허용 목록에 없으므로 전송되지 않은 경우 다음 쿼리를 사용하십시오.
