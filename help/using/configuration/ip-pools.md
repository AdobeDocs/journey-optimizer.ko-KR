...
**이 페이지에서:** 하위 도메인 IP 주소를 그룹화하는 IP 풀을 만들고, 편집하고, 삭제하여 전자 메일 배달 기능을 개선하고, 보낸 사람의 평판을 보호하는 방법을 알아봅니다.
...
1. 드롭다운 목록에서 풀에 포함할 IP 주소를 선택한 다음 **[!UICONTROL 제출]**&#x200B;을 클릭합니다. 인스턴스와 함께 프로비저닝된 모든 IP 주소를 목록에서 사용할 수 있습니다.
...
1. 원하는 대로 속성을 편집합니다. 설명을 수정하고 IP 주소를 추가하거나 제거할 수 있습니다. IP 풀 이름은 편집할 수 없습니다. 이름을 바꾸려면 풀을 삭제하고 새 풀을 만드십시오.
...
>[!NOTE]
>
>* [채널 구성을 만들](channel-surfaces.md#create-channel-surface) 때 선택한 하위 도메인과 연결된 적이 없는 **[!UICONTROL 처리 중]** 상태의 IP 풀을 선택하면 구성 만들기를 진행할 수 없습니다. [자세히 알아보기](channel-surfaces.md#create-channel-surface)
>* IP 풀이 성공적으로 업데이트되면 몇 분 후에 실시간 메시지에 적용되거나 배치 메시지에 대한 다음 배치 작업이 적용될 때까지 기다리십시오.
