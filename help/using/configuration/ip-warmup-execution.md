...
>title="캠페인 대상자 제외"
>abstract="현재 단계에서 대상자를 제외할 캠페인을 선택하십시오. 이렇게 하면 이전에 연락한 프로필이 다시 타겟팅되지 않습니다. 여정을 통해 커뮤니케이션을 받은 사람만 제외됩니다."
...
IP 준비 계획의 단계를 정의하려면 각 단계에 대한 캠페인을 선택하고, 도메인 및 대상에 대한 제외를 구성하고, 프로필 타깃팅을 관리해야 합니다. 각 단계에는 다음 섹션에서 구성할 여러 실행이 포함될 수 있습니다. 다음 단계를 수행합니다.
...
1. IP 준비 계획의 첫 번째 단계와 연결할 캠페인을 선택합니다.
   ...
   >* 다른 IP 준비 계획에서 이미 사용 중인 캠페인은 선택할 수 없습니다. 동일한 캠페인은 동일한 플랜의 여러 단계에서 사용할 수 있습니다.

1. 현재 단계에 대해 캠페인을 선택하면 프로필, 캠페인 대상자 및 도메인 그룹을 제외하는 섹션이 표시됩니다. 실행이 활성화되면 새 단계로 [실행을 분할](#split-phase)하지 않으면 제외를 수정할 수 없습니다.

   1. **[!UICONTROL 제외된 도메인 그룹]** 섹션에서 해당 단계에서 제외할 도메인을 선택합니다.

      >[!NOTE]
      >
      >도메인 제외에는 실행 안 된 단계가 필요하므로 제외를 추가하려면 [실행 중인 단계를 분할](#split-phase)해야 할 수 있습니다. 또한 [IP 준비 계획 템플릿](ip-warmup-plan.md#prepare-file)에 추가된 사용자 지정 도메인 그룹만 제외할 수 있습니다. 그렇지 않으면 템플릿을 사용자 지정 도메인 그룹으로 업데이트하고 [계획을 다시 업로드](#re-upload-plan)하십시오.
   ...
   1. **[!UICONTROL 이전 실행에서 타겟팅된 프로필]** 섹션에서 해당 단계의 이전 실행에서 프로필이 항상 제외되어 있음을 확인할 수 있습니다(이 섹션은 읽기 전용). 예를 들어 실행 시 타겟팅되#1 처음 4800명에서 프로필이 포함된 경우 시스템은 동일한 프로필이 실행 시 이메일을 수신하지 않도록 자동으로 #2.
...
1. **[!UICONTROL 단계 삭제]** 단추를 사용하여 원치 않는 단계를 제거하십시오. 이 작업은 한 단계에서 실행이 실행되지 않는 경우에만 사용할 수 있습니다. <!--Once a run is executed, deletion is not allowed.-->
   ...
   >[!CAUTION]
   >
   >**[!UICONTROL 단계 삭제]** 작업을 실행 취소할 수 없습니다. 모든 단계를 삭제하면 계획을 다시 업로드하는 것이 좋습니다. [자세히 알아보기](#re-upload-plan)
...
>title="참여 필터링"
>abstract="이 열은 예를 들어 지난 20일 동안 브랜드에 참여한 사용자만 타겟팅하는 필터입니다. **실행 편집** 옵션을 통해 이 설정을 변경할 수도 있습니다."
...
>title="적격 프로필 보기"
>abstract="이 열에는 적격 프로필의 수가 표시됩니다. 대상에 대한 실행이 평가되면 적격 프로필보다 타깃팅된 프로필이 더 많으면 **오류가 발생한 경우 활성화된 실행 취소** 옵션을 활성화하지 않는 한 실행이 계속 실행됩니다. 이 경우 실행이 취소되었습니다."

IP 웜업 계획의 단계를 정의한 후에는 각 단계 내에서 개별 실행을 구성해야 합니다. 각 실행에는 일정이 필요하며, 선택적으로 참여 필터, 오류 처리 및 재시도 기간을 구성하여 최적의 실행을 보장할 수 있습니다. 다음 단계를 수행합니다.
...
1. 선택적으로 [대상 평가](https://experienceleague.adobe.com/docs/experience-platform/segmentation/home.html#how-segmentation-works){target="_blank"}가 지연되는 경우 IP 준비 캠페인을 실행할 수 있는 기간을 정의할 수 있습니다. 이렇게 하려면 플랜 이름 옆에 있는 왼쪽 상단의 속성 아이콘을 클릭하고 **[!UICONTROL 실행 시간 다시 시도]** 드롭다운 목록을 사용하여 최대 240분(4시간)의 기간을 선택합니다.
   ...
   >[!NOTE]
   >
   >정의된 기간이 끝날 때까지 30분마다 다시 시도합니다. 기간을 지정하지 않으면 전송 시간에 실행이 시도되며 대상 평가가 완료되지 않으면 실패합니다.
...
1. 필요한 경우 추가 작업 아이콘에서 **[!UICONTROL 실행 편집]**&#x200B;을 선택합니다. 여기에서 각 열의 주소 수를 업데이트할 수 있습니다. 예를 들어 지난 20일 동안 브랜드에 참여한 사용자만 타겟팅하도록 **[!UICONTROL 마지막 참여]** 필드를 업데이트할 수도 있습니다.
   ...
   >[!NOTE]
   >
   >게재 가능성 전문가와 상의하여 이 숫자들을 수정하는 것이 좋습니다. 실행에 대한 참여 기간을 사용하지 않으려면 **[!UICONTROL 마지막 참여]** 필드에 0을 입력하십시오.
...
>[!CAUTION]
>
>각 실행은 실제 전송 시간 전 및 일일 배치 세분화 작업 전 최소 12시간 전에 활성화되어야 합니다. 그렇지 않으면 대상 평가가 완료되지 않을 수 있습니다.
...
* 모든 실행을 활성화할 때:
   ...
   * 다음 명명 규칙을 사용하여 캠페인이 전송될 대상에 해당하는 [대상 구성](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/audience-composition.html){target="_blank"}을 만듭니다. `<warmupName>-Phase<phaseNo>-Run<runNo>`.
...
플랜에서 다른 캠페인을 사용할 수 있으므로 [캠페인 메뉴](../campaigns/manage-campaigns.md#access)에서 보고서에 액세스할 수도 있습니다. [일정에서 보기 →](../campaigns/manage-campaigns.md#calendar)
