...
우선 순위 또는 순위 공식을 사용하는 대신 **AI 모델**&#x200B;을 사용하여 훈련된 모델 점수를 기반으로 여정의 등급을 동적으로 지정할 수 있습니다. UI의 **[!UICONTROL 오케스트레이션 순위]** 섹션에서 AI 모델을 만든 후 여정 집합에 사용하여 규칙에 적용할 수 있습니다.

[!DNL Journey Optimizer]에서 사용 가능한 AI 모델 유형에 대한 개요는 의사 결정 섹션에서 [AI 모델 시작](../experience-decisioning/ranking/ai-models.md#ai-model-types)을 참조하십시오.
...
1. **[!UICONTROL 최적화 지표]** 섹션은 AI 모델에서 사용하는 전환 이벤트에 대한 정보를 제공합니다. [!DNL Journey Optimizer]은(는) **전환율**&#x200B;을(를) 기반으로 순위를 지정합니다(전환율 = 총 전환 이벤트 수 / 총 노출 이벤트 수). 전환율은 다음을 사용하여 계산됩니다.
...
이제 규칙 세트를 구성할 때 AI 모델을 사용할 수 있습니다.

## 규칙 세트에 AI 모델 할당 {#assign-ai-model-to-ruleset}

AI 모델을 사용하여 여정의 등급을 지정하려면 공식에서 사용하고 이 공식을 규칙 세트에 지정해야 합니다.

1. 만든 AI 모델을 사용하여 등급 공식을 만듭니다. [방법 알아보기](journey-ranking-formulas.md#create-journey-ranking-formula)
