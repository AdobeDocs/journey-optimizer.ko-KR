...
>title="카탈로그의 스키마 액세스 및 편집"
>abstract="사용자 정의 속성은 결정 항목에 할당할 수 있으며 필요에 맞게 조정된 특정 속성입니다. 이는 결정 항목의 카탈로그 스키마에 생성됩니다."

Decisioning에서 카탈로그는 의사 결정 항목을 구성하는 중앙 컨테이너 역할을 합니다. 각 카탈로그가 [!DNL Adobe Experience Platform] 스키마에 연결되어 있으며 결정 항목에 할당할 수 있는 모든 특성을 포함합니다.

지금은 생성된 모든 결정 항목이 단일 &quot;오퍼&quot; 카탈로그 내에 통합되어 **[!UICONTROL 카탈로그]** 메뉴를 통해 액세스할 수 있습니다.

![오퍼 카탈로그를 표시하는 항목 카탈로그 목록](assets/catalogs-list.png)
...
1. 카탈로그의 스키마가 아래 구조에 따라 새 탭에서 열립니다.
   ...
   * **`_<imsOrg>`** 노드에는 사용자 지정 결정 항목 특성이 있습니다. 여기서 `<imsOrg>`은(는) 조직 이름(예: Luma 회사의 경우 `_luma`)으로 대체됩니다. 기본적으로 사용자 지정 속성은 구성되지 않지만 요구 사항에 맞게 필요한 만큼 추가할 수 있습니다. 완료되면 사용자 지정 속성이 표준 속성과 함께 의사 결정 항목 만들기 화면에 표시됩니다.
   ...
   ![경험 및 조직 노드가 있는 카탈로그 스키마 구조](assets/catalogs-schema.png)
...
1. 원하는 사용자 지정 속성이 추가되면 스키마를 저장합니다. 이제 **[!UICONTROL 사용자 지정 특성]** 섹션 내의 결정 항목 만들기 화면에서 새 필드를 사용할 수 있습니다.
   ...
   ![사용자 지정 특성 섹션이 있는 결정 항목 만들기 화면](assets/custom-attributes.png)
