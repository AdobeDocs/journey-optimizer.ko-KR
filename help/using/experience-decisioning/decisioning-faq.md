...
+++**AI 모델에서 대상의 역할은 무엇입니까?**

[개인화된 최적화 모델](ranking/personalized-optimization-model.md)을(를) 구성할 때 데이터 세트와 대상은 각각 다른 용도로 사용됩니다.
...
두 구성 요소 모두 효과적인 개인화된 최적화 모델 성능을 위해 필요합니다. [AI 모델](ranking/ai-models.md)에 대해 자세히 알아보세요.

+++

+++**자동 최적화 또는 개인화된 최적화 모델을 사용하는 경우 오퍼 컬렉션 변경 사항이 AI 모델에 어떻게 영향을 줍니까?**

두 모델 모두 지난 30일 동안의 트래픽 데이터를 기반으로 다음으로 사용 가능한 오퍼에 트래픽을 제공합니다.

여러 오퍼가 동시에 제거되고 나머지 오퍼가 30일 창 내에서 최소 트래픽 데이터를 갖는 경우 모델은 다음을 포함하는 차선의 동작을 나타낼 수 있습니다.
* 임의 분포 패턴
* 제한된 노출 데이터를 기반으로 전환율이 더 높은 오퍼에 대한 편향
...
자동 최적화 및 개인화된 최적화 모델은 모두 전용 제어 그룹의 필요성을 제거하는 탐색-익스플로잇(explore-exploit) 전략을 사용합니다.
...
트래픽 예외 항목은 30일 순환 기간 내에 비례적으로 모델에 통합됩니다.

**영향 평가:**
비정상 트래픽은 30일 데이터 세트의 작은 부분을 나타내므로 임시 트래픽 스파이크(예: 일일 트래픽 2배)는 전체 모델 성능에 미치는 영향이 최소화됩니다.
