...
생성 AI를 사용하여 여러 채널에서 콘텐츠를 만들고 개인화하여 콘텐츠 생성 프로세스를 가속화하면서 브랜드 일관성을 유지할 수 있습니다.

### 지원되는 채널

콘텐츠 생성을 위한 AI Assistant는 다음 경우에 사용할 수 있습니다.

* **전자 메일** - 제목 줄, 본문 및 이미지 생성
* **푸시 알림** - 매력적인 알림 메시지 만들기
* **SMS** - 간결하고 매력적인 텍스트 메시지 작성
* **웹** - 웹 페이지 콘텐츠 및 경험 생성

### 주요 기능

* **텍스트 생성** - 브랜드 음성 및 목표에 따라 매력적인 사본 만들기
* **이미지 생성** - Adobe Firefly을 사용하여 사용자 지정 이미지 생성
* **콘텐츠 변형** - A/B 테스트를 위해 여러 변형을 생성합니다.
* **브랜드 정렬** - 생성된 콘텐츠가 브랜드 지침과 일치하는지 확인합니다.
* **템플릿 지원** - 기존 전자 메일 템플릿 활용
...
* 시간에 민감한 운영 메시지(주문 확인, 암호 재설정)
* 긴급 알림(비행 지연, 긴급 경고)
* 특정 시간 요구 사항이 있는 이벤트 기반 메시지

### 구성 팁

* **30일 대기** - 활성화하기 전에 최소 30일 동안의 내역 전자 메일 또는 푸시 데이터를 수집합니다.
* **최적의 대기 시간 설정** - 최상의 결과를 얻으려면 6~24시간을 사용합니다(더 짧은 제한 최적화, 더 길면 콘텐츠가 오래된 것일 수 있음).
* **올바른 지표 선택** - 클릭 수(액션 유도 메시지) 또는 열기 횟수(인식 메시지)에 최적화
* **아침 전송 예약** - 푸시 알림의 경우 야간 배달을 방지하기 위해 하루 일찍 시작합니다.
...
[의사 결정을 위한 AI 모델에 대해 자세히 알아보기](../experience-decisioning/ranking/ai-models.md)
...
* **콘텐츠 생성을 위한 AI 도우미** - &quot;콘텐츠 생성&quot; 권한이 필요합니다.
* **AI Assistant 제품 지식** - Adobe 생성 AI 사용자 지침에 동의해야 합니다.
* **여정 분석 에이전트** - 여정 보기/관리 및 세그먼트 보기/관리 권한이 필요합니다.
* **실험 에이전트** - 실험 보기 및 실험 메타데이터 관리 권한이 필요합니다.
...
아니요. 정확성 및 브랜드 적절성은 항상 AI가 생성한 콘텐츠를 검토하십시오. 피드백 도구(썸네일 위쪽/아래쪽)를 사용하여 모델을 개선합니다.

+++

+++**주요 제한 사항은 무엇입니까?**

* **전송 시간 최적화** - 여정에서 전자 메일 및 푸시에만 사용할 수 있으며 30일 교육 기간이 필요합니다.
* **AI 콘텐츠 생성** - DM, 콘텐츠 카드, LINE 또는 WhatsApp에는 사용할 수 없습니다.
* **AI 등급 모델** - 조직당 최대 5개 모델, 최소 상호 작용 데이터 필요

+++

+++**이 기능에 액세스하려면 어떻게 해야 합니까?**

대부분의 AI 기능이 Adobe Journey Optimizer에 포함되어 있습니다. 전송 시간 최적화 또는 AI 에이전트와 같은 일부 기능은 Adobe에서 활성화해야 할 수 있습니다. 특정 라이선스 및 사용 가능한 기능에 대한 자세한 내용은 Adobe 담당자에게 문의하십시오.

+++

>[!MORELIKETHIS]
>
>* [콘텐츠 생성을 위해 AI Assistant 시작](../content-management/gs-generative.md)
>* Experience Platform의 [AI 길잡이](https://experienceleague.adobe.com/ko/docs/experience-platform/ai-assistant/home){target="_blank"}
>* [여정 분석 에이전트 설명서](https://experienceleague.adobe.com/ko/docs/experience-cloud-ai/experience-cloud-ai/agents/ajo-agent-analyze){target="_blank"}
