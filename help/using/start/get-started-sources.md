...
>[!NOTE]
>
>**오케스트레이션된 캠페인용 데이터 수집** - 오케스트레이션된 캠페인에 사용되는 파일 기반 변경 데이터 캡처 소스의 경우 `_change_request_type` 필드가 필요합니다. 지원되는 값은 `u`(업서트) 또는 `d`(삭제)입니다. 이 값은 대문자 `U` 및 `D`이(가) 아닌 소문자 `u` 및 `d`(으)로 입력해야 합니다. [오케스트레이션된 캠페인 가드레일 및 제한 사항에 대해 자세히 알아보기](../orchestrated/guardrails.md)

## 자세히 알아보기 {#learn-more}

![](assets/sources-home.png)

이 비디오를 시청하여 소스 커넥터에 대해서와 Journey Optimizer에서 소스 커넥터 구성하는 방법을 알아보세요.
