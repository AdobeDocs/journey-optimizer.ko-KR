...
>[!NOTE]
>
>**오케스트레이션된 캠페인에 대한 데이터 수집** - 오케스트레이션된 캠페인에 사용되는 파일 기반 변경 데이터 캡처 소스의 경우 `_change_request_type` 필드가 필요합니다. 지원되는 값은 `u`(업데이트) 또는 `d`(삭제)입니다. 이 값은 대문자 `u` 및 `d`이(가) 아닌 소문자 `U` 및 `D`이어야 합니다. [오케스트레이션된 캠페인 보호 및 제한 사항에 대해 자세히 알아보세요](../orchestrated/guardrails.md)
...
* 소스가 Journey Optimizer의 전체 데이터 설정에 어떻게 적합한지 이해하려면 [데이터 관리 시작하기](../data/gs-data.md) 개요를 검토하십시오
