...
**이 페이지에서:** Adobe Journey Optimizer의 시스템, 여정, 대상, 채널 및 콘텐츠 제한을 검토하여 실패 없이 확장되는 배포를 계획할 수 있습니다.
...
>[!TIP]
>
>**다음 중 무엇을 의미합니까:** **50-활동 제한** 및 **활성 여정 제한**&#x200B;은(는) 크기를 조정할 때 대부분의 팀이 먼저 경험하는 두 개의 보호 기능입니다. 샌드박스 처리량 경합을 방지하기 위해 여정 분할 작업을 조기에 계획하고 대상 읽기 시작 시간을 최소 5~10분 간격으로 분산합니다.
...
* 모든 사용자 지정 작업, 호스트 및 샌드박스에 대해 **1분 간 300,000번의 최대 호출 제한**&#x200B;이(가) 정의됩니다. 이 상한은 응답 시간이 0.75초 미만인 엔드포인트에 대해 샌드박스별, 엔드포인트별로 슬라이딩 윈도우 방식으로 적용됩니다. 응답 시간이 0.75초보다 큰 끝점의 경우, 30초당 **150,000개 호출 제한**(슬라이딩 기간)이 별도로 적용됩니다.
* 사용자 정의 작업 URL은 동적 매개 변수를 지원하지 않습니다.
* POST, PUT 및 GET 호출 메서드가 지원됩니다.
* 쿼리 매개 변수 또는 헤더의 이름은 “.” 또는 &quot;$&quot;입니다.
* IP 주소는 URL에서 허용되지 않습니다. 대신 호스트 이름을 사용하십시오.
* 내부 Adobe 주소(`.adobe.*`)는 URL 및 API에 사용할 수 없습니다.
...
* 사용자 지정 작업으로 타깃팅된 모든 끝점은 **200개 이상의 TPS**&#x200B;를 지원해야 합니다. 스로틀링 구성은 200TPS 미만일 수 없습니다. 예상 처리량에 따라 응답 시간이 길면 실제 처리량에 영향을 줄 수 있습니다.

>[!TIP]
>
>**이 방법은 다음을 의미합니다.** 기본 호출 수/분 제한은 외부 끝점이 여정 처리량에 의해 압도되지 않도록 보호합니다. 끝점에서 더 많은 로드를 처리할 수 있는 경우 [API 최대 가용량](../configuration/capping.md) 또는 [API 제한](../configuration/throttling.md)을 사용하여 이 제한을 늘릴 수 있습니다. Journey Optimizer이 외부 시스템에 연결하는 방법에 대한 자세한 개요는 [이 페이지](../configuration/external-systems.md)를 참조하십시오. 더 높은 조직 제한이 필요한 경우 Adobe 담당자에게 문의하십시오.
...
* 각 조직은 모든 샌드박스 및 여정에서 최대 **5개** **대상자 읽기** 인스턴스를 동시에 실행할 수 있습니다(예약됨 또는 비즈니스 이벤트가 트리거됨). 정확히 같은 시간에 시작하는 **대상자 읽기**&#x200B;가 포함된 여정이 5개를 초과하지 않도록 해야 합니다. 5~10분 간격으로 나누는 것이 좋습니다. [이 섹션](../building-journeys/entry-management.md#journey-processing-rate)에서 여정 처리 속도에 대해 자세히 알아보십시오.
* 샌드박스 처리량: 시스템은 모든 **대상 읽기** 활동에서 최대 **초당 20,000개의 프로필을 공유**&#x200B;하여 샌드박스당 처리를 관리합니다. 개별 활동은 **초당 500~20,000개의 프로필**&#x200B;로 구성할 수 있습니다. 샌드박스 제한에 도달하면 작업이 대기열로 보내질 수 있습니다.
* 작업 처리 시간 초과: **대상자 읽기** **12시간** 내에 처리할 수 없는 작업은 자동으로 정리되며 실행되지 않습니다.
* 내보내기 작업을 검색하는 동안 대상에서 트리거된 여정에 대해 기본적으로 다시 시도가 적용됩니다. 내보내기 작업을 만드는 동안 오류가 발생하면 10분마다 최대 **1시간** 동안 다시 시도합니다. 이후 여정은 실패한 것으로 간주되므로 예약된 시간 후 최대 1시간까지 실행할 수 있습니다.
* 보조 ID를 사용하는 여정의 경우 각 여정 인스턴스에 대한 **대상자 읽기** 활동의 읽기 속도는 최대 **초당 500개의 프로필**&#x200B;로 제한됩니다.
