...
* 기본으로 제공되는 **반응** 이벤트를 사용하면 즉시 사용 가능한 작업에 반응할 수 있습니다. [이 페이지](../building-journeys/reaction-events.md)에서 자세히 알아보십시오. 사용자 지정 작업을 통해 전송된 메시지에 반응하려면 전용 이벤트를 구성해야 합니다.
...
* 사용자 정의 작업을 사용하여 타깃팅할 엔드포인트를 선택할 때 다음을 확인하십시오.
   ...
   * 이 엔트포인트는 제한하는 [Throttling API](../configuration/throttling.md) 또는 [Capping API](../configuration/capping.md)의 구성을 사용하여 여정의 처리량을 지원할 수 있습니다. 스로틀링 구성은 200TPS 미만일 수 없습니다. 타깃팅된 모든 끝점은 최소 200개의 TPS를 지원해야 합니다.
...
**[!UICONTROL 프로필 업데이트]** 활동에 특정 보호 기능이 적용됩니다. [이 페이지](../building-journeys/update-profiles.md)에 나열됩니다.


### 대상자 읽기 {#read-segment-g}

**[!UICONTROL 대상자 읽기]** 활동에 다음 보호 기능이 적용됩니다.
...
* 여정은 대상자 읽기 활동을 하나만 가질 수 있습니다.
* [이 페이지](../building-journeys/read-audience.md)에서 대상자 읽기 활동을 사용하는 방법에 대한 권장 사항도 참조하세요.


### 대상자 자격 조건 {#audience-qualif-g}

**[!UICONTROL 대상 자격]** 활동에 다음 보호가 적용됩니다.

* 대상 자격 활동은 Adobe Campaign 활동과 함께 사용할 수 없습니다.


### 표현식 편집기  {#expression-editor}

* 경험 이벤트 필드 그룹은 대상자 읽기, 대상자 자격 조건 또는 비즈니스 이벤트 활동으로 시작하는 여정에서 사용할 수 없습니다. 새 대상을 만들고 여정에서 inaudience 조건을 사용해야 합니다.
...
* 인앱 표시는 여정 지속 시간에 연결되어 있습니다. 즉, 특정 프로필에 대한 여정이 종료되면 해당 여정 내의 모든 인앱 메시지는 해당 프로필에 표시되지 않습니다.  따라서 여정 활동에서 바로 인앱 메시지를 중지할 수는 없습니다. 대신, 인앱 메시지가 프로필에 표시되지 않도록 하려면 전체 여정을 종료해야 합니다.
...
**[!UICONTROL Jump]** 활동에 특정 보호 기능이 적용됩니다. [이 페이지](../building-journeys/jump.md#jump-limitations)에 나열됩니다.

### 캠페인 활동 {#ac-g}

**[!UICONTROL Campaign v7/v8]** 및 **[!UICONTROL Campaign Standard]** 활동에 다음 보호 기능이 적용됩니다.

* Adobe Campaign 활동은 대상자 읽기 또는 대상자 자격 활동과 함께 사용할 수 없습니다.
* 이러한 활동은 인앱 활동과 함께 사용할 수 없습니다.
...
| Edge 세그멘테이션을 사용한 Edge Decisioning API 요청 | 초당 1500 |
| Edge 세그멘테이션 없이 Edge Decisioning API 요청 | 5,000/초 |
