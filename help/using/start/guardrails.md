...
* 각 프로필 및 여정 버전에 대해 여정 런타임에서는 하나가 처리되는 동안 최대 10개의 보류 중인 이벤트에 대한 내부 대기열을 유지합니다. 이 제한에 도달하면 스택이 소모될 때까지 `maxInstanceStackEventsReached` 이유와 함께 추가 이벤트가 무시됩니다. [maxInstanceStackEventsReached가 있는 삭제된 이벤트](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached)을(를) 참조하십시오.
...
* Journey Optimizer에서 여정을 트리거하기 위해서는 이벤트를 Data Collection Core Service(DCCS)로 스트리밍해야 합니다. 일괄적으로 수집된 이벤트, **쿼리 서비스**&#x200B;를 통해 삽입된 이벤트 또는 내부 Journey Optimizer 데이터 세트의 이벤트(메시지 피드백, 전자 메일 추적 등)는 여정을 트리거하는 데 사용할 수 없습니다. 스트리밍 이벤트를 가져올 수 없는 사용 사례의 경우에는 대신 이 이벤트를 기반으로 대상자를 작성하고 **대상자 읽기** 활동을 사용해야 합니다. 대상 자격은 기술적으로 사용할 수 있지만, 사용된 작업에 따라 다운스트림 문제를 일으킬 수 있으므로 권장되지 않습니다.
