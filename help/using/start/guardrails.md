...
* 한 프로필의 여정 인스턴스 최대 크기는 1MB입니다. 여정 실행의 일환으로 수집한 모든 데이터는 해당 여정 인스턴스에 저장됩니다. 따라서 수신 이벤트의 데이터, Adobe Experience Platform에서 검색한 프로필 정보, 사용자 지정 작업 응답 등은 해당 여정 인스턴스에 저장되고 여정 크기에 영향을 줍니다. 여정이 이벤트로 시작하는 경우 여정 실행 시 몇 가지 활동 후에 위의 제한에 도달하지 않도록 해당 이벤트 페이로드의 최대 크기를 제한(예: 800KB 미만)하는 것이 좋습니다. 제한에 도달하면 프로필이 오류 상태가 되어 여정에서 제외됩니다.
* 각 프로필 및 여정 버전에 대해 여정 런타임에서는 하나가 처리되는 동안 최대 10개의 보류 중인 이벤트에 대한 내부 대기열을 유지합니다. 이 제한에 도달하면 스택이 소모될 때까지 `maxInstanceStackEventsReached` 이유와 함께 추가 이벤트가 무시됩니다. 차단된 여정 인스턴스로 인해 삭제된 [이벤트를 참조하십시오](../building-journeys/troubleshooting-execution.md#max-instance-stack-events-reached).
...
* Journey Optimizer에서 여정을 트리거하기 위해서는 이벤트를 Data Collection Core Service(DCCS)로 스트리밍해야 합니다. 일괄 수집된 이벤트, **쿼리 서비스**&#x200B;를 통해 삽입된 이벤트 또는 내부 Journey Optimizer 데이터 세트의 이벤트(메시지 피드백, 전자 메일 추적 등) 은(는) 여정을 트리거하는 데 사용할 수 없습니다. 스트리밍 이벤트를 가져올 수 없는 사용 사례의 경우에는 대신 이 이벤트를 기반으로 대상자를 작성하고 **대상자 읽기** 활동을 사용해야 합니다. 대상 자격은 기술적으로 사용할 수 있지만, 사용된 작업에 따라 다운스트림 문제를 일으킬 수 있으므로 권장되지 않습니다.
...
* **대상자 읽기** 활동은 여정의 첫 번째 활동이나 비즈니스 이벤트 활동 후에만 사용할 수 있습니다.
* 여정은 하나의 **대상자 읽기** 활동만 가질 수 있습니다.
* **대상자 읽기** 활동은 여정 당 한 대상자만 타깃팅할 수 있습니다. 여러 대상이 필요한 경우 먼저 단일 대상으로 병합합니다. [컴포지션 워크플로를 사용하여 대상자를 결합하는 방법을 알아보세요](../audience/get-started-audience-orchestration.md).
* 각 조직은 모든 샌드박스 및 여정에서 최대 5개의 **대상자 읽기** 인스턴스(예약됨 또는 비즈니스 이벤트 트리거됨)를 동시에 실행할 수 있습니다. 정확히 같은 시간에 시작하는 **대상자 읽기**&#x200B;를 사용하는 여정이 5개 이상 있는 것을 피하십시오. 5분에서 10분 간격으로 분산하십시오. [이 섹션](../building-journeys/entry-management.md#journey-processing-rate)에서 여정 처리 속도에 대해 자세히 알아보십시오.
* 샌드박스 처리량: 시스템은 모든 **대상 읽기** 활동에서 초당 최대 20,000개의 프로필을 공유하여 샌드박스당 처리를 관리합니다. 개별 활동은 초당 500~20,000개의 프로필로 구성할 수 있습니다. 샌드박스 제한에 도달하면 작업이 큐에 대기할 수 있습니다.
* 작업 처리 시간 초과: **대상자 읽기** 12시간 이내에 처리할 수 없는 작업은 자동으로 정리되며 실행되지 않습니다.
...
대상 읽기 활동에 대한 [권장 사항 및 구성](../building-journeys/read-audience.md#must-read)도 참조하세요.
