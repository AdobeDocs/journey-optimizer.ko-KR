...
여정을 저장하거나 게시할 때 Journey Optimizer은 안정성과 성능을 유지하기 위해 총 여정 페이로드 크기를 확인합니다.

**기본 구성**

* **기본 최대 요청 크기**: 2MB(2,000,000바이트). 일부 조직에는 Adobe으로 구성된 사용자 지정 제한이 있을 수 있습니다.
* **경고 임계값**: 최대 제한의 90%.
* **오류 임계값**: 최대 제한의 100%. 저장 또는 게시가 차단되고 요청에서 **HTTP 413 요청 엔터티가 너무 큼**&#x200B;을(를) 반환합니다.

**사용자 경험 시나리오**

* **페이로드가 제한의 90% 미만임**: 여정이 저장되고 게시되었습니다. 경고 또는 오류가 표시되지 않습니다.
* **페이로드가 제한의 90~99%**: 여정이 저장하고 게시되었습니다. 최적화하라는 경고가 표시됩니다. 경고 메시지: **경고**: 여정 페이로드 크기가 한도에 가깝습니다. 가장 큰 노드: &#39;[NodeName]&#39;(형식: &#39;[NodeType]&#39;, 크기: [N]바이트).
* **페이로드 >= 100% 제한**: 여정 저장 또는 게시가 오류와 함께 차단되었습니다. 오류 메시지: **오류**: 여정 페이로드 크기가 한도를 초과합니다. 가장 큰 노드: &#39;[NodeName]&#39;(형식: &#39;[NodeType]&#39;, 크기: [N]바이트).

**오류 응답 세부 정보**

요청이 최대 허용 크기를 초과하는 경우 응답이 **요청 엔터티가 너무 큼**&#x200B;을(를) 포함합니다. 여정 페이로드가 최대 허용 크기를 초과합니다. 오류 세부 정보를 검토하고 여정을 최적화합니다.

**문제 해결 및 권장 사항**

* 경고 또는 오류에서 강조 표시된 가장 큰 노드를 검토합니다.
* 조건을 단순화하고, 데이터 매핑을 줄이고, 불필요한 단계 또는 매개 변수를 제거합니다.
* 필요한 경우 여정을 더 작은 여정으로 분할하는 것이 좋습니다.
* 조직에서 더 높은 제한이 필요하다고 판단되는 경우 Adobe 담당자에게 문의하십시오.
