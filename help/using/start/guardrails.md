...
* **대상 자격**(첫 번째 활동)에서 선택한 대상 및 네임스페이스는 새 버전에서 변경할 수 없습니다.
...
* Journey Optimizer은 모든 샌드박스에서 단일 이벤트의 경우 초당 최대 **5,000개의 인바운드 여정 이벤트**&#x200B;를 지원하고 대상 읽기 기반 여정 이벤트의 경우 초당 **5,000개의 인바운드 여정 이벤트**&#x200B;를 지원합니다. [이 페이지](../event/about-events.md#event-thoughput)에서 이 제한에 대해 자세히 알아보십시오.
...
* 한 번에 최대 **25**&#x200B;개의 여정이 모든 라이브, 닫힘, 일시 중지, 테스트 모드 및 시험 실행 여정에서 단일 이벤트를 참조할 수 있습니다. 이 한도에 도달하면 해당 이벤트를 사용하는 추가 여정 게시가 차단됩니다.
* 단일 XDM 스키마는 모든 라이브, 닫힘, 일시 중지, 테스트 모드 및 시험 실행 여정에서 한 번에 최대 **100**&#x200B;개의 이벤트를 통해 참조할 수 있습니다. 이 한도에 도달하면 해당 스키마를 참조하는 여정 노드가 있는 이벤트 게시가 차단됩니다.
...
* 샌드박스는 모든 라이브, 닫힘, 일시 중지, 테스트 모드 및 시험 실행 여정에서 최대 **300**&#x200B;개의 대상 자격 활동을 포함할 수 있습니다. 이 제한은 종료 기준으로 사용되는 대상 자격 활동에도 적용됩니다. 이 한도에 도달하면 대상 자격 활동을 추가로 포함하는 여정 게시가 차단됩니다.
