...
* 여정을 게시하면 처리량과 안정성을 최대화하기 위해 자동으로 규모를 조절합니다. 한 번에 500개의 라이브 여정이 있는 이정표에 가까워지면 이 도전 과제에 대한 알림이 UI에 표시됩니다. 이 알림이 표시되고 여정을 한 번에 500개의 라이브 여정 이상으로 확장해야 하는 경우 고객 지원 센터에 대한 티켓을 만드십시오. 그러면 목표를 달성할 수 있도록 돕겠습니다.
