...
## 채널 및 메시징 {#channel-guardrails}

이 섹션에서는 이메일, SMS, 인바운드 채널(웹, 인앱, 코드 기반, 콘텐츠 카드) 및 트랜잭션 메시지를 포함한 모든 통신 채널의 가드레일을 다룹니다.
...
이 섹션에서는 랜딩 페이지, 하위 도메인 및 조각을 포함하여 콘텐츠 생성 및 관리를 위한 가드레일을 다룹니다.
...
## 대상 및 프로필 {#audiences-profiles}

이 섹션에서는 고객 관리, 프로필 처리 및 참여 가능한 프로필 고려 사항에 대한 가드레일을 다룹니다.
...
이 섹션에서는 일반 여정 제한 사항, 여정 구성 요소(작업, 이벤트, 데이터 소스), 여정 활동 및 사용자 지정 작업 및 표현식 편집기와 같은 특정 기능을 포함하여 여정에 대한 보호 기능 및 제한 사항에 대해 설명합니다.
