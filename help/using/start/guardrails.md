...
| 영역 | 제한 | 값 | 유형 |
|---|---|---|---|
| **여정** | 여정 당 최대 활동 수 | **50** | 한도 제한 |
| **여정** | 최대 라이브/일시 중지/드라이 실행 여정 | **100** | 소프트 제한(확장 가능) |
| **여정** | 여정 인스턴스 크기 | **1MB** | 한도 제한 |
| **여정** | 여정 페이로드 크기(게시) | **2MB**(90% 경고) | 한도 제한 |
| **여정** | 전역 여정 시간 제한 | **91일** | 한도 제한 |
| **여정** | 프로필당 보류 중인 이벤트 큐 | **10개 이벤트** | 한도 제한 |
| **여정** | 동시 읽기 대상 인스턴스 | 모든 샌드박스에서 **5** | 한도 제한 |
| **여정** | 대상 읽기 샌드박스 처리량 | **20,000개 프로필/초**(공유) | 한도 제한 |
| **여정** | 대상자 읽기 작업 시간 초과 | **12시간** | 한도 제한 |
| **채널** | 초당 인바운드 요청 수 | **5,000RPS** | 한도 제한 |
| **채널** | 최대 활성 인바운드 작업 | **500** | 한도 제한 |
| **채널** | 트랜잭션 메시지/초(캠페인) | **500** | 한도 제한 |
| **채널** | 초당 인바운드 여정 이벤트 | **5,000** | 한도 제한 |
| **사용자 지정 작업** | 분당 호출 수(응답 &lt; 0.75초) | 호스트/샌드박스당 **300,000 / 분** | 하드 제한(구성 가능) |
| **사용자 지정 작업** | 30초당 호출 수 (응답 > 0.75초) | 호스트/샌드박스당 **150,000 / 30초** | 하드 제한(구성 가능) |
| **콘텐츠** | 게시 시 이메일 메시지 콘텐츠 | **2MB**(1MB 미만 작성자) | 한도 제한 |
| **콘텐츠** | 인앱 메시지 콘텐츠 | **2MB** | 한도 제한 |
| **콘텐츠** | 시각적 조각 크기 | **100KB** | 한도 제한 |
| **콘텐츠** | 표현식 조각 크기 | **200KB** | 한도 제한 |
| **콘텐츠** | 여정 조각 노드 | **20개 노드/조각**, 200개 활성/샌드박스 | 한도 제한 |
| **대상자** | 샌드박스당 대상 구성 | **10** | 한도 제한 |
| **데이터 세트** | 프로필 저장소 TTL(새 조직/샌드박스) | **90일** | 한도 제한 |
| **데이터 세트** | 데이터 레이크 TTL(새 조직/샌드박스) | **13개월** | 한도 제한 |

>[!NOTE]
>
>**하드 제한(구성 가능)**(으)로 표시된 제한은 Adobe 담당자 또는 Adobe 고객 지원 센터에 문의하여 늘릴 수 있습니다.
