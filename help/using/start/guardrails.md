...
## SMS 보호 기능 {#sms-guardrails}

* MMS 기능은 Sinch에서만 사용할 수 있습니다.
* 지원되는 URL을 통해 MMS용 미디어 파일을 포함할 수 있습니다. 미디어 파일이 별도로 업로드되었는지 확인하십시오.
* 현재 MMS에서 메시지 피드백 동기화를 사용할 수 없습니다.
* 동의 관리는 MMS에 대한 SMS 채널 수준에서 작동합니다.
...
게재 처리량은 지정된 시간 내에 의사 결정 관리 앱 서비스에서 게재할 수 있는 의사 결정 응답 수에 해당합니다. 초당 결정 수는 아래 표에 표시되어 있다.

| API | 초당 결정 수 |
|---------|----------|
| Decisioning API 요청 | 초당 500 |
| Edge Decisioning API 요청 | 초당 5000 |

### 제한 사항 {#offers-limitations}

의사 결정 관리 제한 사항은 다음과 같습니다.

* **승인된 맞춤형 오퍼 + 대체 오퍼** - 승인된 개인화된 오퍼와 승인된 대체 오퍼를 결합하여 최대 10,000개.
* **결정** - 최대 10,000개의 결정
* **라이브 결정** - Offer decisioning 앱 서비스는 최대 1,000개의 라이브 의사 결정을 지원합니다.
* **응답당 반환된 오퍼** - Offer decisioning은 요청의 모든 결정 범위에서 요청당 최대 100개의 오퍼가 반환되도록 지원합니다.
* **컬렉션** - 최대 10,000개의 컬렉션
* **결정당 컬렉션** - 의사 결정당 최대 30개의 컬렉션
* **의사 결정 규칙 + 순위 함수** 최대 10,000개의 결합된 의사 결정 규칙 및 순위 함수.
* **배치** - 최대 1,000개의 배치.
* **결정당 배치** - 의사 결정당 최대 30개의 배치.
* **의사 결정당 순위 방법** - Offer decisioning 앱 서비스는 의사 결정당 최대 30개의 순위 함수를 지원합니다.
* **AI 순위 모델** - Offer decisioning 앱 서비스는 최대 5개의 AI 등급 모델을 지원합니다.
* **오퍼 또는 컬렉션별 컬렉션 한정자** - Offer decisioning 앱 서비스는 단일 개인화된 오퍼 또는 단일 컬렉션에서 최대 20개의 컬렉션 구분자를 지원합니다.
* **총 컬렉션 한정자** - Offer decisioning 앱 서비스는 최대 1,000개의 컬렉션 한정자를 지원합니다.