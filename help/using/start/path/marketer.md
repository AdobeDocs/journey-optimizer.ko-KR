...
데이터 및 대상 구성에 대해 [데이터 엔지니어](data-engineer.md)와 공동 작업:

* 개인화 및 세분화를 위해 새 계산된 속성 요청
* 오케스트레이션된 캠페인에 대한 관계형 스키마 조정
* 대상 품질 및 데이터 정확도에 대한 피드백 제공
* 고급 세분화를 위한 다중 엔티티 데이터 요구 사항 충족

>[!TAB 개발자와 작업]

이벤트 추적 및 구현에 대해 [개발자](developer.md)와(과) 공동 작업:

* 여정 이벤트를 트리거해야 하는 사용자 상호 작용 조정
* 시작 전 모바일 및 웹 구현 테스트
* 컨텐츠 성능 및 사용자 참여에 대한 추적 유효성 검사
* 메시지 게재 또는 개인화 문제 해결
