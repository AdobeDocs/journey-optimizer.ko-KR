...
**이 페이지에서:** 구매 또는 충성도 마일스톤과 같은 특정 사용자의 행동을 기반으로 여정을 실시간으로 트리거할 수 있도록 단일 이벤트를 구성하십시오.
...
단일 이벤트는 특정 프로필에 연결됩니다. 단일 이벤트는 규칙 기반이거나 시스템 생성일 수 있습니다.  단일 이벤트 [이 섹션](../event/about-events.md)에 대해 자세히 알아보세요.

다음은 새 이벤트를 구성하는 첫 번째 단계입니다.

1. 관리 메뉴 섹션에서 **[!UICONTROL 구성]**(으)로 이동하고 **[!UICONTROL 이벤트]** 섹션에서 **[!UICONTROL 관리]**&#x200B;를 클릭합니다. 그러면 이벤트 목록이 표시됩니다.
...
이벤트가 수신될 때, 키의 값은 시스템이 이벤트와 연관된 사람을 식별할 수 있게 한다. [ID 형식](../event/about-creating.md#select-the-namespace)과(와) 연결된 키를 사용하여 Adobe Experience Platform에서 쿼리를 수행할 수 있습니다. [이 페이지](../building-journeys/about-journey-activities.md#orchestration-activities)를 참조하십시오.
