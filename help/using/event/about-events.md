...
이벤트 구성에서 여정에서 예상되는 이벤트를 구성합니다. 수신되는 이벤트 데이터는 Adobe Experience 데이터 모델(XDM)에 따라 표준화됩니다. 이벤트는 인증된 이벤트와 인증되지 않은 이벤트(예: Adobe Mobile SDK 이벤트)를 위한 수집 API 스트리밍에서 옵니다. 여정의 각 단계에서 여러 이벤트를 사용할 수 있으며 여러 여정에서 같은 이벤트를 사용할 수도 있습니다.
...
## 단일 이벤트와 비즈니스 이벤트 개요 {#event-comparison}
...
1. 이름이 `newYorkPurchases`인 이벤트 **E1**&#x200B;이(가) `city=='New York'`을(를) 나타내는 규칙 정의로 만들어졌습니다. 이 이벤트는 10여정에서 사용될 수 있지만, 표시될 때 1개의 이벤트로만 계산됩니다.

1. 이제 **E1**&#x200B;과(와) 동일한 이벤트 스키마에 `purchaseValue > 1000`을(를) 규칙 정의로 사용하는 이름이 `highValuePurchases`인 이벤트 **E2**&#x200B;도 생성된다고 가정해 보겠습니다. 이 경우 두 규칙 `newYorkPurchases` 및 `highValuePurchases`에 대해 동일한 들어오는 이벤트가 평가됩니다. 이제 뉴욕 구매도 높은 가치의 구매가 될 수 있습니다.
