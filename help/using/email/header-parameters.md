...
## 발신자 헤더 {#sender-header}

>[!CONTEXTUALHELP]
>id="ajo_admin_preset_sender_header"
>title="발신자 헤더"
>abstract="전송 엔티티(발신자)가 작성 엔티티(작성자)와 다른 경우(예: 상위 기업이 하위 브랜드에 대한 메시지를 발송하는 경우 또는 여러 고객을 위한 메시지를 보내는 에이전시) 이러한 옵션 필드를 사용합니다. 이를 지원하는 이메일 클라이언트는 일반적으로 이를 &#39;작성자를 대신하여 전송한 발신자&#39;로 표시하거나 &#39;via&#39;로 표시합니다."
