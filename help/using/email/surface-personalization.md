...
1. [개인화 편집기](../personalization/personalization-build-expressions.md)가 열립니다. 원하는 대로 조건을 정의하고 변경 사항을 저장합니다.
   ...
   <!--
아래 예제에서는 현재 사용할 수 없는 이벤트 매개 변수를 사용합니다.

    영업 도우미를 대신하여 전송된 동적 전자 메일을 처리하고 이벤트 또는 캠페인 컨텍스트 매개 변수에서 영업 도우미를 검색한다고 가정해 보겠습니다. 예를 들어, [여정](../building-journeys/journey-gs.md)에서 구매 이벤트가 특정 매장의 판매 도우미에 연결되면 이벤트 속성에서 가져온 판매 도우미 매개 변수로 이메일 헤더(발신자 이름, 발신자 이메일, 회신 주소)를 개인화할 수 있습니다. 영업 도우미가 외부에서 시작한 [API 트리거된 캠페인](../campaigns/api-triggered-campaigns.md)에서는 영업 도우미와 캠페인 컨텍스트 매개 변수에서 가져온 헤더 개인화 값을 대신하여 트리거된 이메일을 보낼 수 있습니다.
-->
