...
이 기능을 사용하면 자동 생성된 텍스트나 관련 없는 컨텍스트가 아닌 AI 지원 받은 편지함 경험에서 오퍼, 작업 호출 및 의도한 세부 정보를 표시할 수 있도록 전용 텍스트 버전의 메시지를 세분화할 수 있습니다.
...
이러한 문제를 방지하기 위해 [!DNL Journey Optimizer]은(는) 쿠폰, 할인 범위, 콜 투 액션 및 기타 우선 순위가 명확한 선형 복사본에 맨 앞에 표시되도록 추가 텍스트 버전의 메시지를 만듭니다.

>[!NOTE]
>
>이 전용 텍스트 버전은 메시지의 기본 또는 사용자 지정 일반 텍스트 버전과 동일하지 않습니다. [자세히 알아보기](text-version-email.md)
...
>
>AI 받은 편지함에 대한 이메일 텍스트 최적화 기능은 Journey Optimizer에서 전용 텍스트 버전만 생성합니다. 타사 도우미가 메시지를 해석하거나 표시하는 방식을 보장하지는 않습니다. 타사 받은 편지함 AI의 [제한 사항 및 위험](#inbox-ai-risks)에 대해 자세히 알아보세요.
...
* **받은 편지함 Q&amp;A 제어** — 받는 사람이 도우미에게 *이메일이 무엇인지* 또는 *오퍼가 무엇인지*&#x200B;를 물을 것으로 예상할 때 강력한 ai 받은 편지함 버전을 사용하면 부분 요약이 줄어들고 승인된 복사본에 연결되지 않은 웹 보충 답변을 사용하지 않습니다.
...
1. 생성된 버전이 표시됩니다.
   ...
   >[!NOTE]
   >
   >**AI 받은 편지함에 대한 이메일 텍스트 최적화**&#x200B;은(는) HTML 디자인, 레이아웃 또는 이미지를 변경하지 않습니다.

1. 자동으로 생성된 콘텐츠를 변경하려면 **[!UICONTROL 편집 사용]** 토글을 선택하고 필요에 따라 콘텐츠를 수동으로 편집하십시오.

1. 버전이 마음에 들면 **[!UICONTROL 전자 메일 최적화]** 단추를 클릭하여 확인하세요.

1. 이제 이메일이 AI 받은 편지함에 성공적으로 최적화되었습니다.

1. 최적화된 버전에 액세스하거나 편집하려면 **[!UICONTROL AI 받은 편지함에 맞게 최적화]** 단추를 클릭하세요.
...
1. 최적화된 버전이 표시됩니다. **[!UICONTROL 최적화를 제거]**&#x200B;하거나 **[!UICONTROL 다시 최적화]**&#x200B;을 클릭하여 새 버전을 생성할 수 있습니다.
   ...
   >[!NOTE]
   >
   >원래 HTML 콘텐츠를 변경하는 경우 AI 받은 편지함에 대한 버전을 다시 최적화해야 합니다.
...
* **브랜드 및 법적 노출** — 잘못되거나 불완전한 AI 요약으로 인해 여전히 고객 혼란 또는 프로모션, 약관 또는 옵트아웃 언어에 대한 분쟁이 발생할 수 있습니다. **AI 받은 편지함에 대한 전자 메일 최적화**&#x200B;는 타사 모델이 사용자의 전자 메일의 최적화된 버전을 충실하게 재현하지 못하도록 합니다.
