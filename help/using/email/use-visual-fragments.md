...
### 조각에서 동적 콘텐츠를 사용할 때의 제한 사항 {#fragment-dynamic-content}

>[!CAUTION]
>
>다이내믹 콘텐츠(조건부 콘텐츠)가 포함된 조각을 사용하여 작업할 때 다음 제한 사항에 유의하십시오.
>
>**동적 내용이 포함된 조각 중첩은 지원되지 않습니다.** 동적 콘텐츠가 포함된 잠금 해제된 조각 안에 동적 콘텐츠가 포함된 조각을 배치할 수 없습니다. 이 지원되지 않는 구성으로 인해 다음과 같은 문제가 발생할 수 있습니다.
>
>* 조건부 콘텐츠 매핑 손실
>* 이메일 Designer의 호환성 모드 경고
>* 일관성 없는 이메일 렌더링
>
>**권장 방법:** 전자 메일에서 동적 콘텐츠와 함께 여러 조각을 사용하는 경우 각 조각을 전자 메일 수준의 자체 구조 블록에 직접 추가합니다. 이렇게 하면 기능이 적절히 설정되고 위에서 언급한 문제가 방지됩니다.

## 다이내믹 콘텐츠가 있는 조각에 대한 우수 사례 {#fragment-best-practices}

시각적 조각 및 다이내믹 콘텐츠(조건부 콘텐츠) 작업 시 다음 모범 사례를 따르십시오.

* **전자 메일을 올바르게 구조화**: 다이내믹 콘텐츠가 포함된 조각으로 전자 메일을 빌드하는 경우 각 조각을 전자 메일 수준의 전용 구조 블록에 추가하십시오. 동적 콘텐츠가 포함되어 있는 잠금 해제된 다른 조각 내에 동적 콘텐츠가 있는 조각을 중첩하지 마십시오.

* **미리 계획하기**: 전자 메일에 조각을 추가하기 전에 동적 콘텐츠가 포함된 조각을 식별하고 그에 따라 레이아웃을 계획합니다. 이렇게 하면 구성 문제를 방지하고 처음부터 깔끔한 구조를 만들 수 있습니다.

* **재사용 가능한 조각을 신중하게 디자인**: 다이내믹 콘텐츠를 포함할 조각을 만들 때 사용 방법을 고려하십시오. 조각을 다른 조각 내에 중첩해야 하는 경우 다이내믹 콘텐츠를 상위 조각과 하위 조각 모두에 추가하지 마십시오.

* **문제 해결**: 조건부 콘텐츠 매핑 또는 호환성 모드 경고가 손실된 경우:
   * 이메일 구조에 다이내믹 콘텐츠가 포함된 중첩된 조각이 있는지 확인
   * 다이내믹 콘텐츠가 포함된 각 조각을 이메일 수준에서 자체 구조 블록으로 이동하여 재구성
   * 조건부 콘텐츠 매핑이 제대로 복원되었는지 저장 및 확인
