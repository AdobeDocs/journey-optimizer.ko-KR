...
>[!AVAILABILITY]
>
>Journey Optimizer의 라이브 활동은 iOS과만 호환됩니다.

라이브 활동은 모바일 앱 내에서 실시간 업데이트와 대화형 환경을 제공하여 사용자가 디바이스의 화면에서 직접 진행 중인 이벤트 또는 작업에 대한 정보를 유지할 수 있도록 합니다.

이 기능은 사용자가 앱을 열지 않아도 진행 추적, 이벤트 업데이트 또는 대화형 컨텐츠와 같은 라이브 정보를 전달하여 참여를 향상시킵니다.
...
* 대상 기반 캠페인에 대해 **API 트리거된 마케팅** 선택
  ...
  스포츠 점수, 이벤트 업데이트, 공유 경험 등 동일한 업데이트가 여러 사용자에게 전송되는 대상 또는 세그먼트 기반 커뮤니케이션용으로 설계되었습니다.

* 개별 캠페인에 대해 **API 트리거 트랜잭션** 선택

  프로필로 식별되는 의도한 개별 사용자(예: 주문 상태, 게재 추적).

## 빠른 시작 안내서

아래 단계를 완료하여 애플리케이션에서 라이브 활동을 구성하고 구현하십시오.

1. **[Adobe Journey Optimizer 구성](mobile-live-configuration.md)**

   모바일 구성을 만들어 환경을 설정합니다.

1. **[Adobe Experience Platform Mobile SDK 통합](mobile-live-configuration-sdk.md)**

   Adobe Experience Platform Mobile SDK과 통합하여 잠금 화면 및 Dynamic Island에서 실시간 동적 업데이트를 가능하게 합니다.
