...
| 증상 | 사용 사례 | 다음으로 이동 |
|---------|----------|-------|
| API가 200 OK를 반환하지만 라이브 활동이 디바이스에 표시되지 않음 | 모두 | [시나리오 1: 프로필 또는 푸시 토큰 문제](#scenario-1-profile-or-push-token-issues) |
| 라이브 활동이 표시되지만 업데이트되거나 종료되지 않음 | 단일(1:1) | [시나리오 4: 업데이트 토큰이 동기화되지 않음](#scenario-4-live-activity-update-token-not-synced) |
| 캠페인과 토큰이 올바르게 보이지만 전달이 여전히 실패합니다 | 모두 | [시나리오 3: 게재 실패 및 오류 분석](#scenario-3-delivery-failures-and-error-analysis) |
| 페이로드 구성 또는 API 구조가 명확하지 않음 | 모두 | [시나리오 2: 캠페인 구성 및 페이로드 문제](#scenario-2-campaign-configuration-and-payload-issues) |
| 특정 대상 멤버가 브로드캐스트를 받지 못함 | 브로드캐스트 | [시나리오 7: 프로필이 대상 또는 오래된 스냅숏에 없습니다](#scenario-7-profile-not-in-audience-or-stale-audience-snapshot) |
| 프로그래밍 방식으로 실행 상태를 확인해야 함 | 단일(1:1) | [시나리오 5: API를 통해 실행 상태 확인](#scenario-5-checking-execution-status-via-the-api) |
| Assurance 액세스 없음, 프로덕션 수준 디버깅 필요 | 모두 | [고급: 데이터 세트 쿼리를 통해 디버깅](#advanced-debugging-via-dataset-queries) |

## 두 가지 사용 사례 이해

디버깅하기 전에 캠페인에 적용되는 사용 사례를 확인하십시오. 근본 원인과 디버깅 경로는 두 경로 간에 상당한 차이가 있습니다.

| | 단일(1:1) | 브로드캐스트 |
|---|---|---|
| **AJO 캠페인 유형** | API 트리거 트랜잭션 | API 트리거된 마케팅 |
| **타깃팅** | `recipients[].userId`을(를) 통한 개별 프로필 | `audience.id`을(를) 통한 대상 세그먼트 |
| **시작할 토큰** | Push-to-start 토큰(프로필별) | `input-push-channel`(브로드캐스트 인스턴스당) |
| **업데이트할 토큰/끝낼 토큰** | 토큰 업데이트(라이브 활동 인스턴스당) | 시작과 동일한 `input-push-channel` |
| **대상 새로 고침 위험** | 적용할 수 없음 | 최대 24시간 부실(일괄 처리 평가) |

## 용어 용어집

| 용어 | 정의 |
|------|------------|
| **Push-to-Start 토큰** | iOS 17+에서 생성한 APNs 토큰으로, 앱을 열지 않고도 AJO에서 장치에서 라이브 활동을 원격으로 시작할 수 있습니다. Mobile SDK에서 등록하고 AEP 프로필에 저장합니다. |
| **토큰 업데이트** | 장치에서 라이브 활동이 시작될 때 생성되는 인스턴스당 APNs 토큰입니다. 단일 `update` 및 `end` 이벤트에 필요합니다. 각 라이브 활동 인스턴스에는 고유한 업데이트 토큰이 있습니다. |
| **입력-푸시-채널** | 브로드캐스트 라이브 활동을 위해 Apple Developer Portal에 생성된 고유 채널 식별자. 브로드캐스트 업데이트 토큰 역할을 하며, 이 채널을 구독하는 모든 디바이스는 동일한 라이브 활동 이벤트를 수신합니다. |
| **liveActivityData** | Adobe SDK `LiveActivityAttributes` 프로토콜의 필수 속성입니다. 단일 항목의 경우 `liveActivityID`을(를) 포함하고, 브로드캐스트의 경우 `channelID`을(를) 포함합니다. 시작 이벤트 페이로드의 `attributes` 필드에 포함되어야 합니다. |
| **브로드캐스트 채널 ID** | `input-push-channel`의 값입니다. 브로드캐스트 페이로드의 `liveActivityData.channelID`과(와) 정확히 일치해야 합니다. |
| **attributeType / attributes-type** | Swift `ActivityAttributes` 구조체의 이름입니다. AEP 프로필 속성에 `attributeType`(camelCase)로 저장되고 APS JSON 페이로드에서 `attributes-type`(하이픈이 연결됨)로 전송됩니다. 이는 다른 표현에서 동일한 값입니다. |
| **liveActivityPushNotificationDetails** | 장치의 Push-To-Start 토큰 `appId`, `platform` 및 `attributeType`을(를) 저장하는 AEP 프로필 특성입니다. 원격 시작이 작동하려면 이(가) 있어야 하며 유효해야 합니다. |
| **APS 페이로드** | `context.requestPayload.aps` 아래의 API 요청 내에서 전송된 JSON 구조입니다. 라이브 활동 이벤트, `content-state`, `attributes` 및 제어 필드를 포함합니다. |
| **보증** | 연결된 테스트 장치를 위한 실시간 디버깅 도구인 Adobe Experience Platform Assurance 프로덕션 최종 사용자 디바이스에는 사용할 수 없습니다. |

## 사전 요구 사항

문제 해결 전에 다음을 확인하십시오.

+++ 라이브 활동에 대한 Assurance 플러그인

Adobe Experience Platform Assurance의 라이브 활동 보기는 앱의 설정을 확인하고 활동 이벤트를 검사하며 테스트 세션에서 활동을 원격으로 시작, 업데이트 또는 종료할 수 있습니다.

>[!IMPORTANT]
>
> Assurance 세션은 테스트 및 QA 디바이스에만 해당됩니다. 최종 사용자 프로덕션 장치가 Assurance에 연결되어 있지 않습니다. 프로덕션 진단의 경우 이 안내서 끝에 있는 [고급: 데이터 세트 쿼리를 통한 디버깅](#advanced-debugging-via-dataset-queries) 섹션을 사용하십시오.

### 요구 사항

| 요구 사항 | 세부 사항 |
|---|---|
| iOS 장치 | iOS 16.1 이상을 실행하는 물리적 디바이스 |
| Xcode 시뮬레이터 | iOS 16.1 또는 최신 **로컬 시작 전용**, APNs를 통한 원격 푸시는 시뮬레이터에서 지원되지 않습니다. |
| Assurance에서 원격 시작 | iOS 17.1 이상, 유효한 Push-to-Start 토큰 및 유효한 채널 구성 |
| Mobile SDK | Adobe Experience Platform Mobile SDK 5.11.0 이상 |
| Session | 활성 Assurance 세션 |

### 탭 3개

| 탭 | 표시 내용 |
|-----|---------------|
| **클라이언트 정보** | 장치, 프로필 및 App Store 자격 증명. 녹색 확인 = 올바르게 구성됨, 인라인 경고 = 제안된 수정 사항 관련 문제. 아래 각 시나리오의 사전 검사에 직접 매핑됩니다. |
| **활동** | 선택한 클라이언트에 대한 라이브 활동: 유형(단일/브로드캐스트), ID, 상태 및 이벤트 수입니다. 하위 탭: 개요(기본 정보 + 업데이트 보내기 버튼), 활동 흐름(검사 가능한 페이로드가 있는 라이프사이클 타임라인), 이벤트 세부 사항(이벤트당 전체 페이로드 검사). |
| **이벤트** | 라이브 활동 시작, 업데이트 및 종료 이벤트를 포함한 클라이언트의 모든 Assurance 이벤트. |

### Assurance 시작, 업데이트 및 종료

**라이브 활동 시작**. 등록된 활동 유형을 선택하고, 단일 또는 브로드캐스트를 선택하고, 브로드캐스트 채널 ID(브로드캐스트 전용)를 입력하고, 미리 채워진 APS JSON 페이로드를 편집하는 대화 상자를 엽니다. 시작-푸시 토큰, iOS 버전 또는 채널 구성 요구 사항이 충족되지 않으면 버튼이 비활성화되거나 오류를 반환합니다.

**업데이트 보내기**. 기존 활동의 개요 탭에서 업데이트(새 콘텐츠 푸시) 또는 종료 이벤트를 보냅니다. 단일 플러그인의 경우 플러그인은 활동의 업데이트 토큰을 자동으로 사용합니다. 브로드캐스트의 경우 동일한 브로드캐스트 채널 ID를 구독한 모든 디바이스를 타겟팅합니다. 페이로드는 유효한 JSON이어야 하며 활동의 속성 스키마와 일치해야 합니다. 그렇지 않으면 요청이 거부됩니다.

설정 및 세션 연결 단계는 [Adobe Experience Platform Assurance 설명서](https://experienceleague.adobe.com/en/docs/experience-platform/assurance/home.html)를 참조하세요.
...
* 캠페인 이름 및 ID
* Campaign 버전(해당되는 경우)
* 캠페인 유형: **트랜잭션**(단일) 또는 **마케팅**(브로드캐스트)
* 표면 구성: 라이브 활동에 사용되는 iOS 앱 표면
* 활동 유형: 캠페인에 구성된 `AttributeType` 구조체 이름
...
### 시나리오 1: 프로필 또는 푸시 토큰 문제 {#scenario-1-profile-or-push-token-issues}
...
+++ &#x200B;1. Adobe Experience Platform에 프로필이 있는지 확인
...
### 시나리오 2: 캠페인 구성 및 페이로드 문제 {#scenario-2-campaign-configuration-and-payload-issues}
...
**브로드캐스트 사용 사례에 대한 참고 사항**: 캠페인은 **API로 트리거된 마케팅**(트랜잭션 아님)이어야 합니다. 페이로드에서 개별 `profile` 대신 `audience`을(를) 사용합니다. 브로드캐스트별 페이로드 구조는 [이 섹션](#broadcast-config)을 참조하고, 전체 API 사양은 [Adobe Developer 설명서](https://developer.adobe.com/journey-optimizer-apis/references/messaging#operation/postIMAudienceMessageExecution)를 참조하십시오.

#### 사전 확인

* Campaign은 **API 트리거 트랜잭션**(단일) 또는 **API 트리거 마케팅**(브로드캐스트)이며 **높은 처리량** 옵션은 **활성화되지 않음**&#x200B;이어야 합니다. 실시간 활동과 호환되지 않기 때문입니다.
* [위의 시나리오](#scenario-1-profile-or-push-token-issues)을(를) 사용하여 프로필이 있고 토큰이 올바르게 동기화되었는지 확인하십시오.
...
1. **라이브 활동 구성**&#x200B;을 확인하세요. 프로필 `liveActivityPushNotificationDetails`의 `appId`과(와) 일치하는 번들 식별자로 iOS 앱에 대한 표면을 구성해야 합니다. 예를 들어 프로필에 `"appId": "com.example.myapp"`이(가) 있는 경우 표면이 동일한 앱을 대상으로 해야 합니다.
1. 캠페인 구성의 **활동 유형**&#x200B;이(가) 프로필 `liveActivityPushNotificationDetails`의 `attributeType`과(와) 정확히 일치하는지 확인하십시오. 예를 들어 프로필에 `"attributeType": "FoodDeliveryLiveActivityAttributes"`이(가) 있는 경우 캠페인에서 이와 동일한 활동 유형을 지정해야 합니다.
...
### 시나리오 3: 게재 실패 및 오류 분석 {#scenario-3-delivery-failures-and-error-analysis}

[!BADGE 단일 및 브로드캐스트 사용 사례에 모두 적용]{type=Positive}

이 시나리오에서는 모든 이전 검사를 통과했습니다.

* 프로필이 [유효한 라이브 활동 푸시 토큰](#scenario-1-profile-or-push-token-issues)에 있음
* 캠페인이 올바르게 [적절한 페이로드로 구성됨](#scenario-2-campaign-configuration-and-payload-issues)
* [업데이트 토큰이 동기화됨](#scenario-4-live-activity-update-token-not-synced)(업데이트/종료 이벤트의 경우 단일 사용 사례만 해당)
...
1. 다음 섹션을 검토하십시오.
   ...
   1. 게재 성공을 이해하려면 **전송 통계** 지표를 확인하십시오.
      ...
      | 지표 | 의미 | 찾을 내용 |
      |-|-|-|
      | 타깃팅됨 | 대상자에 적합한 프로필 수 | 테스트 프로필을 포함해야 함 |
...
+++ &#x200B;3. Assurance에서 APNs에 라이브 활동 전달 확인
...
### 시나리오 4: 라이브 활동 업데이트 토큰이 동기화되지 않음 {#scenario-4-live-activity-update-token-not-synced}
...
* **프로필 및 캠페인 유효성 검사**: 프로필, 토큰 및 캠페인 구성이 올바른지 확인하기 위한 [시나리오 1](#scenario-1-profile-or-push-token-issues) 및 [시나리오 2](#scenario-2-campaign-configuration-and-payload-issues) 검사를 완료합니다.
...
+++ &#x200B;3. Assurance에서 라이브 활동 전달 이벤트 확인
...
### 시나리오 5: API를 통해 실행 상태 확인 {#scenario-5-checking-execution-status-via-the-api}

[!BADGE 단일(1:1) 사용 사례에만 적용]{type=Informative}

단일 라이브 활동을 트리거한 후 **GET 메시지 실행 API**&#x200B;는 현재 실행 상태를 반환합니다. 피드백 이벤트가 데이터 세트에 도착할 때까지 기다리지 않고 실행이 큐에 있는지, 진행 중인지, 완료되었는지 또는 실패했는지 여부를 확인하는 데 사용합니다.

`executionId`은(는) 트리거 응답에서 반환된 메시지 ID입니다. 단일 실행의 경우 앞에 `HUOC-`이(가) 붙습니다.

#### 엔드포인트 및 예제 요청

**끝점**: `GET https://cjm.adobe.io/imp/message/executions/{executionId}`
...
| 코드 | 의미 |
|------|---------|
| 200개 | 실행이 발견되었으며 정상적으로 반환됨 |
| 401 승인되지 않음 | 누락되었거나 잘못된 전달자 토큰 |
| 403 금지 | 토큰은 유효하지만 권한이 충분하지 않음 |
| 404 찾을 수 없음 | 실행 ID가 존재하지 않음 |

#### 실행 상태 값

200 응답의 `status` 필드는 실행 진행률을 나타냅니다.

| 상태 | 설명 |
|--------|-------------|
| `PENDING` | 실행이 큐에 있지만 아직 시작되지 않음 |
| `INPROGRESS` | 실행이 현재 처리 중입니다. |
| `COMPLETED` | 실행이 정상적으로 완료됨 |
| `FAILED` | 실행 도중 오류 발생 |

200 응답도 실행을 트리거하는 캠페인 또는 여정에 다시 연결하는 메타데이터(`campaignId`, `journeyId`, `batchInstanceId`, 에셋 정보)가 있는 `source` 블록, `executionType`(`unitary` 또는 `batch`), `executionRunMode`(`default` 또는 `test`)을 반환합니다.

## 브로드캐스트 특정 시나리오

### 시나리오 6: 브로드캐스트 캠페인 구성 및 페이로드 문제{#broadcast-config}

[!BADGE 브로드캐스트 사용 사례에만 적용]{type=Informative}
...
| `input-push-channel` | 브로드캐스트에 필요 - 이 브로드캐스트 인스턴스의 고유 식별자 | 누락되었거나 `liveActivityData`의 `channelID`과(와) 일치하지 않습니다. |
| `timestamp` | 항상 현재/최신 Unix Epoch 시간(초)이어야 함 | 이전/캐시된 타임스탬프 사용 |
| `event` | `"start"`, `"update"` 또는 `"end"`이어야 합니다. | 잘못된 이벤트 유형 |
...
+++ &#x200B;3. 컨텐츠 상태, 속성 및 입력-푸시 채널을 iOS 구현에 맞게 조정

페이로드 필드가 iOS 앱의 `ActivityAttributes` 구현과 일치하고 `input-push-channel`이(가) `liveActivityData`의 `channelID`과(와) 일치하는지 확인하십시오.
...
* `aps`의 루트에 있는 `input-push-channel` 값은 `liveActivityData`의 `channelID`과(와) 정확히 일치해야 합니다.
...
* 모든 이벤트 유형에 대해 `content-state`에 모든 `ContentState` 필드를 포함하십시오.
* 시작 이벤트에 대해서만 `attributes`의 모든 사용자 지정 `LiveActivityAttributes` 필드를 포함합니다.
...
### 시나리오 7: 프로필이 대상 또는 부실 대상 스냅샷에 없음 {#scenario-7-profile-not-in-audience-or-stale-audience-snapshot}
...
| 방법 | 평가 빈도 | 데이터 새로 고침 | 다음에 대한 우수 사례 |
|-|-|-|--|
...
## 고급: 데이터 세트 쿼리를 통한 디버깅 {#advanced-debugging-via-dataset-queries}

>[!BEGINSHADEBOX]

**대상**: 개발자 및 데이터 엔지니어 Adobe Experience Platform 쿼리 서비스를 통해 Journey Optimizer 데이터 세트에 대한 SQL 액세스 권한이 필요합니다.

**사용해야 하는 경우**: Assurance은 활성 세션 중에 연결된 테스트 장치로 제한됩니다. 데이터 세트 쿼리를 사용하여 프로덕션 최종 사용자가 보고한 문제를 조사하거나 사후 게재 내역을 감사할 수 있습니다. 데이터 세트는 Assurance 플러그인과 동일한 라이프사이클 및 실패 정보를 노출합니다.

>[!ENDSHADEBOX]

### 데이터 세트 찾기

1. Journey Optimizer에서 **데이터 관리** `>` **데이터 세트**(으)로 이동합니다.

1. **시스템 데이터 세트 표시** 토글을 사용하도록 설정하고 **AJO 메시지 피드백 이벤트 데이터 세트**&#x200B;를 엽니다.

세부 정보 페이지에 표시된 정확한 테이블 이름을 확인합니다. 아래 쿼리는 `ajo_message_feedback_event_dataset`을(를) 사용합니다. 다른 경우 실제 테이블 이름으로 바꾸십시오.

### 라이브 활동 ID별 쿼리

라이브 활동 ID(예: 주문 UUID 또는 배송 추적 ID)를 알고 전체 라이프사이클에서 모든 피드백 이벤트를 연결하려는 경우 이 ID를 사용합니다.
...
영향을 받는 프로필의 ECID를 알 때 사용합니다. `<YOUR_ECID>`을(를) 프로필에서 검색한 ECID 값으로 바꿉니다.
...
| 값 | 의미 |
|-------|---------|
| `sent` | APNs에 전달 — 장치 렌더링을 확인하지 않습니다(아래 참고 참조). |
| `error` | 게재 오류 — 자세한 내용은 `failure_reason` 검사 |
| `exclude` | 보내기 전에 제외된 프로필(토큰 누락, 동의 문제 또는 유형화 규칙) |
| `delay` | 게재 지연, 재시도 예정 |

>[!NOTE]
>
> `la_event` 필드의 경우 데이터 집합은 초기 게재 이벤트에 대해 `start`이(가) 아닌 `remotestart`을(를) 기록합니다. 이벤트 유형별로 쿼리할 때 그에 따라 필터링합니다.
