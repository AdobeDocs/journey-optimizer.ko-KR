...
**[!UICONTROL 일정 열기]** 아이콘을 선택하여 도전 실행 시기를 구성합니다.
...
1. 챌린지가 완료된 것으로 간주되는 시기를 지정합니다. 사용 가능한 설정은 과제 유형에 따라 다릅니다.
   ...
   **[!UICONTROL 작업 완료 요구 사항]** - 다음 중 선택:

   * **[!UICONTROL 고객이 완료할 작업 1개 선택]**: 고객은 보상 받을 단일 작업을 선택하고 완료할 수 있습니다.
   * **[!UICONTROL 고객이 특정 수의 작업을 완료합니다]**: 고객은 정의된 수의 작업을 완료해야 합니다. 필요한 수를 지정하십시오. *예: 작업 5개 중 3개를 완료하십시오*
   ...
   * **[!UICONTROL 연속 형식]**:
      ...
      * **연속**: 고객은 연속해서 며칠씩 쉬지 않고 작업을 완료해야 합니다. *예: 월요일, 화요일, 수요일에 구매 - 연속 중단되는 일 누락*

      * **연속적이지 않음**: 고객은 완료 사이에 간격을 두고 작업을 완료할 수 있습니다. *예: 30일 동안 7회 구매 완료, 휴식 시간 허용*

   * **[!UICONTROL 연속 길이]**: 작업을 완료해야 하는 횟수를 지정합니다. *예: &quot;7일 연속 구매&quot;에 대해 7로 설정*
   ...
   * **[!UICONTROL 고객이 완료할 작업 1개 선택]**: 고객은 보상 받을 단일 작업을 선택하고 완료할 수 있습니다.
   * **[!UICONTROL 고객이 특정 수의 작업을 완료합니다]**: 고객은 사용자가 정의한 정확한 순서로 정의된 수의 작업을 완료해야 합니다. 작업이 누락되거나 건너뛸 경우 시퀀스가 중단됩니다. 필요한 수를 지정합니다(예: 5개 작업 중 3개 완료).

   *예: 작업 1(구매) → 작업 2(검토) → 작업 3(공유) - 이 순서로 완료해야 함*

   +++

1. 기본적으로 표준 및 순차적 과제는 고객이 여러 트랜잭션에서 작업을 완료할 수 있도록 해 줍니다. 모든 작업을 한 번의 트랜잭션으로 완료하도록 하려면 ![](assets/do-not-localize/settings-icon.svg) **[!UICONTROL 설정]** 아이콘을 선택하고 아래 옵션을 켜십시오.
...
리워드는 고객이 도전을 완료하여 얻는 충성도 포인트 또는 혜택입니다. 보상이 전달되는 시기와 방법을 구성합니다.

1. **[!UICONTROL 보상 게재]** 드롭다운에서 보상을 게재할 시기를 선택합니다.
...
1. 선택한 게재 방법에 따라 보상 금액을 구성합니다.
   ...
   **예**: 첫 번째 작업을 완료하면 고객에게 10점을 부여하고, 두 번째 작업을 완료하면 50점을 추가하여 도전이 완료되면 총 60점을 부여합니다.
...
1. 각 단계에 대해 **[!UICONTROL [단계] 메시지 추가]**(여기서 [단계]은(는) 시작, 진행 중 또는 완료를 나타냄)을 선택하여 해당 단계에 대한 메시지를 만듭니다.
...
특정 채널을 위한 메시지를 만드는 방법을 알아봅니다.

* [인앱 메시지](../in-app/get-started-in-app.md)
* [이메일 메시지](../email/get-started-email.md)
* [푸시 알림](../push/get-started-push.md)
...
1. **[!UICONTROL 대상]** 탭으로 이동하여 **[!UICONTROL 대상 선택]** 단추를 선택합니다.

   ![](assets/challenge-create-audience.png)

1. 사용 가능한 Adobe Experience Platform 대상자 목록에서 타겟 대상자를 선택합니다. [대상자를 사용하여 작업하는 방법을 알아봅니다](../audience/about-audiences.md).

1. **[!UICONTROL 대상 추가]**&#x200B;를 선택합니다.

이제 과제 구조, 콘텐츠, 메시징 및 타겟 대상으로 완벽하게 구성되었습니다. 마지막 단계는 여정을 생성하고 게시하는 것입니다.

## 여정 생성 및 게시 {#review-and-publish}

과제 전달 및 고객 상호 작용을 오케스트레이션할 여정을 생성합니다. 이렇게 하려면 **[!UICONTROL 여정 생성]**&#x200B;을 선택하세요.
...
필요한 경우 여정 구성을 검토한 다음 여정을 게시하여 고객이 과제를 사용할 수 있도록 합니다. [여정 게시 방법을 알아보세요](../building-journeys/publish-journey.md).
