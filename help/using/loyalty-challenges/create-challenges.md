...
>title="챌린지 일정"
>abstract="시작 날짜 및 종료 날짜, 시간, 시간대를 사용하여 챌린지 활성화 시점을 설정하거나, &#39;종료 일자 없음&#39;을 선택하여 챌린지를 무기한으로 진행할 수 있습니다. 작업 완료 창에서 고객이 챌린지 기간 동안 작업을 완료할 수 있는 시점을 선택합니다."
...
>title="보상"
>abstract="고객이 포인트를 얻는 시점(전체 챌린지 완료 시 또는 진행 중인 작업 마일스톤 시)를 선택합니다. 보상 공급자(포인트 및 보상을 관리하는 충성도 솔루션)를 선택한 다음 금액을 설정합니다. 전체 완료에 대한 단일 총액 또는 마일스톤별 작업당 값으로 설정하고, 보상을 지급하려는 작업에 대해서만 보상을 활성화합니다."
...
>title="메시지"
>abstract="메시지는 챌린지 라이프사이클 전반의 참여를 돕습니다. 메시징 탭에서 시작(챌린지 공지 및 참여자 초대), 진행 중(참여자의 참여 유지 및 작업 완료 중), 종료(완료 축하 및 참여자에게 보상 알림) 등 각 단계에 대한 메시지를 추가합니다. 각 단계에 대해 메시지 추가 버튼을 클릭하고, 채널을 선택하고, 채널 구성을 선택한 다음 편집을 선택하여 메시지 콘텐츠를 디자인합니다."
