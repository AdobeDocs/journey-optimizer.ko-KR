...
1. 보상 공급자와 관련된 아래 리소스를 구성합니다. 자세한 내용을 보려면 각 섹션을 확장하십시오.
   ...
   공급자가 지원하는 보상당 하나의 항목(예: 프로그램 포인트 또는 스타, 금전 크레딧). 각 정의에 대해 다음 작업을 수행합니다.
   ...
   * 이행 호출과 함께 전송된 **페이로드**&#x200B;를 지정하십시오.
   ...
   이행 호출을 엔드포인트로 직접 이동하는 대신 중간 서버를 통해 라우팅합니다.
...
1. 인벤토리 목록에서 가져온 파일을 검토합니다. 목록에는 항목당 하나의 행이 표시됩니다. **에 포함된**&#x200B;그룹 열에는 해당 항목이 속한 모든 제품 그룹이 표시됩니다. 각 그룹은 알약으로 나타납니다(품목이 여러 그룹에 있는 경우 여러 알약).
...
1. 제외 목록에서 가져온 파일을 검토합니다. 목록에는 항목당 하나의 행이 표시됩니다. **에 포함된**&#x200B;그룹 열에는 해당 항목이 속한 모든 제외 그룹이 표시됩니다. 각 그룹은 알약으로 나타납니다(품목이 여러 그룹에 있는 경우 여러 알약).
