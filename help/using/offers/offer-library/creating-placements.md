...
1. 다음 **[!UICONTROL 요청 설정]** 및 **[!UICONTROL 응답 형식]** 섹션은 추가 매개 변수를 제공합니다.
   ...
   * **[!UICONTROL 배치 간 중복 허용]**: 다른 배치에서 동일한 오퍼를 여러 번 제안할 수 있는지 여부를 제어합니다. 활성화되면 시스템은 여러 배치에 대해 동일한 오퍼를 고려합니다.
   ...
   * **[!UICONTROL 콘텐츠 포함]** / **[!UICONTROL 메타데이터 포함]**: 오퍼의 컨텐츠 및 메타데이터가 API 응답에서 반환되어야 하는지 여부를 지정합니다. 모든 메타데이터 또는 특정 필드만 포함할 수 있습니다.
   ...
   >[!NOTE]
   >
   >을 사용하여 작업하는 경우 [Edge Decisioning API](https://experienceleague.adobe.com/docs/journey-optimizer/using/offer-decisioning/api-reference/offer-delivery-api/edge-decisioning-api.html?), 이러한 매개 변수를 요청에 설정할 수 없습니다. 이 화면에서 정의해야 합니다. 을 사용하여 작업하는 경우 [일괄 의사 결정 API](../api-reference/offer-delivery-api/batch-decisioning-api.md), 이 화면에 설정된 매개 변수는 배치 배달 전송을 위해 고려되지 않으므로 이러한 매개 변수를 API 요청에 직접 포함해야 합니다.
