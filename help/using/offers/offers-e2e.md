...
1. 오퍼를 만들기 전에 [구성 요소](#define-components)를 정의하십시오
...
1. [만든 ](#create-collection) 개인화된 오퍼를 포함할 컬렉션을 만듭니다
...
**[!UICONTROL Decision Management]** > **[!UICONTROL Components menu]** 아래에서 찾을 수 있습니다.

1. 먼저 오퍼에 대해 **배치**&#x200B;를 만듭니다.
...
1. **의사 결정 규칙**&#x200B;을 만듭니다.
   ...
   **[!UICONTROL XDM Individual Profile > Person > Gender]** 속성을 사용하여 두 개의 간단한 규칙을 구성합니다.
...
1. **태그**&#x200B;를 만들 수도 있습니다.
   ...
   태그를 만드는 자세한 단계는 [이 섹션](../../using/offers/offer-library/creating-tags.md)에 설명되어 있습니다.

1. 주어진 배치에 대해 어떤 오퍼가 먼저 제시되어야 하는지 결정하는 규칙을 정의하려는 경우(오퍼의 우선순위 점수를 고려하지 않고) **등급 공식**&#x200B;을 만들 수 있습니다.

   등급 공식을 만드는 자세한 단계는 [이 섹션](../../using/offers/offer-library/create-ranking-formulas.md#create-ranking-formula)에 설명되어 있습니다.

   >[!NOTE]
   >
   >이 예제에서는 우선순위 점수만 사용합니다. [자격 규칙 및 제한](../../using/offers/offer-library/creating-personalized-offers.md#eligibility)에 대해 자세히 알아보십시오.
...
1. 이 오퍼에서 세 개의 표현을 만듭니다. 각 표현은 이전에 생성한 배치와 자산의 조합이어야 합니다.
   ...
   * *Web - Image* 배치에 해당하는 하나
   * *Email - Image* 배치에 해당하는 하나
   * *비디지털 - 텍스트* 배치에 해당하는 하나

   >[!NOTE]
   >
   >오퍼를 메시지의 다른 위치에 표시하여 다른 배치 컨텍스트에서 오퍼를 사용할 더 많은 기회를 만들 수 있습니다.

   [이 섹션](../../using/offers/offer-library/creating-personalized-offers.md#representations)의 표현에 대해 자세히 알아보십시오.

1. 처음 두 배치에 적합한 이미지를 선택합니다. *비디지털 - 텍스트* 배치에 대한 사용자 지정 텍스트를 입력합니다.
...
1. **[!UICONTROL Priority]** 을 입력합니다. 이 예에서 *25*&#x200B;를 추가합니다.

1. 오퍼를 검토하고 **[!UICONTROL Save and approve]** 를 클릭합니다.

   ![](../assets/offers-e2e-review.png)

1. 이 예에서, 동일한 표현이지만 다른 자산으로 세 개의 오퍼를 더 만듭니다. 다음과 같이 다양한 규칙 및 우선 순위를 사용하여 할당합니다.

   * 첫 번째 오퍼 - 의사 결정 규칙:*여성 고객*, 우선순위:*25*
   * 두 번째 오퍼 - 결정 규칙:*여성 고객*, 우선순위:*15*
   * 세 번째 오퍼 - 결정 규칙:*남성 고객*, 우선순위:*25*
   * 네 번째 오퍼 - 결정 규칙:*남성 고객*, 우선순위:*15*
...
1. 대체 오퍼를 검토하고 **[!UICONTROL Save and approve]** 를 클릭합니다.
...
1. *요가* 태그를 사용하여 이전에 만든 4개의 개인화된 오퍼를 선택하십시오.
...
1. *웹 - 이미지*, *이메일 - 이미지* 및 *비디지털 - 텍스트* 배치를 선택합니다.
...
1. [구성 요소를 작성할 때 등급을 정의한 경우 결정에서 배치에 할당할 수 있습니다. ](#define-components) 이 배치에서 여러 오퍼를 표시할 수 있는 경우 이 수식을 사용하여 먼저 전달할 오퍼를 계산합니다.
...
1. 결정을 검토하고 **[!UICONTROL Save and approve]** 을 클릭합니다.
...
1. 이메일을 만든 다음 [이메일 디자이너](../../using/design-emails.md)를 열어 해당 콘텐츠를 구성합니다.

1. 왼쪽 팔레트에서 구조 구성 요소를 추가합니다.

1. **[!UICONTROL Offer decision]** 컨텐츠 구성 요소를 추가합니다. [이 섹션](../../using/content-components.md)에서 컨텐츠 구성 요소를 사용하는 방법을 알아봅니다.

   ![](../assets/offers-e2e-decision-component.png)

1. 패턴을 선택합니다. 오른쪽 팔레트에서 **[!UICONTROL Select offer decision]** 을 클릭하여 결정을 추가합니다.

   ![](../assets/offers-e2e-select-offer-decision.png)

1. **[!UICONTROL Placements]** 드롭다운 목록에서 표시할 오퍼에 해당하는 배치를 선택합니다.

   이 경우 이 예제의 일부로 이전에 만든 배치에서 이메일에서 결정을 사용할 때 **Email - Image** 배치만 사용할 수 있습니다. [배치 만들기](../../using/offers/offer-library/creating-placements.md)에 대해 자세히 알아보십시오.

   ![](../assets/offers-e2e-select-placement-in-decision.png)

1. **이메일 - 이미지** 배치와 일치하는 의사 결정이 표시됩니다. 콘텐츠 구성 요소에서 사용할 결정을 선택한 다음 **[!UICONTROL Add]** 을 클릭합니다.
...
**[!UICONTROL Offers]** 섹션이나 컨텐츠 구성 요소 화살표(오른쪽 및 왼쪽 화살표)를 사용하여 데이터를 찾아봅니다. 고객 프로필과 함께 결정의 일부인 다른 오퍼를 표시할 수도 있습니다. 자세한 내용은 [이 섹션](../../using/deliver-personalized-offers.md#preview-offers-in-email)을 참조하십시오.

변경 사항을 저장한 후 메시지가 게시되면 여정의 일부로 메시지를 보낼 때 관련 프로필에 오퍼를 표시할 수 있습니다.

**관련 항목:**

* [이 섹션](../../using/preview.md#preview-your-messages)에서 메시지 미리 보기를 확인하는 방법을 알아봅니다.

* [이 섹션](../../using/publish-manage-message.md)에 메시지를 게시하는 방법을 알아봅니다.

* [이 섹션](../building-journeys/journey.md)에 있는 하나 이상의 여정에 의해 메시지가 트리거되는 방법을 알아봅니다.
