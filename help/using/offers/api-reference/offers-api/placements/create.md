...
컨테이너 ID를 제공하는 동안 [!DNL Offer Library] API에 POST 요청을 만들어 배치를 만들 수 있습니다.
...
성공적인 응답은 해당 고유 인스턴스 ID 및 배치 `@id`을 포함하여 새로 생성된 배치의 세부 사항을 반환합니다. 이후 단계에서 인스턴스 ID를 사용하여 배치를 업데이트하거나 삭제할 수 있습니다. 나중에 자습서에서 고유한 배치 `@id`을 사용하여 결정, 의사 결정 규칙 및 대체 오퍼를 만들 수 있습니다.
