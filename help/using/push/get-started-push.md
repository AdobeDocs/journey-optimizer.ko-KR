...
[!DNL Journey Optimizer]는 푸시 알림을 지원하며 업계 최고 수준의 처리 속도로 적절한 알림을 보내는 데 도움이 됩니다. 푸시 알림에 개인화와 여정 기반 컨텍스트를 포함하여 브랜드가 [!DNL Adobe CX Enterprise]에서 보유한 데이터 인사이트를 활용할 수 있습니다.
...
| 이점 | 이유 | 사용 사례 예시 |
| --- | --- | --- |
| 시간에 민감한 업데이트 | 사용자가 앱을 사용하지 않을 때에도 즉시 전달됩니다. | 비행 지연 알림, 주문 상태 변경, 최신 뉴스 |
| 재참여 | 일정 기간 활동이 없으면 앱으로 돌아가라는 메시지를 사용자에게 표시합니다. | 장바구니 포기 알림, 재참여 캠페인 |
| 비용 절감 대 SMS | SMS와 달리 메시지당 이동통신사 요금 없음 | 대량의 프로모션 또는 트랜잭션 알림 |
| 풍부한 대화형 콘텐츠 | 이미지, 작업 단추 및 딥링크 지원 | 탭하여 구매할 수 있는 버튼, 리치 미디어 미리보기가 포함된 제품 프로모션 |
| 디바이스 기본 기능 | 다른 채널에서 사용할 수 없는 OS 수준 기능 활용 | 진동 알림, 앱 아이콘 배지, 지오펜스 기반 위치 트리거 |
| 높은 옵트인 가능성 | 앱 설치 또는 최초 실행 단계에 옵트인하라는 메시지가 표시됩니다 | 온보딩 플로우, 첫날 참여 캠페인 |

## 사용하지 말아야 할 경우

푸시 알림이 모든 메시지에 적합하지는 않습니다. 다음과 같은 상황에서는 다른 채널을 고려하세요.

* 메시지가 대상에게 아예 전달되지 않을 수 있으므로, 대상의 푸시 알림 수신 동의율이 낮거나 알림 수신을 꺼리는 경향을 보인 경우
* 이메일이 긴 콘텐츠에 더 적합하고 세부적인 서식을 지원하므로, 메시지에 긴 형식의 콘텐츠가 필요한 경우
* 콘텐츠가 민감하거나 비공개이며, 디바이스 근처의 누구나 볼 수 있는 잠금 화면에 표시되지 않아야 하는 경우
* 대부분의 사용자가 모바일 앱보다 데스크탑에서 서비스에 액세스하여 푸시 알림이 제한되어 있거나 연결할 수 없는 경우
