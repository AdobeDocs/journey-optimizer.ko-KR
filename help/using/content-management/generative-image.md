...
title: AI로 이미지 생성
description: Journey Optimizer에서 컨텐츠 생성을 사용하여 이미지를 생성하는 방법을 알아봅니다.
...
# AI로 이미지 생성 {#generative-image}

>[!BEGINSHADEBOX]

**이 페이지에서:** Adobe Journey Optimizer에서 AI를 사용하여 전자 메일, 웹, 랜딩 페이지 및 푸시 알림 콘텐츠에 대한 브랜드 이미지를 생성, 구체화 및 선택하는 방법을 알아봅니다.
...
AI를 사용하여 이메일, 웹, 랜딩 페이지 및 푸시 알림에서 메시지를 개선하는 매력적인 시각적 콘텐츠를 생성합니다. 콘텐츠 생성 은 에셋을 최적화하고 개선하는 데 도움이 되며 대상자에게 보다 사용자 친화적이고 매력적인 경험을 제공합니다.

## 이메일 및 웹 채널용 {#email-web-channels}

콘텐츠 생성 은 이메일 캠페인, 웹 경험 및 랜딩 페이지에 대한 완전한 시각적 경험을 생성할 수 있습니다. 이 기능을 사용하면 디지털 터치포인트에서 대상자와 공감하는 온브랜드, 주목을 끄는 이미지를 만들 수 있습니다.

### 액세스 및 구성 {#access-configure}

콘텐츠 생성을 통해 이미지 생성을 시작하려면 먼저 캠페인이나 여정을 설정하고 콘텐츠 편집기를 엽니다. 아래 단계에 따라 작업 공간을 준비하고 콘텐츠 생성 패널에 액세스합니다.
...
1. 콘텐츠 생성으로 변경할 에셋을 선택합니다.

1. 오른쪽 메뉴에서 **[!UICONTROL 콘텐츠 생성]**(또는 웹용 **[!UICONTROL 콘텐츠 도우미 표시]**)를 선택합니다.

   ![이미지 자산을 선택하고 컨텐츠 생성 패널을 열었습니다](assets/image-genai-1.png){zoomable="yes"}

### 이미지 생성 {#generate-content}

콘텐츠 생성을 통해 효과적인 프롬프트를 만들고 시각적으로 매력적인 이미지를 생성하도록 이미지 설정을 구성하는 방법에 대해 알아봅니다. 종횡비, 시각적 강도 및 조명과 같은 매개 변수를 사용자 지정하여 브랜드 및 캠페인 목표에 맞는 이미지를 만들 수 있습니다.

1. 콘텐츠 생성에 **[!UICONTROL 참조 스타일]** 옵션을 사용하여 참조 콘텐츠를 기반으로 새 콘텐츠를 개인화합니다. 이미지를 업로드하여 변형에 컨텍스트를 추가할 수도 있습니다.
...
콘텐츠 생성 을 사용하면 푸시 알림에 대한 흥미로운 이미지를 생성하여 관심을 사로잡고 대상자와 공감하는 시각적으로 매력적인 모바일 커뮤니케이션을 만들 수 있습니다.

### 액세스 및 구성 {#mobile-access-configure}

푸시 알림에 대해 콘텐츠 생성을 사용하려면 푸시 게재를 설정하고 콘텐츠 편집기로 이동해야 합니다. 다음 단계는 게재를 만들고 콘텐츠 생성 기능에 액세스하는 과정을 안내합니다.
...
1. **[!UICONTROL 콘텐츠 생성 표시]** 메뉴에 액세스합니다.
   ...
   ![콘텐츠 생성 표시 메뉴를 보여 주는 스크린샷](assets/push-genai-1.png){zoomable="yes"}

### 이미지 생성 {#mobile-generate-content}

콘텐츠 생성에 액세스한 후 생성 설정을 조정하여 내 브랜드에 맞는 이미지를 만들고 푸시 알림 목표를 지원할 수 있습니다. 모바일 디스플레이에 최적화된 시각적 개체를 생성하도록 프롬프트 및 이미지 매개 변수를 구성합니다.
...
1. **[!UICONTROL 이미지 설정]** 선택:
   ...
   * **[!UICONTROL 컴포지션]**: 이미지 프레임 내의 요소 배열을 참조합니다
     ...
     ![푸시할 콘텐츠 이미지 생성](assets/push-gen-img-3.png){zoomable="yes"}
