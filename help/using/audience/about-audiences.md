...
**이 페이지에서:** Adobe Experience Platform 대상을 탐색, 빌드 및 관리하고 Adobe Journey Optimizer 여정 및 캠페인에서 타깃팅하는 방법에 대해 알아봅니다.
...
* **세그먼트 정의**: Adobe Experience Platform 세분화 서비스를 사용하여 새 대상 정의를 만듭니다. 대상은 세그먼트 정의에서 생성되며 평가 유형에 따라 다른 시간에 새로 고쳐집니다.
   ...
   * 배치 세그멘테이션: 대상자가 24시간마다 새로 고쳐지며, 고정된 간격으로 프로필 스냅샷을 캡처합니다. 여정에서 사용할 경우 새로 자격을 갖춘 세그먼트 멤버는 다음 스냅샷이 표시될 때까지 나타나지 않을 수 있습니다. [타이밍에 대해 자세히 알아보기](../building-journeys/audience-qualification-events.md#timing-segment-membership).
...
>[!NOTE]
>
>여정, 캠페인 또는 의사 결정 활동에 상관없이 대상 활성화를 통해 참여한 프로필은 조직의 **참여 가능한 프로필** 라이선스 지표에 포함됩니다. 각 프로필은 롤링 12개월 기간 동안 샌드박스당 한 번 계산됩니다. [참여 가능한 프로필 수 모니터링](license-usage.md)
