...
**이 페이지에서:** 여정 또는 캠페인을 통해 웹 경험을 만들고, 콘텐츠를 편집하고 테스트하고, 방문자를 다른 URL로 리디렉션하고, Adobe Journey Optimizer에서 게시하거나, 활성화하거나, 중지하는 방법에 대해 알아봅니다.
...
>title="웹 작업"
>abstract="웹 경험 인바운드 작업은 프로필이 이 여정 단계에 도달하면 프로필에 웹 경험을 표시합니다. 레이블은 여정 캔버스에서 활동을 식별하고, 작업은 표시된 콘텐츠를 정의하는 웹 구성을 참조합니다. **최적화** 섹션에는 콘텐츠 실험 또는 타기팅 규칙이 포함될 수 있습니다. 이 활동 후(기본적으로 3일) **대기** 노드가 자동으로 삽입되므로 웹 경험을 볼 시간이 프로필에 제공됩니다."
...
1. 실행할 캠페인 유형 선택
   ...
   * **API 트리거됨 - 마케팅/트랜잭션**: API 호출을 사용하여 캠페인을 실행하십시오. API 트리거 캠페인은 마케팅 또는 트랜잭션 메시지(예: 암호 재설정, 장바구니 구매 등 개인이 수행한 작업에 따라 발송된 메시지)를 보내는 것을 목표로 합니다. [API를 사용하여 캠페인을 트리거하는 방법에 대해 알아보세요](../campaigns/api-triggered-campaigns.md)
...
1. 여정이 유효하고 오류가 없는지 확인합니다. [자세히 알아보기](../building-journeys/troubleshooting.md#activity-errors)
