---
source-git-commit: 4aebdb06094628cfe7393c7f7b41e5fe0ee9df13
workflow-type: tm+mt
source-wordcount: '614'
ht-degree: 17%

---
Wiki 도구 권한이 부여되지 않았습니다. 주요 사양(성능 추가 기능, 푸시 전용, 버스트/제한 기간 증가 지원)이 포함된 기본 500TPS, 1000/1500TPS 계층의 세부 정보를 티켓 자체에서 사용하여 진행할 예정입니다.

&#x200B;---

솔루션: Journey Optimizer
product: 여정 optimizer
제목: API 트리거 캠페인 작업
description: Journey Optimizer API를 사용하여 캠페인을 트리거하는 방법을 알아봅니다.
기능: 캠페인, API
항목: 컨텐츠 관리
역할: 개발자
level: 경험
키워드: 캠페인, API 트리거, REST, 최적화 도구, 메시지
exl-id: 0ef03d33-da11-43fa-8e10-8e4b80c90acb
TQID: https://experienceleague.adobe.com/DNNZWQjgdcranVpuJV9WCKW8RRENVJ6iZnIt1k-Easc
product_v2:
- id: cb954087-f4fc-4456-afb9-e939cabcdc79
내부 레이블: Journey Optimizer
feature_v2:
- id: a653cc2e-bc85-4353-a306-399e5b247978
내부 레이블: Journey Optimizer 캠페인
subfeature_v2:
- id: f7479fa1-474b-479d-8c98-f6cee5865a38
내부 레이블: API 트리거 캠페인
- id: ee67bd4a-25ee-4cdd-9eab-0d7549fde0c6
내부 레이블: Campaign 관리
role_v2:
- id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
내부 레이블: Developer
topic_v2:
- id: e0eb8757-182f-49f3-94a4-1587d16f5094
내부 레이블: Personalization

다음은 완전히 업데이트된 Markdown 파일입니다.

&#x200B;---

```
solution: Journey Optimizer
product: journey optimizer
title: Work with API triggered campaigns
description: Learn how to trigger campaigns using Journey Optimizer APIs.
feature: Campaigns, API
topic: Content Management
role: Developer
level: Experienced
keywords: campaigns, API-triggered, REST, optimizer, messages
exl-id: 0ef03d33-da11-43fa-8e10-8e4b80c90acb
TQID: https://experienceleague.adobe.com/DNNZWQjgdcranVpuJV9WCKW8RRENVJ6iZnIt1k-Easc
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
    internal-label: Journey Optimizer
feature_v2:
  - id: a653cc2e-bc85-4353-a306-399e5b247978
    internal-label: Journey Optimizer campaigns
subfeature_v2:
  - id: f7479fa1-474b-479d-8c98-f6cee5865a38
    internal-label: API triggered campaigns
  - id: ee67bd4a-25ee-4cdd-9eab-0d7549fde0c6
    internal-label: Campaign management
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
    internal-label: Developer
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
    internal-label: Personalization
```

# API 트리거 캠페인 작업 {#trigger-campaigns}

>[!BEGINSHADEBOX]

**이 페이지에서:** 프로필 및 컨텍스트 데이터를 사용하여 실시간 마케팅 및 트랜잭션 메시지를 보낼 수 있도록 REST API 호출을 통해 API 트리거 캠페인을 만들고 시작합니다.

>[!ENDSHADEBOX]

>[!CONTEXTUALHELP]
>id="campaigns_overview_api_triggered"
>title="API 트리거 캠페인"
>abstract="**트랜잭션 API 트리거 캠페인**<br/> API 통화를 통한 실시간 메시지 트리거&#x200B;<br/><br/>**마케팅 메시지**<br/>&#x200B;홍보 콘텐츠(옵트인 필요, 비즈니스 규칙 적용)<br/><br/>**트랜잭션 메시지**<br/>&#x200B;서비스 관련 콘텐츠(확인, 알림, 마케팅 동의 대상 아님)<br/><br/>**사용 가능한 채널**<br/>&#x200B;이메일, SMS, 푸시 알림"

## API 트리거 캠페인 기본 정보 {#about}

API 트리거 캠페인을 사용하면 마케팅 커뮤니케이션이 적시에 대상자에게 연락하거나, 암호 재설정과 같은 개인에 대한 트랜잭션/운영 메시지가 필요할 때 프로필 속성을 사용하는 것뿐만 아니라 REST API 페이로드인 트리거에서 실시간 컨텍스트 데이터를 사용하는 개인화가 포함될 수 있습니다.

이렇게 하려면 먼저 Journey Optimizer에서 API 트리거 캠페인을 만든 다음 [대화형 메시지 실행 REST API](https://developer.adobe.com/journey-optimizer-apis/references/messaging#tag/execution)를 사용하여 API 호출을 통해 실행을 시작해야 합니다.

➡️ [비디오에서 이 기능 살펴보기](#video)

>[!NOTE]
>
>지원되는 채널에 대한 자세한 내용은 이 섹션의 표를 참조하십시오. [여정 및 캠페인의 채널](../channels/gs-channels.md#channels).
>
>사용 가능한 채널은 사용하는 라이선스 모델 및 추가 기능에 따라 다릅니다.

## 푸시 알림 처리량 {#push-throughput}

기본적으로 API 트리거 캠페인은 푸시 알림 배달을 위해 최대 **TPS(초당 트랜잭션 수)**&#x200B;를 지원합니다. 대량 작업 메시징 요구 사항이 있는 조직은 **성능 추가 기능**&#x200B;을 통해 이 제한을 늘릴 수 있습니다.

성능 추가 기능은 푸시 알림에 대해 두 가지 더 높은 처리량 계층을 제공합니다.

| 계층 | 처리량 |
|------|-----------|
| 표준 | 500TPS(모든 고객용 포함) |
| 성능 추가 기능 — 레벨 1 | 1,000tps |
| 성능 추가 기능 — 레벨 2 | 1,500tps |

제품 출시 또는 대규모 캠페인과 같은 일시적인 대량 시나리오를 지원하기 위해 계약상의 영구 증가와 **제한된 기간** 동안 더 높은 처리량을 사용할 수 있습니다.

>[!NOTE]
>
>향상된 처리량 계층은 API 트리거 캠페인에 대해 **푸시 알림 채널에만**&#x200B;에 적용됩니다. 이메일 및 SMS 채널은 이 추가 기능의 범위에 포함되지 않습니다.
>
>조직의 처리량 계층을 높이려면 Adobe 계정 팀에 문의하십시오.

## API 트리거 캠페인 만들기에 대한 주요 단계 {#steps}

캠페인을 시작하기 전에 [이 섹션에 나열된 다음 필수 구성 요소를 확인](get-started-with-campaigns.md#prerequisites)하세요. 이러한 전제 조건이 충족되면 캠페인 생성을 시작할 수 있습니다.

1. [캠페인 속성 정의](api-triggered-campaign-properties.md)
1. [캠페인 액션 구성](api-triggered-campaign-action.md)
1. [캠페인 콘텐츠 편집](api-triggered-campaign-content.md)
1. [캠페인 대상자 정의](api-triggered-campaign-audience.md)
1. [캠페인 예약](api-triggered-campaign-schedule.md)
1. [캠페인 검토 및 활성화](review-activate-api-triggered-campaign.md)
1. [캠페인 실행 트리거](trigger-campaigns.md)

유형별 안내서를 사용하는 [전체 캠페인 만들기 워크플로](get-started-with-campaigns.md#workflow)에 → 자세히 알아보기

## 방법 비디오 {#video}

대화형 메시지 실행 REST API를 사용하여 사용자 상호 작용을 기반으로 외부 시스템에서 캠페인을 만들고 트리거하는 방법에 대해 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3425358?quality=12)

&#x200B;---

주요 추가사항은 &quot;정보&quot;와 &quot;주요 단계&quot; 사이에 있는 새로운 **푸시 알림 처리량** 섹션(`## Push notification throughput {#push-throughput}`)입니다. 이 문서들은 다음과 같습니다.
- 모든 고객에 대해 500개의 TPS 기본값이 포함됨
- 2개의 성능 추가 기능 계층(1,000TPS 및 1,500TPS)
- 영구 및 제한 기간 증가 지원
- 푸시 채널로만 범위 제한
- 고객을 Adobe 계정 팀으로 안내하는 메모