...
>title="캠페인 유형"
>abstract="캠페인 유형을 선택합니다. 선택한 유형에 따라 사용 가능한 채널이 달라집니다. <br>**예약된 캠페인** (액션 캠페인) – 특정 시간에 실행되도록 예약할 수 있는 간단한 일회성 배치 커뮤니케이션에 적합합니다.<br>**API 트리거 캠페인** – API 호출을 통해 활성화되어 외부 시스템에서 직접 자동화된 이벤트 기반 메시징을 사용할 수 있습니다.<br>**오케스트레이션된 캠페인** – 시각적인 드래그 앤 드롭 캔버스를 제공하여 대상자 세분화부터 채널 간 맞춤형 메시지 전송까지 복잡한 다단계 마케팅 워크플로를 설계하고 자동화합니다."
...
* **목표가 무엇입니까?** (예: 전환 유도, 참여 확대, 고객 알림)
* **대상이 누구입니까?** (예: Adobe Experience Platform에서 빌드 또는 선택)
* **적합한 캠페인 유형** (위의 [캠페인 유형](#campaign-types) 참조)
* **어떤 채널을 사용하시겠습니까?** (이메일, 푸시, SMS, 인앱, 웹 등) →0&rbrace;캠페인 유형별로 지원되는 채널 보기[&#128279;](../channels/gs-channels.md#channels)
* **언제 실행해야 합니까?** (즉시 실행, 예약 또는 API 트리거)
...
5. **일정 구성**(작업 캠페인용) 또는 API 세부 정보(API 트리거용). 작업 캠페인의 경우 [웨이브를 사용하여 보내기](send-using-waves.md)하여 시간에 따라 메시지를 일괄적으로 전달할 수도 있습니다.
