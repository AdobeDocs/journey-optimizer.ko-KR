...
[!DNL Journey Optimizer] 캠페인을 사용하여 여러 채널에서 특정 대상에게 일회성 콘텐츠를 전달할 수 있습니다. 작업을 단계별로 실행하는 여정과 달리 캠페인은 즉시 또는 정의된 일정에 따라 작업을 동시에 수행합니다.

![](assets/gs-campaigns.png)

## 캠페인 유형

[!DNL Journey Optimizer]은(는) 세 가지 캠페인 유형을 지원합니다. 각 유형은 다양한 사용 사례에 적합하며 다양한 채널을 지원합니다.
...
**오케스트레이션된 캠페인** 채널 전반에서 정교하고 브랜드에서 시작한 마케팅 캠페인을 강화하여 규모에 맞게 참여, 매출 및 고객 충성도를 높일 수 있습니다.
...
➡️ [오케스트레이션된 캠페인으로 작업하는 방법을 알아봅니다](../orchestrated/gs-orchestrated-campaigns.md).

>[!TAB 작업 캠페인(또는 예약된 캠페인)]

**작업 캠페인**(예약된 캠페인이라고도 함)을 사용하면 간단한 임시 일괄 처리 커뮤니케이션이 가능합니다.

* **예약됨 - 마케팅** - 프로모션 오퍼, 참여 캠페인, 공지, 법적 고지 사항 또는 정책 업데이트와 같은 마케팅 사용 사례의 경우. 수신자를 옵트인해야 합니다.
* **예약됨 - 트랜잭션** - 마케팅 캠페인과 달리 트랜잭션 캠페인에는 수신자를 옵트인할 필요가 없습니다. 중단, 긴급 상황, 취소와 관련된 통신에 이 범주를 사용하십시오. 지원되는 채널: 이메일, SMS, 푸시 알림.

➡️ [작업 캠페인 작업 방법 알아보기](create-campaign.md)

>[!TAB API 트리거 캠페인]

**API 트리거 캠페인**&#x200B;을(를) 사용하면 API 호출을 사용하여 캠페인 실행을 트리거할 수 있습니다. 이러한 커뮤니케이션은 필요한 경우 암호 재설정과 같은 프로필 속성을 사용할 뿐만 아니라 REST API 페이로드인 트리거에서 실시간 컨텍스트 데이터를 사용할 수 있는 개인화가 포함될 수 있는 위치에 보낼 수 있습니다.

* **API가 트리거됨 - 마케팅** - 개인화된 마케팅 커뮤니케이션을 타깃팅된 대상자에게 보냅니다.
* **API 트리거됨 - 트랜잭션** - 암호 재설정 요청, 장바구니 구매 등과 같이 개인이 수행한 작업에 따라 메시지를 보냅니다.

➡️ [API 트리거 캠페인을 사용하여 작업하는 방법을 알아봅니다](api-triggered-campaigns.md)
...
캠페인을 작업하기 전에 아래의 사전 요구 사항을 검토했는지 확인하십시오.

* **대상자** 대상자는 캠페인을 만들기 전에 사용할 수 있어야 합니다. [대상자 시작하기](../audience/about-audiences.md).

* **채널 구성** - 채널을 선택하려면 해당 채널 구성(즉, 사전 설정)을 만들어 사용할 수 있어야 합니다. [채널 구성을 설정하는 방법을 알아봅니다](../configuration/channel-surfaces.md).

* **권한** - 아래 나열된 적절한 권한이 있는 사용자만 캠페인을 사용할 수 있습니다. 캠페인 기능에 액세스할 수 없는 경우 관리자에게 연락하여 필요한 권한을 요청하십시오. [Journey Optimizer 기본 제공 역할에 대해 자세히 알아보기](../administration/ootb-product-profiles.md)

  | 캠페인 유형 | 권한 |
  |----------------------------|----------------------------------------------------------------------------|
  | **액션 캠페인** | 캠페인 관리자<br>캠페인 승인자<br>캠페인 관리자<br>캠페인 뷰어 |
  | **API 트리거 캠페인** | 캠페인 관리자<br>캠페인 승인자<br>캠페인 관리자<br>캠페인 뷰어 |
  | **오케스트레이션된 캠페인** | 오케스트레이션된 캠페인 관리자<br>오케스트레이션된 캠페인 승인자<br>오케스트레이션된 캠페인 관리자<br>오케스트레이션된 캠페인 뷰어 |
