...
+++ 대상자가 각 예약된 일괄 처리 전에 다시 평가됩니까? 아니면 캠페인 시작 시 수정됩니까?

캠페인이 활성화될 때 대상자는 **한 번 평가**&#x200B;됩니다. 해당 시점에서 자격 있는 프로필의 스냅샷을 가져와 모든 웨이브에 사용합니다. 각 후속 웨이브 전에는 대상 멤버십이 다시 평가되지 않습니다.

그러나 **프로필 특성은 캠페인 활성화가 아닌 각 웨이브 프로세스**&#x200B;에 읽혀집니다. 즉, 여러 날에 걸쳐 전파되는 파도의 경우

* Personalization 속성(예: 프로필의 이름 또는 충성도 계층)은 웨이브가 실행되는 시점의 프로필 상태를 반영합니다.
* **동의 및 비표시 검사는 각 웨이브에 대해 전송 시간에 적용됩니다.** 두 예약된 일괄 처리 사이에서 프로파일이 옵트아웃되면 후속 일괄 처리에서 메시지를 받지 않습니다.

요약하면 *누가* 캠페인에 포함되는지는 미리 수정되지만 *해당 프로필로 보내는 데 사용되는 데이터*&#x200B;는 웨이브가 처리될 때 현재 상태를 반영합니다.
