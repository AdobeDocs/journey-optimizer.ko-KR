...
>title="캠페인 유형"
>abstract="**예약된 캠페인** 은 즉시 또는 지정된 날짜에 실행되며 마케팅 유형 메시지를 보내기 위한 것입니다. **API 트리거됨** 캠페인은 API 호출을 사용하여 실행됩니다. 마케팅 메시지 또는 트랜잭션 메시지(예: 암호 재설정, 장바구니 포기 등 개인이 수행한 작업에 따라 전송된 메시지)를 전송하는 것을 목표로 합니다."

>[!CONTEXTUALHELP]
>id="ajo_campaigns_campaign_category"
>title="캠페인 범주"
>abstract="예약된 캠페인을 만드는 경우 **마케팅** 유형이 자동으로 선택됩니다. API 트리거 캠페인의 경우 다음을 전송할지 여부를 선택합니다. **마케팅** 또는 **트랜잭션** 메시지(예: 암호 재설정, 장바구니 포기 등 개인이 수행한 작업 후 전송된 메시지)"
...
1. 예약된 캠페인을 만드는 경우 **마케팅** 유형이 자동으로 선택됩니다. API 트리거 캠페인의 경우 다음을 전송할지 여부를 선택합니다. **마케팅** 또는 **트랜잭션** 메시지.&quot;
