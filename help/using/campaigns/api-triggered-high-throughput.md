...
**이 페이지에서:** API 트리거 캠페인에 대해 처리량이 많은 모드를 활성화하여 프로필에 의존하지 않고 초당 최대 5000개의 트랜잭션으로 대규모 실시간 트랜잭션 메시지(전자 메일 및 푸시)를 보낼 수 있습니다.
...
* **피드백 및 추적을 위한 데이터 세트** - 처리량이 많은 캠페인에 대한 피드백 및 추적 데이터는 프로필에 대해 활성화되지 않은 전용 데이터 세트에 저장됩니다. 따라서 이러한 이벤트는 일치하는 프로필이 있어도 프로필에 연결되지 않습니다.
  ...
  * **AJO 메시지 피드백 이벤트 데이터 세트 - 프로필이 아님** - 전자 메일과 푸시 피드백 모두에 사용됩니다.
  * **AJO 전자 메일 추적 경험 이벤트 데이터 세트 - 프로필이 아님** - 전자 메일 추적에만 사용됩니다. 푸시에 대한 전용 비프로필 추적 데이터 세트는 아직 사용할 수 없습니다.
...
* **푸시 알림**:
  ...
  * **웹 푸시** — 처리량이 많은 웹 푸시 캠페인은 공유 인프라에서 실행되며 처리량이 많은 모바일 푸시에 예약된 전용 용량의 혜택을 받지 않습니다.
  * **페이로드 크기** — 최대 페이로드 크기는 5KB입니다.
  * **차단 목록 비헤이비어** — 메시지가 차단 목록에 추가하다를 히트할 때 게재 피드백이 유지되지 않습니다. 이는 차단 목록 업데이트 또는 토큰 무효화를 트리거하지 않음을 의미합니다.
...
* 다음과 같은 경우 **표준 API 트리거** 캠페인을 선택하십시오.
  ...
  * 이메일 또는 푸시 이외의 다른 채널을 사용하려고 합니다.
