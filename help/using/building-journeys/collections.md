...
* **TL;DR:** 이 페이지에서는 지원되는 필드 유형, 구성 프로시저 및 중첩된 배열에 대한 알려진 제한 사항을 포함하여 Journey Optimizer의 사용자 지정 작업 매개 변수에 단순 및 개체 컬렉션을 동적으로 전달하는 방법을 설명합니다.

**의도:**
* 컬렉션(단순 또는 개체)을 동적 매개 변수로 수락하도록 사용자 지정 작업을 구성합니다.
* 여정을 작성할 때 고급 표현식 편집기에서 배열 매개 변수를 변수로 정의합니다
* 필터 및 교차 함수를 적용하여 표현식 편집기에서 배열 데이터 조작
* 사용자 지정 작업 요청 페이로드에 대한 중첩된 배열 제한 사항을 이해하고 작업합니다.
* 여정 테스트 모드에서 코드 보기 모드를 사용하여 컬렉션 매개 변수 테스트

**용어집:**
* **단순 컬렉션**: 사용자 지정 작업 매개 변수 *(제품별)로 전달된 기본 스칼라 값 목록(문자열, 숫자, 부울)*
* **개체 컬렉션**: 각각 여러 필드가 있는 구조화된 개체의 목록이 사용자 지정 작업 매개 변수 *(제품별)*(으)로 전달되었습니다.
* **listObject**: 개체 배열 *(제품별)을 나타내기 위해 사용자 지정 작업 구성에 사용되는 필드 형식*
* **listAny**: 항목이 혼합된 *(제품별)*&#x200B;이(가) 있는 다른 배열 또는 배열 배열에 사용되는 필드 형식입니다
* **변수(변수와 상수)**: 작업 매개 변수 구성에서 &quot;variable&quot;로 설정된 필드는 여정 컨텍스트에서 런타임에 동적으로 채워지는 반면 &quot;constant&quot;는 구성 시간 *(제품별)에 설정된 고정 값입니다*

**보호 기능:**
* 요청 페이로드의 중첩된 배열은 고정 수의 항목(상수로 정의됨)이 포함된 경우에만 지원되며, 동적 중첩된 배열은 지원되지 않습니다
* 테스트 모드에서 컬렉션을 테스트하려면 코드 보기 모드가 필요합니다. 코드 보기는 비즈니스 이벤트에 대해 지원되지 않으므로 이 경우 단일 요소 컬렉션만 전송할 수 있습니다
* 컬렉션 필드를 정의하는 데 사용되는 페이로드 예제에는 개체가 하나 이상 있어야 합니다.
* 페이로드 예제의 첫 번째 개체는 전체 컬렉션에 대한 필드를 정의합니다

**용어:**
* 정식 이름: 컬렉션 — 약어: 없음 — 변형: 배열, 목록, 동적 컬렉션
* 동의어: &quot;simple collection&quot; = &quot;scalar 값 목록&quot; ; &quot;object collection&quot; = &quot;오브젝트 배열&quot;
* 혼동하지 마십시오. &quot;listAny&quot; ≠ &quot;listObject&quot;(listAny는 이기종 또는 중첩 배열을 처리하며, listObject는 구조화된 개체의 균일한 배열을 처리합니다.)

**FAQ:**
* **Q: 단순 컬렉션과 개체 컬렉션의 차이점은 무엇입니까?** — 간단한 컬렉션에는 기본 스칼라 값(문자열, 숫자, 부울)이 들어 있고 객체 컬렉션에는 이름이 지정된 여러 필드가 있는 구조화된 객체가 들어 있습니다.
* **Q: 런타임에 컬렉션 매개 변수를 동적으로 만들려면 어떻게 해야 합니까?** — 사용자 지정 작업의 작업 매개 변수 섹션에서 배열 필드를 &quot;변수&quot;로 설정합니다. 그러면 그 안의 모든 객체 필드가 자동으로 변수로 설정됩니다.
* **Q: 중첩된 배열이 사용자 지정 작업 요청 페이로드에서 지원됩니까?** — 부분적으로만 사용할 수 있습니다. 알려진 고정 항목 수가 있는 중첩된 배열은 상수로 정의할 수 있습니다. 동적 항목 수가 있는 중첩된 배열은 요청 페이로드에서 지원되지 않습니다.
* **Q: 여정 테스트 모드에서 컬렉션을 테스트하려면 어떻게 합니까?** — 테스트 인터페이스의 코드 보기 모드를 사용합니다. 비즈니스 이벤트는 코드 보기를 지원하지 않으므로 해당 컨텍스트에서 단일 요소 컬렉션만 테스트할 수 있습니다.
* **Q: 컬렉션에 지원되는 필드 유형은 무엇입니까?** — listString, listInteger, listDecimal, listBoolean, listDateTime, listDateTimeOnly, listDateOnly 및 listObject가 모두 지원됩니다.
