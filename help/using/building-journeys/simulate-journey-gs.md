...
여정은 **대상자 읽기**&#x200B;에 의해 트리거됩니다. 캔버스에는 단일 이벤트 활동이 없으며 프로필은 조건, 대기 및 채널 작업만 통해 이동합니다.

대상자를 읽는 **일괄 여정**&#x200B;을 사용하면 빠른 시뮬레이션 또는 수동 시뮬레이션에 액세스할 수 있습니다.

읽기 전용 대상이 있는 일괄 여정에 대한 ![시뮬레이션 패널](assets/simulate-14.png)

+++

+++ 대상자 읽기 및 단일 이벤트가 포함된 일괄 여정

경로를 따라 하나 이상의 단일 이벤트를 포함하는 세그먼트 트리거 여정. 사용자를에 보낸 후 이벤트 노드에서 기다리는 사용자에 대한 이벤트를 트리거합니다.

대상자 읽기 및 단일 이벤트가 포함된 **일괄 여정**&#x200B;을 사용하면 빠른 시뮬레이션 또는 수동 시뮬레이션에 액세스할 수 있습니다.

![여정 인터페이스의 테스트 모드 단추](assets/simulate-12.png)

+++

+++ 단일 여정

여정 **시작**&#x200B;은(는) 읽기 대상이 아닌 단일 이벤트로 시작됩니다. 시뮬레이션된 사용자는 해당 시작 이벤트가 실행될 때까지 여정에 들어오지 않습니다.

**단일 여정**&#x200B;을 사용하면 수동 시뮬레이션 메뉴에 직접 액세스할 수 있습니다.

단일 여정에 대한 ![시뮬레이션 패널](assets/simulate-13.png)
