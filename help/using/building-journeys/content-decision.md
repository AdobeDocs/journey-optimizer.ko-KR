...
* **TL;DR:** 이 페이지에서는 Journey Optimizer 여정에서 콘텐츠 결정 활동을 구성하고 사용하여 결정 정책을 통해 개인화된 오퍼를 검색하고 조건 및 사용자 지정 작업을 사용하여 해당 오퍼를 라우팅하거나 전달하는 방법을 설명합니다.

**의도:**
* 여정에 콘텐츠 결정 활동 추가 및 결정 정책 구성
* 의사 결정 정책 내에서 의사 결정 항목 및 선택 전략 선택 및 시퀀스 지정
* 활동 최적화 조건에서 컨텐츠 의사 결정 출력을 사용하여 검색된 오퍼를 기반으로 프로필을 분기합니다
* 사용자 지정 작업을 사용하여 검색된 오퍼를 외부 시스템에 전달합니다
* 감사 및 문제 해결을 위해 여정 단계 이벤트의 의사 결정 데이터 검사

**용어집:**
* **콘텐츠 결정 활동**: 결정 정책을 평가하고 각 프로필 *(제품별)에 가장 적합한 오퍼를 검색하는 여정 오케스트레이션 활동*
* **의사 결정 정책**: 평가할 의사 결정 항목 및 선택 전략과 *(제품별)을(를) 반환할 항목 수를 지정하는 구성*
* **선택 전략**: 적합한 오퍼와 채점 방법을 결정하기 위해 의사 결정 정책 내에서 사용되는 등급 평가 방법 *(제품별)*
* **제안**: 선택한 항목과 관련 범위 및 등급 메타데이터 *(제품별)*&#x200B;을(를) 포함하는 의사 결정 정책 실행의 출력 단위입니다.
* **listSize 함수**: 콘텐츠 결정에 의해 반환된 항목 수를 계산하는 데 사용되는 식 편집기 함수입니다(예: `listSize(@decision{Name.items})>0`). *(제품별)*
* **카탈로그 스키마 제공**: 의사 결정 항목에서 사용할 수 있는 특성을 정의하는 스키마입니다. 고급 표현식 편집기 모드 *(제품별)에서 컨텍스트 노드를 통해 액세스할 수 있습니다.*

**보호 기능:**
* 콘텐츠 결정 활동의 출력은 기본 채널 활동(이메일, 푸시, SMS 등)에서 사용할 수 없습니다.
* 콘텐츠 결정 출력은 표현식 편집기의 고급 모드에서만 액세스할 수 있으며 단순 모드에서는 사용할 수 없습니다
* 의사 결정 정책을 작성하려면 의사 결정 권한이 필요합니다
* 동의 정책 업데이트는 의사 결정 정책에 참조된 속성에 대해 적용되는 데 최대 48시간이 소요됩니다
* 동의 정책은 Adobe Healthcare Shield 또는 Privacy and Security Shield 추가 기능이 있는 조직만 사용할 수 있습니다.
* 오퍼 스키마 속성에 대한 DULE(제한된 데이터 사용 레이블)로 인해 거버넌스 정책 위반이 발생할 수 있습니다

**용어:**
* 정식 이름: 컨텐츠 의사 결정 활동 — 약어: 없음 — 변형: 컨텐츠 의사 결정 노드, 의사 결정 활동
* 동의어: &quot;의사 결정 정책&quot; = &quot;오퍼 선택 정책&quot; ; &quot;제안&quot; = &quot;의사 결정 출력&quot;
* 다음과 같이 혼동하지 마십시오. &quot;콘텐츠 결정 활동&quot; ≠ &quot;기본 채널 작업&quot; (콘텐츠 결정은 오퍼를 검색하지만 오퍼를 직접 전달하지는 않습니다. 출력에 대해 작업하려면 사용자 지정 작업 또는 조건이 필요합니다.)

**FAQ:**
* **Q: 콘텐츠 결정 활동에서 반환된 오퍼를 전자 메일에 직접 사용할 수 있습니까?** — 아니요. 컨텐츠 결정 활동의 출력은 기본 채널 활동에서 사용할 수 없습니다. 오퍼를 사용자 지정 작업에 전달하여 외부 시스템으로 보내야 합니다.
* **Q: 프로필에 대해 반환된 오퍼가 있는지 어떻게 확인합니까?** — 고급 표현식 편집기에서 listSize 함수를 사용합니다. `listSize(@decision{ContentdecisionName.items})>0`.
* **Q: 식 편집기에서 콘텐츠 결정 출력에 액세스하는 위치는 어디입니까?** — 고급 모드로 전환하고 컨텍스트 노드를 펼친 다음 의사 결정 정책으로 이동하여 사용 가능한 모든 오퍼 카탈로그 스키마 속성을 확인합니다.
* **Q: 동의 정책 업데이트를 결정 정책에 적용하는 데 얼마나 걸립니까?** — 동의 정책을 업데이트한 후 최대 48시간
* **Q: 여정 단계 이벤트에서 사용할 수 있는 의사 결정 데이터는 무엇입니까?** — 각 단계 이벤트에는 exdRequestID, propositionEventType 및 제안 배열이 포함됩니다. 각각 ID, scopeDetails(결정 공급자, 상관 관계 ID, 결정 정책) 및 ID, 이름, 점수 및 itemSelection 세부 정보가 있는 항목 배열이 포함됩니다.
