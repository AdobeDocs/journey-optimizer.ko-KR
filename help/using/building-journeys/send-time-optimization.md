...
>title="전송 시간 최적화 활성화"
>abstract="클릭 수는 푸시 메시지에 적용할 수 없으므로 푸시 메시지의 기본값은 열람수 옵션으로 설정됩니다. 또한 시스템에서 사용하는 전송 시간을 다음 이내로 전송 옵션에 값을 입력하여 범위 내로 설정할 수 있습니다."

>[!NOTE]
>
>전송 시간 최적화는 여정 내의 기본 제공 전자 메일 및 푸시 동작과 다음 활동을 계속할 최적의 시간을 결정하는 [대기 활동](wait-activity.md#sto-wait)에 사용할 수 있습니다. 현재 캠페인을 통해 전송된 메시지 또는 다른 작업 유형에는 사용할 수 없습니다.
...
>
>* 전송 시간 최적화는 **이메일** 및 **푸시 알림** 채널과 **[!UICONTROL 대기]** 활동에만 적용됩니다.
>
...
## 대기 활동의 전송 시간 최적화 {#sto-wait-activity}

![대기 활동의 전송 시간 최적화 전환](../building-journeys/assets/wait_sto.png)

이메일 및 푸시 동작 외에도 전송 시간 최적화를 **[!UICONTROL 대기]** 활동에 적용할 수도 있습니다. 이는 동일한 전송 시간 최적화 모델을 사용하여 각 프로필의 최적 대기 시간을 계산하지만, 여기에서는 대기 시간이 전송에서 분리됩니다. 이메일 또는 푸시 작업에만 연결되는 대신 사용자 지정 작업과 같은 모든 활동이 뒤에 올 수 있습니다.

[대기 활동에서 전송 시간 최적화를 구성하는 방법에 대해 알아보세요](wait-activity.md#sto-wait).
...
* AI 모델이 동작 데이터를 사용하여 최적의 전송 시간을 예측하는 방법을 이해합니다
* 전송 시간 최적화가 지정된 메시지 유형에 적합한지 확인
* 대기 활동 내에서 전송 시간 최적화를 사용하여 메시지 전송과 분리된 다운스트림 활동 전에 지연시킵니다
...
* 전송 시간 최적화는 여정 내의 이메일 및 푸시 알림 채널, 대기 활동에 적용되며, 캠페인이나 사용자 지정 작업에는 사용할 수 없습니다.
* 전송 시간 최적화에는 자동 시간 규칙에 대한 가시성이 없습니다. 전송 시간 최적화 대기 활동은 다운스트림 채널 작업을 위해 자동 시간 창 내에서 시간을 선택할 수 있습니다. 그런 다음 자동 시간 규칙 구성에 따라 메시지를 대기열에 추가하거나 삭제할 수 있습니다.
...
* **Q: 전송 시간 최적화를 지원하는 채널은 무엇입니까?** — 여정 내의 이메일 및 푸시 알림 채널, 대기 활동. 캠페인 및 사용자 지정 작업은 지원되지 않습니다.
* **Q: 전송 시간 최적화에 자동 시간에 대해 알고 있습니까?** — 아니요. 자동 시간은 프로필이 메시지 작업에 도달할 때만 평가되므로 전송 시간 최적화 대기 활동은 자동 시간 창 내에서 시간을 선택할 수 있습니다. 방해 금지 모드 시간 규칙에 따라 메시지는 방해 금지 모드가 종료될 때까지 대기열에 추가되거나 무시되고 프로필이 여정을 종료합니다. [자세히 알아보기](wait-activity.md#sto-wait)
...
* **Q: 밤에 푸시 알림을 보낼 수 있습니까?** — 예. 사용자의 동작이 야간 참여를 제안하거나 탐색 전송 시간을 선택한 경우. 이를 방지하려면 최대 대기 시간이 짧은 아침 전송 시간을 사용합니다.
