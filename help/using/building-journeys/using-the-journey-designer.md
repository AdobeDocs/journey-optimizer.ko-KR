...
* **TL;DR:** 이 페이지에서는 팔레트에서 이벤트, 오케스트레이션 및 작업 활동을 끌어다 놓아 여러 단계 여정을 만드는 방법을 설명하는 Journey Optimizer 여정 디자이너 캔버스를 소개합니다.

**의도:**

* 여정 디자이너 인터페이스(팔레트, 캔버스, 도구 모음, 활동 구성 창) 탐색
* 여정 캔버스에 이벤트, 오케스트레이션 활동 및 작업 활동 추가
* 시간 초과 또는 오류 시 조건 및 작업 활동에 대한 대체 경로 구성
* 동일한 여정 내 또는 동일한 인스턴스의 다른 여정 간에 활동을 복사하여 붙여넣기
* 이벤트 트리거 또는 대상 읽기 진입점을 사용하여 여정 시작

**용어집:**

* **팔레트**: 캔버스 *(제품별)로 끌어서 놓기 위해 사용 가능한 모든 이벤트, 오케스트레이션 및 작업 활동을 나열하는 여정 디자이너의 왼쪽 패널*
* **캔버스**: 활동을 배치하고 연결하고 구성하는 여정 디자이너의 중앙 디자인 영역 *(제품별)*
* **활동 구성 창**: 캔버스에서 활동을 선택할 때 열리는 오른쪽 패널로, 활동 설정 *(제품별)을 채우는 데 사용됩니다*
* **여정 조각**: 일반 논리 *(제품별)을(를) 다시 빌드하지 않도록 캔버스에 직접 삽입할 수 있는 미리 빌드된 여정 노드의 재사용 가능한 집합*
* **반응 이벤트**: 수신자 추적 상호 작용(열기, 클릭)에 따라 여정을 분기하기 위한 메시지 뒤에 배치되는 이벤트 활동 *(제품별)*

**보호 기능:**

* 작업, 조건, 대기 활동 및 반응 이벤트를 새 여정의 첫 번째 단계로 배치할 수 없습니다.
* 복사/붙여넣기는 동일한 인스턴스 내에서만 지원됩니다. 인스턴스 간 복사/붙여넣기는 지원되지 않습니다.
* 다른 네임스페이스를 사용하는 대상 여정에 이벤트를 복사/붙여넣을 수 없습니다.
* 다른 샌드박스에서 붙여넣은 활동은 대상 여정에 존재하지 않는 데이터를 참조할 수 있습니다.
* 이벤트 및 대기 활동만 동시에 설정할 수 있으며 다른 활동 유형은 동시에 실행할 수 없습니다.
* 대체 경로(시간 초과/오류 대체)는 조건 및 작업 활동에만 사용할 수 있습니다.

**용어:**

* 정식 이름: 여정 Designer — 약어: 없음 — 변형: 여정 캔버스, 오케스트레이션 캔버스
* 동의어: &quot;palette&quot; = &quot;activity panel&quot;; &quot;canvas&quot; = &quot;design area&quot;
* &quot;이벤트&quot;(여정 입력 또는 분기 트리거) ≠ &quot;작업&quot;(고객에게 발생한 일, 예: 메시지 보내기)을 혼동하지 마십시오

**FAQ:**

* **Q: 프로필이 여정을 어떻게 입력합니까?** — 프로필은 구성된 이벤트가 수신될 때 실시간으로 통합적으로 입력하거나 대상자 읽기 활동이 여정을 트리거할 때 일괄적으로 입력합니다.
* **Q: 여정에 여러 이벤트를 추가할 수 있습니까?** — 예. 모두 동일한 네임스페이스를 사용하는 한 여러 이벤트를 추가할 수 있습니다.
* **Q: 작업이 실패할 때 대체 항목을 정의하려면 어떻게 해야 합니까?** — 활동 속성에서 &quot;시간 초과 또는 오류 발생 시 대체 경로 추가&quot; 옵션을 활성화하여 활동 뒤에 대체 경로를 추가합니다.
* **Q: 읽기 전용 여정에서 활동을 복사할 수 있습니까?** — 예. 상태에 관계없이 모든 여정에서 활동을 복사할 수 있지만, 동일한 인스턴스 내에서만 붙여넣을 수 있습니다.
* **Q: 여정 조각이란?** — 일반 논리를 처음부터 다시 작성하지 않도록 캔버스에 직접 삽입할 수 있는 미리 작성된 여정 노드(예: 자격 확인, 환영 시퀀스) 재사용 가능한 세트입니다.
