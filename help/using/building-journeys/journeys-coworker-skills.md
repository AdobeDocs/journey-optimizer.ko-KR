...
## 여정 시뮬레이션 {#journey-simulation}

여정 시뮬레이션 스킬은 AI 기반 빠른 시뮬레이션을 채팅 인터페이스에 도입하여 사용자가 대화식으로 여정 논리의 유효성을 확인할 수 있도록 합니다. 사용자는 Coworker를 통해 시뮬레이션 테스트 데이터를 생성하고 시뮬레이션을 실행 및 관리하며 결과를 검토할 수 있습니다.

### 주요 사용 사례

1. **시뮬레이션된 테스트 데이터 생성**

   * 여정 분기를 실행하는 데 필요한 최소 시뮬레이션 사용자를 생성합니다.
   * 각 분기가 트리거되도록 이벤트가 트리거된 여정에 대한 이벤트 데이터를 생성합니다.
...
1. **시뮬레이션 실행 및 관리**
   ...
   * 시뮬레이션 실행을 재설정합니다.
   * 시뮬레이션 실행 상태를 확인합니다.
   * 실행에 포함된 시뮬레이션된 사용자를 나열합니다.
   * 실행 로그를 검색합니다.

1. **시뮬레이션 결과 검토**

   * 단계별 경로 순회를 포함하여 자세한 결과를 반환합니다.
   * 시뮬레이트된 실행에 대한 분기 결과를 반환합니다.

### 제한 사항

이 기능은 현재 빠른 시뮬레이션 흐름만 지원하며 Journey Optimizer 수동 시뮬레이션 경험을 완전히 대체하지는 않습니다.

빠른 시뮬레이션을 사용하여 여정 논리의 빠르고 자동화된 온전성 검사를 수행할 수 있습니다. 시뮬레이션된 사용자 및 시나리오에 대한 세분화된 제어를 위해 [Journey Optimizer의 수동 시뮬레이션 경험](simulate-journey-gs.md)을 사용하십시오.

이 빠른 시뮬레이션 경험의 일부로 사용자는 다음과 같은 작업을 수행할 수 없습니다.

* 실행에 대해 저장된 시뮬레이션된 기존 사용자를 선택합니다.
* 시뮬레이션을 다시 실행하기 전에 시뮬레이션된 사용자를 편집합니다.
* 채팅을 통해 영구적인 시뮬레이션 사용자를 생성, 탐색, 업데이트 또는 삭제합니다.
* 특정 경로 또는 사용자 지정 테스트 사례를 타깃팅합니다.
