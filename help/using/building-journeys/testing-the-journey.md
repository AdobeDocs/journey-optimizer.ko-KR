...
>id="ajo_journey_test"
>title="여정 테스트"
>abstract="테스트 프로필을 사용하여 게시 전에 여정을 테스트할 수 있습니다. 이를 통해 개인이 게시하기 전에 여정에서 어떻게 이동하는지 분석하고 문제를 해결할 수 있습니다."
...
테스트 프로필만 테스트 모드에서 여정을 입력할 수 있습니다. 새 테스트 프로필을 만들거나 기존 프로필을 테스트 프로필로 전환할 수 있습니다. 다음을 참조하십시오 [섹션](../segment/creating-test-profiles.md).
...
1. 필요한 다양한 필드를 구성합니다. 에서 **프로필 식별자** 필드에서 테스트 프로필을 식별하는 데 사용되는 필드의 값을 입력합니다. 예를 들어 이메일 주소일 수 있습니다. 테스트 프로필과 관련된 이벤트를 보내야 합니다. 자세한 내용은 [이벤트 실행](#firing_events).

   ![](assets/journeyuctest1-bis.png)

1. 이벤트를 받은 후 **[!UICONTROL Show log]** 테스트 결과를 보고 확인하는 단추입니다. 자세한 내용은 [로그 보기](#viewing_logs).
...
다음 **[!UICONTROL Trigger an event]** 버튼을 사용하면 사용자가 여정을 입력할 수 있는 이벤트를 구성할 수 있습니다.
...
테스트 모드 사용 방법에 대한 자세한 내용은 [이 페이지](../building-journeys/testing-the-journey.md).

## 비즈니스 이벤트에 대한 테스트 모드 {#test-business}

비즈니스 이벤트를 사용할 때( [이 섹션](../event/about-events.md)). 여정 모드에서는 이벤트를 시뮬레이션하고 올바른 프로필 ID를 전달할 수 있도록에서 단일 테스트 프로필 입력을 트리거할 수 있습니다. 테스트에 여정을 입력할 이벤트 매개 변수와 테스트 프로필의 식별자를 전달해야 합니다. 를 사용할 수 없습니다 **[!UICONTROL Up to 100 profiles at once]** 다른 세그먼트 기반 여정에 대해 존재하는 옵션. 테스트 모드에서는 비즈니스 이벤트를 기반으로 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.
...
>title="테스트 모드 로그"
>abstract="테스트 결과를 볼 수 있습니다. 이 페이지에는 여정의 현재 정보가 JSON 형식으로 표시됩니다. 버튼을 사용하면 전체 노드를 복사할 수 있습니다. 여정의 테스트 결과를 업데이트하려면 페이지를 수동으로 새로 고쳐야 합니다."
