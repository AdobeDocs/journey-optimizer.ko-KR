...
>id="ajo_journey_test"
>title="여정 테스트"
>abstract="게시하기 전에 테스트 프로필을 사용하여 여정을 테스트합니다. 이를 통해 개인이 게시하기 전에 여정에서 어떻게 이동하는지 분석하고 문제를 해결할 수 있습니다."
...
>[!NOTE]
>
>여정을 테스트하기 전에 모든 오류를 해결해야 합니다(있는 경우). 에서 테스트하기 전에 오류를 확인하는 방법을 알아봅니다. [이 섹션](../building-journeys/troubleshooting.md#checking-for-errors-before-testing).

테스트 모드를 사용하려면 다음 단계를 수행합니다.

1. 테스트 모드를 활성화하려면 **[!UICONTROL 테스트]** 오른쪽 상단 모서리에 있는 전환.

   ![](assets/journeytest1.png)

1. 여정에 하나 이상의 **대기** 활동, 설정 **[!UICONTROL 대기 시간]** 매개 변수를 사용하여 각 대기 활동 및 이벤트 시간 제한이 테스트 모드에서 지속되는 시간을 정의합니다. 기본 시간은 대기 및 이벤트 시간 초과에 대한 10초입니다. 이렇게 하면 테스트 결과가 빠르게 확인됩니다.
...
1. 를 사용하십시오 **[!UICONTROL 이벤트 트리거]** 이벤트를 구성하고 여정에 전송하는 단추.

   ![](assets/journeyuctest1.png)

1. 필요한 다양한 필드를 구성합니다. 에서 **프로필 식별자** 필드에서 테스트 프로필을 식별하는 데 사용되는 필드의 값을 입력합니다. 예를 들어 이메일 주소일 수 있습니다. 테스트 프로필과 관련된 이벤트를 보내야 합니다. [이 섹션](#firing_events)을 참조하십시오.

   ![](assets/journeyuctest1-bis.png)

1. 이벤트를 받은 후 **[!UICONTROL 로그 표시]** 테스트 결과를 보고 확인하는 단추입니다. [이 섹션](#viewing_logs)을 참조하십시오.
...
를 사용하십시오 **[!UICONTROL 이벤트 트리거]** 단추를 클릭하여 사용자가 여정을 입력할 수 있는 이벤트를 구성합니다.

>[!NOTE]
>
>테스트 모드에서 이벤트를 트리거하면 실제 이벤트가 생성되며, 이것은 이 이벤트를 수신하는 다른 여정에도 이 이벤트가 도달함을 의미합니다.

Adobe Experience Platform에서 테스트 프로필로 플래그가 지정된 프로필을 전제 조건으로 알고 있어야 합니다. 실제로 테스트 모드에서는 여정의 이러한 프로필만 허용하며 이벤트에는 ID가 포함되어야 합니다. 예상 ID는 이벤트 구성에 따라 다릅니다. 예를 들어 ECID 또는 이메일 주소일 수 있습니다. 이 키의 값을 **프로필 식별자** 필드.
...
인터페이스를 사용하면 간단한 이벤트 매개 변수를 전달할 수 있습니다. 이 이벤트에서 컬렉션이나 기타 고급 개체를 전달하려면 **[!UICONTROL 코드 보기]** 페이로드의 전체 코드를 확인하고 수정합니다. 예를 들어 기술 사용자가 준비한 이벤트 정보를 복사하여 붙여넣을 수 있습니다.

![](assets/journeytest5.png)

기술 사용자는 이 인터페이스를 사용하여 타사 도구를 사용하지 않고도 이벤트 페이로드를 작성하고 이벤트를 트리거할 수도 있습니다.

를 클릭할 때 **[!UICONTROL 보내기]** 버튼을 클릭하면 테스트가 시작됩니다. 여정에서 개인의 진행 상태는 시각적 플로우로 표시됩니다. 개별 경로가 여정을 이동할 때 점진적으로 녹색으로 바뀝니다. 오류가 발생하면 해당 단계에 경고 기호가 표시됩니다. 커서를 위에 놓으면 오류에 대한 자세한 정보를 표시하고 전체 세부 정보(가능한 경우)에 액세스할 수 있습니다.
...
규칙 기반 이벤트를 사용하는 여정에 대해서도 테스트 모드를 사용할 수 있습니다. 규칙 기반 이벤트에 대한 자세한 내용은 [이 페이지](../event/about-events.md).
...
사용 시 [비즈니스 이벤트](../event/about-events.md)테스트 모드를 사용하여 여정에서 단일 테스트 프로필 입력을 트리거하고 이벤트를 시뮬레이션하고 올바른 프로필 ID를 전달합니다. 테스트에 여정을 입력할 이벤트 매개 변수와 테스트 프로필의 식별자를 전달해야 합니다. 를 사용할 수 없습니다 **[!UICONTROL 한 번에 최대 100개의 프로필]** 다른 세그먼트 기반 여정에 대해 존재하는 옵션. 테스트 모드에서는 비즈니스 이벤트를 기반으로 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.

비즈니스 이벤트를 처음 트리거할 때 동일한 테스트 세션에서 비즈니스 이벤트 정의를 변경할 수 없습니다. 동일한 개인 또는 다른 개인만 동일하거나 다른 식별자를 전달하는 여정을 입력할 수 있습니다. 비즈니스 이벤트 매개 변수를 변경하려면 테스트 모드를 중지하고 다시 시작해야 합니다.
...
>title="테스트 모드 로그"
>abstract="로그 표시 단추에는 테스트 결과가 JSON 형식으로 표시됩니다. 이러한 결과는 여정 내의 개인 수와 해당 상태를 표시합니다."

다음 **[!UICONTROL 로그 표시]** 단추를 사용하면 테스트 결과를 볼 수 있습니다. 이 페이지에는 여정의 현재 정보가 JSON 형식으로 표시됩니다. 버튼을 사용하면 전체 노드를 복사할 수 있습니다. 여정의 테스트 결과를 업데이트하려면 페이지를 수동으로 새로 고쳐야 합니다.
