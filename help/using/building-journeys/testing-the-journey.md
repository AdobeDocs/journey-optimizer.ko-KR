...
테스트 프로필만 테스트 모드에서 여정에 들어갈 수 있습니다. 새 테스트 프로필을 만들거나 기존 프로필을 테스트 프로필로 만들 수 있습니다. 에서 테스트 프로필에 대해 자세히 알아보기 [이 섹션](../audience/creating-test-profiles.md).
...
사용 시 [비즈니스 이벤트](../event/about-events.md), 테스트 모드를 사용하여 여정에서 단일 테스트 프로필 시작을 트리거하고 이벤트를 시뮬레이션하며 올바른 프로필 ID를 전달합니다. 테스트에 여정을 입력할 테스트 프로필의 식별자와 이벤트 매개 변수를 전달해야 합니다. 를 사용할 수 없습니다. **[!UICONTROL 한 번에 최대 100개의 프로필]** 다른 대상 기반 여정에 대해 존재하는 옵션입니다. 테스트 모드에서는 비즈니스 이벤트를 기반으로 하는 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.
