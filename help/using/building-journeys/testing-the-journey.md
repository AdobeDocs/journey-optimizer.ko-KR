...
비즈니스 이벤트([이 섹션](../event/about-events.md) 참조)를 사용할 때 테스트 모드에서는 여정에서 단일 테스트 프로필 입력을 트리거하고 이벤트를 시뮬레이션하고 올바른 프로필 ID를 전달할 수 있습니다. 테스트에 여정을 입력할 이벤트 매개 변수와 테스트 프로필의 식별자를 전달해야 합니다. 다른 세그먼트 기반 여정에 대해 존재하는 **[!UICONTROL Up to 100 profiles at once]** 옵션을 사용할 수 없습니다. 테스트 모드에서는 비즈니스 이벤트를 기반으로 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.
