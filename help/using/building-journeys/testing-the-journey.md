...
1. 여정에 하나 이상의 **대기** 활동, 설정 **[!UICONTROL Wait time]** 매개 변수를 사용하여 각 대기 활동 및 이벤트 시간 제한이 테스트 모드에서 지속되는 시간을 정의합니다. 기본 시간은 대기 및 이벤트 시간 초과에 대한 10초입니다. 이렇게 하면 테스트 결과가 빠르게 확인됩니다.
...
1. 를 사용하십시오 **[!UICONTROL Trigger an event]** 이벤트를 구성하고 여정에 전송하는 단추.
...
1. 이벤트를 받은 후 **[!UICONTROL Show log]** 테스트 결과를 보고 확인하는 단추입니다. [이 섹션](#viewing_logs)을 참조하십시오.
...
를 사용하십시오 **[!UICONTROL Trigger an event]** 단추를 클릭하여 사용자가 여정을 입력할 수 있는 이벤트를 구성합니다.
...
를 클릭할 때 **[!UICONTROL Send]** 버튼을 클릭하면 테스트가 시작됩니다. 여정에서 개인의 진행 상태는 시각적 플로우로 표시됩니다. 개별 경로가 여정을 이동할 때 점진적으로 녹색으로 바뀝니다. 오류가 발생하면 해당 단계에 경고 기호가 표시됩니다. 커서를 위에 놓으면 오류에 대한 자세한 정보를 표시하고 전체 세부 정보(가능한 경우)에 액세스할 수 있습니다.
...
사용 시 [비즈니스 이벤트](../event/about-events.md)테스트 모드를 사용하여 여정에서 단일 테스트 프로필 입력을 트리거하고 이벤트를 시뮬레이션하고 올바른 프로필 ID를 전달합니다. 테스트에 여정을 입력할 이벤트 매개 변수와 테스트 프로필의 식별자를 전달해야 합니다. 를 사용할 수 없습니다 **[!UICONTROL Up to 100 profiles at once]** 다른 세그먼트 기반 여정에 대해 존재하는 옵션. 테스트 모드에서는 비즈니스 이벤트를 기반으로 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.
