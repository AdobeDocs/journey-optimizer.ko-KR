...
테스트 프로필만 테스트 모드에서 여정에 들어갈 수 있습니다. 새 테스트 프로필을 만들거나 기존 프로필을 테스트 프로필로 만들 수 있습니다. 에서 테스트 프로필에 대해 자세히 알아보기 [이 섹션](../segment/creating-test-profiles.md).

>[!NOTE]
>
>여정을 테스트하기 전에 모든 오류가 있는 경우 이를 해결해야 합니다. 에서 테스트하기 전에 오류를 확인하는 방법 알아보기 [이 섹션](../building-journeys/troubleshooting.md#checking-for-errors-before-testing).
...
1. 여정에 하나 이상의 **대기** 활동, 설정 **[!UICONTROL 대기 시간]** 테스트 모드에서 각 대기 활동 및 이벤트 시간 초과가 지속되는 시간을 정의하는 매개 변수. 대기 및 이벤트 시간 초과에 대한 기본 시간은 10초입니다. 이렇게 하면 테스트 결과를 빠르게 얻을 수 있습니다.
...
1. 사용 **[!UICONTROL 이벤트 트리거]** 버튼을 클릭하여 이벤트를 구성하고 여정에 보냅니다.

   ![](assets/journeyuctest1.png)

1. 예상과 다른 필드를 구성합니다. 다음에서 **프로필 식별자** 필드에 테스트 프로필을 식별하는 데 사용되는 필드 값을 입력합니다. 예를 들어 이메일 주소일 수 있습니다. 테스트 프로필과 관련된 이벤트를 보내야 합니다. [이 섹션](#firing_events)을 참조하십시오.
...
* 실시간 고객 프로필 서비스에서 &quot;테스트 프로필&quot;로 플래그가 지정된 개인만 테스트된 여정에 입력할 수 있습니다. 이 [섹션](../segment/creating-test-profiles.md)을 참조하십시오.
...
사용 **[!UICONTROL 이벤트 트리거]** 버튼을 클릭하여 여정을 입력할 수 있는 이벤트를 구성합니다.

>[!NOTE]
>
>테스트 모드에서 이벤트를 트리거하면 실제 이벤트가 생성됩니다. 즉, 이 이벤트를 수신하는 다른 여정에 이벤트가 발생합니다.

전제 조건으로 Adobe Experience Platform에서 테스트 프로필로 플래그가 지정된 프로필을 알아야 합니다. 실제로 테스트 모드에서는 여정의 이러한 프로필만 허용하며 이벤트에는 ID가 포함되어야 합니다. 예상 ID는 이벤트 구성에 따라 다릅니다. 예를 들어 ECID 또는 이메일 주소일 수 있습니다. 이 키의 값을 다음에 추가해야 합니다. **프로필 식별자** 필드.
...
인터페이스를 사용하면 간단한 이벤트 매개 변수를 전달할 수 있습니다. 이벤트에서 컬렉션이나 기타 고급 객체를 전달하려면 다음을 클릭하면 됩니다. **[!UICONTROL 코드 보기]** 페이로드의 전체 코드를 확인하고 수정합니다. 예를 들어 기술 사용자가 준비한 이벤트 정보를 복사하여 붙여넣을 수 있습니다.

![](assets/journeytest5.png)

기술 사용자는 또한 이 인터페이스를 사용하여 서드파티 도구를 사용하지 않고도 이벤트 페이로드를 작성하고 이벤트를 트리거할 수 있습니다.

클릭 시 **[!UICONTROL 보내기]** 버튼을 누르면 테스트가 시작됩니다. 여정에서 개인의 진행은 시각적인 흐름에 의해 표현된다. 개인이 여정을 가로질러 이동할 때 경로가 점진적으로 녹색으로 바뀝니다. 오류가 발생하면 해당 단계에 경고 기호가 표시됩니다. 커서를 위에 놓으면 오류에 대한 자세한 정보가 표시되고 전체 세부 정보에 액세스할 수 있습니다(가능한 경우).
...
여정 기반 이벤트를 사용하는 테스트에도 테스트 모드를 사용할 수 있습니다. 규칙 기반 이벤트에 대한 자세한 내용은 [이 페이지](../event/about-events.md).

이벤트를 트리거할 때 **이벤트 구성** 화면에서는 테스트에서 전달할 이벤트 매개 변수를 정의할 수 있습니다. 오른쪽 위 모서리에 있는 도구 설명 아이콘을 클릭하여 이벤트 ID 조건을 볼 수 있습니다. 규칙 평가의 일부인 각 필드 옆에서도 도구 설명을 사용할 수 있습니다.

![](assets/jo-event8.png)

## 비즈니스 이벤트의 테스트 모드 {#test-business}

사용 시 [비즈니스 이벤트](../event/about-events.md), 테스트 모드를 사용하여 여정에서 단일 테스트 프로필 시작을 트리거하고 이벤트를 시뮬레이션하며 올바른 프로필 ID를 전달합니다. 테스트에 여정을 입력할 테스트 프로필의 식별자와 이벤트 매개 변수를 전달해야 합니다. 를 사용할 수 없습니다. **[!UICONTROL 한 번에 최대 100개의 프로필]** 다른 세그먼트 기반 여정에 대해 존재하는 옵션입니다. 테스트 모드에서는 비즈니스 이벤트를 기반으로 하는 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.
...
* _currentstep_ > _fetchError_: 이 단계 중에 발생한 데이터 가져오기 오류에 대한 정보입니다.
* _외부 키_: 이벤트에 정의된 키 공식 값.
* _보강된 데이터_: 여정이 데이터 소스를 사용하는 경우 여정이 검색한 데이터입니다.
...
* _시간 초과_: 시간이 너무 오래 걸린 단계 때문에 개인이 여정에서 중지됩니다.
