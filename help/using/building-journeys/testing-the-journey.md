...
여정을 테스트하려면 먼저 모든 오류를 해결해야 합니다. [이 섹션](../building-journeys/troubleshooting.md#section_h3q_kqk_fhb)을 참조하십시오.
...
1. 여정을 테스트하기 전에이 유효한지 그리고 오류가 없는지 확인하십시오. 오류가 있는 여정 테스트를 시작할 수 없습니다. [이 섹션](../building-journeys/troubleshooting.md#section_h3q_kqk_fhb)을 참조하십시오. 오류가 있으면 경고 기호가 표시됩니다.
...
Adobe Experience Platform에서 테스트 프로필로 플래그가 지정된 프로파일을 전제 조건으로 알고 있어야 합니다. 실제로 테스트 모드에서는 여정에 이러한 프로파일만 사용할 수 있으며 이벤트에는 ID가 포함되어야 합니다. 예상 ID는 이벤트 구성에 따라 다릅니다. 예를 들어 ECID 또는 이메일 주소를 사용할 수 있습니다. 이 키의 값을 **프로필 식별자** 필드에 추가해야 합니다.
...
비즈니스 이벤트([이 섹션](../event/about-events.md) 참조)를 사용할 때 테스트 모드를 사용하면 여정에서 단일 테스트 프로필 시작을 트리거하여 이벤트를 시뮬레이션하고 올바른 프로필 ID를 전달할 수 있습니다. 테스트에 여정을 입력할 테스트 프로필의 식별자 및 이벤트 매개 변수를 전달해야 합니다. 다른 세그먼트 기반 여정에 존재하는 **[!UICONTROL Up to 100 profiles at once]** 옵션은 사용할 수 없습니다. 테스트 모드에서는 비즈니스 이벤트를 기반으로 하는 여정에 사용할 수 있는 &quot;코드 보기&quot; 모드가 없습니다.

비즈니스 이벤트를 처음 트리거하는 경우 동일한 테스트 세션에서 비즈니스 이벤트 정의를 변경할 수 없습니다. 동일한 개인 또는 다른 개별 사람만 동일한 또는 다른 식별자를 전달하는 여정에 입장하도록 할 수 있습니다. 비즈니스 이벤트 매개 변수를 변경하려면 중지했다가 다시 테스트 모드를 시작해야 합니다.
...
* _currentstep_ > 단계:개인 여정 상태(실행 중, 완료, 오류 또는 시간 초과)입니다. 자세한 내용은 아래 내용을 참조하십시오.
