...
이러한 각 채널에 대해 [!DNL Adobe Campaign] 표준 트랜잭션 메시지 **템플릿**&#x200B;을(를) 선택합니다. 기본 제공 이메일, SMS 및 푸시 채널의 경우 트랜잭션 메시지를 사용하여 메시지 전송을 실행합니다. 즉, 여정에서 특정 메시지 템플릿을 사용하려면 [!DNL Adobe Campaign] Standard에 게시해야 합니다. 이 기능을 사용하는 방법에 대해 알아보려면 [이 페이지](https://experienceleague.adobe.com/docs/campaign-standard/using/communication-channels/transactional-messaging/getting-started-with-transactional-msg.html?lang=ko)를 참조하세요.
...
>[!NOTE]
>
>실시간 트랜잭션 메시지(rtEvent)를 보내거나 사용자 지정 작업 덕분에 서드파티 시스템으로 메시지를 라우팅하는 경우 피로, 차단 목록 또는 구독 취소 관리에 특정 설정이 필요합니다. 예를 들어, &quot;unsubscribe&quot; 특성이 [!DNL Adobe Experience Platform] 또는 서드파티 시스템에 저장되어 있는 경우 메시지를 보내기 전에 조건을 추가해야 이 조건을 확인할 수 있습니다.
...
>[!NOTE]
>
>이 범주는 &quot;이벤트&quot; 트랜잭션 메시지를 선택한 경우에만 표시됩니다. &quot;프로필&quot; 메시지의 경우 **[!UICONTROL 주소]** 필드는 시스템에서 [!DNL Adobe Campaign] 표준에서 자동으로 검색됩니다.
...
[!DNL Adobe Campaign] 표준 메시지에 필요한 필드입니다. 이러한 필드는 메시지를 개인화하거나, 조건부 서식을 적용하거나, 특정 메시지 변형을 선택하는 데 사용할 수 있습니다.
