...
워크플로우를 개선하고 보다 복잡한 비즈니스 요구 사항을 처리합니다.
...
* **여러 표준 시간대 지원**: 각 수신자의 현지 표준 시간대에서 월요일 배달을 보장하기 위해 지리적 지역에 따라 별도의 여정을 만드십시오.

### 시간대 관리

`now()` 함수 및 여정 실행은 여정 수준에서 구성된 시간대를 사용합니다. 다음과 같은 주요 사항을 고려하십시오.

* **여정 시간대**: 게시하기 전에 여정 속성에서 구성하여 여정 시간대가 사용자의 요구 사항과 일치하는지 확인하십시오. [시간대 관리에 대해 자세히 알아보기](timezone-management.md)
...
항목 유형에 따라 여정 타이밍을 구성합니다.

* **대상자 여정 읽기**: [대상자 읽기를 예약](read-audience.md#schedule)하여 대상자에게 알맞은 시간에 트리거합니다. 업무 커뮤니케이션에는 새벽 실행(예: 오전 6:00)이 일반적입니다.
...
프로덕션에 게시하기 전에 항상 여정 논리를 테스트하십시오.
...
[테스트 여정에 대해 자세히 알아보기](testing-the-journey.md)

### 재입력 및 빈도

반복 캠페인의 경우 프로필 재입력을 신중하게 관리합니다.

* **다시 입력 구성**: **[!UICONTROL 다시 입력]** 설정을 적절히 설정합니다. [다시 시작 설정에 대해 자세히 알아보기](entry-management.md)
* **일관된 동작**: 프로필이 여정에 다시 입력할 수 있는 경우 프로필은 매일 검사를 받으며 주말 항목이 항상 월요일 큐에 있는지 확인합니다.
* **빈도 제한**: 프로필이 자주 다시 들어갈 수 있는 경우 과도한 메시지를 방지하기 위해 [빈도 제한 규칙](../conflict-prioritization/journey-capping.md)을 추가하는 것이 좋습니다.

## 고급 변형

### 특정일 타기팅

특정 요일(예: 화요일 및 목요일)에만 이메일을 보내는 방법:

1. 특정 요일을 확인하려면 **조건을 수정**&#x200B;하세요.

   ```javascript
   dayOfWeek(now()) == 3 or dayOfWeek(now()) == 5
   ```

2. 다음 화요일이나 목요일까지 일수를 계산하는 다른 모든 요일에 대해 **대기 활동을 추가**&#x200B;합니다.

### 다른 요일에 대해 다른 전송 시간

유연한 예약을 위해 서로 다른 대기 공식을 사용하여 여러 경로를 만듭니다.

* **토요일 → 수요일 배달**: `nowWithDelta(4, "days")` 사용
* **일요일 → 화요일 배달**: `nowWithDelta(2, "days")` 사용

이 접근 방식을 사용하면 비즈니스 요구 사항에 따라 게재 일을 사용자 정의할 수 있습니다.

### 업무 시간 게재

업무 시간 동안 납품하려면

1. 대기 수식에서 **시간 매개 변수를 조정**&#x200B;합니다. 예를 들어 오전 9시가 아닌 오후 2시에 게재하는 경우:

   ```javascript
   setHours(nowWithDelta(1, "days"), 14)
   ```

2. **시간 확인 추가**(선택 사항): 보내기 전 현재 시간이 업무 시간 내인지 확인하기 위해 대기 후 두 번째 조건을 추가합니다.

### 휴일 제외

이메일 전송에서 휴일을 제외하려면 다음 작업을 수행하십시오.

1. 특정 휴일 날짜를 확인하려면 **조건 경로를 추가**&#x200B;하세요.

   ```javascript
   toDateTimeOnly(now()) == toDateTimeOnly("2024-12-25T00:00:00")
   ```

2. 상태가 휴일과 일치하는 경우 **대기 활동을 추가**&#x200B;하여 다음 영업일까지 지연합니다.

[날짜 비교 함수에 대해 자세히 알아보기](functions/date-functions.md)
