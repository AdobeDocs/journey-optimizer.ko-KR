...
워크플로우를 개선하고 더 복잡한 비즈니스 요구 사항을 처리하기 위해 휴일, 시간대 또는 특정 비즈니스 시간을 기본 평일 검사 이상으로 고려하여 수식을 확장할 수 있습니다. 최적의 전송 시간과 일치하도록 대기 공식에서 시간 매개 변수(H)를 조정합니다. 예를 들어 오전 10시에 참여율이 더 좋으면 공식을 변경하여 시간 10을 사용합니다. 여러 시간대를 지원하는 경우 월요일 배달을 각 수신자의 현지 시간대로 보장하기 위해 지리적 지역별로 별도의 여정을 만드는 것이 좋습니다.

+++

+++**표준 시간대 관리**

`now()` 함수 및 여정 실행은 여정 수준에서 구성된 시간대를 사용합니다. 게시 전에 여정 속성에서 여정 시간대를 구성하여 사용자의 요구 사항과 일치하는지 확인하십시오([시간대 관리에 대해 자세히 알아보기](timezone-management.md)). 대상이 여러 시간대에 걸쳐 있는 경우 요일 검사는 수신자의 현지 시간대가 아니라 여정이 구성한 시간대에서 수행됩니다. 시간대별 게재의 경우, 다른 지역에 대해 별도의 여정을 만들거나 대상 읽기 활동에서 시간대 설정을 사용합니다.

+++

+++**여정 항목 및 시간**

일괄 처리 여정의 경우 대상자에 적합한 시간에 트리거하도록 [대상자 읽기를 예약](read-audience.md#schedule)합니다. 새벽 실행(예: 오전 6:00)은 비즈니스 커뮤니케이션에서 일반적입니다. 이벤트 기반 여정의 경우, 이벤트가 수신되면 즉시 조건이 평가되고 주말에 입력한 프로필은 월요일까지 자동으로 대기합니다([이벤트에 대해 자세히 알아보기](../event/about-events.md)). [여정 시간 제한 설정](journey-properties.md#timeout)이 최대 대기 기간(토요일에서 월요일까지 최대 2일)을 수용하는지 확인하십시오.

+++

+++**테스트가 필수임**

구현 안내서에서 강조했듯이 항상 여정 논리를 테스트하여 모든 것이 예상대로 작동하는지 확인하십시오. **테스트 모드**&#x200B;를 사용하여 실제 전자 메일을 보내지 않고 다양한 시작 시나리오를 시뮬레이션하세요. 세 개의 경로(토요일 항목, 일요일 항목 및 평일 항목)를 모두 테스트하고, 대기 기간 계산이 올바른지 확인하고, 지정된 시간에 월요일 게재가 발생하는지 확인한 다음, 여정 시각화를 확인하여 경로 라우팅이 적절한지 확인합니다.

+++

+++**다시 시작 및 빈도**

반복 캠페인의 경우 **[!UICONTROL 다시 시작]** 설정을 적절히 구성하십시오([다시 시작 설정에 대해 자세히 알아보기](entry-management.md)). 프로필이 여정에 다시 입장할 수 있는 경우 매번 요일 확인을 받게 되므로 주말 게시물이 항상 월요일 큐에 올라가 있습니다. 프로필이 자주 다시 들어갈 수 있는 경우 과도한 메시지를 방지하기 위해 [빈도 제한 규칙](../conflict-prioritization/journey-capping.md)을 추가하는 것이 좋습니다.
...
주말 동작마다 대기 공식이 다른 여러 경로를 만들 수 있습니다. 예를 들어, 토요일부터 수요일까지 배달하려면 `nowWithDelta(4, "days")`을(를) 사용하고, 일요일부터 화요일까지 배달하려면 `nowWithDelta(2, "days")`을(를) 사용하십시오. 이렇게 하면 전송 일정을 보다 유연하게 조정할 수 있습니다.
