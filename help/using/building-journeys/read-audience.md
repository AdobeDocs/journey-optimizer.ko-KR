...
* 일괄 처리 세그먼트를 사용할 때는 여정이 시작되기 전에 수집 및 일별 스냅샷 업데이트가 완료되었는지 확인하십시오. 세그먼트가 같은 날 수집된 데이터를 반영해야 하는 경우 추가 대기 기간을 고려하십시오. 즉각적인 프로필 새로 고침이 중요한 경우 이벤트 기반 또는 스트리밍 접근 방식을 사용하거나 **대기** 활동을 추가하여 평가하기 전에 업데이트된 데이터가 전파되도록 하십시오.
