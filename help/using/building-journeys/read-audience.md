...
* 를 사용하는 여정의 경우 **대상자 읽기** 활동. 정확히 동시에 시작할 수 있는 최대 여정 수가 있습니다. 재시도는 시스템에서 수행되지만 5개 이상의 여정이 있는 것은 피함(사용) **대상자 읽기**, 예약됨 또는 &quot;가능한 한 빨리&quot; 시작), 정확히 같은 시간에 시작. 가장 좋은 방법은 예를 들어 5분에서 10분 간격과 같이 시간이 지남에 따라 이를 분산시키는 것입니다.

* 경험 이벤트 필드 그룹은 다음으로 시작하는 여정에서 사용할 수 없습니다. **대상자 읽기** 활동, **[대상 자격 조건](audience-qualification-events.md)** 활동 또는 비즈니스 이벤트 활동.
...
* 대상 [csv 파일에서 가져옴](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/overview.html#import-audience) 또는 다음의 결과물 [컴포지션 워크플로](../audience/get-started-audience-orchestration.md) 에서 선택할 수 있습니다. **대상자 읽기** 활동. 이들 대상자는 다음에서 사용할 수 없습니다. **대상 자격 조건** 활동.
...
1. 다음에서 **[!UICONTROL 대상자]** 필드에서 여정을 입력할 Adobe Experience Platform 대상을 선택한 다음 을(를) 클릭합니다 **[!UICONTROL 저장]**. 을 사용하여 생성된 모든 Adobe Experience Platform 대상을 선택할 수 있습니다. [세그먼트 정의](../audience/creating-a-segment-definition.md).
   ...
   >[!NOTE]
   >
   >또한 을 사용하여 만든 Adobe Experience Platform 대상을 타깃팅할 수도 있습니다. [대상자 구성](../audience/get-started-audience-orchestration.md) 또는 [csv 파일에서 업로드됨](https://experienceleague.adobe.com/docs/experience-platform/segmentation/ui/overview.html#import-audience){target="_blank"}.
...
1. 다음 **[!UICONTROL 대상자 읽기]** 활동을 통해 대상자가 여정을 입력하는 시간을 지정할 수 있습니다. 이렇게 하려면 **[!UICONTROL 여정 일정 편집]** 여정 속성에 액세스한 다음 **[!UICONTROL 스케줄러 유형]** 필드.
   ...
   **증분 읽기** 옵션: 여정에 반복이 있는 경우 **대상자 읽기** 은 처음으로 실행되며 대상자의 모든 프로필이 여정에 들어갑니다. 이 옵션을 사용하면 여정의 마지막 실행 이후 대상에 들어온 개인만 첫 번째 발생 후 타깃팅할 수 있습니다.
       ...
       >[!NOTE]
       >
       >여정에서 [사용자 정의 여정 대상](../audience/about-audiences.md#segments-in-user-optimizer)을 타겟팅하는 경우 이러한 대상이 수정되므로 반복 여정에서 이 옵션이 활성화된 경우 첫 번째 반복에서만 프로필이 검색됩니다.
