...
| 날짜 | 끔 | [!DNL Journey Optimizer]은(는) 마지막 실행 이후 대상자에 추가된 프로필만 읽습니다. 여정에서 여전히 활성 상태인 프로필은 자동으로 재설정되지 않습니다. |
| 끔 | 날짜 | [!DNL Journey Optimizer]은(는) 다음 실행을 시작하기 전에 현재 여정 실행에서 활성 참가자를 제거한 다음 전체 대상을 다시 읽습니다. 이렇게 하면 프로필이 새로운 발생 시 새로 시작할 수 있습니다. |
| 날짜 | 날짜 | [!DNL Journey Optimizer]은(는) 다음 실행을 시작하기 전에 현재 여정 실행에서 활성 참가자를 제거한 다음 마지막 실행 이후 대상에 추가된 프로필만 읽습니다. 강제 재입력은 활성 여정 기여도를 재설정하지만 증분 읽기는 여전히 선택을 새로 추가된 대상 구성원으로 제한합니다. |
