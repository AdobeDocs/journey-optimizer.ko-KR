...
대상자 읽기 활동을 사용하여 정의된 대상자로 여정을 시작합니다. 대상자와 대상 실행 시기를 선택한 다음 [조건을 사용하여 분기, 타이머 및 작업을 세그먼트화, 제외 또는 병합하고](#audience-targeting-in-journeys), 타이머 및 작업을 통해 각 프로필의 경로를 개인화합니다.
...
**주요 제한 사항:** 여정 당 읽기 대상(첫 번째 활동 또는 비즈니스 이벤트 후 두 번째여야 함), 활동당 하나의 대상, 조직당 최대 5개의 동시 읽기 대상 실행, 샌드박스당 초당 20,000개의 프로필, 12시간 작업 시간 제한. [보호 기능 및 권장 사항](#must-read)의 전체 세부 정보.
...
>title="네임스페이스"
>abstract="여정에 들어가는 개인을 식별하는 데 사용되는 ID(예: 이메일, ECID)를 선택합니다. 비즈니스 규칙 및 캡핑과 가장 잘 호환되도록 하려면 목록에서 맨 위 옵션을 선택합니다."
...
* 한 여정에서 **[!UICONTROL 대상자 읽기]** 활동을 하나만 사용할 수 있습니다. 캔버스에서 첫 번째 활동이어야 합니다. 단, **비즈니스 여정** 활동으로 시작하는 이벤트는 예외이며, 이 경우 두 번째 활동입니다.
...
>[!TIP]
>
>아웃바운드 메시지를 한 번에 전달하는 대신 시간에 따라 일괄적으로 전달하려면 여정 일정에서 웨이브 전송을 구성할 수 있습니다. [여정에서 웨이브를 사용하여 보내는 방법 알아보기](send-using-waves.md)
...
>[!NOTE]
>
>[**최적화** 활동](optimize.md)은(는) 실험 및 타깃팅 규칙을 포함한 고급 경로 라우팅에 대한 대안을 제공합니다. 현재 제한된 가용성 상태입니다. 자세한 내용은 Adobe 담당자에게 문의하십시오.
...
위의 단계를 수행한 후에도 개수 불일치 또는 제로 프로필 실행이 지속되는 경우 [Adobe 지원 센터에 문의](../start/user-interface.md#support-ticket-guidelines)하십시오. 준비 완료: 대상 이름/ID, 여정 이름/ID, 예약된 실행 시간, 샌드박스 및 불일치에 대한 간단한 설명(예: &quot;대상은 10K를 실현했으며 [날짜]&quot;에 여정을 입력한 것은 2K에 불과합니다.).
