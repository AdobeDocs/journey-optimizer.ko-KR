...
>[!NOTE]
>
>대상 읽기 활동을 사용하는 여정의 경우 정확히 동시에 시작할 수 있는 최대 여정 수가 있습니다. 재시도는 시스템에서 수행되지만 5분에서 10분 간격과 같이 시간에 따라 분산하여 정확히 같은 시간에 시작하는 5개 이상의 여정(대상 읽기, 예약 또는 &quot;가능한 한 빨리&quot; 시작)를 사용하지 마십시오.
>
...
1. 설정 **[!UICONTROL 조절률]** 대상 읽기 활동의 처리량 제한에 대한 필드입니다.

   이 값은 여정 버전 페이로드에 저장됩니다. 기본값은 초당 5,000개의 메시지입니다. 이 값은 초당 500개에서 20,000개의 메시지로 수정할 수 있습니다.

   >[!NOTE]
   >
   >샌드박스당 전체 전송률이 초당 20,000개의 메시지로 설정됩니다. 따라서 동일한 샌드박스에서 동시에 실행되는 모든 읽기 대상의 전송률이 초당 최대 20,000개의 메시지를 추가합니다. 이 캡은 수정할 수 없습니다.
...
다음 **[!UICONTROL 대상자 읽기]** 활동을 사용하면 단일 프로필에서 여정을 테스트하거나, 대상자에게 적합한 프로필 중에서 선택한 100개의 프로필을 무작위로 테스트할 수 있습니다.
