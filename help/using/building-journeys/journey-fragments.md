...
여정 조각은 한 번 생성하여 샌드박스 내 모든 여정에 배치할 수 있는 재사용 가능한 여정 노드 세트입니다. 자격 확인, 기본 채널 라우팅 논리 또는 환영 시퀀스 중 어느 것이든 조각은 팀이 더 빨리 이동하고 일관성을 유지하는 데 도움이 됩니다. 매번 처음부터 동일한 논리를 다시 작성하지 않아도 됩니다. [사용 사례 예제를 참조하십시오.](#examples)

생성된 조각은 전용 **[!UICONTROL 조각 인벤토리]**&#x200B;에 저장되며 **[!UICONTROL 여정 조각]** 활동을 사용하여 모든 여정에 삽입할 수 있습니다.
...
>title="여정 조각 속성"
>abstract="인벤토리에서 조각을 열면 노드, 속성, 태그 또는 레이블을 수정할 수 있습니다. 활성 조각을 편집하려면 먼저 비활성화해야 합니다."
...
**여정 조각은 조각(콘텐츠 조각)과 어떻게 다릅니까?**

**여정 조각**&#x200B;은(는) **[!UICONTROL 여정 조각]** 활동을 사용하여 여정에 삽입하는 재사용 가능한 여정 노드 집합(예: 자격 확인 또는 채널 라우팅 논리)입니다. **[조각](../content-management/fragments.md)**&#x200B;은(는) 캠페인 및 여정의 전자 메일 내에서 사용되는 재사용 가능한 콘텐츠 구성 요소(예: 머리글이나 바닥글)입니다. 즉, 여정 조각은 재사용 가능한 *논리*&#x200B;이지만 콘텐츠 조각은 재사용 가능한 *콘텐츠*&#x200B;입니다.

**여정 조각은 AEM 콘텐츠 조각과 어떻게 다릅니까?**

**[AEM 콘텐츠 조각](../integrations/aem-fragments.md)**&#x200B;은(는) Adobe Experience Manager에서 작성되고 [!DNL Journey Optimizer]개의 메시지에서 재사용되는 콘텐츠입니다. 여정 논리가 아닙니다. 반면 여정 조각은 [!DNL Journey Optimizer] 내에 빌드 및 저장되며 연결된 여정 노드 집합을 나타냅니다.

**여정 조각을 업데이트하면 기존 여정도 업데이트됩니까?**

아니요. 여정 조각에서 **복사 동작**&#x200B;을 사용합니다. 조각을 삽입하면 해당 노드의 정적 복사본이 만들어집니다. 원래 조각에 대한 모든 업데이트는 이미 사용한 여정에 반영되지 않습니다.
