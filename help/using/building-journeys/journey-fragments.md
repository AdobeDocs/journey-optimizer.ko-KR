...
>[!NOTE]
>이 첫 번째 릴리스에서 여정 조각은 **복사 동작**&#x200B;을 사용합니다. 여정에 조각을 삽입하면 원본 노드의 정적 복사본이 만들어집니다. 원래 조각에 대한 모든 업데이트는 이미 사용한 여정에 자동으로 반영되지 않습니다.
...
>title="여정 조각으로 저장"
>abstract="조각의 고유 이름을 입력하고 저장 을 클릭합니다. 선택한 노드는 조각 인벤토리에서 사용할 수 있는 재사용 가능한 조각으로 저장됩니다."
...
>[!TIP]
>여정에서 조각을 만드는 경우 [여정을 테스트하고](testing-the-journey.md) 조각을 저장하는 **이전**&#x200B;을 통해 선택한 노드가 예상대로 작동하는지 확인하십시오.
...
>[!CAUTION]
>조각 편집기에서 테스트 모드를 사용할 수 없습니다. 즉, 조각이 활성화되고 여정에 삽입되기 전에 구성된 활동의 동작을 확인할 수 없습니다. 논리 정확도가 중요한 조각의 경우 먼저 [전체 여정에서 노드를 빌드하고 테스트](testing-the-journey.md)한 다음 위의 캔버스 탭에서 조각으로 저장하는 것이 좋습니다.
...
>
>* 조각 편집기에서 테스트 모드를 사용할 수 없습니다. 노드를 조각으로 저장하기 전에 전체 여정에서 여정 수준 논리를 테스트합니다.
>
...
**[!UICONTROL 초안]** 조각을 활성화하려면 해당 조각을 열고 **[!UICONTROL 활성화]** 아이콘을 사용하십시오. **[!UICONTROL Active]** 조각을 비활성화하려면 해당 조각을 열고 **[!UICONTROL Deactivate]** 아이콘을 사용합니다.
...
* **[!UICONTROL 복제]**: **[!UICONTROL 추가 작업]**(...) 아이콘에서 조각 복사본을 만듭니다.
* **[!UICONTROL 삭제]**: **[!UICONTROL 추가 작업]**(...) 아이콘에서 활성 인벤토리에서 조각을 삭제합니다.
* **[!UICONTROL 태그 편집]** — **[!UICONTROL 추가 작업]**(...) 아이콘에서 조각의 태그를 추가하거나 제거합니다.
...
>title="여정 조각 추가"
>abstract="선택기에서 **[!UICONTROL Active]** 조각만 사용할 수 있습니다. 조각을 삽입하면 해당 노드의 **정적 복사본**&#x200B;이 만들어집니다. 원본 조각에 대한 이후의 모든 업데이트는 여정에 반영되지 않습니다."

여정에 조각을 삽입하려면 다음 작업을 수행하십시오.

1. 여정을 열고 왼쪽 레일에서 **[!UICONTROL 여정 조각]** 활동을 드래그합니다.
1. 기존 분기에 드롭합니다. 조각 선택기가 나타납니다.
1. 사용할 조각을 찾아보거나 검색합니다. 조각을 삽입하기 전에 미리 보거나 다른 탭에서 열 수 있습니다.
1. 조각을 선택합니다. 해당 노드는 드롭 지점에서 캔버스에 복사됩니다.

>[!NOTE]
>선택기에서 **[!UICONTROL Active]** 조각만 사용할 수 있습니다. 조각을 삽입하면 해당 노드의 **정적 복사본**&#x200B;이 만들어집니다. 원본 조각에 대한 이후의 모든 업데이트는 여정에 반영되지 않습니다.
...
* 캔버스의 **기존 분기**&#x200B;에 조각을 삽입해야 합니다.

**일반**

* 조각은 [여정 조각](../start/search-filter-categorize.md) 범주 아래의 **[!UICONTROL 통합 검색]** 막대를 사용하여 찾을 수 있습니다.
...
[대상자 읽기](read-audience.md) 노드 및 자격 필터와 같은 표준 시작 패턴은 조각으로 캡슐화할 수 있습니다. 따라서 팀은 설정 시간을 줄이면서 프로필이 여정에 진입하는 방식에서 일관성을 유지할 수 있습니다. 조각은 [조건](condition-activity.md)만 될 수도 있고 대상과 조건을 함께 읽을 수도 있습니다.

![자격 검사 조각 예제](assets/journey-fragments-uc-eligibility-check.png)

**기본 채널**

조각은 프로필의 기본 통신 채널(이메일, 푸시 또는 SMS)을 평가하고 그에 따라 프로필을 라우팅할 수 있습니다. 이 논리는 아웃바운드 메시징과 관련된 모든 여정에서 재사용할 수 있으므로 일관된 채널 환경 설정을 관리할 수 있습니다. 조각에는 [조건](condition-activity.md) 및 세 개의 채널 분기가 모두 포함될 수 있습니다.
