...
1. **[!UICONTROL 이벤트]** 범주를 펼친 후 **[!UICONTROL 대상 자격]** 활동을 캔버스에 드롭하십시오.
   ...
   ![여정 팔레트의 대상 자격 이벤트](assets/segment5.png)
...
1. **[!UICONTROL 대상]** 필드를 클릭하고 활용할 대상을 선택합니다.
   ...
   자격 이벤트 구성에 대한 ![대상 선택 드롭다운](assets/segment6.png)
...
### 세그먼트 멤버십 업데이트 시간 {#timing-segment-membership}

여정에서 일괄 처리 스냅숏을 사용할 때 새 세그먼트 멤버십은 후속 스냅숏에만 반영될 수 있습니다. 즉시 또는 당일 세그먼트 추가가 필요한 경우 스트리밍 세그먼테이션 또는 다음 스냅샷에 의해 세그먼트 업데이트가 캡처되는지 확인하는 것을 고려하십시오.
...
>* 조건 노드의 **`inAudience()`— 대기 활동(또는 단일 이벤트 여정) 후:** 이 컨텍스트에서는 스트리밍(단일) 프로젝션에서 세그먼트 멤버십을 읽습니다. 예상 대기 시간은 [Adobe Experience Platform 스트리밍 수집 설명서](https://experienceleague.adobe.com/ko/docs/experience-platform/ingestion/streaming/overview){target="_blank"}를 참조하십시오. 이 경로는 일반적으로 최근 프로필 변경 사항에 더 적합합니다.
...
* 프로덕션 여정에서 대상을 사용하기 전에 매일 이 대상에 대해 자격이 있는 개인의 양을 평가하십시오. 이렇게 하려면 **[!UICONTROL 대상자]** 메뉴를 확인하고 대상자를 연 다음 **[!UICONTROL 시간 경과에 따른 프로필]** 그래프를 보십시오.
  ...
  ![실시간 처리에 너무 많은 이벤트가 있는 경우 경고 메시지](assets/segment-overload.png)
