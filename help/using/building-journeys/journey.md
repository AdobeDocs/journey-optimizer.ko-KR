...
* 시작하기 전에 테스트 모드 또는 시험 실행을 사용하여 여정의 유효성을 검사합니다.
...
* **테스트 모드**: 실행 전 문제를 포착하기 위해 테스트 프로필로 실행을 시뮬레이션하는 여정 유효성 검사 모드입니다. *(제품별)*
* **시험 실행**: 실제 프로필 데이터나 메시지 전송에 영향을 주지 않고 실행을 시뮬레이션하는 여정 미리 보기 모드입니다. *(제품별)*
...
* 혼동하지 마십시오: &quot;시험 실행&quot; ≠ &quot;시험 모드&quot; — 시험 실행은 실제 데이터를 건드리지 않고 실행을 미리 봅니다. 시험 모드는 여정을 실제 테스트 프로필과 시뮬레이션합니다
...
* **Q: 실제 메시지를 보내지 않고 여정의 유효성을 검사하려면 어떻게 해야 합니까?** — 실제 데이터나 메시지 전송에 영향을 주지 않고 여정 실행을 미리 보는 시험 실행 모드를 사용합니다.
