...
>[!CAUTION]
>
>2025년 7월 8일부터 새로운 고객 조직에서는 경험 이벤트를 사용하여 표현식을 만드는 것이 여정 조건에 사용되는 표현식 편집기에서 더 이상 지원되지 않습니다. 따라서 표현식을 만드는 데 [Experience Platform 데이터 소스](../datasource/adobe-experience-platform-data-source.md)의 경험 이벤트를 사용할 수 없습니다.
>
...
이 페이지에서는 [!DNL Adobe Journey Optimizer]의 경험 이벤트를 최대한 활용하는 데 도움이 되는 일반적인 패턴 및 확장 가능한 접근 방식에 대해 간략히 설명합니다. 이러한 사용 사례는 옵트아웃 관리, 메시지 빈도 제어, 사용자 행동을 기반으로 콘텐츠 개인화, 실시간 신호 반응과 같은 자주 발생하는 문제를 해결하는 데 도움이 되도록 설계되었습니다.
...
* [모바일 메시지에 대한 옵트아웃 관리](../mobile/mobile-opt-out.md)


## 바운스 기반 제외

전자 메일 반송이 발생한 프로필을 제외하려면 반송된 주소에 대해 [!DNL Adobe Journey Optimizer]의 자동 제외 목록을 활용하십시오. 이 기본 제공 메커니즘은 사용자 지정 논리 없이 잘못되었거나 연결할 수 없는 이메일이 향후 전송에서 제외되도록 합니다.
...
* [조건에서 대상 사용](../building-journeys/conditions.md#using-a-segment)
...
* [[!DNL Adobe Experience Platform] 데이터 Distiller](https://experienceleague.adobe.com/ko/docs/experience-platform/query/data-distiller/overview){target="_blank"}에 액세스할 수 있는 경우 자동화된 쿼리를 구성하여 이벤트에서 필요한 데이터를 추출하고, 사용 사례에 맞게 조작한 다음 활성화를 위해 [프로필 사용 데이터 집합](https://experienceleague.adobe.com/ko/docs/experience-platform/catalog/datasets/user-guide#enable-profile){target="_blank"}에 다시 씁니다.
...
* **TL;DR:** 이 페이지에서는 여정 표현식 편집기에서 직접 경험 이벤트 조회가 더 이상 사용되지 않는 상황에서 Adobe Journey Optimizer 여정에서 경험 이벤트 데이터를 사용하기 위한 대체 패턴 및 모범 사례에 대해 간략히 설명합니다.

**의도:**

* 경험 이벤트 표현식 대신 기본 제공 동의 관리를 사용하여 옵트아웃 프로필 제외
* AJO 자동 제외 목록을 사용하여 반송된 이메일 주소 제외
* 이벤트 기반 기준으로 일괄 대상을 사용하여 일반 제외 논리 작성
* 빈도 제한 규칙 또는 시간 기반 대상 조건을 적용하여 과다 통신 방지
* AEP Data Distiller 또는 계산된 속성을 사용하여 포기한 장바구니 개인화 또는 커뮤니케이션 검색

**용어집:**

* **경험 이벤트**: Adobe Experience Platform *(제품별)에 저장된 고객 동작 또는 행동에 대한 타임스탬프가 지정되고 변경할 수 없는 레코드입니다.*
* **계산된 특성**: 시간에 따른 경험 이벤트 데이터를 집계 또는 요약하여 파생된 프로필 수준 특성으로, 여정 표현식 *(제품별)에서 사용할 수 있습니다*
* **제외 목록**: 하드 바운스 또는 스팸 불만으로 인해 향후 전송에서 자동으로 제외되는 AJO의 기본 제공 이메일 주소 목록 *(제품별)*
* **빈도 제한**: 프로필이 정의된 기간 *(제품별) 내에 받을 수 있는 메시지 수를 제한하는 비즈니스 규칙*
* **Data Distiller**: SQL 기반 일괄 처리 쿼리가 이벤트 데이터를 추출하여 프로필이 활성화된 데이터 세트 *(제품별)로 변환할 수 있는 AEP 기능입니다.*

**보호 기능:**

* 2025년 7월 8일부터 새 고객 조직은 경험 표현식 편집기에서 여정 이벤트 속성을 사용하여 표현식을 만들 수 없습니다.
* 2026년 4월 1일부터 지난 90일 동안 여정 표현식에서 경험 이벤트 속성을 사용하지 않은 조직은 이 기능에 액세스할 수 없게 됩니다.
* 여정 조건에서 직접 경험 이벤트 조회가 중단됩니다. 대체 요소에는 배치 대상, 계산된 속성 및 AEP 데이터 Distiller이 포함됩니다.
* 종료의 영향을 받지 않는 기능에는 이벤트로 여정 트리거, 여정 내 이벤트 수신, 트리거 이벤트의 여정 컨텍스트 데이터 사용, 이벤트 구성 및 반응 이벤트 감지 등이 포함됩니다.

**용어:**

* 정식 이름: 경험 이벤트 조회 — 약어: EE 조회 — 변형: 경험 이벤트 표현식, 이벤트 속성 조회
* 동의어: 제외/포함 메커니즘으로 &quot;이벤트 기반 로직으로 대상 일괄 처리&quot; = &quot;이벤트 기반 세그먼트&quot;
* 혼동하지 마십시오. &quot;표현식 편집기에서 여정 조회&quot; ≠ &quot;이벤트로 경험 트리거&quot; — 이벤트로 여정 트리거는 중단되지 않습니다.

**FAQ:**

* **Q: 경험 이벤트를 사용하여 여정을 계속 트리거할 수 있습니까?** — 예. 단일 또는 비즈니스 이벤트로 여정을 트리거하는 것은 이 변경의 영향을 받지 않습니다.
* **Q: 여정 조건에서 경험 이벤트 조회에 대한 대체 권장 사항은 무엇입니까?** — 복잡한 변환을 위해 AEP 세그먼트 빌더 이벤트 기반 논리, 계산된 속성 또는 AEP Data Distiller으로 구축된 배치 대상을 사용합니다.
* **Q: 내 기존 조직이 영향을 받습니까?** — 새로운 조직은 2025년 7월 8일부터 영향을 받습니다. 기존 조직은 지난 90일 동안 기능을 사용하지 않은 경우에만 2026년 4월 1일부터 영향을 받습니다.
* **Q: 다이렉트 이벤트 조회 없이 장바구니 포기 개인화를 처리하려면 어떻게 해야 합니까?** — AEP Data Distiller 를 사용하여 프로필 지원 데이터 세트에 이벤트 데이터를 추출하고 기록하거나 계산된 속성을 사용하여 프로필의 최신 중단 상태를 캡처합니다.
* **Q: 이 사용 중지의 영향을 받지 않는 기능은 무엇입니까?** — 이벤트로 여정 트리거, 여정 내 이벤트 수신, 표현식에서 이벤트 컨텍스트 데이터 트리거 사용, 이벤트 구성 및 반응 이벤트 감지(예: 이메일 열기)는 모두 영향을 받지 않습니다.
