---
title: 여정 문제 해결
description: 여정의 오류 문제 해결 방법 알아보기
...
이벤트는 소스에서 여정으로 직접 푸시되지 않습니다. 실제로 여정은 Adobe Experience Platform의 스트리밍 통합 API를 사용합니다. 따라서 이벤트 관련 문제가 발생하면 [이 페이지](https://experienceleague.adobe.com/docs/experience-platform/ingestion/streaming/troubleshooting.html)에서 수집 API 스트리밍 문제 해결을 참조할 수 있습니다.

## 사람들이 여정에 들어오는지 확인{#section_x4v_zzs_dgb}

여정 보고 기능은 여정의 입구를 실시간으로 측정합니다.
...
여정 보고는 여정 내의 개인의 진도를 측정합니다. 사람들이 어디에서 왜 멈췄는지를 쉽게 파악할 수 있습니다.
...
* Journey Optimizer은 메시지 전송 요청을 올바로 고려했습니다. 비즈니스 사용자는 전송하기로 되어 있는 메시지에 액세스하여 최근 실행 시간이 여정의 실행 시간과 일치하는지 확인할 수 있습니다. 또한 최신 API 호출/수신된 이벤트를 확인할 수 있습니다.
* Journey Optimizer에서 메시지를 보냈습니다. 메시지의 전송 로그에서 각 실행 상태를 볼 수 있습니다. 녹색인지, 빨간색인지 그리고 무엇이 문제인지를 볼 수 있습니다. 비즈니스 사용자는 이 화면에 액세스하고 추가 조사를 위해 관리자에게 로그를 보낼 수 있습니다.
