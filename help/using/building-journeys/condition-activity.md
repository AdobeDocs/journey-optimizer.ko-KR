...
>title="조건 활동"
>abstract="이 활동을 통해 개인이 여정에서 어떻게 이동할지 정의할 수 있습니다. 다양한 기준을 기반으로 여러 경로가 만들어집니다. 시간 초과 또는 오류 발생 시 대체 경로를 만들 수도 있습니다."
...
>title="단순 표현식 편집기 정보"
>abstract="단순 표현식 편집기 모드에서는 필드 조합을 기반으로 간단한 쿼리를 수행할 수 있습니다. 사용 가능한 모든 필드가 화면 왼쪽에 표시됩니다. 필드를 기본 영역으로 끌어다 놓습니다. 서로 다른 요소를 결합하려면 서로 인터로크하여 서로 다른 그룹 및/또는 그룹 수준을 만듭니다. 그런 다음 논리 연산자를 선택하여 동일한 수준에서 요소를 결합할 수 있습니다."
...
단순 모드에서는 필드 조합을 기반으로 간단한 쿼리를 수행할 수 있습니다. 사용 가능한 모든 필드가 화면 왼쪽에 표시됩니다. 필드를 기본 영역으로 끌어다 놓습니다. 서로 다른 요소를 결합하려면 서로 인터로크하여 서로 다른 그룹 및/또는 그룹 수준을 만듭니다. 그런 다음 논리 연산자를 선택하여 동일한 수준에서 요소를 결합할 수 있습니다.
...
단순 편집기에서는 이벤트 및 데이터 소스 카테고리 아래에 여정 속성 카테고리도 있습니다. 이 카테고리에는 지정된 프로필의 여정과 관련된 기술 필드가 포함되어 있습니다. 이는 여정 ID 또는 발생한 특정 오류와 같은 라이브 여정에서 시스템이 검색한 정보입니다. [자세히 알아보기](expression/journey-properties.md)
...
>[!NOTE]
>
>시간대는 조건에 따라 달라지지 않으며 여정 속성의 여정 수준에서 정의됩니다. 을(를) 참조하십시오 [이 페이지](../building-journeys/timezone-management.md).
...
테스트 모드에서는 분할에 도달할 때 항상 상단 분기가 선택됩니다. 검사에서 다른 경로를 선택하려는 경우 분할된 분기의 위치를 재구성할 수 있습니다. 을(를) 참조하십시오 [이 페이지](../building-journeys/testing-the-journey.md)
