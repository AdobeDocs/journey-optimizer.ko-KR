...
여정은 아래에 설명된 대로 글로벌 시간 초과도 사용합니다.
...
<table style="table-layout:auto">
  ...
  <tr style="border: 1;">
    ...
    <td>
      <p>프로필은 원래 여정 버전의 게시 시간에 맞추어 30일(HIPAA의 경우 7일)의 TTL을 유지 관리합니다. 강제 재입력이 있는 반복 여정의 경우 TTL은 반복 기간과 일치합니다.</p>
    </td>
  </tr>
  <tr style="border: 1;">
    <td>
...
종료 기준을 추가하면, 이벤트가 일어나거나(예: 구매) 프로필이 대상에 대해 자격이 되는 즉시 프로필이 여정을 종료하게 됩니다. 그러면 해당 사용자가 더 이상 해당 여정의 커뮤니케이션을 받지 않게 됩니다.
...
프로필 속성 기반 종료 기준을 사용하면 여정이 다시 시작되기 전에 특정 프로필을 자동으로 제거하는 규칙을 정의할 수 있으므로 일시 중지된 여정을 보다 세밀하게 제어할 수 있습니다. 위치, 상태 또는 기본설정과 같은 프로필 속성에 따라 종료 조건을 설정하여 여정이 재개된 후에도 관련 프로필만 계속 표시되도록 할 수 있습니다.

예를 들어, [여정을 일시 중지](journey-pause.md)하고, 프랑스에 있는 모든 프로필을 제거하는 종료 조건을 추가하고, 다음 작업 단계에서 해당 프로필이 제외된다는 사실을 알고 여정을 다시 시작할 수 있습니다. 이 논리는 여정에 이미 있는 프로필과 여정이 다시 시작된 후 자격이 있는 새 프로필에 모두 적용됩니다.
...
[여정 종료 기준](#exit-criteria-desc) 기능에 다음 보호 기능 및 제한 사항이 적용됩니다.

* 종료 기준은 초안 상태에서만 정의됩니다
* 이벤트와 이벤트 기반 종료 기준 간의 네임스페이스 일관성 여정

[프로필 특성 기반 종료 기준](#profile-exit-criteria) 기능을 사용하는 경우 다음 보호 기능이 적용됩니다.

* **종료 기준이 작업 수준에 적용됨**\
  &quot;프로필 속성&quot; 종료 기준은 작업 단계에서만 평가됩니다. 다른 종료 기준 유형과 달리, 이 유형은 여정 전체에 적용되지 않습니다.\
  여정을 다시 시작하고 일부 프로필이 종료 조건을 충족하는 경우 해당 프로필은 다음 작업 노드에서 제외됩니다.\
  재개 후 여정에 들어가는 새 프로필도 조건을 충족하는 경우 첫 번째 작업 노드에서 평가되고 제외됩니다.

* **프로필당 하나의 여정 기반 종료 규칙**\
  프로필당 하나의 &quot;여정 속성&quot; 종료 기준만 정의할 수 있습니다. 이 제한은 명확성을 유지하고 여정 논리의 충돌을 방지하는 데 도움이 됩니다.
...
* [여정 시작 및 종료 기준 안내서](entry-exit-criteria-guide.md) - 실제 사례 및 모범 사례를 포함한 전체 안내서
* [여정 종료 방법](end-journey.md) - 자연어 여정 완료 및 프로필 종료 이해
* [여정 일시 중지](journey-pause.md) - 프로필 특성 종료 기준을 사용하여 여정을 일시 중지하고 다시 시작합니다.
* [표준 시간대 관리](timezone-management.md) - 여정 및 프로필 표준 시간대 구성
* [충돌 관리 및 우선 순위 지정](../conflict-prioritization/conflicts.md) - 여정 및 캠페인 간 충돌을 식별하고 해결합니다.
