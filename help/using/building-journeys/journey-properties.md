...
* **TL;DR:** 이 페이지에서는 시작 규칙, 시간대, 시작/종료 날짜, 시간 초과 동작, 종료 기준, 페이로드 크기 및 충돌 관리를 포함하여 여정에 대한 모든 전역 설정을 구성하고 관리하는 방법을 설명합니다.
...
* 동의어: &quot;전역 여정 시간 초과&quot; = &quot;TTL&quot; = &quot;Time-to-Live&quot;
* 혼동하지 마십시오. &quot;전역 여정 시간 초과(91일)&quot; ≠ &quot;보고 기간(~91일)&quot; — 시간 초과는 여정의 개별 프로필 기간을 제한합니다. 보고 기간은 분석 데이터의 UI 표시 제한입니다

**FAQ:**

* **Q: 프로필이 여정에 얼마나 오래 있을 수 있습니까?** — 최대 91일(전역 여정 시간 제한). 이 기간이 지나면 프로필이 자동으로 종료되고 해당 데이터가 삭제됩니다.
* **Q: 여정이 라이브되는 동안 여정 속성을 편집할 수 있습니까?** — 라이브 여정의 경우 등록 정보 패널에 게시 날짜와 게시자 이름만 표시되며, 구조를 변경하려면 새 버전이 필요합니다.
* **Q: 여러 종료 기준이 구성되면 어떻게 됩니까?** — 여정의 모든 단계에서 OR 논리를 통해 하향식으로 평가됩니다. 한 가지 기준이 충족되면 프로필이 종료됩니다.
* **Q: 프로필이 여정에 다시 들어가는 것을 방지하려면 어떻게 해야 합니까?** — 여정 속성에서 &quot;재입력 허용&quot; 옵션의 선택을 취소합니다. 선물 오퍼와 같은 일회성 경험에 적합합니다.
* **Q: 여정 시간 제한과 종료 날짜 사이의 차이는 무엇입니까?** — 종료 날짜는 모든 새 항목을 중단하고 해당 특정 날짜에 활성 프로필을 자동으로 종료합니다. 여정 종료 날짜와 상관없이 입력한 시점부터 프로필당 91일 글로벌 시간 초과가 적용됩니다.
* **Q: 여정에 대한 병합 정책은 어떻게 결정됩니까?** — 여정 유형에 따라 다릅니다. 대상 읽기 및 대상 자격 여정은 대상의 병합 정책을 사용하고, 단일 이벤트 여정은 기본 병합 정책을 사용하며, 비즈니스 이벤트 여정은 후속 대상 읽기 활동에서 타깃팅된 대상의 병합 정책을 사용합니다.
