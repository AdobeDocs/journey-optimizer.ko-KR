...
* **오케스트레이션 활동**: 조건, 대기, 대상 읽기, 여정 조각 및 종료와 같은 논리 구성 요소
...
* 개별 고객 작업에 실시간으로 대응해야 합니다(예: 장바구니 포기, 신규 구성원 온보딩).
* 각 고객은 원하는 속도로 진행해야 합니다
* 특정 이벤트를 기반으로 트리거하려고 합니다.

다음과 같은 경우 **대상 여정 읽기**&#x200B;를 사용합니다.

* 그룹(예: 월간 뉴스레터, 프로모션 캠페인)에 일괄 통신을 보내고 있습니다.
...
+++ 장바구니가 중단된 후 즉시 이메일을 보내려면 어떻게 해야 합니까?

**단일 이벤트 트리거 여정 만들기**:

1. 관련 장바구니 세부 정보로 &quot;장바구니 포기&quot; 이벤트 구성
2. 이벤트를 여정 진입점으로 추가
3. 이메일 작업 즉시 수행
4. 개인화된 제품 세부 정보로 장바구니 복구 이메일 디자인
5. 여정 게시

이 여정은 장바구니 포기 이벤트가 수신될 때마다 자동으로 트리거되어 실시간으로 복구 이메일을 보냅니다.

[이벤트 구성](../event/about-events.md) 및 [전자 메일 동작](journey-action.md)에 대해 자세히 알아보세요.

+++

+++ 다른 사람이 메시지를 열거나 클릭하지 않으면 메시지를 다시 보낼 수 있습니까?
...
시간 제한이 있는 **[!UICONTROL 반응]** 이벤트를 사용하여 이벤트가 트리거된 여정 만들기:

1. **&quot;장바구니 중단&quot; 이벤트 구성**: 항목이 추가되었지만 일정 내에 체크아웃이 완료되지 않은 경우 트리거됩니다.
2. **초기 메시지 보내기**(선택 사항): 장바구니 항목을 확인하는 전자 메일
3. **채널 동작 바로 뒤에 [!UICONTROL 반응] 이벤트를 추가**: 구매 이벤트를 수신하도록 구성합니다.
4. **시간 제한 기간 설정**: **[!UICONTROL 반응]** 이벤트에서 시간 제한(예: 1~2시간)을 정의하여 고객이 자연스럽게 완료할 수 있는 시간을 제공합니다
5. **두 개의 경로 만들기**:
   * **구매 이벤트가 발생하는 경우**: 여정을 종료하거나 구매 후 흐름을 계속합니다.
   * **시간 제한 경로(구매 안 함)**: 장바구니 콘텐츠가 포함된 포기 알림 전자 메일을 보냅니다.
6. **선택 사항**: 다른 **[!UICONTROL 반응]** 이벤트 추가 **시간 후** 시간 제한(24시간)이 있는 미리 알림 전자 메일을 추가하고 인센티브(예: 10% 할인)가 포함된 두 번째 미리 알림을 보냅니다.
...
+++ 여정 조각이란 무엇이며 언제 사용해야 합니까?

**여정 조각**&#x200B;은(는) 한 번 빌드하고 샌드박스의 모든 여정에 삽입하는 재사용 가능한 여정 노드 집합입니다. 여정 캔버스에서 오케스트레이션 활동으로 사용할 수 있습니다.

**여정 조각을 사용하는 경우**:

* 여러 여정(예: 자격 확인, 선호 채널 라우팅, 환영 시퀀스)에 걸쳐 반복되는 논리가 있습니다
* 팀 간에 일관성을 유지할 수 있도록 하려면 패턴을 한 번 정의한 다음 모든 곳에서 다시 사용합니다.
* 공통 노드 시퀀스를 처음부터 다시 빌드하지 않도록 하여 여정 생성 속도를 높이려고 합니다

**인식할 주요 동작**:

* 조각을 삽입하면 해당 노드의 **정적 복사본**&#x200B;이 만들어집니다. 원본 조각에 대한 업데이트는 이미 해당 조각을 사용하는 여정에 전파된 **아님**&#x200B;입니다.
* **Active** 조각만 여정에 삽입할 수 있습니다.
* 조각은 샌드박스 범위로 지정되며 샌드박스당 최대 20개의 노드 및 200개의 활성 조각을 지원합니다.
* [이동](jump.md) 활동은 조각 내에서 허용되지 않습니다.

**점프 활동과의 차이점**: [점프 활동](jump.md)이 런타임 시 프로필을 다른 라이브 여정으로 리디렉션합니다. 여정 조각은 디자인 타임에 현재 여정에 노드를 복사합니다. 런타임 라우팅 메커니즘이 아닌 빌드 시간 재사용 메커니즘입니다.

[여정 조각](journey-fragments.md)에 대해 자세히 알아보세요.
...
Journey Optimizer에서는 세 가지 유효성 검사 방법을 제공합니다.

* **여정 시뮬레이션**: 수동으로 만들었거나 자동으로 생성한 임시 시뮬레이션된 사용자를 사용하여 여정의 유효성을 검사합니다. 실제 테스트 프로필을 만들거나 기다릴 필요가 없습니다. 여정 디자인 중에 빠른 반복에 가장 적합합니다.
* **테스트 모드**: 여정을 단계별로 실제(지정된 테스트) 프로필을 살펴보고 라이브로 전환하기 전에 논리, 조건 및 작업을 확인할 수 있습니다.
...
전체 비교를 위해 [여정 테스트 및 유효성 검사 방법 선택](choose-validation-method.md)을 참조하거나 [시뮬레이션](simulate-journey-gs.md), [테스트 모드](testing-the-journey.md) 및 [시험 실행](journey-dry-run.md)에 대해 자세히 알아보세요.
...
[여정 버전](journey-ui.md#journey-filter)에 대해 자세히 알아보세요.
...
* [점프 활동](jump.md)을 사용하여 여러 여정으로 세분화하는 중
* 반복된 논리를 [여정 조각](journey-fragments.md)(으)로 추출하여 처음부터 다시 빌드하지 않고 여정 간에 재사용합니다.
* 보다 효율적인 조건으로 논리 단순화
* 모든 활동이 필요한지 검토
...
* **TL;DR:** 이 페이지는 Adobe Journey Optimizer의 여정 오케스트레이션 개념, 여정 구축, 테스트 및 게시, 실행 모니터링, 고급 기능 및 모범 사례에 대한 포괄적인 FAQ입니다.

**의도:**
* 네 가지 여정 유형(단일, 대상 읽기, 대상 자격, 비즈니스 이벤트)과 각 유형을 사용해야 하는 시점을 이해합니다
* 주어진 사용 사례에 대해 여정과 캠페인 간 결정
* 프로필이 동일한 여정을 입력하는 빈도를 제어하도록 재입력 설정을 구성합니다.
* 프로필이 입력되지 않은 이유 또는 메시지가 전송되지 않은 이유 문제 해결
* 여러 여정 간에 메시지가 피로하지 않도록 여정 제한 규칙을 적용합니다
* 여정 조각을 사용하여 여정 간 공통 노드 시퀀스 재사용

**용어집:**
* **단일 여정**: 구매 또는 등록과 같은 실시간 이벤트에 의해 여정이 한 번에 하나의 프로필을 트리거했습니다. *(제품별)*
* **대상 여정 읽기**: 일괄 처리 대상의 모든 프로필을 한 번에 또는 일정 *(제품별)*&#x200B;에 처리하는 여정
* **대상 자격 여정**: 프로필이 스트리밍 대상 세그먼트 *(제품별)에 들어오거나 나갈 때 트리거되는 여정*
* **여정 한도**: 프로필이 시간 범위 내에 여정을 입력할 수 있는 횟수 또는 프로필이 동시에 *(제품별)에 있을 수 있는 여정 수를 제한하는 구성*
* **여정 조각**: 한 번 빌드되고 디자인 타임 *(제품별)에 여러 여정에 삽입된 재사용 가능한 정적 여정 노드 집합입니다*
* **STO(전송 시간 최적화)**: 참여를 최대화하기 위해 각 개별 프로필에 대해 최적의 전송 시간을 예측하는 AI 기반 기능입니다. *(제품별)*
* **보조 식별자**: 프로필에서 다른 엔터티(예: 개별 여정)에 대해 동일한 주문을 여러 번 입력할 수 있는 추가 식별자 *(제품별)*

**보호 기능:**
* 여정 당 최대 50개 활동
* 최대 여정 기간은 91일입니다(전역 시간 초과).
* 업로드 대상 및 연합 대상 구성 대상은 대상 자격 여정에서 지원되지 않습니다
* 반응 이벤트는 채널 작업 직후 사이에 대기 활동 없이 배치해야 합니다
* 여정 조각 내에서는 이동 활동이 허용되지 않습니다.
* 여정 조각은 최대 20개의 노드를 지원하고 샌드박스는 최대 200개의 활성 조각을 지원합니다
* 이미 대상에 있는 프로필에 대한 여정 게시 후 스트리밍 대상 자격이 최대 10분 지연될 수 있습니다
...
* **Q: 프로필이 내 여정을 입력하지 않은 이유는 무엇입니까?** — 일반적인 원인으로는 트리거 이벤트가 수신되지 않음, 대상 기준이 충족되지 않음, 재진입을 차단하는 재진입 여정, 게시가 취소됨 또는 네임스페이스 불일치 등이 있습니다.
* **Q: Live 여정 구조를 수정할 수 있습니까?** — 아니요. 구조를 변경하려면 새 여정 버전을 만들어야 합니다. 새 버전 없이 메시지 콘텐츠를 업데이트할 수 있습니다.
* **Q: 일시 중지, 새 등록 마감 및 중지 간의 차이점은 무엇입니까?** — 일시 중지는 비행 중인 프로필을 유지하거나 삭제하는 동안 여정을 일시적으로 중단합니다. 새 시작에 닫으면 새 시작은 중지되지만 기존 프로필은 완료됩니다. Stop을 선택하면 모든 프로필이 즉시 종료됩니다.
* **Q: 언제 Jump 활동 대신 여정 조각을 사용해야 합니까?** — 조각을 사용하여 디자인 시 공통 노드 논리를 재사용합니다(복사-붙여넣기 동작). 런타임 시 점프를 사용하여 프로필을 다른 라이브 여정으로 리디렉션합니다.
* **Q: 동일한 고객에게 너무 많은 메시지를 보내지 않으려면 어떻게 해야 합니까?** — 여정 제한 규칙(시작 제한 또는 동시 실행 제한)을 적용하고 개별 채널 작업에 빈도 제한 비즈니스 규칙을 사용합니다.
