...
![아래쪽 표시줄에서 두 개의 라이브 여정 일괄 일시 중지](assets/bulk-pause-journeys.png){width="80%" align="left"}
...
| 여정 활동 | 프로필 관리 | 참고 |
|-------------------------|--------------------------------------------------|------------------------|
| [대상 자격](audience-qualification-events.md) | 첫 번째 노드에서: 삭제됨 <br> 다른 노드에서: 라이브 여정과 동일한 동작이지만, 대상 자격이 작업 활동 후이고 사용자가 해당 작업에서 일시 중지된 경우 대상 자격이 삭제됩니다. |          |
| [비즈니스 이벤트](general-events.md) | 삭제됨 |    |
| [단일 이벤트](general-events.md) | 첫 번째 노드: 삭제됨 <br>다른 노드: 라이브 여정과 동일한 동작이지만 이벤트가 작업 활동 후이고 사용자가 해당 작업에서 일시 중지된 경우에는 이벤트가 삭제됩니다. | 메시지 작성 |
| [대상자 읽기](read-audience.md) | 몇 가지 특성을 가진 라이브 여정과 동일한 동작: <br> 대상 읽기 활동이 시작된 후 일시 중지를 누르면 여정에 들어간 프로필이 다음 작업 활동까지 계속됩니다. 여정이 특정 속도로 대상자를 읽을 때 전체 대상자를 아직 입력하지 않은 경우 대기열의 나머지 프로필은 무시됩니다. | - 단일 실행의 경우: 예약된 날짜가 다시 시작 날짜 이전인 경우 다시 시작 시간에 오류가 표시되지 않습니다. 해당 일정은 무시됩니다. <br>- 증분 여정의 경우: <br> 첫 번째 발생 전에 일시 중지가 발생하면 전체 대상이 재생됩니다. <br>예를 들어, 일시 중지가 일별 되풀이 중 4일에 발생하고 여정이 9일까지 일시 중지된 상태로 유지되면 4일부터 9일까지 입력한 모든 프로필이 다시 시작됩니다 |
| [반응](reaction-events.md) | 라이브 여정과 동일한 동작이지만, 작업 활동 후가 반응이고 해당 작업에서 사용자가 일시 중지된 경우 이벤트가 무시됩니다. |
...
| [프로필 업데이트](update-profiles.md) 및 [이동](jump.md) |  |       |
