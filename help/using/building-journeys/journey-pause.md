...
여정이 일시 중지되면 프로필 속성에 따라 글로벌 필터를 적용할 수 있습니다. 이 필터를 사용하면 재개 시 정의된 표현식과 일치하는 프로필을 제외할 수 있습니다. 현재 여정에 있는 기준과 일치하는 프로필은 종료되고 입력을 시도하는 새 프로필은 차단됩니다.

예를 들어 프랑스로의 마케팅 커뮤니케이션에서 모든 프랑스 고객을 제외하려면 다음 단계를 수행합니다.
...
1. 전역 필터 설정에서 프로필 속성에 따라 필터를 정의합니다.

1. 국가 속성이 프랑스와 같은 프로필을 제외하도록 표현식을 설정합니다.

1. 여정을 다시 시작합니다.

   재개 시 국가 속성이 프랑스로 설정된 모든 프로필은 자동으로 여정에서 제외됩니다. 여정에 들어가려고 하는 국가 속성이 프랑스로 설정된 새 프로필은 모두 차단됩니다.
