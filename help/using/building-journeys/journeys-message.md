...
>[!NOTE]
>
>메시지를 보내기 위한 특정 작업을 설정할 수도 있습니다. [자세히 알아보기](#recommendation)

여정에 기본 제공 채널 작업을 추가하려면 아래 단계를 수행합니다.

1. [이벤트](general-events.md) 또는 [대상자 읽기](read-audience.md) 활동으로 여정을 시작하십시오.

1. 팔레트의 **작업** 섹션에서 아웃바운드(**이메일**, **푸시**, **SMS**) 또는 인바운드(**인앱**, **웹**, **코드 기반 경험**, **콘텐츠 카드**) 활동을 캔버스로 끌어다 놓습니다.

   ![](assets/journey-web-activity.png)

1. 활동을 구성합니다.

   * 메시지 콘텐츠를 만드는 자세한 단계를 다음과 같이 알아보십시오.
...
Adobe의 AI 서비스를 기반으로 하는 Adobe Journey Optimizer의 전송 시간 최적화 기능을 사용하면 **이메일** 또는 **푸시 메시지**&#x200B;를 보내는 데 가장 적합한 시간을 예측하여 이전 공개 및 클릭률을 기반으로 참여를 극대화할 수 있습니다. 머신 러닝 모델을 사용하여 각 사용자에 대해 개인화된 전송 시간을 예약하여 메시지 열람률과 클릭률을 높일 수 있습니다.
...
&quot;시간&quot;에 있는 순위는 -83에서 84까지이지만 168개의 고유한 값으로 프로필을 혼동하지 않도록 단일 값으로 함께 충돌합니다. 세 세트의 168개의 점수 각각에 대해, 그 등급은 -83에서 84로 올라간다.

이 값은 최적화 알고리즘에서 읽습니다. 이 값은 사람이 읽을 수 있도록 디자인되지 않았습니다.

순위가 높을수록 수신자와 상호 작용하기 위해 더 좋은 시간을 선택하였다. 여정의 시작 및 기간을 정의할 수 있으므로 최상의 순위(84)는 해당 시간 범위에 포함되지 않을 수 있습니다. 이 경우 등급 값이 가장 높은 시간을 선택하는 것이 좋습니다.
+++
