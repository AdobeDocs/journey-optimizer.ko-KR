...
다음&#x200B;**옵션 내에**&#x200B;보내기 옵션에 값을 입력하여 시스템에서 사용하는 전송 시간을 대괄호로 묶도록 선택할 수도 있습니다. 값으로 &quot;6시간&quot;을 선택하는 경우 [!DNL Journey Optimizer]은(는) 각 사용자 프로필을 확인하여 여정 실행 시간으로부터 6시간 이내에 최적의 전송 시간이 발생하는지 확인하고 전송-시간 최적화에 의해 결정된 시간을 선택합니다. 이 시간이 다음 6시간 내에 없으면 [!DNL Journey Optimizer]은(는) 기본적으로 여정 실행 시 메시지를 전송하도록 설정됩니다.
