...
1. 다음 **[!UICONTROL 세그먼트 읽기]** 활동을 통해 세그먼트가 여정을 입력할 시간을 지정할 수 있습니다. 이렇게 하려면 **[!UICONTROL 여정 예약 편집]** 링크를 클릭하여 여정 속성에 액세스한 다음 **[!UICONTROL 스케줄러 유형]** 필드.
   ...
   **증분 읽기** 옵션: 반복 여정 시 **세그먼트 읽기** 은 처음으로 실행되며, 세그먼트의 모든 프로필은 여정을 입력합니다. 이 옵션을 사용하면 첫 번째 발생 후 여정의 마지막 실행 이후 세그먼트에 입력한 개인만 타깃팅할 수 있습니다.

   **되풀이 시 강제 재진입**: 이 옵션을 사용하면 모든 프로필이 여정에 여전히 남아 있게 하여 다음 실행 시 자동으로 종료하도록 할 수 있습니다. 예를 들어 일별 재귀 여정에서 2일 대기하는 경우 이 옵션을 활성화하면 다음 실행 대상인지 여부에 관계없이 프로필이 다음 여정 실행(즉, 다음 날)에서 항상 이동됩니다. 이 여정에 있는 프로필의 수명이 되풀이 횟수보다 길 수 있는 경우에는 이 옵션을 활성화하여 프로필에서 여정을 완료할 수 있는지 확인하십시오.
