...
1. 예약된 실행 후 여정은 안전 버퍼 기간 동안 **[!UICONTROL Live]** 상태로 유지됩니다.
1. 안전 버퍼가 경과(여정의 예약된 실행 시간 후~96시간)되면 여정은 잠시 후 자동으로 **[!UICONTROL 중지됨]** 상태로 전환됩니다.

이 동작은 **반복되지 않는 대상 읽기 여정**&#x200B;에만 적용됩니다. 반복 여정은 영향을 받지 않습니다.

* **자동 중지 시간:** 안전 버퍼는 두 개의 창을 고려합니다. **24시간 유휴 상태 창**(기내 전송을 완료하도록 허용) 및 **72시간 방해 금지 시간**(방해 금지 시간은 전송을 최대 72시간 지연시킬 수 있음). 총 버퍼는 여정의 예약된 실행 시간 후 약 **96시간(~4일)**&#x200B;입니다. 이 기간 동안 여정은 **[!UICONTROL Live]** 상태로 유지됩니다. 이는 예상되는 동작이며 문제를 나타내지는 않습니다.

* **예약된 예약된 예약된 예약된 예약된 예약된 여정:** 이 자동 중지 동작은 예약된 여정 및 전송 시간 최적화를 사용하는 여정에 적용되지 않습니다. 이러한 여정은 예약된 모든 예약된 전파에서 활성 상태로 유지되며 수동으로 닫거나 중지하지 않는 한 표준 [91일 전역 시간 제한](../building-journeys/journey-properties.md#global_timeout)으로만 중지됩니다.
...
* 장기 실행 대기, 반응 또는 이벤트 트리거 노드가 없는 반복되지 않는 대상 읽기 여정은 예약된 실행 후 약 96시간(~4일) 후에 중지됨으로 자동 전환됩니다. 여정은 이 버퍼 동안 라이브 상태로 유지됩니다. 예약된 일괄 처리 기반 여정 및 전송 시간 최적화를 사용하는 여정은 이 자동 중지에서 제외되며, 수동으로 닫거나 중지하지 않는 한 91일 글로벌 시간 초과의 영향을 받습니다.
...
* **Q: 반복되지 않는 여정이 실행 후 며칠 동안 Live 상태로 유지되는 이유는 무엇입니까?** — 이는 예상되었습니다. AJO은 ~96시간(~4일)의 안전 버퍼를 적용합니다. 기내 전송을 완료하려면 24시간과 조용한 시간 지연의 경우 72시간이 적용됩니다. 여정은 버퍼가 경과한 직후 중지됨으로 전환됩니다.
* **Q: ~96시간 후에 예약된 예약된 예약된 일괄 처리 여정을 자동으로 중지합니까?** — 아니요. 예약된 모든 웨이브에서 활성 상태를 유지할 수 있도록 웨이브 기반 여정 및 전송 시간 최적화를 사용하는 여정은 이 자동 중지에서 제외됩니다. 수동으로 닫거나 중지하지 않는 한 표준 91일 여정 시간 제한을 따릅니다.
* **Q: 대상자 읽기 여정은 언제 완료됨 상태에 도달합니까?** — 반복되지 않는 대상 읽기 여정의 경우: 예약된 실행 후 약 96시간(~4일) 후에 중지됨으로 자동 중지됩니다(안전 버퍼: 24시간 유휴 기간 + 72시간 방해 금지). 여정은 이 버퍼 동안 라이브 상태로 유지됩니다. 대기, 반응 또는 이벤트 노드가 프로필을 활성 상태로 유지하는 경우, 표준 91일 전역 시간 제한이 대신 적용됩니다. 종료된 여정이 91일의 글로벌 시간 초과 또는 완료된 정의 테이블의 반복 여정 규칙에 도달하면 완료됨으로 표시됩니다.
