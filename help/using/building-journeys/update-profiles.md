...
**[!UICONTROL 프로필 업데이트]** 작업 활동을 사용하여 이벤트, 데이터 원본에서 가져온 정보 또는 특정 값으로 기존 [!DNL Adobe Experience Platform] 프로필을 업데이트합니다.

## 주요 개념 {#key-concepts}

* **프로필 업데이트** 작업은 네임스페이스가 있는 여정에서만 사용할 수 있습니다.
* 작업은 기존 필드만 업데이트하고 새 프로필 필드는 만들지 않습니다.
...
## 데이터 집합 선택 {#dataset-selection}

**프로필 업데이트** 활동에는 업데이트를 저장할 전용 데이터 집합이 필요합니다. 이 활동은 프로필 저장소만 업데이트하므로(Datalake가 아님), 모든 업데이트는 **프로필 업데이트** 작업에 대해 특별히 지정된 프로필 사용 데이터 집합에 저장되어야 합니다. 일괄 처리 또는 스트리밍 인수에 사용되는 데이터 집합을 사용하면 새로 온보딩된 데이터가 **프로필 업데이트** 작업의 변경 내용을 덮어씁니다.

또한 **프로필 업데이트** 활동 구성에는 ID 네임스페이스가 필요하지 않습니다. 따라서 선택한 데이터 집합에서 이 네임스페이스처럼 여정을 시작한 작업에서 사용한 것과 동일한 **ID 네임스페이스**&#x200B;을(를) 사용하는지 확인하십시오. 선택한 데이터 집합에서 ID 맵을 사용할 수도 있습니다. 올바른 네임스페이스를 가진 데이터 집합 또는 ID 맵을 사용하는 데이터 집합을 선택하지 않으면 프로필 업데이트 작업이 실패합니다.
...
1. **필드**&#x200B;를 클릭하여 업데이트할 필드를 선택합니다. 필드는 하나만 선택할 수 있습니다.
...
1. 목록에서 데이터 세트를 선택합니다.
   ...
   >[!NOTE]
   >
   >**프로필 업데이트** 작업은 프로필 데이터를 실시간으로 업데이트하지만 데이터 집합은 업데이트하지 않습니다. 프로필이 데이터 세트와 관련된 레코드이므로 데이터 세트를 선택해야 합니다.
...
![필드 구성을 사용하여 여정의 프로필 업데이트 활동](assets/profileupdate1.png)
...
테스트 프로필만 테스트 모드로 여정을 시작할 수 있습니다. 새 테스트 프로필을 만들거나 기존 프로필을 테스트 프로필로 만들 수 있습니다. [!DNL Adobe Experience Platform]에서 csv 파일 가져오기 또는 API 호출을 통해 프로필 특성을 업데이트할 수 있습니다. 더 간단한 방법은 **프로필 업데이트** 작업 활동을 사용하고 테스트 프로필 부울 필드를 false에서 true로 변경하는 것입니다.
