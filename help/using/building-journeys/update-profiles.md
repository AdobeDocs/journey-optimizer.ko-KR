...
**이 페이지에서:** 고객이 여정을 진행할 때 프로필 업데이트 작업 활동을 사용하여 기존 Adobe Experience Platform 프로필을 보강하거나 수정하는 방법을 알아봅니다.
...
* **TL;DR:** 이 페이지에서는 고객이 여정 과정을 진행할 때 여정 이벤트, 데이터 소스 또는 정적 값의 데이터로 기존 Adobe Experience Platform 프로필을 보강하거나 수정하도록 프로필 업데이트 활동을 구성하는 방법을 설명합니다.

**의도:**

* 프로필 업데이트 활동을 구성하여 여정 중에 기존 프로필 속성 수정
* 프로필 업데이트 작업 전용 프로필 지원 데이터 세트 선택
* 여정 이벤트, 데이터 소스 또는 정적 값의 필드 값을 프로필 속성에 매핑
* 단일 활동에서 여러 프로필 속성(최대 5개) 업데이트
* 여정 테스트 모드에서 프로필 업데이트 테스트

**용어집:**

* **프로필 활동 업데이트**: 프로필이 *(제품별) 여정을 이동할 때 실시간으로 Adobe Experience Platform 프로필의 기존 필드에 새 값을 쓰는 작업 활동*
* **프로필 저장소**: 데이터 레이크 *(제품별)와(과) 구별되는 실시간 고객 프로필 데이터를 보관하는 Adobe Experience Platform 저장소*
* **ID 네임스페이스**: 업데이트 중인 프로필과 일치하는 데 사용되는 ID 컨텍스트(예: 이메일, CRM ID)를 구분하는 레이블 *(제품별)*
* **프로필 사용 데이터 세트**: 통합 프로필 *(제품별)에 레코드를 기여하도록 구성된 Adobe Experience Platform 데이터 세트*

**보호 기능:**

* 프로필 업데이트 작업은 네임스페이스가 정의된 여정에서만 사용할 수 있습니다.
* 이 작업은 기존 XDM 필드만 업데이트합니다. 새 프로필 필드를 만들 수 없습니다.
* 단순 필드 유형(문자열, 숫자, 부울)만 지원됩니다. 열거형, 개체 배열 및 복잡한 컬렉션은 지원되지 않습니다.
* 작업에서 구매 등의 경험 이벤트를 생성할 수 없습니다.
* 하나의 프로필 업데이트 작업에서 최대 5개의 필드/값 쌍을 업데이트할 수 있습니다.
* 다른 수집 실행은 프로필 업데이트 변경 사항을 덮어쓰게 되므로 배치 또는 스트리밍 수집 프로세스와 전용 데이터 세트를 공유하지 마십시오.
* 프로필 업데이트는 동일한 여정 실행에서 다운스트림에서 즉시 사용하지 못할 수 있습니다.
* 활동은 프로필 저장소만 업데이트하며 데이터 레이크는 업데이트하지 않습니다.

**용어:**

* 정식 이름: 프로필 업데이트 — 약어: 없음 — 변형: 프로필 활동 업데이트, 프로필 업데이트 작업
* 동의어: &quot;Profile Store&quot; = &quot;Real-Time Customer Profile Store&quot;
* 혼동하지 마십시오. &quot;프로필 저장소&quot;(이 활동에 의해 업데이트됨) ≠ &quot;데이터 레이크&quot;(이 활동에 의해 업데이트되지 않음)

**FAQ:**

* **Q: 프로필 업데이트 활동이 새 프로필 필드를 만들 수 있습니까?** — 아니요. 선택한 XDM 프로필 스키마에 이미 있는 필드만 업데이트할 수 있습니다.
* **Q: 프로필 업데이트 작업에 전용 데이터 세트를 사용해야 하는 이유는 무엇입니까?** — 일괄 처리 또는 스트리밍 수집으로 데이터 세트를 공유하면 다른 수집 실행이 프로필 업데이트 활동의 변경 사항을 덮어쓸 수 있습니다.
* **Q: 프로필 업데이트가 동일한 여정의 다운스트림 활동에 즉시 표시됩니까?** — 아니요. 프로필 업데이트 활동에서 필드를 작성한 직후 작업에서 동일한 필드를 읽는 경우 업데이트된 값이 아직 반영되지 않을 수 있습니다.
* **Q: 하나의 프로필 업데이트 작업에서 몇 개의 필드를 업데이트할 수 있습니까?** — &quot;다른 필드 업데이트&quot; 버튼을 사용하여 단일 활동에서 최대 5개의 필드/값 쌍을 구성할 수 있습니다.
* **Q: 프로필 업데이트가 테스트 모드에서 적용됩니까?** — 예. 테스트 모드에서는 업데이트가 테스트 프로필에 즉시 적용되며 시뮬레이션되지 않습니다.
