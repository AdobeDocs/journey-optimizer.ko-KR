...
+++ 대상이 각 예약된 일괄 처리 전에 다시 평가됩니까? 아니면 여정 시작 시 수정됩니까?

여정이 트리거될 때 대상자는 **한 번 평가**&#x200B;됩니다. 해당 시점에서 자격 있는 프로필의 스냅샷을 가져와 모든 웨이브에 사용합니다. 각 후속 웨이브 전에는 대상 멤버십이 다시 평가되지 않습니다.

그러나 **프로필 특성은 여정 시작이 아니라 각 예약된 일괄 처리**&#x200B;할 때 읽혀집니다. 즉, 여러 날에 걸쳐 전파되는 파도의 경우
...
요약하면 *여정에 포함된 사람*&#x200B;은(는) 미리 수정되지만 *프로필로 보내는 데 사용되는 데이터*&#x200B;은(는) 웨이브가 처리될 때 현재 상태를 반영합니다.
