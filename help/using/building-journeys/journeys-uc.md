...
* **TL;DR:** 이 페이지에서는 두 가지 실용적인 여정 사용 사례를 제공합니다. 대상 읽기, 반응 이벤트, 이메일 및 푸시를 결합한 다중 채널 메시지 흐름과 복잡한 여정을 관리 가능한 하위 여정으로 분해하기 위해 [이동] 활동을 사용하는 다단계 충성도 여정 패턴입니다.

**의도:**

* 고객이 초기 이메일을 여는지 여부에 따라 후속 이메일 또는 푸시를 보내는 다중 채널 여정을 작성합니다
* 여정 내에서 감사 푸시 알림을 트리거하는 구매 이벤트 구성
* 반응 이벤트를 사용하여 이메일 열기 비헤이비어를 기반으로 여정 분기
* 복잡한 다상 여정을 Jump 활동으로 연결된 더 작은 하위 여정으로 분해
* 여정 트리거로 사용할 규칙 기반 이벤트 만들기 및 구성
* 타깃팅된 여정 항목에 대한 구/군/시 및 출생 연도 속성을 기반으로 대상자를 정의합니다.

**용어집:**

* **반응 이벤트**: 프로필이 메시지와 상호 작용할 때(예: 이메일을 열거나 링크를 클릭할 때) 트리거되는 여정 이벤트로, 동작 기반 분기를 활성화합니다. *(제품별)*
* **대상 활동 읽기**: 여정을 시작하기 위해 지정된 Adobe Experience Platform 대상의 모든 프로필을 로드하는 여정 항목 활동입니다. *(제품별)*
* **이동 활동**: 프로필을 한 여정(원본)에서 다른 여정(대상)로 푸시하여 모듈식 하위 프로필 아키텍처를 활성화하는 작업 활동입니다. *(제품별)*
* **규칙 기반 이벤트**: 트리거 조건이 오케스트레이션 ID가 아닌 규칙 식으로 정의되는 이벤트 유형으로, 구매 또는 동작 트리거에 유용합니다. *(제품별)*

**보호 기능:**

* 정의된 기간 내에 메시지와 상호 작용하지 않는 프로필을 처리하도록 반응 이벤트 시간 제한 경로를 구성해야 합니다
* 사용 사례에 사용되는 대상자는 여정을 빌드하기 전에 만들어야 합니다
* 구매 이벤트를 여정에서 사용하려면 먼저 구성해야 합니다
* Jump를 통해 연결된 하위 여정은 원본 여정과 동일한 네임스페이스를 사용해야 합니다.
* 이메일 주소 재정의(매개 변수 재정의)는 기본 주소의 일반적인 대체가 아니라 특정 사용 사례에만 사용해야 합니다.

**용어:**

* 정식 이름: 반응 이벤트 — 약어: 없음 — 변형: 반응 활동, 메시지 반응
* 동의어: &quot;원본 여정&quot; = &quot;원본 여정&quot;; &quot;대상 여정&quot; = &quot;대상 여정&quot;
* 혼동하지 마십시오. &quot;대상 활동 읽기&quot; ≠ &quot;대상 자격 활동&quot; — 대상 읽기 는 모든 대상 구성원을 한 번에 일괄적으로 로드합니다. 대상 자격 트리거는 멤버십 변경에 따라 프로필별로 실시간으로 트리거됩니다.

**FAQ:**

* **Q: 이메일을 열지 않은 고객에게만 후속 메시지를 보내려면 어떻게 해야 합니까?** — 시간 제한 경로를 사용하여 반응 이벤트(이메일 열림)를 추가합니다. 시간 제한 기간 내에 열리지 않는 프로필은 후속 이메일이 배치되는 시간 제한 경로를 따라 흐릅니다.
* **Q: 구매 이벤트가 다중 채널 사용 사례에서 어떻게 구성됩니까?** — 스키마, 페이로드 필드(제품, 날짜, 구매 ID), 네임스페이스 및 프로필 식별자로 구성된 `purchaseMessage="thank you"`과(와) 같은 조건을 사용하는 규칙 기반 이벤트입니다.
* **Q: 복잡한 여정을 하위 여정으로 분해하는 이유는 무엇입니까?** — 복잡한 여정은 20개 이상의 고유한 고객 경로를 노출할 수 있으며 복잡성은 각 접점에 따라 기하급수적으로 증가합니다. 하위 여정은 각 단계를 읽기 쉽고, 테스트 가능하며, 독립적으로 유지 관리할 수 있도록 합니다.
* **Q: 이동 후 프로필이 원본 여정과 대상 프로필 모두에 동시에 있을 수 있습니까?** — 예. 여정이 [이동] 단계에 도달하면 원본 여정에서 계속 진행하면서 동시에 대상 프로필로 들어갑니다.
* **Q: 다단계 충성도 예제에 사용되는 하위 여정 수는 몇 개입니까?** — Jump 활동을 사용하여 순차적으로 연결된 1단계(앱 다운로드), 2단계(첫 번째 트랜잭션), 3단계(두 번째 트랜잭션)의 세 가지 하위 여정.
