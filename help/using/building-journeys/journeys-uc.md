...
* 애틀랜타, 샌프란시스코 또는 시애틀에 거주하며 1980년 이후에 출생한 모든 고객을 위한 세그먼트입니다.
* 구매 이벤트

### 세그먼트 만들기

이 여정에서 고객의 특정 세그먼트를 활용하려고 합니다. 세그먼트에 속하는 모든 개인이 여정을 입력하고 다른 단계를 따릅니다. 이 예에서는 Atlanta, San Francisco 또는 Seattle에 거주하며 1980년 이후 출생한 모든 고객을 타겟팅하는 세그먼트가 필요합니다.

세그먼트에 대한 자세한 내용은 다음을 참조하십시오. [페이지](../segment/about-segments.md).

1. 고객 메뉴 섹션에서 을(를) 선택합니다. **[!UICONTROL 세그먼트]**.

1. 다음을 클릭합니다. **[!UICONTROL 세그먼트 만들기]** 세그먼트 목록의 오른쪽 상단에 있는 단추입니다.

1. 다음에서 **[!UICONTROL 세그먼트 속성]** 창에서 세그먼트 이름을 입력합니다.
...
이제 세그먼트가 만들어지고 여정에서 사용할 준비가 되었습니다. 사용 **세그먼트 읽기** 활동에서는 세그먼트에 속하는 모든 개인이 여정을 입력하도록 할 수 있습니다.
...
1. 다음으로 여정 시작 **세그먼트 읽기** 활동. 이전에 만든 세그먼트를 선택합니다. 세그먼트에 속한 모든 개인이 여정을 입력합니다.
...
1. 첫 번째 경로에서 **반응** 이벤트 및 선택 **푸시 열림**. 이벤트는 세그먼트에 속하는 개인이 첫 번째 메시지의 푸시 버전을 열 때 트리거됩니다.
