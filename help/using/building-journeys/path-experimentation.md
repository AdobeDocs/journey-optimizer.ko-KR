...
* **TL;DR:** 이 페이지에서는 A/B 또는 Multi-armed bandit 방법을 사용하여 Adobe Journey Optimizer 여정에서 경로 실험을 구성하고 실행하는 방법과 가장 효과적인 처리를 자동 또는 수동으로 확장하는 방법을 설명합니다.

**의도:**
* 여정에서 A/B 또는 Multi-armed bandit 경로 실험 설정
* 실험 성과를 평가하기 위한 성공 지표 정의
* 처리 경로 간 트래픽을 균등하게 할당하거나 사용자 지정 백분율로 할당
* 홀드아웃 그룹을 추가하여 모든 처리에서 대상자 일부 제외
* 자동 크기 조절을 활성화하여 가장 성과가 좋은 처리 자동으로 롤아웃
* 실험 결과를 검토한 후 채택 처리 수동 비율 조정

**용어집:**
* **활동 최적화**: 실험 또는 타깃팅을 위해 프로필을 다른 경로로 분할하는 데 사용되는 여정 캔버스 활동 *(제품별)*
* **처리**: 경로 실험의 단일 경로 변형(예: 처리 A, 처리 B) *(제품별)*
* **성공 지표**: 실험에서 성과가 가장 좋은 처리를 평가하는 데 사용되는 KPI *(제품별)*
* **Multi-armed bandit**: 기본 지표 성능 *(제품별)을 기반으로 7일마다 트래픽 분할이 자동으로 조정되는 실험 유형*
* **우승자 크기 조정**: 자동 또는 수동으로 전체 나머지 대상자에게 채택 처리를 롤아웃하는 기능입니다. *(제품별)*
* **홀드아웃 그룹**: 모든 실험 처리에서 제외된 대상자의 세그먼트로, 컨트롤 그룹 *(제품별)*

**보호 기능:**
* 우승자 비율 조정 은 단일 여정(이벤트 트리거 및 대상 자격)에만 사용할 수 있으며 대상 읽기 여정에는 사용할 수 없습니다.
* 자동 크기 조정 시간은 실험 종료 날짜 이전에 예약해야 합니다. 그렇지 않으면 여정이 게시되지 않습니다.
* 자동 확장이 발생하면 수동 확장을 더 이상 사용할 수 없습니다.
* 예약된 자동 크기 조정 시간이 자동 크기 조정을 취소하기 전에 수동으로 승자 크기를 조정합니다.
* 치료의 스케일링은 최대 1시간이 소요될 수 있습니다.

**용어:**
* 정식 이름: 경로 실험 — 약어: 없음 — 변형: 여정 실험, A/B 경로 테스트
* 동의어: &quot;활동 최적화&quot; = &quot;실험 활동&quot; = &quot;경로 분할 활동&quot;
* 혼동하지 마십시오: &quot;A/B 실험&quot; ≠ &quot;Multi-armed bandit&quot; (A/B는 트래픽 분할을 고정했습니다. Multi-armed bandit는 7일마다 가중치를 동적으로 조정합니다.)

**FAQ:**
* **Q: A/B 실험과 Multi-armed bandit 간의 차이점은 무엇입니까?** — A/B 실험에서는 시작 시 정의된 고정된 트래픽 분할을 사용하는 반면, Multi-armed bandit 에서는 기본 지표 성능을 기반으로 7일마다 트래픽 가중치를 자동으로 조정합니다.
* **Q: 대상자 읽기 여정에서 우승자 크기 조절을 사용할 수 있습니까?** — 아니요. 우승자 비율 조정 은 단일(이벤트 트리거 및 대상 자격) 이벤트에만 사용할 수 있습니다.
* **Q: 자동 크기 조정 시간에 우승자를 찾을 수 없으면 어떻게 됩니까?** — 대체(fallback)를 구성할 수 있습니다. 예약된 종료까지 실험을 계속하거나 지정된 시간 후에 대체 처리를 확장할 수 있습니다.
* **Q: 처리 비율을 수동으로 구성하지 않으면 트래픽이 어떻게 분배됩니까?** — 균등 분포 토글을 활성화하여 모든 처리 간에 트래픽을 균등하게 분할할 수 있습니다.
* **Q: 여정을 게시한 후 경로 실험을 편집할 수 있습니까?** — 여정이 게시 후 읽기 전용 모드로 전환됩니다. 변경하려면 여정의 새 버전을 만드십시오.
