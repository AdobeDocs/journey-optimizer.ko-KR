...
* **완료됨**: 기본 전역 시간 제한 30일 이후에 여정이 자동으로 이 상태로 전환됩니다. 이미 여정에 있는 프로필은 여정을 정상적으로 완료합니다. 새 프로필은 더 이상 여정에 들어갈 수 없습니다.
* **라이브**: 여정이 다음을 사용하여 게시되었습니다. **게시** 단추를 클릭합니다.
* **중지됨**: 여정이 **중지** 단추를 클릭합니다. 모든 개인이 즉시 여정을 빠져나갑니다.

>[!NOTE]
>
>또한 여정 작성 라이프사이클에는 &quot;게시&quot;(&quot;초안&quot;과 &quot;라이브&quot; 사이), &quot;테스트 모드 활성화&quot; 또는 &quot;테스트 모드 비활성화&quot;(&quot;초안&quot;과 &quot;초안(테스트)&quot; 사이), &quot;중단&quot; 및 &quot;라이브&quot;와 &quot;중단됨&quot; 사이)과 같은 필터링에 사용할 수 없는 중간 상태 집합이 포함됩니다. 여정이 중간 상태일 때는 읽기 전용입니다.
...
다음을 추가할 수도 있습니다. **종료일**. 이렇게 하면 해당 일자가 되었을 때 프로필이 자동으로 종료됩니다. 종료 날짜를 지정하지 않으면 프로필은 [글로벌 여정 시간 초과](#global_timeout) (일반적으로 30일이며, Healthcare Shield 추가 기능 서비스를 통해 7일로 단축됩니다.) 유일한 예외는 다음과 같은 반복 대상자 읽기 여정입니다. **재발 시 강제 재진입** 활성화됨, 다음 발생의 시작 날짜에 종료.
...
이 전역 시간 제한은 여정에서 개인 사용자의 진행을 중지합니다 **30일** 입장한 후에 이 시간 제한은 다음으로 줄어듭니다. **7일** Healthcare Shield 추가 기능 제공 이는 개인의 여정이 30일(또는 7일)을 초과할 수 없음을 의미합니다. 이 시간 제한 기간이 지나면 개인의 데이터가 삭제됩니다. 시간 제한 기간이 끝날 때 여전히 여정에 유입되는 개인은 중지되며, 보고에서 고려되지 않습니다. 따라서 종료하는 것보다 더 많은 사람들이 여정에 들어가는 것을 볼 수 있습니다.
