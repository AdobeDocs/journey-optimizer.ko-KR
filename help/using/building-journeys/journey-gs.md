...
**이 페이지에서:** 시작 대상 또는 여정을 정의하는 것부터 작업을 추가하고 실시간으로 게시하는 것까지 Adobe Journey Optimizer에서 첫 번째 대상자를 만드는 주요 단계를 알아봅니다.
...
* **TL;DR:** 이 페이지에서는 시작점 정의, 캔버스 디자인, 테스트 모드 또는 시험 실행, 게시 등 Adobe Journey Optimizer에서 첫 번째 여정을 만드는 네 가지 주요 단계를 안내하고 올바른 시작점 유형 선택에 대한 지침을 제공합니다.

**의도:**
* 새 여정을 만들고 여정 관리 메뉴에서 속성을 구성합니다
* 주어진 사용 사례에 대한 올바른 진입점(대상 읽기, 대상 자격, 단일 이벤트 또는 비즈니스 이벤트)을 선택합니다
* 이벤트, 오케스트레이션 활동 및 채널 작업을 캔버스로 드래그 앤 드롭하여 여러 단계로 구성된 여정을 디자인합니다
* 시뮬레이션을 사용하여 여정 테스트, 지속적인 AEP 테스트 프로필을 사용한 테스트 모드 또는 게시 전 시험 실행
* 드라이 실행을 실행하여 고객에게 연락하지 않고 실제 프로덕션 데이터로 대상 타깃팅을 확인합니다.
* 여정을 게시하여 라이브로 만들고 보고 도구를 사용하여 성능을 모니터링합니다.

**용어집:**
* **대상자 읽기**: 일괄 처리 대상의 모든 프로필을 한 번에 또는 일정 *(제품별)*&#x200B;에 따라 처리하는 시작 활동
* **대상 자격**: 프로필이 스트리밍 대상 *(제품별)에 들어오거나 나갈 때 실시간으로 트리거되는 시작 활동*
* **단일 이벤트**: 특정 작업이 발생할 때 여정에 한 번에 하나의 프로필을 입력하는 실시간 트리거 *(제품별)*
* **비즈니스 이벤트**: 자동 대상자 읽기 단계 *(제품별)를 통해 동시에 여러 프로필에 대한 여정을 트리거하는 비프로필 이벤트(예: 비행 취소, 재고 보충)*
* **테스트 모드**: 영구 Adobe Experience Platform 테스트 프로필(명시적으로 테스트 프로필로 플래그가 지정됨)을 사용하여 게시 전에 초안 여정을 트래버스하는 유효성 검사 모드 *(제품별)*
* **시뮬레이션**: 즉석에서 생성된 임시 시뮬레이션 사용자를 사용하는 유효성 검사 모드입니다. 시뮬레이션된 사용자는 Adobe Experience Platform *(제품별)에서 지속되지 않습니다*
* **시험 실행**: 실제 고객에게 연락하거나 프로필을 업데이트하지 않고 실제 프로덕션 데이터를 사용하여 여정 논리를 확인하는 특수 게시 모드 *(제품별)*

**보호 기능:**
* 여정에 오류가 있으면 게시할 수 없습니다. 모든 오류를 먼저 해결해야 합니다
* 여정을 빌드하려면 데이터 엔지니어가 이벤트 기반 엔트리의 경우 이벤트 구성을 완료해야 합니다
* 여정 보호 및 제한 사항은 별도로 문서화되어 있으며 규모에 맞게 설계하기 전에 검토해야 합니다
* Adobe Experience Platform에서의 대상 만들기는 대상 기반 여정의 필수 조건입니다

**용어:**
* 정식 이름: 여정 — 약어: 없음 — 변형: 고객 여정, 오케스트레이션 흐름
* 동의어: &quot;테스트 모드&quot; = &quot;여정 테스트&quot;; &quot;시험 실행&quot; = &quot;시험 실행 모드&quot;
...
**FAQ:**
* **Q: 이벤트가 트리거된 여정을 만들기 전에 가장 먼저 수행해야 하는 작업은 무엇입니까?** — 데이터 엔지니어와 함께 이벤트를 구성하여 트리거와 전달하는 데이터를 정의한 다음 여정 진입점으로 이벤트를 참조합니다.
* **Q: Journey Optimizer을 처음 사용하는 사용자에게 어떤 진입점이 권장됩니까?** — 대상 읽기 활동을 사용하여 대상 기반 여정으로 시작합니다. 사전 이벤트 구성이 필요하지 않으며 캔버스에 익숙해지는 가장 쉬운 방법입니다.
* **Q: 여정이 라이브로 전환되기 전에 테스트할 수 있습니까?** — 예. 임시 시뮬레이션 사용자를 사용한 시뮬레이션, 지속적 AEP 테스트 프로필을 사용한 테스트 모드 또는 시험 실행을 사용하여 통신을 보내지 않고 실제 프로덕션 데이터에 대해 실행할 수 있습니다.
* **Q: 게시하려고 할 때 여정에 오류가 발생하면 어떻게 됩니까?** — 오류가 있는 여정은 게시할 수 없습니다. 게시하기 전에 모든 구성 오류를 해결해야 합니다.
* **Q: 복잡한 여정을 여러 단계로 나누려면 어떻게 해야 합니까?** — Jump 활동을 사용하여 더 작은 하위 여정을 연결하여 복잡성을 줄이고 각 하위 여정을 개별적으로 테스트하기 쉽게 할 수 있습니다.
