...
다음을 추가할 수도 있습니다. **종료일**. 이렇게 하면 해당 일자가 되었을 때 프로필이 자동으로 종료됩니다. 종료 날짜를 지정하지 않으면 프로필은 [글로벌 여정 시간 초과](#global_timeout) (일반적으로 30일이며, Healthcare Shield 및 보안/Privacy Shield 추가 기능 서비스를 통해 7일로 단축됩니다.) 유일한 예외는 다음과 같은 반복 대상자 읽기 여정입니다. **재발 시 강제 재진입** 활성화됨, 다음 발생의 시작 날짜에 종료.
...
이 전역 시간 제한은 여정에서 개인 사용자의 진행을 중지합니다 **30일** 입장한 후에 이 시간 제한은 다음으로 줄어듭니다. **7일** Healthcare Shield 및 Privacy and Security Shield 추가 기능 오퍼링을 통해 제공됩니다. 이는 개인의 여정이 30일(또는 7일)을 초과할 수 없음을 의미합니다. 이 시간 제한 기간이 지나면 개인의 데이터가 삭제됩니다. 시간 제한 기간이 끝날 때 여전히 여정에 유입되는 개인은 중지되며, 보고에서 고려되지 않습니다. 따라서 종료하는 것보다 더 많은 사람들이 여정에 들어가는 것을 볼 수 있습니다.
