...
**이 페이지에서:** 여정이 전송되었는지 확인하고, 프로필이 여정을 통해 입력 및 진행되는지 확인하고, 메시지가 전달되는지 확인하는 등 실시간 실행 문제를 해결하는 방법을 알아봅니다.
...
* **ID 네임스페이스 불일치** - 네임스페이스 불일치로 인해 자동 드롭이 발생합니다. 이벤트가 수락되고 성공 응답을 반환하지만 프로필이 여정에 들어가지 않으며 UI에 오류가 표시되지 않습니다. **프로필 식별자**&#x200B;의 네임스페이스가 이벤트 스키마에 정의된 네임스페이스와 정확히 일치하는지 확인합니다(대/소문자 구분). 자세한 내용은 [프로필 식별자 식 형식](testing-the-journey.md#trigger-events-prerequisites)을 참조하세요.
...
>[!NOTE]
>
>여정의 활성 날짜 창 밖에서 전송된 이벤트는 로그 항목 `DISPATCHER DISCARD #16 — unqualified on journey version enablements`과(와) 함께 자동으로 삭제되며 UI 오류가 없습니다. 여정 진행 문제를 해결할 때 항상 테스트 타이밍 구성을 먼저 확인합니다.
...
**아니요.** 영향은 로깅으로만 제한됩니다. [!DNL Adobe Journey Optimizer]은(는) 메시지 실행 계층에 다음과 같은 기본 제공 중복 제거 메커니즘을 가지고 있습니다.

* 한 개의 메시지(이메일, SMS, 푸시 알림 등)만 각 프로필로 전송됩니다.
* 작업은 한 번만 실행됩니다
* 여정 실행이 올바르게 진행됨
