...
## maxInstanceStackEventsHit이 있는 삭제된 이벤트 {#max-instance-stack-events-reached}

이 삭제 이유는 여정 런타임이 특정 여정 버전에 대해 10개의 이벤트라는 프로필당 내부 이벤트 스택 제한에 도달했음을 의미합니다. 동일한 프로필에 대한 다른 이벤트가 계속 처리되는 동안 너무 많은 보류 이벤트가 쌓이지 않도록 하는 안전 보호대입니다.
