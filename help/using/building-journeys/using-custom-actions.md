...
* **TL;DR:** 이 페이지에서는 URL 구성, 헤더/쿼리 매개 변수 매핑, 작업 매개 변수 매핑, 데이터 거버넌스 및 동의 정책 적용 등 JSON 페이로드로 서드파티 REST API를 호출하도록 여정에서 사용자 지정 작업 활동을 추가하고 구성하는 방법에 대해 설명합니다.

**의도:**

* 여정에 사용자 지정 작업 활동을 추가하여 REST API를 통해 데이터를 서드파티 시스템으로 전송합니다
* 표현식 편집기에서 필드와 정적 텍스트를 연결하여 동적 URL 경로 구성
* 여정 이벤트 또는 데이터 소스의 동적 헤더 및 쿼리 매개 변수 값 매핑
* 작업 매개 변수(변수로 정의됨)를 이벤트 필드, 데이터 소스 필드 또는 정적 값에 매핑
* 데이터 거버넌스 및 동의 정책을 적용하여 사용자 지정 작업을 통해 내보내지는 데이터를 제어합니다.

**용어집:**

* **사용자 지정 작업**: JSON 형식의 페이로드를 사용하여 외부 REST API 끝점을 호출하여 서드파티 시스템 *(제품별)*&#x200B;을(를) 통합하는 여정 작업 활동
* **동적 경로**: 여정 컨텍스트 *(제품별)의 필드를 사용하여 실행별로 정의된 사용자 지정 작업 URL의 변수 부분*
* **작업 매개 변수**: 사용자 지정 작업 구성에서 &quot;변수&quot;로 정의된 메시지 페이로드 필드가 여정 수준 *(제품별)에서 여정 데이터에 매핑됨*

**보호 기능:**

* URL의 정적 부분은 여정에서 수정할 수 없습니다. 전역 사용자 지정 작업 구성에서 설정해야 합니다.
* 동적 헤더 및 쿼리 매개 변수 필드는 여정이 아닌 작업 구성 화면에서 변수로 정의됩니다.
* 데이터 거버넌스 및 동의 정책을 적용해 특정 필드의 내보내기 방지나 미동의 고객 제외 등이 가능하다.

**용어:**

* 정식 이름: 사용자 지정 작업 — 약어: 없음 — 변형: 사용자 지정 작업, 서드파티 작업
* 동의어: &quot;action parameters&quot; = &quot;message parameters defined as Variable&quot;
* 혼동하지 마십시오. &quot;정적 URL 부분&quot;(전역 작업 구성에서 설정됨, 여정에서 편집할 수 없음) ≠ &quot;동적 경로&quot;(실행당 여정에서 설정됨)

**FAQ:**

* **Q: 여정 내에서 사용자 지정 작업의 기본 URL을 변경할 수 있습니까?** — 아니요. 여정에서 동적 경로 부분만 설정할 수 있습니다. URL의 정적 부분은 전역 사용자 지정 작업 구성에 구성됩니다.
* **Q: 프로필 ID가 포함된 동적 URL 경로를 만들려면 어떻게 해야 합니까?** — 경로 필드를 고급 표현식 편집기와 함께 사용하여 ID 필드를 정적 문자열로 연결합니다(예: `_id + '/messages'`).
* **Q: 동의 규칙을 사용자 지정 작업에 적용하려면 어떻게 해야 합니까?** — 관련 커뮤니케이션 수신에 동의하지 않은 고객을 제외하도록 사용자 지정 작업에 대한 동의 정책을 구성합니다. 자세한 내용은 동의 페이지를 참조하십시오.
* **Q: 동적 헤더의 값은 어디에서 매핑합니까?** — 활동 창의 URL 구성 섹션에서 동적 헤더 필드 내부를 클릭하거나 연필 아이콘을 사용하여 이벤트 또는 데이터 소스에서 원하는 필드를 선택합니다.
* **Q: 작업 매개 변수에 할당할 수 있는 값 유형은 무엇입니까?** — 매개 변수를 이벤트 필드, 데이터 소스 필드에 매핑하거나, 값을 수동으로 전달하거나, 고급 표현식 편집기를 사용하여 데이터를 조작할 수 있습니다.
