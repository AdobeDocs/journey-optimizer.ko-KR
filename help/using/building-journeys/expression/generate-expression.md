...
* **TL;DR:** 이 페이지에서는 여정 고급 표현식 편집기에서 AI를 사용하여 일반 언어 프롬프트에서 유효한 여정 표현식을 생성하는 방법을 설명합니다.

**의도:**

* AI를 사용하여 자연어 설명에서 여정 표현식 생성
* 적용 단추를 사용하여 생성된 표현식을 고급 표현식 편집기에 직접 적용
* 최적화 활동, 조건 활동 및 사용자 지정 날짜 대기 활동 내에서 AI 기반 표현식 생성을 사용합니다
* 이벤트 기반 조건 및 `dateTimeOnly` 대기 식에 대한 예제 프롬프트를 제공합니다.
* 프로필이 Gold Loyalty 대상에 있는지 확인하는 것과 같이 자연어 프롬프트를 사용하여 대상 멤버십을 확인합니다
* 유효한 필드 이름 및 데이터 소스를 참조하도록 프롬프트를 수정하여 생성 실패 문제 해결

**용어집:**

* **AI를 사용하여 표현식 생성**: 일반 언어 프롬프트를 유효한 여정 표현식으로 변환하는 여정 고급 표현식 편집기에 포함된 AI 기반 생성 기능 *(제품별)*
...
* 정식 이름: AI를 사용하여 표현식 생성 — 약어: none — 변형: AI 표현식 생성, 여정 표현식 생성기
* 동의어: &quot;AI로 표현식 생성&quot; = &quot;AI 표현식 생성기&quot;
* 혼동하지 마십시오: AI(AI 기반 생성기) ≠ 고급 표현식 편집기(수동 코드 편집기 자체)를 사용하여 표현식을 생성합니다.

**FAQ:**

* **Q: AI가 있는 표현식 생성은 어디에 있습니까?** — 조건 활동, 활동 최적화 및 사용자 지정 날짜가 포함된 대기 활동을 포함하여 여정 고급 표현식 편집기가 열릴 때마다 사용할 수 있습니다.
* **Q: AI가 올바른 식을 생성할 수 없으면 어떻게 됩니까?** — 오류 메시지가 나타납니다. 여정 구성에 있는 필드 이름과 데이터 소스를 사용하도록 프롬프트를 수정해야 합니다.
* **Q: 생성된 식을 편집기에 삽입하려면 어떻게 해야 합니까?** — 길잡이 패널의 **적용** 단추를 클릭하여 고급 표현식 편집기의 현재 커서 위치에 직접 삽입합니다.
* **Q: AI로 식을 생성하여 대기 활동에 대한 `dateTimeOnly` 식을 만들 수 있습니까?** — 예. 예를 들어 &quot;지금부터 30일 후 오후 10시를 날짜 시간으로만&quot; 프롬프트가 표시되면 적절한 `dateTimeOnly` 식이 생성됩니다.
+++
