---
solution: Journey Optimizer
product: journey optimizer
title: 여정 시뮬레이션
description: 여정 시뮬레이션 방법 알아보기
feature: Journeys, Test Profiles
topic: Content Management
role: User
level: Intermediate
keywords: 테스트, 여정, 확인, 오류, 문제 해결
version: Journey Orchestration
feature_v2: []
subfeature_v2: []
source-git-commit: 2094f1503f726a352193c86d3a2ccadc8a2caa24
workflow-type: tm+mt
source-wordcount: 3009
ht-degree: 0%

---

# 여정 시뮬레이션 {#simulate-journey}

>[!BEGINSHADEBOX]

**이 페이지에서:** 게시 전에 시뮬레이션 사용자와 빠른 시뮬레이션 및 수동 시뮬레이션을 실행하여 여정 경로를 확인하고 결과를 검토하는 방법을 알아봅니다.

>[!ENDSHADEBOX]

시뮬레이션이 당신에게 적합한 방법인지 확신할 수 없습니까? [세 가지 유효성 검사 옵션 모두 비교](choose-validation-method.md).

게시하기 전에 **[!UICONTROL 시뮬레이션]**&#x200B;을 사용하여 **시뮬레이션된 사용자**&#x200B;와(과) 여정의 유효성을 검사하십시오. 이 페이지에서는 **[!UICONTROL 빠른 시뮬레이션]** 및 **[!UICONTROL 수동 시뮬레이션]**, 시뮬레이션된 여정 생성 및 전송, 사용자에게 필요할 때 단일 이벤트 트리거 및 **[!UICONTROL 결과]** 로그 검토를 안내합니다.

>[!IMPORTANT]
>
>* **[!UICONTROL 시뮬레이션]**&#x200B;을(를) 사용하려면 **[!UICONTROL 여정]** 기능에서 하나 이상의 권한을 할당하십시오. **여정 시뮬레이션**, **여정 게시** 또는 **여정 승인 및 게시**. 동일한 권한을 사용하여 시뮬레이션된 사용자를 만들고 관리할 수 있습니다. **[!UICONTROL 시뮬레이션된 사용자]** 권한은 필요하지 않습니다. [자세히 알아보기](../administration/permissions.md)
>
>* **[!UICONTROL 시뮬레이션]** 없이 시뮬레이션된 사용자를 관리하려면 **[!UICONTROL 시뮬레이션된 사용자]** 기능에서 **시뮬레이션된 사용자 관리** 또는 **시뮬레이션된 사용자 보기**&#x200B;를 지정하십시오.
>
>* 시뮬레이션 중인 AI의 경우(**[!UICONTROL 빠른 시뮬레이션]**, AI 생성 사용자, **[!UICONTROL 이벤트 값 생성]**) **[!UICONTROL AI 길잡이]** 기능에서 **[!UICONTROL 콘텐츠 생성]**&#x200B;을 지정하십시오.

여정 유형별 개요는 [여정 시뮬레이션 시작](simulate-journey-gs.md)을 참조하십시오.

## 시뮬레이션 유형 {#simulation-types}

활성화 후 대상자 항목 읽기가 포함된 일괄 여정은 시뮬레이션을 실행하는 두 가지 방법을 제공합니다.

* **[!UICONTROL 빠른 시뮬레이션]**&#x200B;은(는) Journey Agent에서 제공하는 생성된 사용자, 생성된 이벤트 값 및 기본 테스트 설정을 통해 처음부터 끝까지 실행됩니다. 최소한의 개입으로 여정을 끝까지 시뮬레이션하는 빠른 방법입니다. 이 옵션을 선택하면 바로 빠른 시뮬레이션이 시작됩니다. [빠른 시뮬레이션 비디오 보기](simulate-journey-gs.md#video)

* **[!UICONTROL 수동 시뮬레이션]**&#x200B;을 사용하면 시뮬레이션을 수동으로 단계별로 실행할 수 있습니다. 시뮬레이션된 사용자(수동으로 또는 Journey Agent을 사용하여)를 만들고, 사용자를 여정에 트리거하고, 이벤트 페이로드를 정의하고(수동으로 또는 Journey Agent을 사용하여), 대기를 무시합니다.

![여정 캔버스 옆에 빠른 시뮬레이션 및 수동 시뮬레이션 옵션이 있는 시뮬레이션 설정 패널](assets/quick-simulation-1.png)

### 빠른 시뮬레이션 {#quick-simulation}

**[!UICONTROL 시뮬레이션]**&#x200B;의 모든 여정에서 **[!UICONTROL 빠른 시뮬레이션]**&#x200B;은(는) 생성된 사용자, 여정 값 및 미리 채워진 설정으로 이벤트를 실행합니다.


1. **[!UICONTROL 빠른 시뮬레이션]**&#x200B;을 선택하세요.

1. 실행을 위해 Adobe Journey Optimizer이 수집한 필드를 검토합니다. 테스트 설정 및 실행 주소를 변경하거나 변경하지 않고 계속하려면 **[!UICONTROL 값 업데이트]**&#x200B;를 클릭하십시오.

   이 단계는 여정이 대기 또는 채널을 사용하는 경우에만 나타납니다. 시뮬레이션된 사용자의 모든 대기 기간과 실행 주소를 조정할 수 있습니다. 예를 들어 실행 중 메시지가 받은 편지함으로 이동하도록 사용자 고유의 이메일을 사용합니다.

   ![업데이트 값을 사용하여 정보 수집 단계에서 빠른 시뮬레이션 대화 상자 및 다음 단계로 계속](assets/quick-simulation-2.png)

1. **[!UICONTROL 값 업데이트]**&#x200B;를 연 경우 설정을 편집하십시오. 예를 들어 메시지 증명에 사용되는 주소를 편집한 다음 시뮬레이션을 시작하도록 확인하십시오.

   >[!NOTE]
   >
   >미리 채워진 실행 이메일, 전화 및 푸시 토큰 필드는 이전에 여정 시뮬레이션에 사용되고 브라우저에 캐시된 값에서 가져옵니다.

   ![빠른 시뮬레이션 업데이트 값 단계(대기 시간 재정의 및 증명 이메일, 전화 및 푸시 토큰 필드 포함)](assets/quick-simulation-3.png)

1. Journey Agent은 여정 정의에서 시뮬레이션된 사용자 세트를 생성합니다.

   이메일, SMS 또는 푸시 노드가 있는 여정의 경우 에이전트는 사용할 이메일 주소, 전화번호 또는 푸시 토큰을 확인하라는 메시지를 표시합니다. 시뮬레이션된 사용자는 해당 값을 사용하여 생성됩니다. 완료되면 **[!UICONTROL 생성]**&#x200B;을 클릭하세요.

1. 실행이 완료되면 **[!UICONTROL 결과 보기]**&#x200B;를 클릭하여 경로, 오류 및 검색된 분기를 검토합니다. [결과 보기](#viewing-results)를 참조하세요.

   ![빠른 시뮬레이션이 완료되었으며 모든 단계가 성공했으며 결과를 볼 수 있습니다](assets/quick-simulation-4.png)

또한 빠른 시뮬레이션은 이벤트가 트리거된 여정 및 이벤트 활동을 포함하는 여정을 지원합니다. 이벤트 값은 생성된 모든 시뮬레이션된 사용자에 대해 자동으로 설정되고 실행됩니다. 사용자가 여정에 들어오면 각 이벤트는 해당 Wait에 도달하는 즉시 트리거됩니다.

### 수동 시뮬레이션 {#manual-simulation}

시뮬레이션된 각 사용자를 선택하고 전송 순서를 제어하며 이벤트 페이로드를 구성하고 실행 기간을 **[!UICONTROL 대기]**&#x200B;해야 하는 경우 **[!UICONTROL 수동 시뮬레이션]**&#x200B;을(를) 선택하십시오.

[시뮬레이션된 사용자 만들기 및 관리](#test-users), [이벤트 트리거](#firing-events) 및 [결과 보기](#viewing-results)를 계속합니다.

## 시뮬레이션된 사용자 만들기 및 관리 {#test-users}

>[!CAUTION]
>
>시뮬레이션된 사용자를 보내면 실제 메시지 보내기가 트리거됩니다. 영향을 받는 데이터 세트(예: 피드백 이벤트 또는 추적 이벤트)에 프로필이 활성화된 경우 실행이 시뮬레이션임에도 불구하고 Adobe Experience Platform에서 해당 사용자에 대한 영구 프로필이 생성될 수 있습니다.

시뮬레이션된 사용자는 **[!UICONTROL 시뮬레이션 설정]**&#x200B;에서 정의한 임시 프로필과 같은 엔터티입니다. 이 섹션에서는 이러한 구성 요소를 만들고, 재사용하기 위해 저장하고, 목록에서 조정 또는 제거하고, 여정으로 보내는 방법을 다룹니다.

여정에서 외부 대상(예: CSV 또는 Federated Audience Composition 대상)을 사용하고, 조건, 표현식 또는 개인화가 이 대상의 데이터 보강 속성에 의존하는 경우 프로필 속성과 마찬가지로 시뮬레이션된 사용자에 대해 이러한 속성을 시뮬레이션할 수 있습니다. 이렇게 하면 실제 대상을 실행하지 않고도 설정한 값을 기반으로 사용자를 특정 분기로 유도할 수 있습니다.

1. **[!UICONTROL 테스트 사용자]** 목록을 채우는 것부터 시작하십시오.

   +++ AI로 사용자 생성

   Adobe Journey Optimizer은 여정 정의에서 시뮬레이션된 사용자 세트를 생성합니다.

   이메일, 푸시 또는 SMS 노드가 있는 여정의 경우 AI가 사용할 이메일 주소, 전화번호 또는 푸시 토큰을 확인하라는 메시지를 표시합니다. 시뮬레이션된 사용자는 정의된 이러한 값을 사용하여 생성됩니다. 완료되면 [!UICONTROL 생성]을 클릭하세요.

   생성된 시뮬레이션된 사용자는 표현식 또는 개인화 등 현재 여정에서 사용되는 속성만 표시합니다.

   >[!NOTE]
   >
   >이메일, 전화 및 푸시 토큰 필드는 이전에 여정 시뮬레이션에 사용되고 브라우저에 캐싱된 값에서 미리 채워집니다.

   ![실행 전자 메일, 휴대폰 및 푸시 토큰 필드가 포함된 시뮬레이션된 사용자 대화 상자 생성 및 생성 단추](assets/simulate-generate.png)

   +++

   +++ 인벤토리 찾아보기

   **[!UICONTROL 인벤토리 찾아보기]**&#x200B;를 선택하여 양식 또는 JSON에서 만든 사용자 또는 AI 생성 실행 후 유지한 사용자 등 이미 저장한 시뮬레이션 사용자를 추가합니다.

   인벤토리에서 시뮬레이트된 사용자를 선택하면 다른 여정에 설정된 다른 속성에 대한 값이 있는 경우에도 현재 여정에 사용된 속성만 표시됩니다.

   ![검색, 사용자 테이블 및 선택 단추가 있는 시뮬레이션된 사용자 인벤토리 대화 상자](assets/simulate-inventory.png)

   +++

   +++ 양식에서 만들기

   1. 이 시뮬레이션된 사용자를 식별하려면 **[!UICONTROL 표시 이름]**, **[!UICONTROL ID 네임스페이스]** 및 **[!UICONTROL 설명]**&#x200B;을(를) 입력하십시오.

      ![표시 이름, ID 네임스페이스, 설명 및 공용 구조체 스키마 특성을 사용하여 시뮬레이션된 사용자 양식을 만듭니다](assets/simulate-form.png)

   1. 그런 다음 이 사용자에 대해 채울 유니온 스키마에서 속성을 선택합니다. 여기에서는 표현식 또는 개인화 등 현재 여정에 사용되는 속성만 사용할 수 있습니다.

      속성은 프로필 속성에 대해 원점별로 별도의 섹션으로 그룹화되고, 여정이 CSV 또는 Federated Audience Composition 대상과 같은 외부 대상자를 읽는 경우 해당 대상자의 보강 속성에 대해 표시됩니다.

   1. 세그먼트 멤버십을 시뮬레이션하려면 **[!UICONTROL 대상 멤버십 추가]**&#x200B;를 클릭하십시오.

   1. **[!UICONTROL 시뮬레이션된 사용자 만들기]** 창에서 **[!UICONTROL 시뮬레이션된 사용자 추가]**&#x200B;를 클릭하여 하나의 세션에서 시뮬레이션된 사용자를 여러 개 정의합니다.

      사용자가 목록에 표시되는 방식을 변경하거나, 누적 보기에서 모든 카드를 축소하거나, 사용자의 속성 메타데이터를 열 수 있습니다.

      ![시뮬레이션된 사용자 추가, 모두 축소 및 레이아웃 보기 컨트롤을 사용하여 시뮬레이션된 사용자 바닥글을 만듭니다](assets/simulate-form-3.png)

   1. 시뮬레이션된 사용자 메뉴에서 **[!UICONTROL 복제]**&#x200B;를 사용하여 사용자를 복사하거나 **[!UICONTROL 다른 사용자에게 모든 특성을 적용]**&#x200B;하여 한 사용자의 특성을 세션의 다른 모든 사용자에게 복사하거나 **[!UICONTROL 삭제]**&#x200B;를 사용하여 사용자를 제거합니다.

      ![중복되는 시뮬레이션 사용자 카드를 만들고, 다른 사용자에게 모든 특성을 적용하고, 각 사용자에 대해 삭제합니다](assets/simulate-form-2.png)

   1. 이 세션에서 사용자 구성을 마치면 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

   +++

   +++ JSON에서 만들기

   **[!UICONTROL 시뮬레이션된 사용자 만들기]**&#x200B;에서 JSON 템플릿을 편집하여 사용자를 정의한 다음 **[!UICONTROL JSON 형식 지정]** 및 **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

   **[!UICONTROL JSON에서 만들기]** 및 **[!UICONTROL JSON에서 업데이트]**&#x200B;에서는 프로필 속성 및 해당되는 경우 CSV 또는 Federated Audience Composition 대상과 같은 외부 대상의 데이터 보강 속성을 포함하여 시뮬레이션된 사용자에 저장된 모든 속성에 액세스할 수 있습니다.

   ![사용자 템플릿 및 형식 JSON 컨트롤을 사용하여 시뮬레이션된 사용자 JSON 편집기 만들기](assets/simulate-json.png)

   +++

<!--
    To reuse attribute values from a profile or [test profile](../audience/creating-test-profiles.md) in [!DNL Adobe Experience Platform]:

    1. Browse to the profile you want to use as a reference. On the profile detail page, click **[!UICONTROL View JSON]**. [Learn more](../audience/get-started-profiles.md)

        ![Profile JSON view in Adobe Experience Platform](assets/simulate-json-1.png)

    1. Copy the JSON from the viewer.

    1. In the journey, open **[!UICONTROL Simulation settings]**, start **[!UICONTROL Create Simulated Users]**, and choose **Create from JSON**.

    1. Paste the JSON into the matching part of the simulated user template (for example, the attribute block for one user). Click **[!UICONTROL Format JSON]** to validate the structure.

        ![Create Simulated Users JSON editor with pasted profile attributes](assets/simulate-json-2.png)

    1. Remove properties that exist on the [!DNL Adobe Experience Platform] profile only tied to the source profile, such as mergePolicyId or lastModifiedAt.

    1. Set the fields required by the simulated user template: **[!UICONTROL Display name]**, **[!UICONTROL Identity namespace]**, identity value, and channel execution addresses.

    1. Click **[!UICONTROL Save]**. Use ![Edit icon](assets/do-not-localize/Smock_Edit_18_N.svg) on the saved simulated user to review the data before you run **[!UICONTROL Simulation]**.

        ![Create Simulated Users JSON editor with users template and Format JSON control](assets/simulate-json-3.png)

        If you paste profile JSON, remove or replace all production identifiers and contact points (email, phone, ECID, push token, and similar). Simulation will send messages using the data you provide.
-->

1. 만든 시뮬레이션 사용자가 **[!UICONTROL 테스트 사용자]** 목록에 나타납니다. 각 항목에 대해 다음 중 하나를 선택합니다.

   * ![편집 아이콘](assets/do-not-localize/Smock_Edit_18_N.svg): 시뮬레이션된 사용자의 세부 정보를 업데이트합니다.
   * ![보내기 아이콘](assets/do-not-localize/Smock_Send_18_N.svg): 이 시뮬레이션된 사용자에 대해서만 시뮬레이션을 실행합니다.

     시뮬레이션된 여정 시작은 전송되는 이벤트에 의해 트리거되므로 이벤트로 시작하는 사용자에게는 이 옵션을 사용할 수 없습니다. [자세히 알아보기](#firing-events)

   * ![지우기 아이콘](assets/do-not-localize/Smock_Close_18_N.svg): 이 목록에서 사용자를 제거합니다. 시뮬레이션된 사용자는 삭제되지 않으며 시뮬레이션된 사용자 선택에서 계속 사용할 수 있습니다.

   ![캔버스에 강조 표시된 편집, 전송 및 제거 작업과 시뮬레이션된 경로를 사용하여 사용자 목록을 테스트합니다](assets/simulate-4-2.png)

1. 선택 후 목록을 변경하려면 **[!UICONTROL 사용자 관리]**&#x200B;를 클릭하여 인벤토리에서 또는 새 사용자를 만들어 시뮬레이션된 사용자를 더 추가합니다. 이 실행에 대한 **[!UICONTROL 사용자 테스트]** 목록에서 모든 사용자를 제거하려면 **[!UICONTROL 모든 사용자 지우기]**&#x200B;를 선택하십시오.

   ![사용자 추가 옵션을 사용하여 사용자 관리 메뉴가 열리고 모든 사용자 지우기](assets/simulate-manage.png)

1. 여정에 **[!UICONTROL 대기]** 활동이 포함된 경우 **[!UICONTROL 테스트 설정]** 탭을 열어 시뮬레이션 중 대기 시간을 미세 조정하십시오. 예를 들어 라이브 **[!UICONTROL 대기]** 활동이 며칠 동안 구성된 경우 시뮬레이션된 사용자가 다음 활동으로 이동하기 전에 해당 기간만 노드에서 보내도록 10초로 재정의할 수 있습니다.

1. **[!UICONTROL 모두 보내기]**&#x200B;를 클릭하여 목록의 모든 시뮬레이션된 사용자를 여정으로 보내거나 ![보내기 아이콘](assets/do-not-localize/Smock_Send_18_N.svg)을 클릭하여 해당 사용자만 보냅니다. 시뮬레이션된 사용자가 여정을 성공적으로 입력하면 `Simulated users have entered the journey successfully.` 확인 메시지가 나타납니다.

   사용자가 캔버스에 성공 메시지와 경로를 포함한 여정을 입력한 후 ![사용자 테스트 탭](assets/simulate-5-2.png)

1. 여정에 단일 이벤트가 포함된 경우 트리거할 이벤트를 선택해야 합니다. [이벤트 트리거](#firing-events)를 참조하세요.

1. **[!UICONTROL 결과]** 탭에 액세스하여 실행 로그를 열고 각 단계가 실행되는 방식을 검토하십시오. 자세한 내용은 [결과 보기](#viewing-results)를 참조하세요.

1. 테스트를 마치면 **[!UICONTROL 시뮬레이션 관리]** 메뉴를 엽니다.

   * 현재 시뮬레이션 세션을 종료하려면 **[!UICONTROL 시뮬레이션을 중지]**&#x200B;하십시오.
   * **[!UICONTROL 시뮬레이션을 다시 설정]**&#x200B;하여 현재 실행, 선택된 시뮬레이션된 사용자, 정의된 이벤트 값 및 기타 테스트 설정에서 모든 데이터를 지우고 새 시뮬레이션을 처음부터 시작할 수 있습니다.

     ![시뮬레이션 다시 설정 및 시뮬레이션 닫기 옵션으로 시뮬레이션 관리 메뉴 열기](assets/simulate-15.png)

**[!UICONTROL 시뮬레이션]**&#x200B;에서 여정의 유효성을 검사한 후 **[!UICONTROL 결과]** 로그를 검토하십시오. 오류가 나타나면 **[!UICONTROL 시뮬레이션]**&#x200B;을 종료하고 필요한 변경 내용을 여정에 적용한 다음 실행이 올바르게 나타날 때까지 **[!UICONTROL 시뮬레이션]**&#x200B;을 다시 실행하십시오. 그런 다음 여정을 게시할 수 있습니다. [여정 게시](../building-journeys/publish-journey.md)를 참조하십시오.

## 이벤트 트리거 {#firing-events}

>[!NOTE]
>
>시뮬레이션에서 트리거된 이벤트는 다음과 같이 레이블이 지정된 Adobe Experience Platform의 전용 데이터 세트에 저장됩니다. ``JOtestmode - <schema of your event>``.

여정에 하나 이상의 단일 이벤트가 포함된 경우 시뮬레이션이 활성화되어 있을 때 트리거할 수 있습니다. 이벤트에서 시작하지 않지만 여정이 포함된 경우 시뮬레이션된 사용자가 여정에 들어갈 때까지 이 섹션이 표시되지 않습니다.

1. **[!UICONTROL 이벤트 유형 선택]**&#x200B;에서 이 시뮬레이션에 대해 실행할 이벤트를 선택합니다.

   ![시뮬레이션 설정의 테스트 이벤트 섹션에서 이벤트 유형 선택 드롭다운이 열림](assets/simulate-10-2.png)

1. 목록의 모든 사용자에게 동일한 변경 사항을 적용하려면 **[!UICONTROL 이벤트 관리]** 옵션을 사용하여 다음을 수행합니다.

   * Journey Agent에서 AI를 사용하여 모든 페이로드를 생성할 수 있도록 **[!UICONTROL 이벤트 값을 생성]**&#x200B;합니다. 값이 생성되면 사용자는 **[!UICONTROL 전송 준비 완료]**(으)로 표시됩니다.
   * 목록에서 시뮬레이션된 모든 사용자에 대한 페이로드를 변경하려면 **[!UICONTROL 이벤트 데이터를 편집합니다]**.

   ![AI로 생성 및 모든 옵션 편집으로 테스트 이벤트의 이벤트 관리 메뉴](assets/simulate-9-2.png)

1. 사용자 옆에 있는 ![이벤트 편집](assets/do-not-localize/Smock_Edit_18_N.svg)을 클릭하여 각 사용자에 대한 이벤트 페이로드를 구성하십시오.

   * Journey Agent에서 AI를 사용하여 페이로드를 생성하도록 하려면 **[!UICONTROL 이벤트 값을 생성]**&#x200B;하십시오. 값이 생성되면 사용자는 **[!UICONTROL 전송 준비 완료]**(으)로 표시됩니다.
   * **[!UICONTROL 이벤트 데이터를 편집]**&#x200B;하여 시뮬레이션된 사용자의 페이로드만 변경합니다.

   ![이벤트 값 생성 및 이벤트 데이터 편집 옵션을 사용하여 이벤트 테스트의 사용자별 메뉴](assets/simulate-8-2.png)

1. **[!UICONTROL 테스트 이벤트]**&#x200B;에서 **[!UICONTROL 모두 보내기]**&#x200B;를 선택하여 **[!UICONTROL 테스트 사용자]**&#x200B;에 나열된 모든 시뮬레이션 사용자에 대해 이 이벤트를 보내거나 ![전송 아이콘](assets/do-not-localize/Smock_Send_18_N.svg)을 선택하여 해당 사용자에 대해서만 트리거할 단일 이벤트에 대해 이 이벤트를 보내십시오.

   ![준비 상태로 표시된 사용자에 대한 모든 사용자 및 사용자별 전송 컨트롤을 포함한 테스트 이벤트 섹션](assets/simulate-11-2.png)

1. 이벤트가 실행되면 각 사용자의 진행률을 반영하도록 캔버스가 업데이트됩니다.

1. **[!UICONTROL 결과]** 탭에 액세스하여 실행 로그를 열고 각 단계가 실행되는 방식을 검토하십시오. 자세한 내용은 [결과 보기](#viewing-results)를 참조하세요.

1. 테스트를 마치면 **[!UICONTROL 시뮬레이션 관리]** 메뉴를 엽니다.

   * 현재 시뮬레이션 세션을 종료하려면 **[!UICONTROL 시뮬레이션을 중지]**&#x200B;하십시오.
   * **[!UICONTROL 시뮬레이션을 다시 설정]**&#x200B;하여 현재 실행, 선택된 시뮬레이션된 사용자, 정의된 이벤트 값 및 기타 테스트 설정에서 모든 데이터를 지우고 새 시뮬레이션을 처음부터 시작할 수 있습니다.

     ![시뮬레이션 다시 설정 및 시뮬레이션 중지 옵션으로 시뮬레이션 관리 메뉴 열기](assets/simulate-15.png)

## 결과 보기 {#viewing-results}

**[!UICONTROL 결과]** 탭에서 테스트 결과를 볼 수 있습니다. **[!UICONTROL 사용자 테스트]** 드롭다운에서 실행을 검사할 시뮬레이션된 사용자를 선택합니다. 시뮬레이션된 단일 사용자를 선택하면 캔버스에 사용자가 여정을 따라 이동한 정확한 경로가 강조 표시되므로 예상한 분기로 들어갔는지 확인할 수 있습니다.

실행 시 시뮬레이션된 모든 사용자 간에 집계된 결과를 보려면 **[!UICONTROL 모두]**&#x200B;를 선택하십시오. 그러면 캔버스에 실행이 다루는 모든 경로가 표시되므로 시뮬레이션된 사용자 한 명을 먼저 선택하지 않고도 프로필 간의 적용 범위를 비교하고 활동, 결과 및 오류를 포함한 전체 시뮬레이션을 한 눈에 스캔할 수 있습니다.

![시뮬레이션 요약, 테스트 사용자 필터 및 여정 캔버스의 경로 범위가 포함된 결과 탭](assets/simulate-6-2.png)

각 활동에 대해 로그는 시뮬레이션된 사용자가 단계를 시작했는지 또는 종료했는지 여부, 각 단계에 대한 타임스탬프 및 분기 결정, 시뮬레이션 중에 발생한 오류를 표시할 수 있습니다.

**대기** 활동의 경우 로그에 두 개의 기간 관련 값이 포함됩니다.

* **정의된 기간**: 게시된 여정의 **대기** 활동에 지정되고 여정이 라이브되면 적용되는 기간입니다. 로그는 시뮬레이션이 여정에 정의된 값에만 의존하지 않고 테스트 설정에서 재정의를 적용하는지 여부(예: 10초)를 기록합니다.
* **실제 기간**: 시뮬레이션된 사용자가 **대기** 활동에 남아 있는 경과 시간입니다. 이 값은 **[!UICONTROL 테스트 설정]** 탭에서 설정합니다.

로그에 오류가 나타나면 **시뮬레이션**&#x200B;을 종료하고 필요한 변경 내용을 여정에 적용한 다음 **시뮬레이션**&#x200B;을 다시 실행하십시오. 유효성 검사가 성공하면 여정을 게시합니다. [여정 게시](../building-journeys/publish-journey.md)를 참조하십시오.

+++ AI 기술 자료 참조

이 단원에는 이 주제와 관련된 해석, 검색 및 질문 답변을 지원하기 위한 구조화된 지식이 포함되어 있습니다.

이해를 돕기 위해 이 정보를 이 페이지의 설명서와 통합해야 합니다. 두 소스 모두 독립적으로 사용하기 위한 것은 아닙니다. 이 페이지에서는 기능에 대해 설명하지만, 용어, 의도, 적용 가능성 및 제약 조건을 명확히 하는 데 도움이 되는 추가 컨텍스트를 제공합니다.

* **TL;DR:** 이 페이지에서는 시뮬레이트된 사용자를 만들고 관리하는 방법, 단일 이벤트를 트리거하는 방법, 대기 시간을 재정의하는 방법, 결과 로그를 해석하는 방법 등 Adobe Journey Optimizer에서 빠른 시뮬레이션 및 수동 시뮬레이션을 실행하는 방법에 대한 단계별 지침을 제공합니다.

**의도:**
* 빠른 시뮬레이션을 실행하여 최소한의 수동 입력으로 여정의 전체 유효성을 검사합니다.
* 시뮬레이션된 사용자 생성, 이벤트 페이로드 및 대기 재정의를 제어하려면 수동 시뮬레이션을 설정하십시오.
* AI 생성, 인벤토리 찾아보기, 양식 입력 또는 JSON을 통해 시뮬레이션된 사용자 만들기
* 활성 시뮬레이션 세션 중에 시뮬레이션된 사용자에 대한 단일 이벤트 트리거
* 시뮬레이션 실행 후 결과 로그를 검토하여 오류 및 발견된 분기를 식별합니다
* 시뮬레이션 세션을 재설정하거나 닫아 새로 시작하거나 종료합니다.

**용어집:**
* **빠른 시뮬레이션**: Journey Agent을 사용하여 사용자 및 이벤트 값을 생성하고 최소 수동 단계 *(제품별)로 전체 여정을 실행하는 자동화된 시뮬레이션 모드*
* **수동 시뮬레이션**: 전문가가 사용자 생성, 이벤트 페이로드 및 타이밍을 개별적으로 제어하는 단계별 시뮬레이션 모드 *(제품별)*
* **시뮬레이션된 사용자**: 시뮬레이션에 사용되는 임시 프로필과 유사한 엔터티입니다. 시뮬레이션된 사용자를 보내면 실제 메시지 보내기가 트리거되어 현재 Adobe Experience Platform *(제품별)*&#x200B;에서 영구 프로필이 생성될 수 있습니다.
* **Journey Agent**: AI 지원 시뮬레이션 중 시뮬레이션된 사용자 및 이벤트 페이로드를 생성하는 AI 구성 요소 *(제품별)*
* **테스트 설정**: 시뮬레이션 실행 *(제품별)에 대해 대기 기간 및 실행 주소(이메일, 전화, 푸시 토큰)가 재정의될 수 있는 시뮬레이션 패널 탭*
* **결과 로그**: [결과] 탭에서 실행 로그에 액세스하여 시뮬레이션된 각 사용자의 활동별 결과, 기간 및 오류를 표시합니다. *(제품별)*

**보호 기능:**
* 다음 중 하나 이상 필요: 여정 시뮬레이션, 여정 게시 또는 여정 승인 및 게시 권한
* AI 기능(빠른 시뮬레이션, AI로 생성, 이벤트 값 생성)에는 AI Assistant 기능의 콘텐츠 생성 권한이 필요합니다
* 이벤트가 트리거된 여정의 경우 사용자당 전송 아이콘을 사용할 수 없습니다. 테스트 이벤트 섹션을 통해 항목이 트리거됩니다
* 대기 기간 재정의 및 실행 주소 설정은 여정에 대기 또는 채널 활동이 포함된 경우에만 표시됩니다
* 채널 증명 및 사용자 지정 작업 또는 외부 데이터 소스는 시뮬레이션 중에 실제 아웃바운드 호출을 실행할 수 있습니다. 비프로덕션 연락처를 사용하고 시뮬레이션된 사용자의 실제 고객 PII를 피하십시오
* 결과 로그의 오류는 시뮬레이션을 종료하고, 여정을 수정하고, 게시 전에 다시 실행해야 합니다

**용어:**
* 정식 이름: 빠른 시뮬레이션 — 약어: 없음 — 변형: 없음
* 정식 이름: 수동 시뮬레이션 — 약어: 없음 — 변형: 없음
* 정식 이름: 시뮬레이션된 사용자 — 약어: 없음 — 변형: 테스트 사용자(테스트 사용자 목록의 UI 레이블)
* 동의어: &quot;모두 보내기&quot; = 나열된 모든 시뮬레이션 사용자를 여정에 동시에 트리거
* 혼동하지 마십시오: &quot;시뮬레이션 재설정&quot; ≠ &quot;시뮬레이션 닫기&quot; — 재설정하면 모든 데이터와 설정이 지워지고 닫기 는 현재 세션만 종료됩니다

**FAQ:**
* **Q: 빠른 시뮬레이션과 수동 시뮬레이션의 차이점은 무엇입니까?** — 빠른 시뮬레이션은 AI가 생성한 사용자와 이벤트를 사용하여 전체 여정을 자동으로 실행하며, 수동 시뮬레이션을 사용하면 페이로드와 타이밍을 완벽하게 제어하여 사용자와 이벤트를 단계별로 생성할 수 있습니다.
* **Q: 시뮬레이션 세션 간에 시뮬레이션된 사용자를 재사용할 수 있습니까?** — 예. 인벤토리에 저장된 사용자는 후속 세션에서 인벤토리 찾아보기 를 통해 검색할 수 있습니다.
* **Q: 시뮬레이션 중에 대기 활동 기간을 재정의하려면 어떻게 해야 합니까?** — 테스트 설정 탭을 열고 10초와 같이 더 짧은 기간을 설정하므로 시뮬레이트된 사용자가 대기 노드를 빠르게 이동할 수 있습니다.
* **Q: 시뮬레이션된 특정 사용자에 대해 단일 이벤트를 트리거하는 방법은 무엇입니까?** — 이벤트 테스트 섹션에서 사용자 옆에 있는 편집 아이콘을 클릭하여 이벤트 페이로드를 구성한 다음 해당 행의 보내기 아이콘을 클릭하여 해당 사용자의 이벤트만 트리거합니다.
* **Q: 대기 활동에 대한 결과 로그에서 정의된 기간 및 실제 기간 필드는 무엇을 의미합니까?** — 정의된 기간은 라이브 여정이 구성한 대기 시간입니다. 실제 기간은 시뮬레이션된 사용자가 대기 노드에서 실제로 사용한 재정의된 테스트 기간입니다.
* **Q: 결과 로그에 오류가 표시되면 어떻게 해야 합니까?** — 시뮬레이션을 종료하고 여정에 필요한 수정 사항을 적용한 다음 게시 전에 실행에 오류가 표시되지 않을 때까지 시뮬레이션을 다시 실행합니다.

+++
