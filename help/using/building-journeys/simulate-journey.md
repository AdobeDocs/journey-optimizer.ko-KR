...
1. **[!UICONTROL 값 업데이트]**&#x200B;를 연 경우 설정을 편집하십시오. 예를 들어 메시지 증명에 사용되는 주소를 편집한 다음 시뮬레이션을 시작하도록 확인하십시오.
   ...
   >[!NOTE]
   >
   >미리 채워진 실행 이메일, 전화 및 푸시 토큰 필드는 이전에 여정 시뮬레이션에 사용되고 브라우저에 캐시된 값에서 가져옵니다.

   ![빠른 시뮬레이션 업데이트 값 단계(대기 시간 재정의 및 증명 이메일, 전화 및 푸시 토큰 필드 포함)](assets/quick-simulation-3.png)
...
>[!CAUTION]
>
>시뮬레이션된 사용자를 보내면 실제 메시지 보내기가 트리거됩니다. 영향을 받는 데이터 세트(예: 피드백 이벤트 또는 추적 이벤트)에 프로필이 활성화된 경우 실행이 시뮬레이션임에도 불구하고 Adobe Experience Platform에서 해당 사용자에 대한 영구 프로필이 생성될 수 있습니다.
...
여정에서 외부 대상(예: CSV 또는 Federated Audience Composition 대상)을 사용하고, 조건, 표현식 또는 개인화가 이 대상의 데이터 보강 속성에 의존하는 경우 프로필 속성과 마찬가지로 시뮬레이션된 사용자에 대해 이러한 속성을 시뮬레이션할 수 있습니다. 이렇게 하면 실제 대상을 실행하지 않고도 설정한 값을 기반으로 사용자를 특정 분기로 유도할 수 있습니다.
...
1. **[!UICONTROL 테스트 사용자]** 목록을 채우는 것부터 시작하십시오.
   ...
   이메일, 푸시 또는 SMS 노드가 있는 여정의 경우 AI가 사용할 이메일 주소, 전화번호 또는 푸시 토큰을 확인하라는 메시지를 표시합니다. 시뮬레이션된 사용자는 정의된 이러한 값을 사용하여 생성됩니다. 완료되면 [!UICONTROL 생성]을 클릭하세요.

   생성된 시뮬레이션된 사용자는 표현식 또는 개인화 등 현재 여정에서 사용되는 속성만 표시합니다.

   >[!NOTE]
   >
   >이메일, 전화 및 푸시 토큰 필드는 이전에 여정 시뮬레이션에 사용되고 브라우저에 캐싱된 값에서 미리 채워집니다.

   ![실행 전자 메일, 휴대폰 및 푸시 토큰 필드가 포함된 시뮬레이션된 사용자 대화 상자 생성 및 생성 단추](assets/simulate-generate.png)
   ...
   **[!UICONTROL 인벤토리 찾아보기]**&#x200B;를 선택하여 양식 또는 JSON에서 만든 사용자 또는 AI 생성 실행 후 유지한 사용자 등 이미 저장한 시뮬레이션 사용자를 추가합니다.
   ...
   인벤토리에서 시뮬레이트된 사용자를 선택하면 다른 여정에 설정된 다른 속성에 대한 값이 있는 경우에도 현재 여정에 사용된 속성만 표시됩니다.
   ...
   1. 그런 다음 이 사용자에 대해 채울 유니온 스키마에서 속성을 선택합니다. 여기에서는 표현식 또는 개인화 등 현재 여정에 사용되는 속성만 사용할 수 있습니다.

      속성은 프로필 속성에 대해 원점별로 별도의 섹션으로 그룹화되고, 여정이 CSV 또는 Federated Audience Composition 대상과 같은 외부 대상자를 읽는 경우 해당 대상자의 보강 속성에 대해 표시됩니다.
   ...
   **[!UICONTROL JSON에서 만들기]** 및 **[!UICONTROL JSON에서 업데이트]**&#x200B;에서는 프로필 속성 및 해당되는 경우 CSV 또는 Federated Audience Composition 대상과 같은 외부 대상의 데이터 보강 속성을 포함하여 시뮬레이션된 사용자에 저장된 모든 속성에 액세스할 수 있습니다.
...
1. 테스트를 마치면 **[!UICONTROL 시뮬레이션 관리]** 메뉴를 엽니다.
   ...
   * 현재 시뮬레이션 세션을 종료하려면 **[!UICONTROL 시뮬레이션을 중지]**&#x200B;하십시오.
...
1. 테스트를 마치면 **[!UICONTROL 시뮬레이션 관리]** 메뉴를 엽니다.
   ...
   * **[!UICONTROL 시뮬레이션을 다시 설정]**&#x200B;하여 현재 실행, 선택된 시뮬레이션된 사용자, 정의된 이벤트 값 및 기타 테스트 설정에서 모든 데이터를 지우고 새 시뮬레이션을 처음부터 시작할 수 있습니다.
     ...
     ![시뮬레이션 다시 설정 및 시뮬레이션 중지 옵션으로 시뮬레이션 관리 메뉴 열기](assets/simulate-15.png)

## 결과 보기 {#viewing-results}

**[!UICONTROL 결과]** 탭에서 테스트 결과를 볼 수 있습니다. **[!UICONTROL 사용자 테스트]** 드롭다운에서 실행을 검사할 시뮬레이션된 사용자를 선택합니다. 시뮬레이션된 단일 사용자를 선택하면 캔버스에 사용자가 여정을 따라 이동한 정확한 경로가 강조 표시되므로 예상한 분기로 들어갔는지 확인할 수 있습니다.

실행 시 시뮬레이션된 모든 사용자 간에 집계된 결과를 보려면 **[!UICONTROL 모두]**&#x200B;를 선택하십시오. 그러면 캔버스에 실행이 다루는 모든 경로가 표시되므로 시뮬레이션된 사용자 한 명을 먼저 선택하지 않고도 프로필 간의 적용 범위를 비교하고 활동, 결과 및 오류를 포함한 전체 시뮬레이션을 한 눈에 스캔할 수 있습니다.

![시뮬레이션 요약, 테스트 사용자 필터 및 여정 캔버스의 경로 범위가 포함된 결과 탭](assets/simulate-6-2.png)

각 활동에 대해 로그는 시뮬레이션된 사용자가 단계를 시작했는지 또는 종료했는지 여부, 각 단계에 대한 타임스탬프 및 분기 결정, 시뮬레이션 중에 발생한 오류를 표시할 수 있습니다.
