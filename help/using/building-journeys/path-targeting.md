...
1. **[!UICONTROL 규칙 만들기]** > **[!UICONTROL 규칙 선택]**&#x200B;을 클릭하여 **[!UICONTROL 규칙]** 메뉴에서 만든 기존 타깃팅 규칙을 선택할 수도 있습니다. [자세히 알아보기](../experience-decisioning/rules.md)
   ...
   >[!AVAILABILITY]
   >
   >[전용 [!DNL Journey Optimizer] 메뉴에서 타깃팅 규칙을 만드는 중](../experience-decisioning/rules.md#create)은(는) 현재 Decisioning 추가 기능 서비스를 구입한 조직에서 사용할 수 있으며 다른 조직에 대해 필요할 때 사용할 수 있습니다(제한된 가용성).
   >
...
- **TL;DR:** 이 페이지에서는 Adobe Journey Optimizer 여정에서 경로 타깃팅을 사용하여 정의된 규칙에 따라 특정 대상 세그먼트를 지정된 여정 경로로 결정적으로 라우팅하는 방법을 설명합니다.

**의도:**
- 타깃팅 규칙 방법을 사용하여 활동 최적화 를 사용하여 결정론적 경로 타깃팅 구성
- 규칙 메뉴에서 새 타깃팅 규칙을 만들거나 기존 규칙을 다시 사용합니다
- 타깃팅 규칙에 적합하지 않은 프로필에 대한 대체 경로 정의
- 개별 대상 세그먼트(예: 충성도 계층, 비헤이비어, 구매 내역)에 대한 여정 경로 개인화
- 원래 규칙 정의에 영향을 주지 않고 인라인 타깃팅 규칙 수정

**용어집:**
- **활동 최적화**: 실험(무작위) 또는 타깃팅(결정적) *(제품별)을 통해 프로필을 다른 경로로 분할하는 데 사용되는 여정 캔버스 활동*
- **타깃팅 규칙**: 프로필 또는 대상 특성 *(제품별)을(를) 기반으로 프로필이 입력하는 여정 경로를 결정하는 결정론적 자격 조건*
- **대체 여정 경로**: 정의된 타깃팅 규칙 *(제품별)*&#x200B;을(를) 충족하지 않는 프로필에 대한 대체 경로

**보호 기능:**
- 경로 타깃팅은 현재 제한된 가용성입니다. 액세스를 요청하려면 Adobe 담당자에게 문의하십시오.
- 전용 Journey Optimizer 규칙 메뉴에서 타깃팅 규칙을 만들려면 의사 결정 추가 기능이 필요하거나 필요할 때 사용할 수 있습니다(제한된 가용성).
- 규칙 메뉴에서 규칙을 선택하여 여정에 복사하는 경우 원래 규칙에 대한 후속 변경 사항은 여정의 복사본에 영향을 주지 않습니다.
- 규칙 인라인을 편집해도 해당 규칙이 가져온 원래 규칙은 수정되지 않습니다.
- 대체 경로 옵션을 활성화하지 않으면 타깃팅 규칙에 적합하지 않은 프로필이 여정을 완전히 종료합니다.

**용어:**
- 정식 이름: 경로 타깃팅 — 약어: 없음 — 변형: 결정론적 경로 라우팅, 규칙 기반 경로 분할
- 동의어: &quot;타기팅 규칙&quot; = &quot;자격 규칙&quot; = &quot;경로 조건&quot;
- 혼동하지 마십시오. &quot;타깃팅&quot;≠ &quot;실험&quot;(타깃팅은 결정론적이며 실험은 무작위 할당)

**FAQ:**
- **Q: 경로 타깃팅과 경로 실험 간의 차이점은 무엇입니까?** — 타겟팅은 결정적입니다. 프로필은 정의된 규칙에 따라 경로를 입력합니다. 실험은 무작위입니다. 프로필은 우연히 경로에 할당되어 성능을 비교합니다.
- **Q: 타깃팅 규칙에 적합하지 않은 프로필은 어떻게 됩니까?** — 대체 경로 옵션을 활성화하면 대체 경로를 입력합니다. 활성화되지 않은 경우 여정이 완전히 종료됩니다.
- **Q: 규칙 메뉴에서 기존 규칙을 다시 사용할 수 있습니까?** — 예. 하지만 규칙 공식이 여정 활동에 복사됩니다. 규칙 메뉴에서 원래 규칙에 대한 후속 변경 사항은 여정의 복사에는 영향을 주지 않습니다.
- **Q: 타깃팅 규칙을 인라인으로 편집하면 원래 규칙이 변경됩니까?** — 아니요. 인라인 편집을 수행하면 여정 활동 내의 규칙만 업데이트되고 소스 규칙에는 영향을 주지 않습니다.
- **Q: 경로 타깃팅에 액세스할 수 있는 사람은 누구입니까?** — 현재 제한된 가용성입니다. 액세스를 요청하려면 Adobe 담당자에게 문의하십시오.
