...
>title="양식 필드의 렌더링 확인"
>abstract="JSON 또는 HTML 콘텐츠 템플릿에서 편집 가능한 특정 필드를 정의할 수 있습니다. 이렇게 하면 기술 전문가가 아닌 사용자도 코드를 조작하지 않고도 코드 기반 경험에서 콘텐츠를 쉽게 편집할 수 있습니다. 전용 구문을 사용하여 이러한 필드를 만들고 이 버튼을 사용하여 미리 봅니다."
...
1. `{{#each}}` 태그 뒤에 [위](#add-editable-fields)에 설명된 **인라인** 구문을 사용하여 추가하려는 편집 가능한 양식 필드에 해당하는 코드를 삽입합니다. `"name"`을(를) 고유 ID로 바꾸면 편집 가능한 필드를 식별할 수 있습니다. 이 예제에서는 &quot;title&quot;을 사용합니다.
...
>title="양식 필드란?"
>abstract="이 코드 기반 경험에는 개인화 편집기에서 코드를 조작하지 않고도 쉽게 편집할 수 있는 양식 필드가 포함되어 있습니다."
