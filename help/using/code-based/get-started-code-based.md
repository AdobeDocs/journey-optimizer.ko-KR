...
**이 페이지의 내용:** 코드 기반 채널을 통해 앱과 웹 페이지의 특정 위치에 개인화된 콘텐츠를 제공하는 방법과 다른 채널 대신 이 채널을 사용해야 하는 시점을 알아봅니다.
...
| 이점 | 이유 | 사용 사례 예시 |
| --- | --- | --- |
| 심층 개인화 | 시각적 편집기에서 노출하는 것 이상의 복잡한 논리 및 실시간 데이터 지원 | 실시간 인벤토리 또는 사용자 컨텍스트를 기반으로 하는 개인화 |
| 외부 시스템과 통합 | 외부 시스템의 데이터를 사용하여 콘텐츠를 구성할 수 있습니다 | 날씨 기반 프로모션, 실시간 인벤토리 기반 오퍼 |
| 고급, 여러 단계의 조건부 워크플로 | 시각적 채널의 사전 정의된 작업에 국한되지 않음 | 터치포인트 간 여러 단계 의사 결정 논리 |
| 플랫폼 한계 넘어서기 | 개발자가 사용자 지정 대화형 요소를 구축할 수 있습니다 | 기본 제공 채널에서 지원되지 않는 맞춤형 UI 구성 요소 |
| 캠페인 유연성 향상 | 콘텐츠 및 논리는 자체 구현에 의해 정의됩니다 | 헤드리스, API 기반 또는 비브라우저 디지털 속성 |

## 사용하지 말아야 할 경우 {#when-not-to-use}

코드 기반 채널은 개발 노력이 필요하므로 모든 시나리오에 적합한 선택은 아닙니다. 다음과 같은 상황에서는 다른 채널을 고려하세요.

* 캠페인을 빠르게 진행해야 하거나 단순하여 개발 작업 없이 웹이나 인앱과 같은 노코드 채널로 구축할 수 있는 경우
* 사용자 지정 구현을 구축하고 유효성을 검사하는 데 사용할 수 있는 개발자 리소스 또는 테스트 환경이 없는 경우
* 코드 기반 경험에는 더 많은 사전 계획이 필요하므로 사용자 지정 개발에 사용할 수 있는 일정이나 예산이 제한적인 경우
* 기본 제공 채널 기능만으로 표준 메시징 요구 사항을 충족할 수 있어 사용자 지정 개발이 불필요한 경우
* 표면 및 구현에는 지속적인 개발자 지원이 필요하므로 사용자 지정 코드에 대한 장기적인 유지 보수가 우려되는 경우
...
* 더 많은 계획이 필요하고 개발자가 지정하는 사항만 변경할 수 있습니다. 따라서 개인화 또는 테스트를 위해 수정해야 할 애플리케이션의 구성 요소(홈 배너, 히어로 이미지, 메뉴 모음 등)를 파악하고 개발팀과 협력하여 이러한 변경 사항을 처리하는 데 필요한 구현을 구축하는 것이 중요합니다.
