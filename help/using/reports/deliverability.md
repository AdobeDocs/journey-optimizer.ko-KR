...
>[!IMPORTANT]
>
>모든 ISP가 Gmail과 같은 기존 FBL을 제공하는 것은 아닙니다. Gmail은 개별 수준 피드백을 제공하지 않으며, Google Postmaster 도구 내의 집계 수준 보고에 중점을 두고 개별 수신자에게 스팸 불만을 추적하는 데 사용할 수 없습니다. [자세히 알아보기](#providers-no-fbl)


모든 Adobe 고객은 아래 나열된 ISP의 기존 FBL에 자동으로 등록됩니다.
...
Adobe은 사용 가능한 최신 FBL이 추가되도록 정기 감사를 수행합니다.

### 수신자별 FBL이 없는 공급자 {#providers-no-fbl}

모든 ISP가 기존 FBL을 제공하는 것은 아닙니다. 일부 주요 사서함 공급자는 수신자별 스팸 불만을 반환하지 않으므로 해당 공급자를 사용하는 수신자에게 제외 목록에 항목이 없을 것으로 예상됩니다.

| 사서함 공급자 | 수신자별 불만 사항 FBL? |
|---|---|
| **Gmail/Google Workspace** | 아니요 — Google Postmaster Tools(Feedback-ID, 수신자별 아님)를 통해서만 보고를 집계합니다. [자세히 알아보기](https://support.google.com/a/answer/6254652?hl=en){target="_blank"} |
| **Apple iCloud**(icloud.com, me.com, mac.com) | 아니요 — [자세히 알아보기](https://support.apple.com/en-us/102322){target="_blank"} |
| **회사 Microsoft 365/Exchange Online** | 아니요 — JMRP(Junk Mail Reporting Program)는 소비자 Outlook.com 네트워크만 지원하며, 기업 M365 환경의 테넌트 정크 보고서는 발신자에게 반환되지 않습니다. [자세히 알아보기](https://learn.microsoft.com/en-us/answers/questions/5787589/does-the-junk-mail-reporting-program-(jmrp)-send-c){target="_blank"} |

이러한 공급자는 함께 대다수의 소비자 및 비즈니스 받은 편지함을 나타냅니다. 받는 사람당 컴플레인을 반환하지 않으므로 이러한 서비스를 사용하여 메시지를 스팸으로 표시하는 받는 사람은 **not**&#x200B;이(가) 비표시 목록에 자동으로 표시됩니다.
