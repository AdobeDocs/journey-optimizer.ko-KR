...
여정 단계 이벤트는 Adobe Journey Optimizer의 여정을 진행할 때 프로필이 취하는 각 단계에 대한 자세한 정보를 캡처하는 자동으로 생성된 이벤트입니다. 이러한 이벤트는 여정 성능에 대한 포괄적인 가시성을 제공하고 강력한 분석 기능을 가능하게 합니다.

## 여정 단계 이벤트란? {#what-are-step-events}

여정 단계 이벤트는 여정에서 프로필이 한 노드에서 다른 노드로 이동할 때마다 Adobe Journey Optimizer이 자동으로 만들어 Adobe Experience Platform에 전송하는 시스템 생성 XDM(Experience Data Model) 이벤트입니다. 각 이벤트는 고객의 여정 경험에서 특정 작업 또는 전환에 해당합니다.
...
- **시작 이벤트**: 프로필이 여정에 들어갈 때
- **작업 실행**: 메시지를 보내거나 사용자 지정 작업을 수행할 때
- **조건 평가**: 프로필이 결정 지점을 통과할 때
- **대기 활동**: 프로필이 대기 노드에 들어가고 나올 때
- **종료 이벤트**: 프로필이 완료되거나 여정을 종료하는 경우
- **오류 처리**: 여정 실행 중에 오류가 발생하는 경우

>[!NOTE]
>
>여정 단계 이벤트는 기본적으로 모든 인스턴스에서 활성화됩니다. 단계 이벤트에 대한 프로비저닝 중에 생성된 스키마 및 데이터 세트는 수정하거나 업데이트할 수 없습니다. 이러한 스키마와 데이터 세트는 읽기 전용 모드에 있습니다.
...
- **여정 성능 추적**: 프로필이 실시간으로 여정을 통해 어떻게 이동하는지 모니터링합니다.
- **전환 분석**: 중단점 및 성공적인 전환 경로 이해
- **오류 감지**: 문제가 발생할 때 식별하고 해결합니다.

### 데이터 통합 및 통찰력 {#data-integration}

- **플랫폼 간 분석**: 여정 데이터를 다른 Adobe Experience Platform 데이터 소스와 결합
- **Customer 360 보기**: 여정 상호 작용을 포함하는 포괄적인 고객 프로필 만들기
- **속성 모델링**: 여정 터치 포인트를 다운스트림 비즈니스 결과에 연결

### 최적화 기회 {#optimization}

- **A/B 테스트 인사이트**: 다양한 여정 경로의 성능을 분석합니다.
- **Personalization 개선 사항**: 여정 동작 데이터를 사용하여 향후 환경을 개선합니다.
- **운영 효율성**: 병목 현상을 식별하고 여정 디자인을 최적화합니다.
...
1. **데이터 레이크 쿼리**: SQL을 사용하여 `journey_step_events` 데이터 집합을 쿼리합니다.
2. **Customer Journey Analytics**: 고급 분석 도구를 통해 여정 데이터 분석
3. **실시간 보고**: Journey Optimizer의 기본 제공 보고 기능을 통해 데이터에 액세스
...
- **여정 식별**: 여정 ID, 버전 및 이름
- **프로필 정보**: 프로필 ID 및 관련 ID
- **단계 세부 정보**: 노드 이름, 단계 유형 및 실행 상태
- **타임스탬프**: 각 여정 단계의 정확한 타이밍
- **작업 결과**: 성공/실패 상태 및 실행 세부 정보
- **오류 정보**: 문제가 발생할 때 자세한 오류 코드 및 설명
...
- 쿼리 성능 향상을 위해 `journeyVersionID` 대신 `journeyVersionName`을(를) 사용합니다.
- 날짜 범위별로 필터링하여 큰 데이터 세트의 쿼리 속도 개선
- 여정 네임스페이스 구성과 일치하는 프로필 ID 활용
...
- 전체 속성을 위해 여정 단계 이벤트를 메시지 피드백 데이터와 결합
- 시간 기반 분석을 사용하여 여정 속도 및 병목 현상 파악
- 집단 분석을 만들어 다양한 여정 변형 비교

### 고급 분석 기능 {#advanced-analytics}

**Customer Journey Analytics 통합**
여정 단계 이벤트는 Customer Journey Analytics을 사용하여 분석할 수 있습니다.
...
- **[기본 제공 스키마 사전](https://experienceleague.adobe.com/tools/ajo-schemas/schema-dictionary.html?lang=ko)**: 전체 XDM 스키마 참조
...
여정 단계 이벤트는 Adobe Journey Optimizer의 고급 여정 분석의 기초를 형성합니다. 이러한 이벤트를 효과적으로 이해하고 활용함으로써 고객 행동에 대한 깊은 통찰력을 얻고, 여정 성능을 최적화하고, 고객을 위해 보다 개인화된 경험을 만들 수 있습니다.
