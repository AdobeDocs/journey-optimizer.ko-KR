...
**이 페이지에서:** Adobe Journey Optimizer에서 인앱 캠페인 보고서를 읽고 표시 및 클릭 트렌드, 추적 데이터 및 인앱 메시지에 대한 추적 링크를 분석하는 방법을 알아봅니다.
...
* **[!UICONTROL 전송]**: 앱에서 인앱 캠페인을 요청한 횟수입니다. 사용자 세션당 여러 요청(예: 실행 또는 다시 로드 시)을 사용하면 캠페인 데이터가 캐시되지 않은 경우 이 값이 고유한 사용자 수를 초과할 수 있습니다.

* **[!UICONTROL 인바운드 트리거됨]**: 인앱 메시지를 표시하는 것으로 앱이 간주한 횟수입니다. 앱 측 규칙으로 인해 메시지가 표시되지 않는 경우 이 숫자는 총 전송 횟수보다 작을 수 있습니다.
