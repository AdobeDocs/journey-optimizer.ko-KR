...
**이 페이지에서:** 데이터 원본의 정의와 올바른 데이터 액세스 전략을 선택하여 조건, 개인화 및 타이밍을 위해 여정에 추가 데이터를 가져올 수 있도록 하는 방법을 이해합니다.
...
>[!TIP]
>Journey Optimizer에서 데이터 관리를 처음 수행하시나요? 데이터 소스를 구성하기 전에 스키마, 데이터 세트, ID 및 데이터 흐름 방법을 이해하려면 [데이터 관리 시작하기](../data/gs-data.md) 개요로 시작하십시오.
...
>[!NOTE]
>
>이제 응답이 지원되므로 외부 데이터 소스 사용 사례에서 데이터 소스 대신 사용자 정의 작업을 사용해야 합니다. 응답에 대한 자세한 내용은 이 [섹션](../action/action-response.md)을 참조하세요.
...
>[!TIP]
>
>이 옵션은 두 질문에 **예**&#x200B;라고 답하는 경우에 적합합니다.
>* 데이터는 여정 컨텍스트 내에서만 유용하며 다른 곳에서는 필요하지 않습니까? 대상이나 기타 채널에도 데이터가 필요한 경우 옵션 2 또는 3을 고려하십시오.
>* 필요한 속성을 반환하는 API 끝점을 통해 외부 시스템에 액세스할 수 있습니까? 그렇지 않은 경우 먼저 데이터를 데이터 레이크로 수집해야 합니다.
...
>
>이 옵션은 두 질문에 **예**&#x200B;라고 답하는 경우에 적합합니다.
>* 레코드에는 Experience Platform에 이미 저장된 프로필에 액세스하는 데 사용할 수 있는 ID 필드가 포함되어 있습니까? 그렇지 않으면 여정이 프로필에 액세스하여 전달할 수 없습니다.
>* Journey Optimizer 외부에서 [대상자](../audience/about-audiences.md)를 만들거나 ID를 결합하는 데 데이터가 필요하지 않습니까? 있는 경우 옵션 3을 대신 사용하십시오.
...
>[!CAUTION]
>
>**프로필에 대한 데이터 집합을 사용**&#x200B;하기 전에 다음 영역을 평가하십시오.
>* **데이터 동기화** - 수집 실패를 식별하기 위한 경고를 사용하여 외부 데이터베이스를 동기화해야 합니다.
>* **[프로필 보호](https://experienceleague.adobe.com/ko/docs/experience-platform/profile/guardrails){target="_blank"}** — 프로필별 보호 기능은 Experience Platform의 [일반 데이터 수집 보호](https://experienceleague.adobe.com/ko/docs/experience-platform/ingestion/guardrails){target="_blank"}에 추가로 적용됩니다.
>* **ID 무결성** — 정상적인 ID 그래프를 유지하려면 소스 시스템의 ID 데이터를 신중하게 계획해야 합니다.
>* **데이터 레이크 사용률** - 수집 전에 전체 저장소 사용량, 테이블 관계 및 주소 지정 가능한 프로필을 평가해야 합니다.
