...
#### 작동 방식 {#certificate-credential-how-it-works}

인증서 기반 사용자 지정 인증은 [RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523){target="_blank"}에 정의된 대로 JWT 클라이언트 어설션으로 OAuth 2.0 클라이언트 자격 증명을 구현합니다. 이 표준은 Microsoft Entera ID 및 Okta에서 지원하는 표준과 동일합니다. Journey Optimizer은 클라이언트 암호 대신 Adobe의 관리 개인 키로 서명된 JWT를 사용하여 ID를 증명합니다. ID 공급자는 Adobe의 공개 인증서를 사용하여 서명을 확인하며, 이 인증서는 ID 공급자에 한 번 등록됩니다.

토큰 교환은 다음 단계를 따릅니다.

1. Journey Optimizer은 Adobe의 개인 키로 서명된 JWT 클라이언트 어설션을 빌드합니다.
1. 어설션이 `client_id`, `grant_type` 및 `scope`과(와) 함께 토큰 끝점으로 전송됩니다.
1. ID 공급자는 Adobe의 등록된 공개 인증서에 대해 JWT 서명을 확인합니다.
1. ID 공급자가 전달자 액세스 토큰을 반환합니다.
1. Journey Optimizer은 해당 토큰을 사용하여 사용자 지정 작업 엔드포인트를 호출합니다.

#### Adobe 인증서 세부 정보 {#certificate-credential-details}

Adobe은 인증서 및 관련 개인 키를 관리합니다. 다음 표에는 주요 속성이 요약되어 있습니다.
...
| 관리자 | Adobe |
| 알고리즘 | RS256 (RSA) |
| ID 공급자에 등록할 사항 | Adobe의 리프 인증서만 — 중간 또는 루트 CA 아님 |
| 얻는 방법 | [mTLS 공개 인증서 API](https://experienceleague.adobe.com/ko/docs/experience-platform/data-governance/mtls-api/public-certificate-endpoint){target="_blank"}에서 검색합니다(아래 **인증서** 보호 참조). |
| 회전 | Adobe은 만료 60일 전에 인증서를 자동으로 회전합니다(인증서 수명: 13개월). 이전 인증서는 만료 30일 전까지 유효합니다. 현재 고객에게 회전 알림이 전송되지 않습니다. 주기적으로 [mTLS 공개 인증서 API](https://experienceleague.adobe.com/ko/docs/experience-platform/data-governance/mtls-api/public-certificate-endpoint){target="_blank"}를 호출하여 `expiryDate`을(를) 확인하고 이전 인증서가 해지되기 전에 IDP를 다시 구성하십시오. |

Adobe은 만료 60일 전에 인증서를 자동으로 회전합니다. 이전 인증서는 만료 30일 전까지 유효합니다. 현재 고객에게 알림이 전송되지 않습니다. 프로그래밍 방식으로 로테이션을 모니터링하는 방법은 아래의 [**인증서 로테이션** 보호](#certificate-credential-guardrails)를 참조하십시오.

#### JWT 어설션 구조 {#certificate-credential-jwt}

JWT 클라이언트 어설션은 작성하지 않습니다. Journey Optimizer이 생성하고 서명합니다. ID 공급자 팀이 클레임을 확인할 수 있도록 예상 구조가 여기에 제공됩니다.
...
* `exp` − `iat`은(는) 항상 10분 ≤. Okta 및 Entra ID 요구 사항과 일치합니다.
* 각 어설션은 고유한 `jti`을(를) 사용하므로 재생 공격을 안전하게 보호합니다.
* `client_assertion` 및 `client_assertion_type`은(는) 플랫폼에서 자동으로 삽입되며 작성되지 않습니다.

다음은 Microsoft Entra ID의 인증서 자격 증명 인증 유형에 대한 예입니다.
...
다음은 Okta의 동일한 인증서 자격 증명 인증 유형에 대한 예입니다.
...
>* **인증서**: Adobe에서 인증서 및 개인 키를 관리합니다. 인증서를 업로드하거나 입력하지 않습니다. 라이브 여정에서 사용자 지정 작업을 사용하기 전에 ID 공급자에서 **Adobe의 리프 인증서**&#x200B;를 등록해야 합니다. 검색하려면 [mTLS 공개 인증서 API](https://experienceleague.adobe.com/ko/docs/experience-platform/data-governance/mtls-api/public-certificate-endpoint){target="_blank"}를 호출하고 `certCommonName`이(가) `ajo-journeys.aep-mtls.adobe.com`인 항목을 찾습니다. 해당 항목에서 `publicCertificate` 값을 등록합니다. 중간 또는 루트 CA 인증서를 사용하지 마십시오. 현재 고객에게 인증서 순환에 대한 알림이 전송되지 않으므로 주기적으로 mTLS 공개 인증서 API를 호출하여 `expiryDate`을(를) 확인하고 IDP에 등록된 인증서를 업데이트해야 만료 30일 전에 이전 인증서가 해지됩니다.
