...
필드 그룹이 하나 이상의 초안 또는 라이브 여정에서 사용되는 경우 선택한 스키마에서 새 필드를 증분 추가할 수 있지만 이미 선택된 필드는 선택 취소/제거/수정할 수 없습니다. 초안 또는 라이브 여정에서 이미 사용 중인 스키마 필드를 수정하는 경우(예: 필드의 데이터 유형 변경) 필드 그룹을 업데이트할 수 없습니다. 이렇게 하면 여정 손상을 방지할 수 있습니다.
