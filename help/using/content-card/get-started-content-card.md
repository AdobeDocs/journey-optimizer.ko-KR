...
콘텐츠 카드는 메시지가 도착하는 순간 사용자를 방해하는 대신, 사용자가 원하는 속도로 조치를 취할 수 있도록 메시지를 계속 제공하려는 경우에 가장 효과적입니다.

| 이점 | 이유 | 사용 사례 예시 |
| --- | --- | --- |
| 지속적인 가시성 | 사용자가 콘텐츠 카드를 닫을 때까지 해당 카드는 앱이나 웹 사이트에 계속 표시됨 | 지속적인 프로모션, 사용자가 다시 볼 수 있는 주요 기능 |
| 비침입식 게재 | 사용자 인터페이스를 방해하지 않고 자연스럽게 통합 | 다른 콘텐츠와 함께 표시되는 수동 업데이트 |
| 푸시 옵트인 없이 작동 | 푸시 알림 동의 여부와 관계없이 인앱 또는 웹 사이트에 게재됨 | 푸시 알림을 비활성화한 사용자에게 도달 |
| 시각적으로 풍부한 메시지 | 미디어 및 콜 투 액션(CTA) 버튼 지원 | 이미지와 &quot;지금 구매&quot; 버튼이 포함된 제품 하이라이트 |

## 사용하지 말아야 할 경우

콘텐츠 카드는 사용자가 원하는 속도로 볼 수 있는 콘텐츠를 위한 것이므로 모든 시나리오에 적합한 것은 아닙니다. 다음과 같은 상황에서는 다른 채널을 고려하세요.

* 메시지가 시간 제약이 있는 경고인 경우: 사용자가 콘텐츠 카드를 확인하기 위해 앱이나 웹 사이트를 제때 열지 못할 수 있습니다.
* 대상 사용자가 비활성 상태인 경우: 콘텐츠 카드는 앱이나 웹 사이트 내에서만 표시되므로 비활성 사용자는 콘텐츠 카드를 볼 수 없습니다.
* 콘텐츠가 민감하거나 보안 처리가 필요한 경우: 지속적으로 표시되는 카드로는 보안을 보장할 수 없습니다.
* 메시지에 긴급한 조치가 필요한 경우(예: 깜짝 할인 행사): 푸시 알림이나 이메일을 통해 더 빠른 액션을 유도할 수 있습니다.
