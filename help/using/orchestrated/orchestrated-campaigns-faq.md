...
* **미리 전송 가시성**: 타깃팅을 최적화하기 위해 시작하기 전에 정확한 대상자 규모를 확인하십시오.
...
**모범 사례**: 함께 사용 - 트리거되고 반응적인 경험에 대한 여정 및 계획된 일정 기반 이니셔티브에 대한 오케스트레이션된 캠페인입니다.
...
+++ 오케스트레이션된 캠페인을 게시하면 내부적으로 어떻게 됩니까?

**[!UICONTROL 게시]**&#x200B;를 클릭하면 다음 시퀀스가 발생합니다.

1. **스케줄러 활성화** — 일정이 구성된 경우 스케줄러가 시작되고 정의된 시간에 실행을 트리거합니다.
1. **대상자 저장 활동이 먼저 실행** — 모든 대상자 저장 활동이 메시지 활동 전에 실행됩니다. 대상 셸은 Audience Portal에서 만들어지고 자격을 갖춘 프로필이 수집을 시작합니다.
1. **메시지 실행 시작** — 채널 활동이 워크플로우의 첫 번째 메시지 활동에 대한 처리를 시작합니다.
1. **프로필 스냅숏 조회** — 프로필 데이터가 실시간 프로필이 아닌 게시 시 만든 스냅숏에 대해 확인되므로 전체 실행 시 일관성이 보장됩니다.
1. **동의 평가** — 동의는 프로필 레코드에서 직접 부여되며 전송 시 다시 평가되지 않습니다.
1. **프로필 조정** — 받는 사람은 전송 시 Adobe Experience Platform 프로필에 대해 조정됩니다.
1. **게재 로그 만들기** - 게재 이벤트가 `ajo_message_feedback_event` 데이터 집합에 기록됩니다.

**자세히 알아보기**

* [게시 시간 실행 시퀀스](start-monitor-campaigns.md#publication-sequence)
* [오케스트레이션된 캠페인 시작 및 모니터링](start-monitor-campaigns.md)

+++

+++ 캠페인을 게시한 후 메시지가 전송되지 않는 이유는 무엇입니까?

게시 후 메시지가 전송되지 않는 경우가 있습니다. 다음 사항을 순서대로 확인하십시오.

1. **전송 확인 보류 중(가장 일반적인)** — 반복되지 않는 캠페인의 경우 채널 활동의 속성 창에서 전송을 명시적으로 확인할 때까지 기본적으로 메시지 배달이 일시 중지됩니다. 캠페인이 **Live**(으)로 표시되지만 확인될 때까지 메시지가 표시되지 않습니다. [자세히 알아보기](start-monitor-campaigns.md#confirm-sending)

1. **캠페인이 향후 시간으로 예약됨** — 일정이 구성되어 있으면 캠페인이 라이브이지만 실행이 아직 시작되지 않았습니다. 예약 설정을 확인하고 구성된 시작 시간을 기다립니다. [자세히 알아보기](create-orchestrated-campaign.md#schedule)

1. **아직 수집 중인 대상 활동 저장** — 게시 시 메시지 활동 전에 실행되는 대상 활동 저장 대상자 수집이 아직 진행 중인 경우 메시지 실행이 아직 시작되지 않았습니다. 캔버스에서 활동 상태 표시기를 모니터링합니다. [자세히 알아보기](start-monitor-campaigns.md#activities)

1. **대상자가 비어 있음** — 타깃팅 쿼리에서 프로필을 0개 반환했습니다. 다시 게시하기 전에 세그먼테이션 규칙을 검토하고 대상자 규모의 유효성을 검사하십시오.

1. **모든 프로필이 옵트아웃됨** — 동의는 각 프로필에 대해 전송 시 평가됩니다. 모든 타겟팅된 프로필이 관련 채널에서 옵트아웃한 경우 메시지가 전송되지 않습니다. [자세히 알아보기](../action/consent.md)

1. **채널 활동이 오류 상태임** — 채널 활동의 주황색 또는 빨간색 상태 표시기가 차단 문제를 나타냅니다. 오류 및 해결 방법에 대한 자세한 내용을 보려면 **[!UICONTROL 로그]**&#x200B;를 여십시오. [자세히 알아보기](start-monitor-campaigns.md#logs-tasks)

1. **속도 제어 조절 배달** — 채널 활동에서 속도 제어를 사용하도록 설정하면 배달 속도가 예상보다 느려질 수 있습니다. 채널 활동 속성 창에서 속도 제어 설정을 확인합니다. [자세히 알아보기](activities/channels.md#rate-control)
...
+++ 게시에서 실시간 프로필 또는 스냅샷을 사용합니까?

게시 시 프로필 데이터는 실시간 프로필이 아닌 게시 시 **스냅숏**&#x200B;에 대해 확인됩니다. 이렇게 하면 캠페인 실행 전체에 걸쳐 일관성이 보장되며, 모든 활동은 캠페인 실행 기간에 관계없이 동일한 프로필 상태를 처리합니다.

그러나 동의는 항상 현재 프로필 레코드에서 부여되며 전송 시 다시 평가되지 않습니다.

오케스트레이션된 캠페인의 세그멘테이션은 수신자(관계형 스토어)에서 수행되는 반면 메시지 전송 및 동의 확인은 Adobe Experience Platform 프로필에 대해 해결됩니다.

**자세히 알아보기**

* [게시 시간 실행 시퀀스](start-monitor-campaigns.md#publication-sequence)
* [수신자와 프로필 엔티티 간의 관계는 무엇입니까?](#faq-oc)
* [동의 정책 사용](../action/consent.md)
...
예. 오케스트레이션된 캠페인은 크로스채널 오케스트레이션을 지원합니다. 이메일, SMS, 푸시 알림 및 DM 활동을 여러 단계의 캠페인 캔버스에 결합하여 포괄적인 고객 경험을 만들 수 있습니다.
...
채널 구성 요소 및 런타임은 모든 Journey Optimizer 캠페인에 공통되지만, 지원되는 채널은 다릅니다. 오케스트레이션된 캠페인은 이메일, SMS, 푸시 알림 및 DM을 지원합니다.
...
아니요. 웹 및 인앱과 같은 인바운드 채널은 오케스트레이션된 캠페인에서 지원되지 않습니다. 아웃바운드 채널(이메일, SMS, 푸시 알림 및 DM)만 지원됩니다.
...
* 활성화하기 전에 대상자 규모의 유효성을 검사하여 과소 또는 과다 전송을 방지하십시오.
...
오케스트레이션된 캠페인에서 만들어진 오브젝트(예: 대상자 및 워크플로)는 만들어진 샌드박스에 속합니다. 오케스트레이션된 캠페인을 다른 샌드박스(예: 개발, 스테이지 또는 프로덕션)에서 다시 사용하려면 **샌드박스 도구**&#x200B;를 사용하여 복사하십시오. 패키지에 캠페인을 추가하고 패키지를 게시한 다음 대상 샌드박스로 가져옵니다. 가져온 복사본은 **초안**&#x200B;에서 만들어지며, **동일한 패키지를 다시 가져오면 기존 캠페인을 업데이트하지 않고 새 캠페인이 만들어집니다**. 전체 이동은 대개 **두 단계 이상** 걸립니다. 동일한 패키지 또는 추가 패키지 가져오기를 통해 **채널 구성**(대상에서 일치하는 이름), **스키마** 및 **데이터 세트**&#x200B;를 정렬해야 할 수 있습니다. 채널 구성은 캠페인과 함께 복사되지 않습니다. UI에 전체 내보내기 전 확인 목록이 없습니다. 가져오기 매핑 흐름과 **가져오기 후 경고**&#x200B;를 사용하여 설정을 완료하십시오. 자세한 내용 및 제한 사항은 [샌드박스 간 Journey Optimizer 개체 복사](../configuration/copy-objects-to-sandbox.md)를 참조하십시오.

**우수 사례**

* 실험, QA 및 프로덕션을 위해 **별도의 샌드박스**&#x200B;를 유지합니다.
* 가져오기 직후 [캠페인을 복제](../campaigns/manage-campaigns.md#duplicate-a-campaign)하고 중복 항목에서 작업하여 보고에 피드백과 추적 데이터가 올바르게 표시되도록 하십시오.
* 각 가져오기 후에 게시하기 전에 Target 샌드박스에서 캠페인이 종료되는지 확인하십시오.
* 구성을 문서화하고 거버넌스 팀과 협력하여 환경 간의 구성 편차를 줄입니다.

**자세히 알아보기**

* [샌드박스 간에 Journey Optimizer 개체 복사](../configuration/copy-objects-to-sandbox.md)
* [오케스트레이션된 캠페인 시작](gs-orchestrated-campaigns.md)
* [가드레일 및 제한 사항](guardrails.md)
