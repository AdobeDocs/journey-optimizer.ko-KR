...
1. 캔버스에서 **스키마 이름** 옆에 있는 ![](assets/do-not-localize/Smock_AddCircle_18_N.svg)을(를) 클릭하여 특성 추가를 시작합니다.
...
1. 적절한 필드를 **[!UICONTROL 기본 키]** 및 **[!UICONTROL 버전 설명자]**(으)로 할당하십시오.
   ...
   * CDC(변경 데이터 캡처) 수집의 경우, 데이터 변경 유형(예: 삽입, 업데이트, 삭제)을 나타내고 증분 처리를 가능하게 하는 `String` 유형의 이름이 `_change_request_type`인 특수 열입니다. `_change_request_type`은(는) 테이블 스키마에 포함되지 않아야 하며 수집 중에 데이터 파일에만 추가되어야 합니다.
...
이제 스키마를 정의한 후 이를 기반으로 데이터 세트를 만들 수 있습니다. 데이터 세트는 수집된 데이터를 저장하며, 오케스트레이션된 캠페인에 액세스할 수 있도록 활성화해야 합니다.
...
>title="오케스트레이션된 캠페인"
>abstract="데이터 세트를 만든 후 오케스트레이션된 캠페인에 대해 명시적으로 활성화해야 합니다. 이 단계를 통해 데이터 세트를 Adobe Journey Optimizer 내에서 실시간 오케스트레이션 및 개인화할 수 있습니다."


데이터 세트를 만든 후 오케스트레이션된 캠페인에 대해 명시적으로 활성화해야 합니다. 이 단계를 통해 데이터 세트를 Adobe Journey Optimizer 내에서 실시간 오케스트레이션 및 개인화할 수 있습니다.
