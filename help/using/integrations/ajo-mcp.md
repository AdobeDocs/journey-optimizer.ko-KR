...
* **채널 구성 세부 정보 확인** - 여정 또는 캠페인에서 채널 구성을 사용하기 전에 보낸 사람 도메인, 구독 취소 설정 및 IP 풀을 확인하십시오.
* **거버넌스 정책 확인** - 채널 구성에 연결된 마케팅 작업 및 거버넌스 정책을 확인합니다.
...
| 도구 | 설명 |
|---|---|
| **채널 구성 나열** | 이메일, 모바일 메시지, 푸시 알림, WhatsApp, DM, 인앱 메시지, 웹, 코드 기반 경험, 콘텐츠 카드, LINE, 라이브 활동 등 모든 AJO 채널에서 이름, 상태(초안, 활성, 보관됨, 비활성화됨) 또는 채널 유형별로 채널 구성을 필터링합니다. |
| **채널 구성 가져오기** | 보낸 사람/회신 주소, 하위 도메인, IP 풀 및 구독 취소 설정을 포함하여 특정 채널 구성에 대한 전체 구성 세부 정보를 검색합니다. |
| **구성 리소스 나열** | 푸시 자격 증명, 이메일 하위 도메인, IP 풀, SMS 자격 증명, WhatsApp 자격 증명, DM 라우팅, LINE 채널 설정 및 라이브 활동 레지스트리와 같이 채널 구성에서 참조하는 지원 리소스를 나열합니다. |
| **구성 리소스 가져오기** | 유형 및 ID별로 단일 구성 리소스에 대한 전체 세부 정보를 검색합니다. |
| **마케팅 액션 나열** | 데이터 거버넌스 정책 시행에 사용 가능한 마케팅 작업을 나열합니다. |
...
| **채널 구성** | 내 샌드박스에서 사용할 수 있는 채널 사전 설정은 무엇입니까? / 내 이메일 채널 구성을 모두 표시합니다. / WhatsApp 구성이 설정되어 있습니까? / 내 마케팅 이메일 구성에 대해 구성된 발신자 주소 및 회신 주소 |
...
캠페인, 여정, 오퍼, 채널 구성, 구성 리소스 및 샌드박스 정보에 액세스할 수 있습니다. 작업은 읽기 전용(API 검색)이며 쓰기 작업은 현재 릴리스에서 지원되지 않습니다.
+++
