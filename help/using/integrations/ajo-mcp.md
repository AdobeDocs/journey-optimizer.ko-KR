...
**이 페이지에서:** Adobe Journey Optimizer MCP 서버를 사용하여 지원되는 MCP 클라이언트의 일반 언어 프롬프트를 통해 캠페인과 오퍼를 쿼리하고 기능, 도구, 사전 요구 사항 및 제한 사항을 검토할 수 있는 방법을 이해합니다.
...
| 목표 | 예제 프롬프트 |
|---|---|
| **캠페인 개요** | 모든 Journey Optimizer 캠페인 표시 / Journey Optimizer에 설정된 캠페인은 몇 개입니까? |
| **상태 감사** | 현재 어떤 캠페인이 라이브됩니까? / 일시 중지되거나 중지된 캠페인을 나열합니다. |
| **캠페인 세부 정보** | [ID] 캠페인에 대한 전체 세부 정보를 확인하고 [ID] 캠페인에 설정된 모든 내용을 소개합니다. |
...
| 제한 사항 | 설명 | 해결 방법 |
|---|---|---|
| **참여 또는 성능 지표 없음** | MCP 서버는 보고 데이터를 노출하지 않습니다. 도구는 노출 횟수, 클릭스루 비율, 전환 또는 게재 통계를 반환하지 않습니다. | 지표에 Journey Optimizer Reporting UI, CJA MCP 또는 Adobe Analytics MCP를 사용합니다. AEP 쿼리 서비스는 캠페인 실행 ID를 사용하여 원시 이벤트 데이터를 쿼리할 수 있습니다. |
| **캠페인 목록 페이지 매김이 제한됨** | `List Campaigns`은(는) 항상 결과의 첫 페이지를 반환합니다(최대 50개의 캠페인을 사전순으로 정렬). 오프셋 및 제한 값이 적용되지 않으므로 대형 샌드박스에서 전체 열거를 사용할 수 없습니다. | 캠페인 ID 또는 이름이 알려진 경우 `Get Campaign`을(를) 직접 사용하십시오. 전체 목록을 탐색 및 필터링하려면 Journey Optimizer UI를 사용하십시오. |
| **날짜, 채널 또는 일정별 서버측 필터링 없음** | `List Campaigns`은(는) 상태별 필터링만 지원합니다. 게시 날짜, 예약 날짜, 채널 또는 캠페인 유형별 필터링은 서버측에서 사용할 수 없습니다. | 기본 날짜 및 채널 필터링을 지원하는 Journey Optimizer UI 캠페인 목록을 사용합니다. |
| **메시지 콘텐츠 검색을 사용할 수 없음** | 메시지 콘텐츠 도구는 모든 채널 유형(이메일, 코드 기반 등)에 대해 HTTP 502를 반환합니다. 메시지 HTML, 제목 줄, 개인화 토큰 및 오퍼 콘텐츠는 MCP를 통해 검색할 수 없습니다. | **캠페인 > [캠페인] > 컨텐츠** 아래의 Journey Optimizer UI에서 직접 메시지 컨텐츠 및 개인화 토큰을 봅니다. |
...
캠페인, 오퍼 및 샌드박스 정보에 액세스할 수 있습니다. 작업은 읽기 전용(API 검색)이며 쓰기 작업은 현재 릴리스에서 지원되지 않습니다.
+++
