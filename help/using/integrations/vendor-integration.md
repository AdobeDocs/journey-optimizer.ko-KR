...
**이 페이지에서:** 호환 API를 노출하는 외부 플랫폼과 Adobe Journey Optimizer 통합을 사용하는 방법, 작동 보호 기능 및 설정을 안내하는 공급업체 패턴을 알아봅니다.

>[!ENDSHADEBOX]

>[!BEGINSHADEBOX]

고객은 AJO 통합 기능 및 관련 타사 공급업체 또는 통합을 사용하는 것이 HIPAA와 같은 모든 적용 가능한 법률 및 규정을 준수하는지 확인할 책임이 있습니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 이는 Contentable에 의해 유지되거나 공식적으로 지원되지 않습니다. 콘텐츠풀 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Sitecore에서 유지 관리하거나 공식적으로 지원하지 않습니다. Sitecore 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
1. [통합 작업](integrations.md)을 따르십시오. 다운로드 순서 경로에서 **GET**&#x200B;을(를) 구성하고, 사이트코어당 권한 부여 헤더를 설정하고, 컨텍스트에서 `id`을(를) 매핑하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 자산 지연 시간을 조정하십시오.
...
1. 구성 표에 표시된 HTTP 메서드를 선택합니다. 달리 명시되지 않는 한 일반적으로 GET입니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Salsify가 유지 관리하거나 공식적으로 지원하지 않습니다. Salsify 설명서로 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 이 기능은 Contentstack에 의해 유지되거나 공식적으로 지원되지 않습니다. Contentstack 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Akeneo에 의해 유지되거나 정식으로 지원되지 않습니다. Akeneo 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 그것은 매그놀리아에 의해 유지되거나 공식적으로 지지되지 않는다. Magnolia 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Voucherify에 의해 유지되거나 공식적으로 지원되지 않습니다. Voucherify 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 그것은 Talon.One에 의해 유지되거나 정식으로 지원되지 않습니다. Talon.One 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Antavo에 의해 유지되거나 공식적으로 지원되지 않습니다. Antavo 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Salesforce에서 유지 관리하거나 공식적으로 지원하지 않습니다. Salesforce 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 이는 모세관으로부터 유지되거나 공식적으로 지지되지 않는다. Capillary 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Stensul에 의해 유지되거나 공식적으로 지지되지 않는다. Stensul 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 이는 Marigold에 의해 유지되거나 정식으로 지원되지 않습니다. Marigold 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. AccuWeather가 유지 관리하거나 공식적으로 지원하지 않습니다. AccuWeather 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
1. [통합 작업](integrations.md)을 따르십시오. 구독에 다른 요구 사항이 없으면 **GET**&#x200B;을(를) 사용하십시오. `apiKey` 쿼리 매개 변수, 맵 `locationKey` 및 프로필/컨텍스트의 기타 변수를 첨부하고 샘플 JSON, 맵 필드를 붙여 넣은 다음 테스트하십시오.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. ShipStation에서 유지 관리하거나 공식적으로 지원하지 않습니다. ShipStation 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. RevenueCat에서 유지 관리하거나 공식적으로 지원하지 않습니다. RevenueCat 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
1. [통합 작업](integrations.md)을 따르십시오. 아래 모델링된 REST **GET**&#x200B;을 호출하고, 비밀 키 헤더로 인증하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화하십시오.
...
예제 패턴: 프로젝트의 기본 URL 및 버전과 함께 [RevenueCat 문서](https://docs.revenuecat.com/){target="_blank"}에서 RevenueCat의 **제품 가져오기**(또는 이에 상응하는 제품/권한 부여 가져오기)를 사용합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Databricks가 유지 관리하거나 공식적으로 지원하지 않습니다. Databricks 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Bynder에 의해 유지되거나 공식적으로 지원되지 않습니다. Bynder 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
1. [통합 작업](integrations.md)을 따르십시오. 선택한 엔드포인트에서 **GET**&#x200B;을(를) 구성하고(하나의 공통 패턴은 사용자 나열임) [Byender](https://developer.bynder.com/){target="_blank"}당 OAuth를 완료하고, 불필요한 데이터 페이지를 가져오지 않도록 하고, 필드를 매핑하고, 테스트하고, 활성화합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Trustpilot에 의해 유지되거나 공식적으로 지원되지 않습니다. Trustpilot 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. 그것은 Bazaarvoice에 의해 유지되거나 공식적으로 지원되지 않습니다. Bazaarvoice 설명서를 통해 현재 API 세부 정보를 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. OneTrust에서 유지 관리하거나 공식적으로 지원하지 않습니다. OneTrust 설명서를 통해 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Meta에서 유지 관리하거나 공식적으로 지원하지 않습니다. Meta 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
1. [통합 작업](integrations.md)을 따르십시오. 그래프 호출은 종종 버전이 지정된 경로를 가진 **GET**&#x200B;입니다. 토큰 만료를 처리하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화하십시오.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Aprimo에 의해 유지되거나 공식적으로 지원되지 않습니다. Aprimo 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
...
>[!IMPORTANT]
>
>이 구성 예제는 예제 패턴으로서 Adobe에서 독립적으로 테스트되었습니다. Epsilon에 의해 유지되거나 공식적으로 지원되지 않습니다. Epsilon 설명서를 사용하여 현재 API 세부 사항을 확인합니다.
