...
1. [통합 작업](integrations.md)을 따르십시오. 구독에 다른 요구 사항이 없으면 **GET**&#x200B;을(를) 사용하십시오. `apiKey` 쿼리 매개 변수(또는 문서화된 대로)를 첨부하고, 프로필/컨텍스트에서 `locationKey` 및 기타 변수를 매핑하고, 샘플 JSON을 붙여 넣고, 필드를 매핑한 다음 테스트하십시오.
