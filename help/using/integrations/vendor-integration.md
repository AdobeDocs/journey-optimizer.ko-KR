...
1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.
...
인증: 전달자 &lt;CONTENTABLE_DELIVERY_TOKEN>
...
1. [통합 작업](external-sources.md)을 따르십시오. 다운로드 순서 경로에서 **GET**&#x200B;을(를) 구성하고, 사이트코어별 권한 부여 헤더를 설정하고, 컨텍스트에서 `id`을(를) 매핑하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 자산 지연 시간을 조정하십시오.
...
1. 구성 표에 표시된 HTTP 메서드를 선택합니다(달리 명시되지 않은 경우 일반적으로 GET).
...
1. [통합 작업](external-sources.md)을 따르십시오. 벌크 카탈로그 호출보다 단일 제품 검색을 선호하고, 전달자 인증을 설정하고, 샘플 JSON, 맵 필드, 테스트, 활성화를 붙여넣습니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. Contentstack에 필요한 대로 `api_key` 및 `access_token` 헤더를 모두 추가, `environment` 쿼리 매개 변수 포함, 샘플 JSON 붙여넣기, 필드 매핑, 테스트, 활성화.
...
1. [통합 작업](external-sources.md)을 따르십시오. 전달자 토큰과 함께 **GET**&#x200B;을(를) 사용하고, 쿼리 플래그에 필요한 속성 옵션만 요청하고, 샘플 JSON을 붙여 넣고, 최소 속성 집합을 매핑하고, 테스트하고, 활성화하십시오.
...
1. [통합 작업](external-sources.md)을 따르십시오. 모듈이 노출하는 공개 게재 URL 패턴 사용, 목련당 인증 지침(보호되는 콘텐츠의 익명 게재와 토큰), 샘플 JSON 붙여넣기, 필드 매핑, 테스트, 활성화.
...
참고: 배달 API는 로그인이 필요하지 않은 콘텐츠에 대해 rest-anonymous 역할을 사용하는 것입니다. 보호된 데이터에 대한 보안 액세스를 위해 API 토큰 또는 OAuth 2.0과 같은 보다 강력한 방법이 선호됩니다
...
1. [통합 작업](external-sources.md)을 따르십시오. 클러스터의 기본 URL을 설정하고, 필수 헤더(`X-APP-ID`, `X-APP-TOKEN`)를 추가하고, 필터 또는 ID로 목록 끝점을 제한하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. 충성도/REST API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. 해당 지역의 Voucherify OpenAPI 기본 URL에 따라
...
1. [통합 작업](external-sources.md)을 따르십시오. 필요한 프로필 또는 도전 과제 경로에 **GET**&#x200B;을(를) 사용하고, `Authorization: ApiKey-v1 <key>`을(를) 문서화된 대로 설정하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화하십시오.
...
1. [통합 작업](external-sources.md)을 따르십시오. 공급업체의 인증(예: 쿼리의 API 키)으로 **GET**&#x200B;을(를) 구성하고, 정책에 대한 PII를 노출하지 않도록 하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Antavo Enterprise API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. 테넌트에 문서화된 Antavo 스택 기본 URL에 따라
...
1. [통합 작업](external-sources.md)을 따르십시오. 팀이 승인하는 충성도 통합 엔드포인트를 사용하고, Salesforce OAuth를 완료하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 복합 API 제한을 준수하고, 테스트하고, 활성화합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. 필요에 따라 `CAP-API-ACCESS-TOKEN`과(와) 같은 헤더를 구성하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Capillary API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. 해당 지역에 대한 모세관 통합 가이드별
...
1. Stensul 템플릿 API URL을 사용하여 끝점을 구성합니다(예제 패턴). `https://api.stensul.com/v1/templates/{template_id}`

1. 인증을 구성합니다(Stensul API 설명서별 API 키 또는 OAuth).

1. 경로 변수(예: 템플릿 ID)를 정의합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. 영역에 대한 Marigold 호스트를 가리키고, 인증을 설정하고(아래 샘플은 키 및 암호와 함께 `X-Api-Key`을 사용), 샘플 JSON을 붙여넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Marigold REST API(통합 가이드에 따른 엔드포인트)를 사용하여 엔드포인트를 구성합니다. 예제 URL 패턴:

1. Marigold API 설명서에 제공된 기본 URL 및 경로 사용
...
1. 유럽: https://{{customername}}.module.slgnt.eu
...
1. [통합 작업](external-sources.md)을 따르십시오. 게재 호출은 종종 JSON 본문이 있는 **POST**&#x200B;입니다. [Target 인증](https://experienceleague.adobe.com/en/docs/target-dev/developer/api/configure-authentication){target="_blank"}당 OAuth를 구성하고 샘플 응답을 붙여 넣고 필드를 매핑한 다음 예상 볼륨에서 테스트하십시오.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Target Recommendations/게재 API(통합 패턴에 대한 Adobe 설명서별)를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. 사용 사례에 대해서는 Adobe Target 권장 사항 API 설명서 를 참조하십시오
...
1. [통합 작업](external-sources.md)을 따르십시오. 구독에 다른 요구 사항이 없으면 **GET**&#x200B;을(를) 사용하십시오. `apiKey` 쿼리 매개 변수(또는 문서화된 대로)를 첨부하고, 프로필/컨텍스트에서 `locationKey` 및 기타 변수를 매핑하고, 샘플 JSON을 붙여 넣고, 필드를 매핑한 다음 테스트하십시오.
...
1. [통합 작업](external-sources.md)을 따르십시오. 필요한 리소스를 타겟팅(주문과 배송), [ShipStation API당 인증](https://www.shipstation.com/docs/api/){target="_blank"}, 샘플 JSON 붙여넣기, 필드 매핑, 테스트, 활성화.
...
1. [통합 작업](external-sources.md)을 따르십시오. 아래 모델링된 REST **GET**&#x200B;을(를) 호출하고, 비밀 키 헤더로 인증하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.
...
예제 패턴: 프로젝트의 기본 URL 및 버전을 사용하여 **RevenueCat 문서**&#x200B;에서 RevenueCat의 [제품 가져오기](https://docs.revenuecat.com/){target="_blank"}(또는 이에 상응하는 제품/권한 부여 GET)를 사용합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. 좁은 읽기 경로를 선호합니다. **POST** 문 실행을 사용하는 경우 API에 필요한 JSON 본문을 포함하고, 매핑에 대한 샘플 성공 응답을 붙여넣고, 지연을 주의 깊게 테스트하고, 활성화합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. 선택한 엔드포인트에서 **GET**&#x200B;을(를) 구성하고(하나의 공통 패턴은 사용자 목록) [바이트당 OAuth를 완료합니다. ](https://developer.bynder.com/){target="_blank"} 불필요한 데이터 페이지를 가져오지 않고 필드 매핑하고 테스트한 다음 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Byender API v4를 사용하여 끝점을 구성합니다(예: 사용자 목록 패턴). 예제 URL 패턴:
...
1. [통합 작업](external-sources.md)을 따르십시오. 필수 쿼리 인증을 사용하여 **GET**&#x200B;을(를) 구성하고, 프로필 또는 컨텍스트에서 식별자를 매핑하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. **과(와) 함께** GET`passkey`을(를) 대화 API의 쿼리 매개 변수로 사용하고, `Accept: application/json`을(를) 설정하고, 샘플 JSON을 붙여 넣고, 필드를 매핑하고, 테스트하고, 활성화하십시오.
...
1. [통합 작업](external-sources.md)을 따르십시오. 게시된 스키마 또는 환경 설정 센터 경로를 사용하여 구독 문서를 만들고, 필요한 경우 OAuth를 완료하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. OneTrust API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. OneTrust 개발자 포털 기본 URL당
...
1. [통합 작업](external-sources.md)을 따르십시오. 그래프 호출은 종종 버전이 지정된 경로를 사용하는 **GET**&#x200B;입니다. 토큰 만료 처리, 샘플 JSON 붙여넣기, 맵 필드, 테스트, 활성화 등을 수행합니다.
...
1. [통합 작업](external-sources.md)을 따르십시오. 필요한 레코드 경로에서 **GET**&#x200B;을(를) 사용하고, `API-VERSION`과(와) 같은 필수 헤더를 보내고, 샘플 JSON(반환된 대로 HAL 또는 JSON)을 붙여 넣고, 최소 필드 집합을 매핑하고, 테스트하고, 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. Aprimo DAM / Records API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:

1. 임차인에 대한 Aprimo API 기본 URL 및 리소스 경로당
...
1. [통합 작업](external-sources.md)을 따르십시오. 공개 URL은 추측하지 마십시오. Epsilon의 사양을 사용하고, 샘플 JSON을 붙여넣고, 필드를 매핑하고, 테스트 및 활성화합니다.

1. Journey Optimizer에서 구성 > 관리로 이동한 다음 통합 만들기 를 선택합니다.

1. 통합 이름을 공백 없이 입력하십시오.

1. 통합 사양에 따라 Epsilon API를 사용하여 끝점을 구성합니다. 예제 URL 패턴:
...
예제 패턴: `https://{your-instance}.epsilon3.io/api/v1/planning/events` 및 `start` 쿼리 매개 변수와 헤더 기반 API 키가 있는 `end`. 제작 전에 Epsilon과 확인합니다.
