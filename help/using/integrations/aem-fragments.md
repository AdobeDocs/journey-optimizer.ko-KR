...
**이 페이지에서:** 변형 및 Experience Decisioning 작업을 포함하여 Journey Optimizer 캠페인 및 여정에서 Adobe Experience Manager 콘텐츠 조각에 태그를 지정하고, 추가하고, 개인화하는 방법을 알아봅니다.
...
>[!NOTE]
>
>**AEM 콘텐츠 조각**&#x200B;은(는) Adobe Experience Manager에서 작성되고 [!DNL Journey Optimizer]에서 사용됩니다. 다음과 다릅니다.
>
>* **[조각](../content-management/fragments.md)** — [!DNL Journey Optimizer]에서 만들어져 캠페인 및 여정의 전자 메일에 사용되는 재사용 가능한 콘텐츠 구성 요소입니다.
...
자리 표시자 대신 Journey Optimizer 조직 ID와 샌드박스 이름을 사용하여 ID가 `ajo-enabled:{AJO-OrgId}/{AJO-SandboxName}`인 태그를 지정하십시오(예: `ajo-enabled:123A12A123A123A12A@AdobeOrg/prod`).

Experience Manager에서 태그를 만들려면 다음을 수행하십시오.

1. **도구** > **태그 지정**(으)로 이동합니다.

   ![](assets/do-not-localize/aem_tag_1.png)

1. 전체 태그 ID가 위의 포맷과 일치하도록 중첩된 태그 구조를 만듭니다.

   1. 루트 수준에서 `ajo-enabled` 폴더를 만듭니다.

   1. `ajo-enabled`에서 조직 ID(예: `123A12A123A123A12A@AdobeOrg`)에 대한 태그를 만드십시오.

   1. 해당 조직 태그에서 샌드박스에 대한 태그(예: `prod`)를 만듭니다.

   결합된 경로는 `ajo-enabled:123A12A123A123A12A@AdobeOrg/prod`과(와) 같은 태그 ID를 생성합니다.

1. 콘텐츠 조각에 적용하려면 편집기에서 콘텐츠 조각을 엽니다.

1. **속성**&#x200B;에서 만든 태그를 추가합니다.
...
1. 왼쪽 창의 **[!UICONTROL AEM 콘텐츠 조각]** 메뉴에서 **[!UICONTROL AEM 콘텐츠 관리자 열기]**&#x200B;를 클릭합니다.
...
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다. 이제 [이 섹션](../content-management/preview.md)에 자세히 설명된 대로 메시지 콘텐츠를 테스트하고 확인할 수 있습니다.
   ...
   선택한 콘텐츠 조각은 이 메시지에 대해 활성 상태를 유지합니다. 다른 필드 또는 콘텐츠 블록에서 Personalization 편집기를 열 때 **[!UICONTROL AEM 콘텐츠 조각]** 섹션에서 동일한 조각으로 계속 작업하고 **[!UICONTROL AEM 콘텐츠 관리자 열기]**&#x200B;를 다시 열지 않고 필드를 더 추가할 수 있습니다.
...
1. **[!UICONTROL AEM 콘텐츠 조각]**&#x200B;에서 **[!UICONTROL AEM 콘텐츠 관리자 열기]**&#x200B;를 클릭합니다.
