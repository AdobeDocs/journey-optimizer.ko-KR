...
➡️ [Adobe Experience Manager 설명서에서 Adobe Experience Manager Content Advisor에 대해 자세히 알아보기](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/assets/content-advisor/integrate-adobe-non-adobe-applications)
...
정확한 키워드나 파일 이름 대신 자연어를 사용하여 에셋을 검색합니다. 필요한 것을 일반 언어로 설명하십시오. 예를 들어 &quot;산속의 커피&quot;와 같이 AI는 텍스트만 일치하는 것이 아니라 의미와 콘텐츠를 기반으로 컨텍스트에 맞는 에셋을 표시합니다. 다국어 검색도 지원되므로 원하는 언어로 쿼리하고 태그가 지정되었거나 이름이 지정된 언어에 관계없이 올바른 에셋을 찾을 수 있습니다.
...
액세스 권한이 있는 조직의 저장소에서 에셋을 검색하고 선택합니다. 이 기능을 사용하면 다른 조직에 속한 저장소에 저장된 에셋을 탐색하고 사용할 수 있으므로 Journey Optimizer 작성 워크플로를 종료하지 않고도 사용 가능한 에셋 라이브러리에 더 폭넓게 액세스할 수 있습니다.
