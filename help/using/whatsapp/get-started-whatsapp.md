...
WhatsApp은 대상이 이미 플랫폼을 사용하고 있으며 리치 콘텐츠와 실질적인 양방향 대화를 결합하고자 할 때 가장 적합합니다.

| 이점 | 이유 | 사용 사례 예시 |
| --- | --- | --- |
| 높은 글로벌 참여도 | 많은 지역에서 강력한 채택을 통해 널리 사용되는 메시징 플랫폼 | 이미 WhatsApp을 활발하게 사용하는 해외 대상에 도달 |
| 풍부한 대화형 메시지 | 이미지, 비디오, 단추 및 빠른 답글 지원 | 제품 카탈로그, 빠른 회신 옵션을 통한 예약 확인 |
| 양방향 대화 경험 | 수신자는 동일한 스레드 내에서 회신할 수 있습니다. | 고객 지원 대화, 주문 추적 질문 |
| 대화형 멀티스크린 환경 | WhatsApp Flow 템플릿을 사용하면 채팅 내에서 안내형 다단계 상호 작용을 구축할 수 있습니다 | 설문 조사, 잠재 고객 캡처 양식 |
| 공식 API를 통한 규정 준수 및 신뢰 | 발신자 인증을 거쳐 Meta의 인증된 클라우드 API를 통해 전달됨 | 수신자 신뢰를 높이는 브랜드 인증 커뮤니케이션 |
| 다른 채널과 통합 | 다른 채널과 함께 여정 및 캠페인과 계층화할 수 있음 | WhatsApp을 보완 접점으로 사용하는 다중 채널 여정 |

## 사용하지 말아야 할 경우 {#when-not-to-use}

WhatsApp은 대상 채택 및 명시적 동의에 따라 다르므로 모든 시나리오에 적합하지 않습니다. 다음과 같은 상황에서는 다른 채널을 고려하세요.

* 대상이 WhatsApp을 사용하지 않는 경우(지역 및 인구통계에 따라 이용률이 크게 다르므로)
* 수신자로부터 Meta의 메시징 정책에 필요한 명시적 옵트인이 제공되지 않은 경우
* 메시지가 긴급하며 전달이 보장되어야 하는 경우. WhatsApp에는 전달 및 템플릿 검토에 제한이 있기 때문에 SMS 또는 푸시 알림이 더 적합합니다.
* 콘텐츠가 길거나 복잡하여 더 많은 공간과 더 풍부한 포맷을 제공하는 이메일에 더 적합한 경우
* WhatsApp의 양방향 스레드에서는 신속한 답변이 기대되므로 실시간 대화형 지원을 제공하기 어려운 경우
