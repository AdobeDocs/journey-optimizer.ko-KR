...
>title="이름"
>abstract="이 API 자격 증명 집합을 위한 고유한 이름을 입력합니다. WhatsApp 웹후크 및 채널 구성을 구성할 때 선택합니다."

>[!CONTEXTUALHELP]
>id="ajo_admin_whatsapp_config_api_token"
>title="API 토큰"
>abstract="WhatsApp 에셋과 동일한 Business Manager에 속한 시스템 사용자의 Meta 액세스 토큰을 사용합니다. 이 사용자에게는 whatsapp_business_management, whatsapp_business_messaging 및 business_management 권한과 WhatsApp Business 계정에 대한 에셋 수준 액세스 권한이 필요합니다. Meta 토큰은 약 60일 후 만료됩니다. 만료 전에 토큰을 갱신하십시오."

>[!CONTEXTUALHELP]
>id="ajo_admin_whatsapp_config_business_account_id"
>title="Business 계정 ID"
>abstract="Meta Business Portfolio ID(Business Manager ID라고도 함)를 입력합니다. 이 필드에 WhatsApp Business 계정 ID를 입력하지 마십시오."
...
1. 아래에 자세히 설명된 대로 API 자격 증명을 구성합니다.
   ...
   * **API 토큰**: API 토큰을 입력하십시오. [Meta 설명서](https://developers.facebook.com/blog/post/2022/12/05/auth-tokens/)에서 자세히 알아보기
...
WhatsApp API 자격 증명이 정상적으로 생성되면 이제 웹후크를 다음과 같이 구성할 수 있습니다.

* 옵트인 및 옵트아웃 동의 관리를 위한 **인바운드 응답 캡처**
* 읽음 확인 메일(가능한 경우) 및 메시지 배달 상태와 같은 **배달 보고서를 받습니다**
* Adobe Experience Platform 데이터 세트의 분석 및 보고를 위해 **추적 이벤트 활성화**

>[!NOTE]
>
>인바운드 WhatsApp 메시지는 _AJO 전자 메일 추적 데이터 세트_ 시스템 데이터 세트에 캡처됩니다. 들어오는 메시지가 이 데이터 집합에 캡처되기 전에 프로필에 [!DNL Journey Optimizer]에서 보낸 메시지가 하나 이상 있어야 합니다. [자세히 알아보기](../data/get-started-datasets.md#system-datasets)
...
1. 다음과 같은 **[!UICONTROL 인바운드 키워드 범주]**&#x200B;을 선택합니다.
   ...
   * **[!UICONTROL 옵트아웃 키워드]**
   * **[!UICONTROL 도움말 키워드]**
   * **[!UICONTROL 기본값]** - 다른 키워드와 일치하지 않는 모든 인바운드 메시지에 대한 대체 범주. 이 범주를 사용하여 Adobe Experience Platform 데이터 세트에서 이벤트(열기, 게재 보고서) 추적을 활성화합니다.
