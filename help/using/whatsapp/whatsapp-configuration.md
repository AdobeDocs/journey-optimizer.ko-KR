...
>title="이름"
>abstract="이 API 자격 증명 집합의 고유한 이름을 입력하십시오. WhatsApp 웹후크 및 채널 구성을 구성할 때 선택합니다."

>[!CONTEXTUALHELP]
>id="ajo_admin_whatsapp_config_api_token"
>title="API 토큰"
>abstract="WhatsApp 에셋과 동일한 비즈니스 관리자에 있는 시스템 사용자의 Meta 액세스 토큰을 사용합니다. 이 사용자에게는 whatsapp_business_management, whatsapp_business_messaging 및 business_management 권한과 WhatsApp 비즈니스 계정에 대한 에셋 수준 액세스 권한이 필요합니다. Meta 토큰은 약 60일 후에 만료됩니다. 만료되기 전에 토큰을 갱신하십시오."

>[!CONTEXTUALHELP]
>id="ajo_admin_whatsapp_config_business_account_id"
>title="비즈니스 계정 ID"
>abstract="Business Manager ID라고도 하는 Meta Business Portfolio ID를 입력합니다. 이 필드에 WhatsApp 비즈니스 계정 ID를 입력하지 마십시오."
...
>[!NOTE]
>
>인바운드 WhatsApp 메시지는 _AJO 인바운드 활동 이벤트 데이터 세트_ 시스템 데이터 세트에 캡처됩니다. 들어오는 메시지가 이 데이터 집합에 캡처되기 전에 프로필에 [!DNL Journey Optimizer]에서 보낸 메시지가 하나 이상 있어야 합니다. [자세히 알아보기](../data/get-started-datasets.md#system-datasets)
