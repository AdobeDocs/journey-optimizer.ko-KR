...
1. 선택 **Amazon** (으)로 **[!UICONTROL 서버 유형]** 를 사용하여 dm 파일을 내보냅니다.

   ![](assets/file-routing-config-type.png){width="800" align="center"}

1. 서버에 대한 세부 정보 및 자격 증명을 입력합니다.

   * **AWS 버킷 이름**: AWS 버킷 이름을 찾을 수 있는 위치를 알려면 다음을 참조하십시오. [이 페이지](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingBucket.html).

   * **AWS 액세스 키**: AWS 액세스 키 ID를 찾을 수 있는 위치를 알려면 다음을 참조하십시오. [이 페이지](https://docs.aws.amazon.com/IAM/latest/UserGuide/security-creds.html#access-keys-and-secret-access-keys).

   * **AWS 비밀 키**: AWS 비밀 키를 찾을 수 있는 위치를 알려면 다음을 참조하십시오. [이 페이지](https://aws.amazon.com/fr/blogs/security/wheres-my-secret-access-key/).

   * **AWS 지역**: 다음을 선택합니다. **[!UICONTROL AWS 지역]** 서버 인프라가 배치될 위치입니다. AWS 지역은 AWS이 클라우드 인프라를 호스팅하는 데 사용하는 지리적 영역입니다. 일반적으로 DM 공급자 위치와 가장 가까운 지역을 선택하는 것이 좋습니다.
...
1. SFTP 선택 **[!UICONTROL 서버 유형]** 를 사용하여 dm 파일을 내보냅니다.
...
1. Azure 선택 **[!UICONTROL 서버 유형]** 를 사용하여 dm 파일을 내보냅니다.

   ![](assets/file-routing-config-type-azure.png){width="800" align="center"}

1. 서버에 대한 세부 정보 및 자격 증명을 입력합니다.

   * **Azure 연결 문자열**: 을(를) 찾으려면 **Azure 연결 문자열**, 참조 [이 페이지](https://learn.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string#configure-a-connection-string-for-an-azure-storage-account).

     다음 **Azure 연결 문자열** 아래 형식을 따라야 합니다.

     `DefaultEndpointsProtocol=[http|https];AccountName=myAccountName;AccountKey=myAccountKey`

   * **컨테이너 이름**: 을(를) 찾으려면 **컨테이너 이름**, 참조 [이 페이지](https://learn.microsoft.com/en-us/azure/storage/blobs/blob-containers-portal).

     다음 **컨테이너 이름** 슬래시 없이 컨테이너 이름만 포함해야 합니다. 파일을 저장할 컨테이너 내의 경로를 지정하려면 원하는 경로를 포함하도록 DM 캠페인의 파일 이름을 업데이트합니다.
