...
## 다이렉트 메일 메시지 추가 {#create-dm-campaign}

>[!CONTEXTUALHELP]
>id="ajo_journey_action_direct_mail"
>title="다이렉트 메일 작업"
>abstract="다이렉트 메일(DM) 채널 액션은 프로필이 여정의 이 단계에 도달하면 프로필에 대한 DM 콘텐츠를 생성합니다. 레이블은 여정 캔버스에서 활동을 식별하고, 작업은 전달된 콘텐츠를 정의하는 DM 구성을 참조합니다. **최적화** 섹션에는 콘텐츠 실험 또는 타깃팅 규칙이 포함될 수 있고, **다국어** 섹션은 여러 언어로 콘텐츠를 제공할 수 있으며, **시간 초과 또는 오류** 섹션은 작업이 실패할 경우 대체 경로를 정의할 수 있습니다."
...
1. 여정을 열고 팔레트의 **작업** 섹션에서 **[!UICONTROL DM]** 활동을 끌어서 놓습니다.
...
1. DM 공급자에게 보낼 추출 파일을 구성합니다. 이렇게 하려면 **[!UICONTROL 콘텐츠 편집]** 단추를 클릭하십시오.
   ...
   ![DM 활동이 작업 팔레트에서 여정에 추가됨](assets/direct-mail-add-journey.png)
...
1. 표시할 파일 이름 또는 열과 같은 추출 파일 속성을 조정합니다. 추출 파일 속성을 구성하는 방법에 대한 자세한 내용은 [DM 메시지 만들기](../direct-mail/create-direct-mail.md#extraction-file) 섹션을 참조하십시오.
   ...
   ![DM 여정 활동에 대한 추출 파일 콘텐츠 편집기](assets/direct-mail-journey-content.png)

1. 추출 파일의 콘텐츠가 정의되면 **[!UICONTROL 콘텐츠 시뮬레이션]**&#x200B;을 사용하여 미리 봅니다. [콘텐츠를 미리 보고 테스트하는 방법에 대해 알아보세요](../content-management/preview-test.md)

   ![DM 추출 파일에 대한 콘텐츠 미리 보기 시뮬레이션](assets/direct-mail-simulate.png){width="800" align="center"}
...
1. 사용할 **[!UICONTROL DM 구성]**&#x200B;을(를) 선택하거나 만드십시오. [DM 구성을 만드는 방법을 알아보세요](direct-mail-configuration.md#direct-mail-surface).
   ...
   ![예약된 마케팅 캠페인에 구성된 DM 작업](assets/direct-mail-campaign.png){width="800" align="center"}

   >[!AVAILABILITY]
   >
   >DM은 **홀드아웃** 기능을 지원하지만 현재 **처리**&#x200B;를 지원하지 않습니다. [실험 작업 방법 알아보기](../content-management/get-started-experiment.md)

1. 특정 날짜에 대해 캠페인을 예약하거나 정기적으로 반복하도록 설정할 수 있습니다. [이 섹션](../campaigns/campaign-schedule.md)에서 캠페인의 **[!UICONTROL 일정]**&#x200B;을 구성하는 방법을 알아보세요.
...
1. 캠페인 또는 여정 구성 화면에서 **[!UICONTROL 콘텐츠 편집]** 단추를 클릭하여 추출 파일 콘텐츠를 구성합니다.

1. DM 메시지에 결정 정책을 추가하려면 **[!UICONTROL 데이터 필드]** 섹션에서 열을 선택하고 ![](../experience-decisioning/assets/do-not-localize/editor-icon.svg) 아이콘을 사용하여 개인화 편집기를 엽니다. 결정 정책을 만들고 삽입하려면 **[!UICONTROL 결정 정책]** 메뉴로 이동하십시오. 그런 다음 의사 결정 항목 속성을 추출 파일의 열 데이터로 사용할 수 있습니다.

   >[!AVAILABILITY]
   >
   >DM에서의 Experience Decisioning 은 새로운 기능입니다. 이전에는 DM 추출 파일에서 Decisioning 엔진을 사용할 수 없었습니다. 이제 의사 결정 정책을 추가하고 의사 결정 항목 속성을 내보내기에 열 데이터로 포함할 수 있습니다.

   [DM에서 결정 정책을 추가하는 방법을 알아보세요](../experience-decisioning/create-decision-policy.md#add). 일괄 의사 결정 워크플로우 및 예제(개인화된 DM 또는 다운스트림 시스템으로 내보내기)에 대해서는 [DM의 일괄 의사 결정](../experience-decisioning/batch-decisioning-direct-mail.md)을 참조하십시오.
...
1. 추출 파일 속성을 조정합니다.
   ...
   1. 추출 파일의 시작 또는 끝 부분에 정보를 추가해야 하는 경우가 있습니다. 이렇게 하려면 **[!UICONTROL 메모]** 필드를 사용한 다음 메모를 머리글이나 바닥글로 포함할지 여부를 지정합니다.
      ...
      ![파일 이름, 타임스탬프 및 머리글이나 바닥글 메모를 포함한 추출 파일 속성](assets/direct-mail-properties.png){width="800" align="center"}
...
1. 추출 파일에 표시할 열과 정보를 구성합니다.
   ...
   1. 열을 사용하여 추출 파일을 정렬하려면 열을 선택하고 **[!UICONTROL 정렬 기준]** 옵션을 켜십시오. **[!UICONTROL 데이터 필드]** 섹션에서 열 레이블 옆에 **[!UICONTROL 정렬 기준]** 아이콘이 표시됩니다.
      ...
      ![DM 추출 파일 편집기의 데이터 필드 및 열 서식](assets/direct-mail-content.png){width="800" align="center"}
...
* [다이렉트 메일 시작](get-started-direct-mail.md)
* [DM 채널 구성](direct-mail-configuration.md)
* [DM 테스트 및 보내기](test-send-direct-mail.md)
